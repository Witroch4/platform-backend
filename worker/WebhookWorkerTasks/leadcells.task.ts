...
export async function processLeadCellTask(job: Job<ILeadCellJobData>) {
	...
	if ("manuscrito" in data && data.manuscrito) {
		...
		"analise" in data ||
		"analiseSimulado" in data ||
		"analiseValidada" in data ||
		"analiseSimuladoValidada" in data
	) {
...
async function processManuscrito(job: Job<IManuscritoJobData>) {
	...
	try {
		...
		const conteudoUnificado = textoDAprova.map((item) => item.output).join("\n\n---------------------------------\n\n");
		...
		try {
			...
			console.warn(
				`[BullMQ] Não foi possível atualizar timestamp do Lead pai para LeadOabData ${leadID}: ${e?.message || e}`,
			);
...
async function processAnalise(job: Job<IAnaliseJobData>) {
	...
	try {
		...
		if (analiseUrl) {
			...
			message = analiseSimulado ? "Sua análise de simulado foi finalizada!" : "Sua análise foi finalizada!";
			...
			message = analiseSimulado ? "Sua pré-análise de simulado está pronta!" : "Sua pré-análise está pronta!";
...
function limitJobDataForLog(jobData: any) {
	...
	if (jobData.analisePreliminar) {
		const { exameDescricao, inscricao, nomeExaminando, seccional, areaJuridica, notaFinal, situacao } =
			jobData.analisePreliminar;
		limited.analisePreliminar = {
			exameDescricao,
			inscricao,
			nomeExaminando,
			seccional,
	...
	if (payload.leadData) {
		...
		if (payload.leadData.analisePreliminar) {
			...
			limited.leadData.analisePreliminar = {
				...
				pontosPeca: payload.leadData.analisePreliminar.pontosPeca
					? `[${payload.leadData.analisePreliminar.pontosPeca.length} pontos]`
					: undefined,
				argumentacao: payload.leadData.analisePreliminar.argumentacao
					? `[${payload.leadData.analisePreliminar.argumentacao.length} argumentos]`
					: undefined,
			};
		}
	}
