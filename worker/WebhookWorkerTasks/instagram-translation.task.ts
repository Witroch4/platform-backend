...
import { findOptimizedCompleteMessageMapping, recordDatabaseQuery } from "@/lib/instagram/optimized-database-queries";
import { getCachedConversionResult, setCachedConversionResult } from "@/lib/cache/instagram-template-cache";
import {
...
import { getCachedVariablesForUser, replaceVariablesInText } from "../../lib/mtf-diamante/variables-resolver";
...
export async function processInstagramTranslationTask(
	...
	try {
		...
		try {
			...
			instagramTranslationLogger.workerDatabaseQuery(
				logContext,
		...
		if (!usuarioChatwitId) {
			const error = createDatabaseError(
				"usuarioChatwitId missing from message mapping",
				new Error("usuarioChatwitId is required for cache operations"),
				correlationId,
			);
			logError(error);
			throw error;
		...
		const uniqueUserKey = usesLeadVariable
			? job.data.originalPayload?.originalDetectIntentRequest?.payload?.psid || job.data.contactPhone || undefined
			: undefined;

		const cachedResult = await getCachedConversionResult(
			intentName,
			usuarioChatwitId,
			inboxId,
			bodyLength,
		...
		if (cachedResult && cachedResult.templateType !== "incompatible") {
			...
			try {
				...
				const error = createConversionFailedError(sanitizeErrorMessage(conversionError), correlationId, {
		...
		recordInstagramTranslationMetrics({
			...
			timestamp: new Date(),
			retryCount: job.attemptsMade,
			messageType: messageMapping.messageType as
				| "interactive"
		...
		if (buttonContextMeta && fulfillmentMessages.length > 0) {
			...
			first.payload.meta.instagram = {
				...
				sender_action: buttonContextMeta.reaction
					? { type: "react", reaction: buttonContextMeta.reaction, emoji: buttonContextMeta.reactionEmoji }
					: undefined,
			};
		}
		...
		console.log(
			`[Instagram Worker] [${correlationId}] FULFILLMENT MESSAGES GENERATED:`,
			JSON.stringify(fulfillmentMessages, null, 2),
		);
		console.log(
			`[Instagram Worker] [${correlationId}] FULFILLMENT MESSAGES STRUCTURE:`,
			fulfillmentMessages.map((msg, index) => ({
				index,
				...
				instagramPayloadKeys:
					msg.custom_payload && msg.custom_payload.instagram ? Object.keys(msg.custom_payload.instagram) : [],
				instagramPayloadType:
					msg.custom_payload && msg.custom_payload.instagram
						? msg.custom_payload.instagram.attachment?.type
						: "unknown",
			})),
		);
		...
		// Log job failure
		instagramTranslationLogger.workerJobFailed(
			logContext,
			error as Error,
			error instanceof InstagramTranslationError ? error.retryable : false,
		);
...
> {
	...
	const isButton =
		payload.interaction_type === "button_reply" ||
	...
	const buttonId: string | undefined =
		payload.button_id || payload.interactive?.button_reply?.id || payload.interactive?.list_reply?.id;
	...
	try {
		...
		if (resolver) {
			...
			if (resolvedInteractiveContent.header?.content) {
				const resolvedHeader = await resolver.resolveText(String(resolvedInteractiveContent.header.content));
				resolvedInteractiveContent.header.content = resolvedHeader;
			}

			// Body
			if (resolvedInteractiveContent.body?.text) {
				resolvedInteractiveContent.body.text = await resolver.resolveText(String(resolvedInteractiveContent.body.text));
			}
...
async function convertSimpleReplyToInstagram(
	...
	if (!simpleReplyText || simpleReplyText.trim().length === 0) {
		throw createValidationError("simple_reply_text", "Simple reply text cannot be empty", correlationId);
	}
	...
	if (fulfillmentMessages.length > 0) {
		...
		if (template) {
			...
			instagramTranslationLogger.workerValidationPerformed(
				logContext,
				"simple_reply_instagram_template",
...
async function convertInteractiveMessageToInstagram(
	...
	if (!validation.valid) {
		throw createValidationError("interactive_message", validation.errors.join(", "), correlationId);
	}
	...
	try {
		...
		instagramButtons = interactiveMessage.botoes ? convertWhatsAppButtonsToInstagram(interactiveMessage.botoes) : [];
...
function getHasImageFromMapping(messageMapping: any): boolean {
	...
	if (messageMapping.interactiveMessage) {
		return (
			messageMapping.interactiveMessage.headerTipo === "image" && !!messageMapping.interactiveMessage.headerConteudo
		);
	}
	...
	if (messageMapping.enhancedInteractiveMessage) {
		return (
			messageMapping.enhancedInteractiveMessage.headerType === "image" &&
