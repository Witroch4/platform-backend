...
class IntentProcessor {
  ...
  private async processTemplateContent(
    ...
    try {
      ...
      console.error(
        "[Intent Processor] Error processing template content:",
        error
      );
      throw error;
    }
   ...
   */
  private processWhatsAppOfficialTemplate(
    data: any,
    ...
    try {
      ...
      if (processedData.components && Array.isArray(processedData.components)) {
        processedData.components = processedData.components.map(
          (component: any) => {
            if (component.type === "BODY" && component.text) {
              component.text = this.substituteVariables(
                component.text,
                variables
              );
            }
   ...
   */
  private processInteractiveMessageTemplate(
    data: any,
    variables: Record<string, string>
  ): any {
    try {
      ...
      console.error(
        "[Intent Processor] Error processing Interactive Message template:",
        error
      );
      throw error;
    }
   ...
   */
  private processAutomationReplyTemplate(
    data: any,
    variables: Record<string, string>
  ): any {
    try {
      ...
      console.error(
        "[Intent Processor] Error processing Automation Reply template:",
        error
      );
      throw error;
    }
...
export const respostaRapidaWorker = new Worker<RespostaRapidaJobData>(
  RESPOSTA_RAPIDA_QUEUE_NAME,
  async (job: Job<RespostaRapidaJobData>) => {
    const { data } = job.data;
    const startTime = Date.now();
    ...
    try {
      ...
      console.log(
        `[Resposta Rapida Worker] Job completed successfully: ${job.name}`,
        {
          correlationId: data.correlationId,
...
respostaRapidaWorker.on("completed", (job, result) => {
...
respostaRapidaWorker.on("failed", (job, err) => {
...
respostaRapidaWorker.on("error", (err) => {
  console.error("[Resposta Rapida Worker] Worker error:", err);
});

console.log(
  "[Resposta Rapida Worker] Worker initialized and ready to process jobs"
);
// ============================================================================
