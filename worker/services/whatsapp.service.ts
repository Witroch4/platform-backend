...
export class WhatsAppApiManager {
	 ...
	 * Requirement: 2.4, 2.5
	 */
	private async getCredentialsWithFallback(inboxId: string): Promise<WhatsAppCredentials> {
		try {
			...
			if (fallbackCredentials && this.areCredentialsValid(fallbackCredentials)) {
	 ...
	 */
	private async getInboxCredentials(inboxId: string): Promise<WhatsAppCredentials | null> {
		try {
			...
			if (!inbox || !inbox.whatsappApiKey || !inbox.phoneNumberId || !inbox.whatsappBusinessAccountId) {
				return null;
			}
	...
	private async getFallbackCredentials(
		...
		try {
			...
			if (fallbackInbox.whatsappApiKey && fallbackInbox.phoneNumberId && fallbackInbox.whatsappBusinessAccountId) {
				return {
	 ...
	 */
	private async getGlobalCredentials(inboxId: string): Promise<WhatsAppCredentials | null> {
		try {
	 ...
	 * Requirement: 2.5
	 */
	async resolvePhoneNumberId(inboxId: string, fallbackPhoneNumberId?: string): Promise<string | null> {
		try {
