...
import { WorkerResponse, WhatsAppCredentials, ButtonActionMapping } from "../types/types";
...
export class ButtonProcessor {
	 ...
	 * Requirement: 4.3
	 */
	private async findActionByButtonId(buttonId: string, inboxId: string): Promise<ButtonActionMapping | null> {
		try {
			...
			return {
				...
				actionType: mapping.actionType as "SEND_TEMPLATE" | "ADD_TAG" | "START_FLOW" | "ASSIGN_TO_AGENT",
	...
	private async executeAction(
		...
		switch (actionType) {
			case "SEND_TEMPLATE":
				return await this.executeSendTemplateAction(actionPayload, contactPhone, credentials, correlationId, inboxId);
	...
	private async executeSendTemplateAction(
		...
		try {
			...
			if (templateId) {
				...
				messageContent = await this.processTemplateForAction(template, parameters, inboxId);
	 ...
	 */
	private async processTemplateForAction(template: any, parameters: any, inboxId?: string): Promise<any> {
		try {
			console.log(`[Button Processor] Processing template for action: ${template.type}`);
			...
			switch (template.type) {
				...
				case "INTERACTIVE_MESSAGE":
					if (template.interactiveContent) {
						return {
							type: "interactive",
							interactive: await payloadBuilder.buildInteractiveMessagePayload(template.interactiveContent),
						};
					}
					break;
				...
				case "AUTOMATION_REPLY":
					if (template.simpleReplyText) {
						return await payloadBuilder.buildSimpleTextPayload(template.simpleReplyText);
					}
					break;
			}
	...
	private async handleEmojiReactionFallback(
		...
		try {
			...
			if (emoji) {
				...
				const payloadBuilder = new METAPayloadBuilder();
				const messageContent = payloadBuilder.buildReactionPayload(wamid, emoji);
	...
	private async handleTextReactionFallback(
		...
		try {
			...
			const messageContent = await payloadBuilder.buildTextReplyPayload(wamid, textResponse);
