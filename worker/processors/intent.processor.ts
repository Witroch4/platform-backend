...
import { sanitizeTextContent, sanitizePhoneNumber, sanitizeApiKey } from "../../lib/webhook-utils";
...
export class IntentProcessor {
	...
	async processIntent(
		...
		try {
			...
			const templateMapping = await this.findTemplateByIntent(sanitizedData.intentName, sanitizedData.inboxId);

			if (!templateMapping) {
				console.log(`[Intent Processor] No mapping found for intent: ${sanitizedData.intentName}`, {
					correlationId: sanitizedData.correlationId,
					inboxId: sanitizedData.inboxId,
				});
				...
				if (options?.disableFallback) {
					...
					console.log(`[Intent Processor] Fallback disabled by caller; skipping automatic reply for unmapped intent`, {
						correlationId: sanitizedData.correlationId,
					});
					return {
						success: true,
	 ...
	 * Requirement: 4.2
	 */
	private async findTemplateByIntent(intentName: string, inboxId: string): Promise<any> {
		try {
			...
			return {
				...
				template: {
					...
					type: mapping.template.type as "WHATSAPP_OFFICIAL" | "INTERACTIVE_MESSAGE" | "AUTOMATION_REPLY",
	...
	private async resolveTemplate(mapping: TemplateMapping): Promise<any> {
		...
		try {
			...
			if (template.type === "WHATSAPP_OFFICIAL" && template.whatsappOfficialInfo) {
			...
			if (template.type === "INTERACTIVE_MESSAGE" && template.interactiveContent) {
	...
	private async buildMessageContent(
		...
		try {
			...
			console.error("[Intent Processor] Error building message content:", error);
			throw error;
		}
	}
	...
	private async buildInteractiveMessage(
		...
		try {
			...
			console.error("[Intent Processor] Erro ao construir mensagem interativa:", error);
			throw error;
		}
	}
	...
	private async buildTextMessage(
		...
		try {
			...
			console.error("[Intent Processor] Erro ao construir mensagem de texto:", error);
			throw error;
		}
	}
