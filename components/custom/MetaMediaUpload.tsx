...
import { Video, Image as ImageIcon, FileImage, UploadCloud, X, Check, Loader2, ExternalLink, File } from "lucide-react";
...
export default function MetaMediaUpload({
		...
		try {
			...
			if (response.data && response.data.success) {
				...
				setUploadedFiles((prev) => {
					...
					console.log(
						`[MetaMediaUpload] Arquivo atual:`,
						prev.find((f) => f.id === newFile.id),
					);
	...
	const uploadFileToMetaApi = async (fileData: MetaMediaFile) => {
		...
		setUploadedFiles((prev) => prev.map((f) => (f.id === fileData.id ? { ...f, status: "uploading" } : f)));
		...
		try {
			...
			const minioResponse = await axios.post("/api/upload-media", formDataMinIO, {
				...
				onUploadProgress: (progressEvent) => {
					const progress = Math.round((progressEvent.loaded / (progressEvent.total ?? 0)) * 50); // Metade do progresso total para o upload no MinIO
			...
			setUploadedFiles((prev) => prev.map((f) => (f.id === fileData.id ? { ...f, url: minioUrl, progress: 50 } : f)));
			...
			const metaResponse = await axios.post("/api/upload-media", formDataMeta, {
				onUploadProgress: (progressEvent) => {
					const progress = 50 + Math.round((progressEvent.loaded / (progressEvent.total ?? 0)) * 50); // Segunda metade do progresso (50-100%)
								...
								>
									<div className="flex items-center flex-1 p-2">
										...
										<div className="ml-3 flex-1 min-w-0">
											...
											{file.status === "success" && (
												<div className="flex flex-col text-xs text-green-500">
													<div className="flex items-center">
														...
														<span className="truncate">Processado com sucesso</span>
													</div>
													{file.mediaHandle && (
											...
											{file.status === "error" && (
												<div className="text-xs text-red-500 truncate">Erro: {file.error}</div>
											)}

											{file.status === "waiting" && (
												<div className="text-xs text-muted-foreground">Aguardando upload...</div>
											)}
										</div>
									</div>
