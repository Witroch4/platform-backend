...
import {
	FileTypes,
	uploadFileWithAssistants,
	validateFileForOpenAI,
	getFilePurpose,
} from "@/services/assistantsFileHandler";
		...
		try {
			...
			if (!validation.valid) {
				setUploadingFiles((prev) =>
					prev.map((f) =>
						f.id === uploadingFile.id ? { ...f, status: "error", error: validation.error, progress: 0 } : f,
					),
				);
			...
			const result = await uploadFileWithAssistants(uploadingFile.file, purpose, (progress) => {
        ...
        transition-colors duration-200 ease-in-out`}
			>
				<div className="text-center">
					...
					<p className="text-xs text-gray-500">Supported formats: PDF, Images, Text, CSV, JSON (max 25MB)</p>
				</div>
								...
								<div className="flex items-center flex-1 p-2">
									...
									<div className="w-full ml-2 space-y-1">
										<div className="text-sm flex justify-between">
											<p className="text-muted-foreground truncate max-w-[200px]">{file.file.name}</p>
										...
										{file.status === "uploading" && (
											<Progress value={file.progress} className={getFileIconAndColor(file.file).color} />
										)}
