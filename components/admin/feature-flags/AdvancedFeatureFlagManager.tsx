...
export function AdvancedFeatureFlagManager({
	...
	const searchFlags = useCallback(
		async (newFilters?: Partial<SearchFilters>, newOffset?: number) => {
			try {
				setLoading(true);
		...
		<div className="space-y-6">
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>Use os filtros abaixo para encontrar feature flags específicas</CardDescription>
				</CardHeader>
				<CardContent className="space-y-4">
					...
					<div className="grid grid-cols-2 md:grid-cols-5 gap-4">
						<div>
							...
							<Select value={filters.category} onValueChange={(value) => handleSearch({ category: value })}>
								<SelectTrigger>
						...
						<div>
							<Label>Status</Label>
							<Select value={filters.enabled} onValueChange={(value) => handleSearch({ enabled: value })}>
								<SelectTrigger>
						...
						<div>
							...
							<Select value={filters.userSpecific} onValueChange={(value) => handleSearch({ userSpecific: value })}>
								<SelectTrigger>
									<SelectValue placeholder="Todos" />
								</SelectTrigger>
								<SelectContent>
						...
						<div>
							...
							<Select value={filters.sortBy} onValueChange={(value) => handleSearch({ sortBy: value })}>
								<SelectTrigger>
									<SelectValue />
								</SelectTrigger>
								<SelectContent>
						...
						<div>
							<Label>Ordem</Label>
							<Select value={filters.sortOrder} onValueChange={(value) => handleSearch({ sortOrder: value })}>
								<SelectTrigger>
									<SelectValue />
								</SelectTrigger>
								<SelectContent>
					...
					{statistics.status.map((stat: any) => (
						<Card key={stat.enabled.toString()}>
							<CardContent className="p-4">
								...
								<p className="text-sm text-muted-foreground">{stat.enabled ? "Ativas" : "Inativas"}</p>
							</CardContent>
						</Card>
					))}
				</div>
					...
					<CardContent className="p-4">
						<div className="flex items-center justify-between">
							...
							<div className="flex gap-2">
								...
								<Button variant="outline" onClick={() => setSelectedFlags(new Set())}>
									Limpar Seleção
								</Button>
							</div>
						</div>
				...
				<CardHeader>
					<div className="flex items-center justify-between">
						<div>
							...
							<CardDescription>{pagination.total} feature flags encontradas</CardDescription>
						</div>

						<div className="flex items-center gap-2">
							<Button variant="outline" onClick={selectAllFlags} className="flex items-center gap-1">
							...
							{onCreateFlag && <Button onClick={onCreateFlag}>Nova Flag</Button>}
						</div>
					</div>
				</CardHeader>
				<CardContent>
						...
						<div className="space-y-4">
							{flags.map((flag) => (
								<div key={flag.id} className="flex items-start gap-3">
									...
									/>
									<div className="flex-1">
										<FeatureFlagCard flag={flag} onToggle={onToggleFlag} onDelete={onDeleteFlag} updating={false} />
									</div>
								</div>
							))}
						</div>
