...
export function UserFlagOverrideDialog({ flagId, flagName, children }: UserFlagOverrideDialogProps) {
		...
		<Dialog open={open} onOpenChange={setOpen}>
			...
			<DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
				<DialogHeader>
					...
					<DialogDescription>Configure a feature flag "{flagName}" para usuários específicos</DialogDescription>
				</DialogHeader>
											...
											<CardContent className="p-4">
												<div className="flex items-center justify-between">
													<div className="flex items-center gap-3">
														...
														<div>
															<p className="font-medium">{override.user.name || "Sem nome"}</p>
															<p className="text-sm text-muted-foreground">{override.user.email}</p>
														</div>
								...
								<Card>
									<CardContent className="p-4 space-y-4">
										...
										<div className="flex gap-2">
											...
											>
												{updating === selectedUser.id && <Loader2 className="h-4 w-4 animate-spin" />}
												Criar Override
											</Button>
											<Button
												variant="outline"
