...
export function RegisterApiKeyDialog({
	...
	const defaultTrigger = (
		<Button
			variant={userHasToken ? "outline" : "default"}
			size="sm"
			className="gap-1.5 min-h-[36px] sm:min-h-[40px] px-2 sm:px-3 text-xs sm:text-sm shrink-0"
		>
			<Key className="h-4 w-4" />
			<span className="hidden sm:inline">{userHasToken ? "Atualizar API Key" : "Cadastrar API Key"}</span>
			<span className="sm:hidden">{userHasToken ? "API Key" : "API Key"}</span>
		</Button>
	);
		...
		<Dialog open={isOpen} onOpenChange={setIsOpen}>
			...
			<DialogContent className="max-w-2xl">
				...
				<div className="space-y-6">
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>Siga os passos abaixo para obter seu token de acesso:</CardDescription>
						</CardHeader>
						<CardContent className="space-y-3">
							...
							<div className="flex items-start gap-3">
								...
								<div>
									<p className="text-sm">Acesse as configurações do perfil e procure pela seção "Token de acesso"</p>
								</div>
							</div>
							...
							<div className="flex items-start gap-3">
								...
								<div>
									<p className="text-sm">Clique no botão "Copiar" ao lado do token</p>
								</div>
							</div>
							...
							<div className="flex items-start gap-3">
								...
								<div>
									<p className="text-sm">Cole o token e o ID da conta nos campos abaixo e clique em "Registrar"</p>
								</div>
							</div>
						</CardContent>
					</Card>
					...
					<form onSubmit={handleSubmit} className="space-y-4">
						<div className="space-y-2">
							...
							{!!accountId && <span className="text-green-600 text-xs block mt-1">✓ ID configurado: {accountId}</span>}
						</div>
						...
						<div className="space-y-2">
							...
							<p className="text-xs text-muted-foreground">O token deve ter exatamente 24 caracteres alfanuméricos</p>
						</div>
				...
				<DialogFooter>
					...
					<Button type="submit" onClick={handleSubmit} disabled={isLoading || !token.trim() || !accountId.trim()}>
