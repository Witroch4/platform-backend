...
app.prepare().then(async () => {
	...
	if (dev && !process.env.RUN_IN_DOCKER) {
		...
		// Workers (init.ts é o orquestrador único de todos os workers)
		const workerInit = spawn(npxPath, ["ts-node", "-r", "tsconfig-paths/register", "worker/init.ts"], {
			shell: true,
			stdio: "inherit",
		});

		// Worker de automação (caso queira rodar separado)
		const workerInstagram = spawn(npxPath, ["ts-node", "-r", "tsconfig-paths/register", "worker/automacao.worker.ts"], {
			shell: true,
			stdio: "inherit",
		});
