	...
	if (!rows || rows.length === 0) {
		...
		try {
			...
			if (err.code === "42P04" || /already exists/i.test(err.message)) {
				...
				console.error(
					"🔒 Sem permissão para CREATE DATABASE. Crie o DB manualmente ou forneça credenciais com permissão.",
				);
				throw err;
			} else {
				throw err;
...
async function adjustEmbeddingColumnAndIndex(prisma, dims) {
  ...
  `;
	...
	try {
		...
		console.warn(
			`⚠️ Falhou criar ${preferred.toUpperCase()} → fallback para ${preferred === "hnsw" ? "IVFFLAT" : "HNSW"} (${e?.message || e})`,
		);
		try {
			if (preferred === "hnsw") {
				if (!(await hasIndex(idxIvf))) {
