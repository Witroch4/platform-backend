...
class UnifiedModelMigration {
	...
	private async migrateCaixaEntradaToInbox(): Promise<void> {
		...
		try {
			...
			const legacyCaixas = await this.queryLegacyTable(
				"CaixaEntrada",
				`
        SELECT 
	...
	private async migrateInstagramLeads(): Promise<void> {
		try {
			...
			const legacyInstagramLeads = await this.queryLegacyTable(
				"InstagramLead",
				`
        SELECT 
	...
	private async migrateWhatsAppTemplates(): Promise<void> {
		try {
			const legacyTemplates = await this.queryLegacyTable(
				"WhatsAppTemplate",
				`
        SELECT 
	...
	private async migrateInteractiveMessages(): Promise<void> {
		try {
			const legacyMessages = await this.queryLegacyTable(
				"MensagemInterativa",
				`
        SELECT 
      ...
      `,
			...
			for (const message of legacyMessages) {
				try {
					...
					const buttons = await this.queryLegacyTable(
						"BotaoMensagemInterativa",
						`
					...
					await prisma.template.create({
						data: {
							...
							interactiveContent: {
								create: {
									header:
										message.headerTipo && message.headerConteudo
											? {
													create: {
	...
	private async migrateEnhancedInteractiveMessages(): Promise<void> {
		try {
			const enhancedMessages = await this.queryLegacyTable(
				"InteractiveMessage",
				`
        SELECT 
					...
					await prisma.template.create({
						data: {
							...
							interactiveContent: {
								create: {
									header:
										message.headerType && message.headerContent
											? {
													create: {
