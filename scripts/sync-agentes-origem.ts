		...
		for (const usuario of usuariosChatwit) {
			...
			try {
				...
				for (const caixa of usuario.caixas) {
					...
					const hookIdsExistentes = new Set(agentesExistentes.filter((a) => a.hookId).map((a) => a.hookId));
					...
					for (const hook of hooksParaInbox) {
						...
						if (hookIdsExistentes.has(hookIdStr)) {
							...
							if (agenteExistente && agenteExistente.ativo !== hook.status) {
								...
								console.log(
									`  ✏️  Agente ${agenteExistente.nome} atualizado (${hook.status ? "ATIVADO" : "DESATIVADO"})`,
								);
								agentesAtualizados++;
							}
						} else {
