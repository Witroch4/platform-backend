...
async function healthCheck() {
	...
	try {
		...
		console.log(
			`${respostaRapidaActive ? "✅" : "❌"} Fila Resposta Rápida: ${respostaRapidaActive ? "Ativa" : "Inativa"}`,
		);
		if (respostaRapidaHealth) {
			console.log(
				`    • Aguardando: ${respostaRapidaHealth.waiting}, Ativo: ${respostaRapidaHealth.active}, Falhou: ${respostaRapidaHealth.failed}`,
			);
		}
		...
		if (persistenciaHealth) {
			console.log(
				`    • Aguardando: ${persistenciaHealth.waiting}, Ativo: ${persistenciaHealth.active}, Falhou: ${persistenciaHealth.failed}`,
			);
		}
	} catch (error) {
