...
class ErrorHandlingTestRunner {
  ...
  async runValidationTests(): Promise<void> {
    const validationConfigs = this.testConfigs.filter(config => 
      config.name.toLowerCase().includes('validation')
    );
  ...
  async runErrorHandlingTests(): Promise<void> {
    const errorConfigs = this.testConfigs.filter(config => 
      config.name.toLowerCase().includes('error')
    );
  ...
  async runApiTests(): Promise<void> {
    const apiConfigs = this.testConfigs.filter(config => 
      config.name.toLowerCase().includes('api')
    );
