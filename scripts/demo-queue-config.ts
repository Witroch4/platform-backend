...
async function demonstrateQueueConfig() {
	...
	try {
		...
		allConfigs.forEach((config) => {
			console.log(
				`   • ${config.name} (${config.displayName}) - Priority: ${config.priority}, Concurrency: ${config.concurrency}`,
			);
		});
		console.log();
		...
		console.log(
			`✅ Updated queue: ${updatedQueue.name} - New priority: ${updatedQueue.priority}, New concurrency: ${updatedQueue.concurrency}\n`,
		);
