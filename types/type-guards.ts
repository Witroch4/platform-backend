...
export function hasMediaHeader(message: InteractiveMessage): boolean {
	return (
		hasHeader(message) && message.header.type !== "text" && (!!message.header.mediaUrl || !!message.header.mediaId)
	);
}
...
export function hasValidButtons(action: MessageAction): boolean {
	...
	return Array.isArray(action.buttons) && action.buttons.length > 0 && action.buttons.every(isValidButton);
}
...
export function hasValidListSections(action: MessageAction): boolean {
	...
	return Array.isArray(action.sections) && action.sections.length > 0 && action.sections.every(isValidListSection);
}
...
export function hasValidCtaUrl(action: MessageAction): boolean {
	...
	return (
		typeof action.action.displayText === "string" &&
...
export function hasValidFlow(action: MessageAction): boolean {
	...
	return (
		(typeof action.action.parameters.flow_id === "string" || typeof action.action.parameters.flow_name === "string") &&
		typeof action.action.parameters.flow_cta === "string" &&
		action.action.parameters.flow_cta.length > 0 &&
		(!action.action.parameters.mode ||
			action.action.parameters.mode === "draft" ||
			action.action.parameters.mode === "published")
	);
}
...
export function isValidationError(error: any): error is { field: string; message: string } {
	return error && typeof error === "object" && typeof error.field === "string" && typeof error.message === "string";
}
