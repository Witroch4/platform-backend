...
export const {
	...
	callbacks: {
		...
		async jwt({ token, user, trigger, session }) {
			...
			if (user) {
				...
				if (instagramAccount) {
					const partialIgToken = instagramAccount.access_token
						...
						: null;
					console.log(
						`Conta do Instagram encontrada. ProviderAccountId: ${instagramAccount.providerAccountId}, AccessToken parcial: ${partialIgToken}`,
					);
			...
			const partialAccess = token.instagramAccessToken ? token.instagramAccessToken.slice(0, 3) + "..." : undefined;
