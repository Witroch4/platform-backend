	...
	try {
		...
		for (const buttonId of testButtonIds) {
			...
			if (mapping) {
				...
				console.log(`   Emoji: ${(mapping.actionPayload as any)?.emoji || "N/A"}`);
				console.log(`   Texto: ${(mapping.actionPayload as any)?.textReaction || "N/A"}`);
			} else {
		...
		console.error("❌ Erro ao verificar mapeamentos:", error instanceof Error ? error.message : String(error));
	} finally {
