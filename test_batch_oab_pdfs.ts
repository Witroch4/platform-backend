...
async function runBatchTest() {
  ...
  console.log(`┌${'─'.repeat(35)}┬${'─'.repeat(10)}┬${'─'.repeat(12)}┬${'─'.repeat(10)}┬${'─'.repeat(10)}┬${'─'.repeat(10)}┐`);
  console.log(`│ ${'PDF'.padEnd(33)} │ ${'PEÇA'.padEnd(8)} │ ${'QUESTÕES'.padEnd(10)} │ ${'GERAL'.padEnd(8)} │ ${'ESPERADO'.padEnd(8)} │ ${'STATUS'.padEnd(8)} │`);
  console.log(`├${'─'.repeat(35)}┼${'─'.repeat(10)}┼${'─'.repeat(12)}┼${'─'.repeat(10)}┼${'─'.repeat(10)}┼${'─'.repeat(10)}┤`);
  ...
  results.forEach(r => {
    ...
    const status = r.status === 'PASSOU' ? '✅ OK'.padEnd(8) : r.status === 'FALHOU' ? '❌ FALHA'.padEnd(8) : '🚨 ERRO'.padEnd(8);
