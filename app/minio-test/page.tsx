	...
	const testUrl = async () => {
		...
		try {
			...
			if (isImage) {
				...
				setTestResult(
					`Sucesso! A URL retornou uma imagem (${contentType}). <br/><img src="${imageUrl}" alt="Imagem do MinIO" style="max-width: 100%; max-height: 300px;" />`,
				);
			} else {
				...
				<TabsContent value="upload">
					<Card>
						<CardHeader>
							...
							<CardDescription>Faça upload de arquivos para o MinIO e teste as URLs geradas.</CardDescription>
						</CardHeader>
						<CardContent>
				...
				<TabsContent value="test">
					<Card>
						<CardHeader>
							...
							<CardDescription>Cole uma URL do MinIO para testar se ela está acessível.</CardDescription>
						</CardHeader>
						<CardContent>
				...
				<TabsContent value="gallery">
					<Card>
						<CardHeader>
							...
							<CardDescription>Visualize as imagens que você fez upload.</CardDescription>
						</CardHeader>
						<CardContent>
							<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
										...
										<div key={file.id} className="border rounded-md overflow-hidden">
											...
											<div className="p-2 text-sm">
												...
												<div className="flex gap-2 mt-2">
													...
													{file.thumbnail_url && (
														<Button variant="outline" onClick={() => window.open(file.thumbnail_url, "_blank")}>
															Thumbnail
														</Button>
													)}
												</div>
								...
								{uploadedFiles.filter((file) => file.progress === 100 && file.mime_type?.startsWith("image/"))
									.length === 0 && (
