...
export function convertInteractiveMessageToWhatsApp(
	...
	const processedBodyText = variableConverter.generatePreviewText(message.body.text, variables);

	// Process footer text - convert variables to actual values
	let processedFooterText = "";
	if (message.footer?.text) {
		processedFooterText = variableConverter.generatePreviewText(message.footer.text, variables);
	}

	// Process header text if it exists
	let processedHeaderText = "";
	if (message.header?.type === "text" && message.header.text) {
		processedHeaderText = variableConverter.generatePreviewText(message.header.text, variables);
	}
...
export function validateInteractiveMessage(message: InteractiveMessage): { isValid: boolean; errors: string[] } {
	...
	switch (message.type) {
		...
		case "list":
			...
			if (
				!message.action?.sections ||
				!Array.isArray(message.action.sections) ||
				message.action.sections.length === 0
			) {
