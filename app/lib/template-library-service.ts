...
export class TemplateLibraryService {
	...
	static async getLibraryItems(
		...
		const whereClause: any = {
			isActive: true,
			OR: [{ scope: "GLOBAL" }, { scope: "PRIVATE", createdById: userId }],
		};
	 ...
	 */
	static async getApprovalRequests(status?: "pending" | "approved" | "rejected"): Promise<
		Array<
	...
	static async updateTemplate(
		...
		const canUpdate = template.createdById === userId || user.role === "ADMIN" || user.role === "SUPERADMIN";
	...
	static async deleteTemplate(templateId: string, userId: string): Promise<void> {
		...
		const canDelete = template.createdById === userId || user.role === "ADMIN" || user.role === "SUPERADMIN";
	 ...
	 */
	static async getTemplatesByCategory(category: string, userId: string): Promise<TemplateLibraryWithCreator[]> {
		return await getPrismaInstance().template.findMany({
			where: {
				tags: {
					has: category,
