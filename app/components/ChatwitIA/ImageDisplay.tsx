		...
		<div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50 p-4">
			<div className="max-w-4xl max-h-full bg-white rounded-lg overflow-hidden">
				...
				<div className="p-4 max-h-[80vh] overflow-auto">
					<img src={imageUrl} alt={prompt || "Imagem gerada"} className="w-full h-auto rounded-lg shadow-lg" />
						...
						{prompt && (
							<div className="bg-gray-50 p-3 rounded-lg">
								<div className="flex justify-between items-start mb-2">
									...
									<button onClick={() => handleCopyPrompt(prompt)} className="p-1 hover:bg-gray-200 rounded">
						...
						{revisedPrompt && revisedPrompt !== prompt && (
							<div className="bg-blue-50 p-3 rounded-lg">
								<div className="flex justify-between items-start mb-2">
									...
									<button onClick={() => handleCopyPrompt(revisedPrompt)} className="p-1 hover:bg-blue-200 rounded">
										{copiedPrompt ? <Check size={16} className="text-green-600" /> : <Copy size={16} />}
									</button>
								</div>
			...
			<div className={`relative group ${className}`}>
				...
				{(prompt || revisedPrompt) && (
					<div className="mt-2 text-xs text-gray-500">
						<p className="truncate">{revisedPrompt || prompt}</p>
					</div>
				)}
			</div>
