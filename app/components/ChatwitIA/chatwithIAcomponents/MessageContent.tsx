		...
		<>
			<div className="inline-flex items-center gap-3 bg-gradient-to-r from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl p-3 my-2 max-w-sm">
				...
				<div className="flex-1 min-w-0">
					...
					<p className="text-xs text-slate-500 dark:text-slate-400 mb-2">Imagem carregada</p>
						...
						<Dialog open={isExpanded} onOpenChange={setIsExpanded}>
							...
							<DialogContent className="max-w-4xl max-h-[90vh] p-0">
								<DialogHeader className="p-4 border-b">
									...
									<DialogDescription>Visualização da imagem em tamanho completo</DialogDescription>
								</DialogHeader>
...
const GeneratedImage: React.FC<{
	...
	({ src, alt, prompt, onAspectRatioDetected, sharedAspectRatio = "1 / 1", isProgress = false, onReference }) => {
		const [isExpanded, setIsExpanded] = useState(false);
		const [isLoading, setIsLoading] = useState(true);
		...
		const handleImageLoad = (e: React.SyntheticEvent<HTMLImageElement>) => {
			...
			console.log(
				`🖼️ Imagem carregada com sucesso - Dimensões: ${img.naturalWidth}x${img.naturalHeight}, Aspect Ratio: ${aspectRatio.toFixed(3)}`,
			);
		};
		...
		return (
			<>
				<div key={stableKey} className="relative group my-4 w-full max-w-2xl mx-auto">
          ...
          `}
            ...
            `}
							...
							<div className="flex gap-2">
								<Dialog open={isExpanded} onOpenChange={setIsExpanded}>
									...
									<DialogContent className="max-w-5xl max-h-[95vh] p-0">
										<DialogHeader className="p-4 border-b">
											<DialogTitle className="text-lg font-semibold truncate pr-4">{alt}</DialogTitle>
											<DialogDescription>Visualização da imagem gerada em tamanho completo</DialogDescription>
										</DialogHeader>
					...
					{prompt && !isProgress && <div className="mt-2 text-sm text-muted-foreground text-center px-4">{prompt}</div>}
				</div>
			</>
		);
	},
...
export default React.memo(function MessageContent({ content, isStreaming = false, onImageReference }: Props) {
	...
	const hasImageReference =
		content.includes("[") &&
		content.includes("](file_id:") &&
		/\[([^\]]*\.(png|jpg|jpeg|gif|webp|bmp|svg)[^\]]*)\]\(file_id:/i.test(content);
	const hasPdfReference =
		content.includes("[") && content.includes("](file_id:") && /\[([^\]]*\.pdf[^\]]*)\]\(file_id:/i.test(content);
	const hasOtherFileReference =
		content.includes("[") && content.includes("](file_id:") && !hasImageReference && !hasPdfReference;
	...
	const contentParts = useMemo((): ContentPart[] => {
		...
		console.log(
			"🔍 MessageContent - Processando conteúdo:",
		...
		if (
			...
			hasImagePreviews ||
			hasFileReferences ||
			result.filter((p) => p.type === "image" || p.type === "image_preview").length > 0
		) {
	...
	return (
		<div
			className={`${proseClass} w-full min-w-0`}
			style={{
		...
		>
			...
			{hasPdfReference && (
				<div className="flex items-center gap-2 p-2 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-md mb-3">
					...
					<span className="text-sm text-blue-700 dark:text-blue-300">Arquivo PDF anexado à mensagem</span>
				</div>
			)}
			...
			{hasImageReference && (
				<div className="flex items-center gap-2 p-2 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-md mb-3">
					...
					<span className="text-sm text-green-700 dark:text-green-300">Imagem anexada à mensagem</span>
				</div>
			)}
			...
			{hasOtherFileReference && (
				<div className="flex items-center gap-2 p-2 bg-gray-50 dark:bg-gray-900/20 border border-gray-200 dark:border-gray-800 rounded-md mb-3">
					...
					<span className="text-sm text-gray-700 dark:text-gray-300">Arquivo anexado à mensagem</span>
				</div>
			)}
			...
			{contentParts.map((part, index) => {
				if (part.type === "image" && part.src && part.alt) {
					...
					const textContent = part.content.replace(/!\[([^\]]*)\]\((https?:\/\/[^\s)]+)\)/g, ""); // Remove apenas imagens markdown já processadas
