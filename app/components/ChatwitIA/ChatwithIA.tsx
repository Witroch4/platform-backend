...
export default function ChatwitIA({
	...
	const { generateImage, isGenerating: isGeneratingImage } = useImageGeneration(chatId || undefined);
	...
	const handleImageReference = useCallback(
		async (imageUrl: string, prompt?: string, openaiFileId?: string) => {
			...
			try {
				...
				if (openaiFileId && openaiFileId.startsWith("file-")) {
					...
					const searchResponse = await fetch(
						`/api/chatwitia/images/search?imageUrl=${encodeURIComponent(imageUrl)}&sessionId=${chatId}`,
					);
		...
		try {
			...
			if (modelId.includes("latest") || modelId.includes("chatgpt-4o")) {
				if (modelId.includes("4o") || modelId.includes("chatgpt-4o")) {
					...
					console.log(
						`🔄 Frontend: Mapeando ${modelId} para ${modelForImageCheck} para verificação de compatibilidade`,
					);
	...
	return (
		<div className="flex flex-col h-full max-h-full overflow-hidden">
			...
			{showScrollButton && <ScrollToBottomButton unread={unreadMessages} onClick={scrollToBottom} />}
