...
const ChatInputForm: React.FC<ChatInputFormProps> = ({
	...
	const pendingPdfRefs = useMemo(() => {
		const completedAsRefs = completedFiles
	...
	const processFile = useCallback(
		async (file: File, purpose: UploadPurpose, useUrl: boolean) => {
			...
			try {
				...
				setUploadingFiles((prev) =>
					prev.map((f) => (f.id === fileId ? { ...f, storageUrl: uploadResult.url, progress: 50 } : f)),
				);
				...
				if (isImage && !useUrl) {
					...
					setUploadingFiles((prev) =>
						prev.map((f) => (f.id === fileId ? { ...f, status: "syncing", progress: 75 } : f)),
					);
	...
	const encapsulateContent = useCallback(
		(content: string): string => {
			...
			// React/JSX
				...
				content.includes("<") &&
				content.includes(">") &&
				(content.includes("className") || content.includes("jsx") || content.includes("tsx"))
			) {
				language = "jsx";
			}
			// HTML
				...
				content.includes("<") &&
				content.includes(">") &&
				(content.includes("<!DOCTYPE") || content.includes("<html") || content.includes("<div"))
			) {
				language = "html";
			}
			// CSS
		...
		async (message: PromptInputMessage) => {
			...
			try {
				...
				if (completedFiles.length > 0) {
					...
					const fileLinks = completedFiles
						.map((r) => {
	...
	const handleSend = useCallback(
		async (e: React.MouseEvent | React.KeyboardEvent) => {
			...
			if (isLoading || sendingRef.current || isUploading) {
				...
				console.log(
					`⏸️ Bloqueando envio: isLoading=${isLoading}, sending=${sendingRef.current}, uploading=${isUploading}`,
				);
				return;
			}
			...
			if (pendingPdfRefs.length > 0) {
				// Adiciona referências de todos os arquivos (PDFs e imagens)
				const fileLinks = pendingPdfRefs
					.map((r) => {
						// 🔧 CORREÇÃO: Para imagens com useUrl: true, usar formato markdown de imagem
						if (r.useUrl && r.isImage && r.storageUrl) {
							return `![${r.name}](${r.storageUrl})`;
	...
	const handleVariationImage = useCallback(
		(fileId: string) => {
	...
	const handleImageEdit = useCallback(
		(file: File, prompt: string, mask?: File) => {
			if (onEditImage) {
	...
	const uploadMenuItems = useMemo(
		() => [
			{
				purpose: "vision" as UploadPurpose,
			...
			{
				purpose: "vision" as UploadPurpose,
				label: "Carregar arquivo (PDF + Imagens)",
				icon: Upload,
				key: "vision-files-convert",
				useUrl: true,
			},
	...
	const onDrop = useCallback(
		async (acceptedFiles: File[]) => {
			...
			const processPromises = acceptedFiles.map((file) => processFile(file, purpose, useUrl));
	...
	return (
		<>
					...
					<div className="bg-gradient-to-r from-muted/20 to-accent/20 border border-border rounded-lg p-3">
						<div className="flex items-start gap-3">
							...
							<div className="flex-1 min-w-0">
								...
								<p className="text-xs text-muted-foreground mt-1">Digite sua pergunta sobre esta imagem abaixo</p>
							</div>
								...
								<div key={file.id} className="flex items-center gap-3">
									...
									<div className="flex-1 min-w-0">
										<div className="flex items-center gap-2">
											<span className="text-sm text-amber-800 dark:text-amber-200 truncate">{file.name}</span>
								...
								<Tooltip>
									<TooltipTrigger asChild>
										...
										>
											...
											<button
												onClick={() =>
													setCompletedFiles((prev) => prev.filter((f) => (f.openaiFileId || f.id) !== r.id))
												}
			...
			<div {...getRootProps()} className="sticky bottom-0 border-t border-border bg-background py-4 relative">
				...
				{isDragActive && (
					<div className="absolute inset-0 bg-primary/10 backdrop-blur-sm z-50 rounded-lg border-2 border-primary border-dashed flex items-center justify-center">
						<div className="text-center">
							...
							<p className="text-sm text-muted-foreground">PDFs serão convertidos em imagens automaticamente</p>
						</div>
					</div>
				)}
				...
				<div className="container mx-auto max-w-6xl px-4">
					<PromptInput onSubmit={handlePromptSubmit} className="relative">
						{/* 🎯 NOVO: Area de exibição de arquivos anexados */}
						<PromptInputAttachments className="p-2">
							{(attachment) => <PromptInputAttachment key={attachment.id} data={attachment} className="mr-2 mb-2" />}
						</PromptInputAttachments>
						...
						<PromptInputTextarea
							...
							placeholder={
								isUploading
									? "⏳ Aguardando processamento de arquivos..."
									: isDragActive
										? "📁 Solte os arquivos aqui..."
										: "Digite sua mensagem (proteção inteligente contra problemas de formatação)..."
							}
						...
						<PromptInputToolbar className="absolute bottom-3 left-4 right-4 flex items-center justify-between bg-card/90 backdrop-blur-sm p-2 rounded-lg border border-border shadow-sm">
							...
							<div className="flex items-center space-x-2">
								...
								<button
									type="button"
									onClick={() => {
										setGerarImagemActive((prev) => !prev);
										onImageGenerate?.();
									}}
									disabled={isUploading}
							...
							<div className="flex items-center space-x-2">
								...
								>
									{isUploading ? <Loader2 size={20} className="animate-spin" /> : <ArrowUp size={20} />}
								</PromptInputSubmit>
							</div>
						</PromptInputToolbar>
					</PromptInput>
					...
					{showUploadMenu && !isUploading && (
						<div
							ref={uploadMenuRef}
							className="absolute bottom-20 left-4 bg-popover border border-border rounded-lg shadow-lg py-1 min-w-[220px] z-50"
						>
