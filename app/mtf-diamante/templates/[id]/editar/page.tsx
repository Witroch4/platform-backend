...
export default function EditTemplateDetailsPage() {
	...
	const handleSave = async () => {
		...
		try {
			...
			if (response.data.success) {
				toast("Template enviado para análise", {
				...
				<div className="md:col-span-2">
					<Card>
						<CardHeader>
							...
							<CardDescription>Atualize os componentes do template</CardDescription>
						</CardHeader>
						<CardContent>
							<div className="space-y-6">
								{hasHeaderImage && (
									<div>
										...
										<FileUpload uploadedFiles={headerMedia} setUploadedFiles={setHeaderMedia} />
											...
											<img
												...
												onError={(e) => {
													(e.target as HTMLImageElement).src =
														"https://via.placeholder.com/400x200?text=Imagem+Inválida";
												}}
											/>
										</div>
									)}
											...
											<div className="mt-4 border-t pt-3 space-y-2">
												{formData.buttons.map((button: any, index: number) => (
													<div key={index} className="w-full">
														<button className="w-full py-2 px-3 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-center rounded-md text-sm transition">
															{button.texto || "Botão"}
														</button>
													</div>
												))}
