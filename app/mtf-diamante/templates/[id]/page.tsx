	...
	const handleTestSend = async () => {
		...
		try {
			...
			if (headerNamed.length > 0) {
				...
				const headerVal =
					typeof headerNamed[0].value !== "undefined" && headerNamed[0].value !== null
						? headerNamed[0].value
						: headerNamed[0].example || "";
			...
			if (bodyNumeric.length > 0) {
				params.bodyVars = bodyNumeric.map((v) =>
					typeof v.value !== "undefined" && v.value !== null ? v.value : v.example || "",
				);
			}
	...
	return (
		<div className="max-w-6xl mx-auto py-10 space-y-6">
			...
			<div className="flex items-center justify-between">
				<Link href="/mtf-diamante?tab=templates">
									...
									<div className="space-y-4">
										...
										{contactList.length > 0 && (
											<div className="space-y-2">
												...
												<Button onClick={handleMassSend} disabled={isMassSending} className="w-full">
													{isMassSending ? <Loader2 className="animate-spin h-4 w-4 mr-2" /> : null}
													Enviar para Todos
												</Button>
											</div>
										)}
