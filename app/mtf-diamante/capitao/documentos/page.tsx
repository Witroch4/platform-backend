...
export default function CaptainDocumentsPage() {
	...
	async function loadDocuments() {
		const url =
			selectedAssistantId === "all"
				? "/api/admin/ai-integration/documents"
				: `/api/admin/ai-integration/documents?assistantId=${selectedAssistantId}`;
	...
	async function handleCreate() {
		...
		const r = await fetch("/api/admin/ai-integration/documents", {
			method: "POST",
		...
		<div className="p-6 space-y-6">
			<div className="flex items-center justify-between">
				<div className="flex items-center gap-3">
					...
					<div>
						...
						<p className="text-sm text-muted-foreground">
							Cadastre guias/URLs para alimentar o conhecimento dos assistentes.
						</p>
					</div>
				</div>
				<Dialog open={open} onOpenChange={setOpen}>
					<DialogTrigger asChild>
						<Button>
							<Plus className="w-4 h-4 mr-2" /> Criar um novo documento
						</Button>
					</DialogTrigger>
						...
						<div className="space-y-4">
							...
							<div>
								...
								<Textarea
									...
									placeholder="Cole aqui o texto relevante"
									rows={6}
								/>
							</div>
							<div>
			...
			<div className="flex items-center gap-3">
				...
				<select
					...
					onChange={(e) => setSelectedAssistantId(e.target.value as any)}
				>
				...
				{documents.length === 0 ? (
					<div className="text-center py-16 border rounded-md">
						...
						<p className="text-sm text-muted-foreground mb-6">
							Os documentos são usados pelo seu assistente para gerar respostas.
						</p>
