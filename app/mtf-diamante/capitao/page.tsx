...
export default function CaptainAssistantsPage() {
	...
	const handleExport = async () => {
		...
		try {
			...
			if (response.ok) {
				...
				toast.success(
					`Exportação concluída! ${totalAssistants} assistentes, ${totalDocuments} documentos e ${totalFaqs} FAQs exportados.`,
				);
	...
	const handleImport = async () => {
		...
		try {
			...
			if (result.success) {
				toast.success(
					`Importação concluída! ${result.summary.importedAssistants} assistentes importados, ${result.summary.skippedAssistants} ignorados.`,
				);
				await loadAssistants(); // Reload assistants list
			} else if (response.status === 207) {
				// Multi-Status (partial success)
				toast.warning(
					`Importação parcial. ${result.summary.importedAssistants} importados, ${result.summary.errors.length} erros.`,
				);
			} else {
				...
				<div className="flex items-center justify-between">
					<div className="flex-1">
						<div className="rounded-md border border-dashed p-4 bg-muted/30 flex items-center gap-4">
							...
							<a
								href="/mtf-diamante/capitao/saiba-mais"
					...
					<div className="flex gap-2">
						...
						<Dialog open={open} onOpenChange={setOpen}>
							<DialogTrigger asChild>
								<Button>
									<Plus className="w-4 h-4 mr-2" /> Criar um novo assistente
								</Button>
							</DialogTrigger>
								...
								<div className="space-y-4">
									...
									<div className="space-y-3">
										<label className="flex items-center gap-2 text-sm">
											<input
												type="checkbox"
												checked={form.generateFaqs}
												onChange={(e) => setForm({ ...form, generateFaqs: e.target.checked })}
											/>
										...
										<label className="flex items-center gap-2 text-sm">
											<input
												type="checkbox"
												checked={form.captureMemories}
												onChange={(e) => setForm({ ...form, captureMemories: e.target.checked })}
											/>
										...
										<label className="flex items-center gap-2 text-sm">
											<input
												type="checkbox"
												checked={form.proposeHumanHandoff}
												onChange={(e) => setForm({ ...form, proposeHumanHandoff: e.target.checked })}
											/>
										...
										<label className="flex items-center gap-2 text-sm">
											<input
												type="checkbox"
												checked={form.disableIntentSuggestion}
												onChange={(e) => setForm({ ...form, disableIntentSuggestion: e.target.checked })}
											/>
											<span className="flex items-center gap-2">
									...
									<div>
										...
										<Textarea
											placeholder="Defina como este assistente deve agir. Ex.: classificar intenção e extrair entidades..."
											value={form.instructions}
											onChange={(e) => setForm({ ...form, instructions: e.target.value })}
											rows={6}
										/>
										<p className="text-xs text-muted-foreground mt-1">
											Este texto será injetado no prompt que decide a intenção e entidades.
										</p>
									</div>
									<div>
										...
										<select
											...
											value={form.intentOutputFormat}
											onChange={(e) => setForm({ ...form, intentOutputFormat: e.target.value as any })}
										>
											<option value="JSON">
				...
				{isInitialLoading ? (
					...
					<div className="text-center py-16 border rounded-md">
						...
						<p className="text-sm text-muted-foreground mb-6">
							Crie um assistente para responder seus clientes automaticamente. Ele pode aprender com seus documentos e
							conversas anteriores, e transferir para um atendente quando necessário.
						</p>
						...
						<div key={a.id} className="border rounded-md p-4 flex items-start justify-between">
							...
							<DropdownMenu>
								...
								<DropdownMenuContent align="end">
									<DropdownMenuItem onClick={() => router.push(`/mtf-diamante/capitao/${a.id}`)}>
										<Settings2 className="w-4 h-4 mr-2" /> Configurar prompt
									</DropdownMenuItem>
									<DropdownMenuItem onClick={() => router.push(`/mtf-diamante/capitao/${a.id}/inboxes`)}>
				...
				<DialogContent className="w-[96vw] sm:max-w-lg max-h-[85vh]">
					...
					<div className="space-y-4">
						...
						<Alert>
							...
							<AlertDescription>
								O arquivo exportado incluirá todos os assistentes com documentos, FAQs, versões de prompt, configurações
								SocialWise Flow e metadados.
							</AlertDescription>
						</Alert>
					</div>
					<div className="flex justify-end gap-2">
										...
										<div className="space-y-3">
											...
											<div className="flex items-center space-x-2">
												<input
													...
													onChange={(e) =>
														setImportOptions((prev) => ({ ...prev, importPromptVersions: e.target.checked }))
													}
												/>
								...
								<div className="space-y-4">
									<div className="flex items-start gap-2">
										...
										<div>
											...
											<p className="text-sm text-muted-foreground">Resultado do processamento dos assistentes</p>
										</div>
									</div>
												...
												<ul className="text-xs space-y-1">
													...
													{importResult.summary.errors.length > 5 && (
														<li className="text-muted-foreground">
															... e mais {importResult.summary.errors.length - 5} erros
														</li>
													)}
												</ul>
											</AlertDescription>
