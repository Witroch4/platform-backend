...
export function EspecialidadeSelector({
	...
	const [selectedEspecialidade, setSelectedEspecialidade] = useState<string>(especialidadeAtual || "");
	...
	if (especialidadeAtual && !disabled) {
		return (
			<div className="flex items-center gap-2">
				...
				<Button variant="ghost" onClick={() => setSelectedEspecialidade("")} className="h-6 px-2 text-xs">
					Alterar
				</Button>
			</div>
		);
	...
	return (
		<div className="flex items-center gap-2 w-full max-w-xs">
			<Select value={selectedEspecialidade} onValueChange={setSelectedEspecialidade} disabled={disabled || isUpdating}>
				<SelectTrigger className="text-xs h-8">
					<SelectValue placeholder="Selecionar especialidade..." />
				</SelectTrigger>
				<SelectContent>
					{especialidades.map((especialidade) => (
						<SelectItem key={especialidade.value} value={especialidade.value} className="text-xs">
							{especialidade.label}
						</SelectItem>
					))}
				</SelectContent>
			...
			{selectedEspecialidade && selectedEspecialidade !== especialidadeAtual && (
				<Button
					...
					className="h-8 px-2"
				>
					{isUpdating ? "Salvando..." : <Save className="h-3 w-3" />}
				</Button>
			)}
		</div>
	);
