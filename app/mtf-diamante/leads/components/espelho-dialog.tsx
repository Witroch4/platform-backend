...
function StudentMirrorViewer({ payload }: { payload: StudentMirrorPayload }) {
	...
	const pieceItems = useMemo(() => safePayload.itens.filter((item) => item.escopo === "Peça"), [safePayload.itens]);
							...
							<dd className="text-right text-foreground">
								...
								{safePayload.aluno.nota_final_raw && (
									<span className="ml-2 text-xs text-muted-foreground">
										(Imagem: {safePayload.aluno.nota_final_raw})
									</span>
								)}
							</dd>
						</div>
						...
						<div key={label} className="rounded border bg-background p-3">
							...
							{(typeof data.maximo_rubrica === "number" || typeof data.obtido_rubrica === "number") && (
								<div className="mt-2 text-xs text-muted-foreground">
									<div>
										Rubrica: {formatScoreOptional(data.obtido_rubrica)} / {formatScoreOptional(data.maximo_rubrica)}
									</div>
								</div>
							)}
						</div>
...
export function EspelhoDialog({
	...
	useEffect(() => {
		if (isOpen) {
			...
			const hasText =
				textoEspelho &&
				(typeof textoEspelho === "string"
					? textoEspelho.trim().length > 0
					: Array.isArray(textoEspelho)
						? textoEspelho.length > 0
	...
	const handleGenerateTextFromImages = async (imageUrls: string[]) => {
		try {
			...
			const payload = {
				...
				arquivos:
					leadData.arquivos?.map((a: { id: string; dataUrl: string; fileType: string }) => ({
						id: a.id,
						url: a.dataUrl,
						tipo: a.fileType,
						nome: a.fileType,
	...
	const handleCopyStructuredJson = async () => {
		...
		try {
			...
			toast.success("JSON copiado", {
				description: "Conteúdo estruturado disponível na área de transferência",
				duration: 2000,
			});
		} catch (error: any) {
					...
					<DialogHeader className="shrink-0">
						<div className="flex items-center justify-between">
							<div className="flex items-center gap-2">
								...
								<DialogTitle>{batchMode ? "Espelho em Lote" : "Editar Espelho de Correção"}</DialogTitle>
							</div>
							{batchMode && batchInfo && (
						...
						<DialogDescription>
							{batchMode && batchInfo ? (
								<div className="space-y-2">
									...
									<div className="text-xs text-muted-foreground">
										💡 Selecione as imagens que serão usadas como espelho de correção. O texto pode ser gerado
										automaticamente após selecionar as imagens.
									</div>
								</div>
							) : (
					...
					<div className="flex-1 px-1 py-4 space-y-4">
						{aguardandoEspelho ? (
							<div className="flex flex-col items-center justify-center py-8 min-h-[300px]">
								...
								{onCancelarEspelho && (
									<Button variant="destructive" onClick={handleCancelarEspelho} disabled={isCancelando}>
										{isCancelando ? (
											<>
							...
							<>
								...
								<div className="flex justify-between items-center">
									...
									<div className="flex gap-1 sm:gap-2 flex-wrap">
										<Button variant="outline" onClick={handleOpenImageGallery} className="text-xs sm:text-sm">
										...
										{imagens.length > 0 ? (
											imagens.map((url, index) => (
												<div key={index} className="border rounded-md overflow-hidden h-24 sm:h-32">
													<img src={url} alt={`Espelho ${index + 1}`} className="w-full h-full object-contain" />
												</div>
											))
										) : (
							...
							<>
								...
								{!aguardandoEspelho &&
									(canEditContent ? (
										...
										<Button variant="outline" onClick={handleCopyStructuredJson} disabled={!structuredPayload}>
			...
			<Dialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							Deseja enviar as {pendingImages.length} imagem(ns) selecionada(s) para o sistema externo gerar o texto do
