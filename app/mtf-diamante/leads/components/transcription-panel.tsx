...
import { Loader2, CheckCircle2, XCircle, Clock, FileText, Minimize2, Maximize2, X } from "lucide-react";
...
export function TranscriptionPanel({ transcriptions, onViewDetails, onDismiss, onClose }: TranscriptionPanelProps) {
	const [isMinimized, setIsMinimized] = useState(false);
	const [isVisible, setIsVisible] = useState(true);

	const activeTranscriptions = transcriptions.filter((t) => t.status === "queued" || t.status === "processing");
			...
			<Card className="shadow-lg border-2">
				<CardHeader className="pb-3 flex flex-row items-center justify-between space-y-0">
					<div className="flex items-center gap-2">
						...
						<CardTitle className="text-base">Digitações ({transcriptions.length})</CardTitle>
					</div>
							...
							<div className="space-y-3">
								...
								{activeTranscriptions.length > 0 && (
									<div className="space-y-2">
										<h4 className="text-xs font-semibold text-muted-foreground uppercase">Em andamento</h4>
								...
								{completedTranscriptions.length > 0 && (
									<div className="space-y-2">
										<h4 className="text-xs font-semibold text-muted-foreground uppercase">Concluídas</h4>
								...
								{failedTranscriptions.length > 0 && (
									<div className="space-y-2">
										<h4 className="text-xs font-semibold text-destructive uppercase">Com erro</h4>
...
function TranscriptionItem({ transcription, onViewDetails, onDismiss }: TranscriptionItemProps) {
		...
		>
			<div className="flex items-start gap-2">
				...
				<div className="flex-1 min-w-0 space-y-1.5">
								...
								<div className="flex items-center justify-between text-xs text-muted-foreground">
									...
									{estimatedTimeRemaining && (
										<span className="flex items-center gap-1">
											<Clock className="h-3 w-3" />~{formatTime(estimatedTimeRemaining)}
										</span>
									)}
								</div>
					...
					{status === "failed" && error && <p className="text-xs text-destructive line-clamp-2">{error}</p>}

					<div className="flex items-center gap-1 pt-1">
						{onViewDetails && (
							<Button variant="ghost" size="sm" className="h-7 text-xs" onClick={() => onViewDetails(leadID)}>
								Ver detalhes
							</Button>
						)}
						{onDismiss && status !== "processing" && (
							<Button variant="ghost" size="sm" className="h-7 text-xs" onClick={() => onDismiss(leadID)}>
								Dispensar
							</Button>
						)}
					</div>
