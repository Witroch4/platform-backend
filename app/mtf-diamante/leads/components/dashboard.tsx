...
export function LeadsDashboard({ isOpen, refreshCounter, period }: DashboardProps) {
	...
	const totalLeadsCanais = chartData.leadsPorCanal.reduce((total: number, item: any) => total + item.leads, 0);
		...
		<div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-4 bg-background">
				...
				<CardFooter>
					<div className="flex w-full items-start gap-2 text-sm">
						<div className="grid gap-2">
							...
							<div className="flex items-center gap-2 leading-none text-muted-foreground">Últimos 6 meses</div>
						</div>
					</div>
				</CardFooter>
			</Card>
			...
			<Card className="flex flex-col border-border bg-card">
					...
					{isLoading ? (
						...
						>
							<PieChart>
								...
								/>
								<Pie data={chartData.leadsPorCanal} dataKey="leads" nameKey="channel" innerRadius={60} strokeWidth={5}>
									<Label
										content={({ viewBox }) => {
				...
				<CardFooter className="flex-col gap-2 text-sm">
					...
					<div className="leading-none text-muted-foreground">Distribuição de onde vêm os leads</div>
				</CardFooter>
			</Card>
		</div>
	);
