...
import { DialogDetalheLead } from "@/app/mtf-diamante/leads/components/dialog-detalhe-lead";
import { ImageGalleryDialog } from "@/app/mtf-diamante/leads/components/image-gallery-dialog";
import { ProcessDialog, type ProcessType } from "@/app/mtf-diamante/leads/components/process-dialog";
import { ProvaDialog } from "@/app/mtf-diamante/leads/components/prova-dialog";
import { EspelhoDialog } from "@/app/mtf-diamante/leads/components/espelho-dialog";
import { AnaliseDialog } from "@/app/mtf-diamante/leads/components/analise-dialog";
import { AnalisePreviewDrawer } from "@/app/mtf-diamante/leads/components/analise-preliminar-drawer";
import { RecursoDialog } from "@/app/mtf-diamante/leads/components/recurso-dialog";
			...
			<EspelhoDialog
				...
				imagensEspelho={(() => {
					...
					if (localEspelhoState.espelhoCorrecao && localEspelhoState.espelhoCorrecao !== "[]") {
						try {
							const result =
								typeof localEspelhoState.espelhoCorrecao === "string"
			...
			<Dialog open={confirmDeleteEspelho} onOpenChange={setConfirmDeleteEspelho}>
				<DialogContent>
					<DialogHeader>
						<DialogTitle>Confirmar exclusão</DialogTitle>
						<DialogDescription>
							Tem certeza que deseja excluir completamente o espelho de correção do lead "{displayName}"? Esta ação irá
							remover tanto o texto quanto as imagens do espelho e não pode ser desfeita.
						</DialogDescription>
					</DialogHeader>
					<DialogFooter>
						...
						<div className="space-y-2 mt-4">
							<div className="text-sm bg-muted/50 p-3 rounded-md border-l-4 border-destructive/20">
								...
								<ul className="text-muted-foreground space-y-1 list-disc list-inside">
									...
									{((lead.espelhoCorrecao && lead.espelhoCorrecao !== "[]") || lead.textoDOEspelho) && (
										<li>Espelho de correção individual</li>
									)}
									{(localAnaliseState.analiseUrl ||
										localAnaliseState.analisePreliminar ||
										localAnaliseState.aguardandoAnalise) && <li>Análise da prova</li>}
								</ul>
