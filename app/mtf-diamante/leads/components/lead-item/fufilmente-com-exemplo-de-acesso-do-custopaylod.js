...
const configWhatsApp = {
	headers: {
		Authorization:
			"Bearer EAAGIBII4GXQBO2qgvJ2jdcUmgkdqBo5bUKEanJWmCLpcZAsq0Ovpm4JNlrNLeZAv3OYNrdCqqQBAHfEfPFD0FPnZAOQJURB9GKcbjXeDpa83XdAsa3i6fTr23lBFM2LwUZC23xXrZAnB8QjCCFZBxrxlBvzPj8LsejvUjz0C04Q8Jsl8nTGHUd4ZBRPc4NiHFnc",
...
async function handleWelcome(req, res, to) {
	...
	if (snapshot.exists()) {
		...
		await enviarTemplateWhatsApp(
			to,
			"menu_novo",
			"https://amandasousaprev.adv.br/wp-content/uploads/2024/04/logo_quad_quad.png",
		);
...
async function handleIdentificacao(req, res, to) {
	...
	if (!nome) {
		...
		res.send({
			fulfillmentMessages: [{ text: { text: ["Desculpe, não consegui identificar seu nome. Poderia repetir?"] } }],
		});
		return;
	}
...
const intentHandlers = {
	"BPC-LOAS": (req, res, to) =>
		enviarTemplateWhatsApp(to, "bpc_loas", "https://amandasousaprev.adv.br/wp-content/uploads/2024/04/bpc-loas.jpg"),
	"confirmação.nome.menu": (req, res, to) =>
		enviarTemplateWhatsApp(
			to,
			"menu_novo",
			"https://amandasousaprev.adv.br/wp-content/uploads/2024/04/logo_quad_quad.png",
		),
	maternidade: (req, res, to) =>
		enviarTemplateWhatsApp(
			to,
			"maternidade_novo",
			"https://amandasousaprev.adv.br/wp-content/uploads/2024/04/salario-maternidade.jpg",
		),
	invalidez: (req, res, to) =>
		enviarTemplateWhatsApp(to, "invalidez", "https://amandasousaprev.adv.br/wp-content/uploads/2024/04/2.jpg"),
	auxilio: (req, res, to) =>
		enviarTemplateWhatsApp(to, "auxilio", "https://amandasousaprev.adv.br/wp-content/uploads/2024/04/auxilio.jpg"),
	"consulta.juridica": (req, res, to) =>
		enviarTemplateWhatsApp(
			to,
			"consulta_juridica",
			"https://amandasousaprev.adv.br/wp-content/uploads/2024/04/atendimento-wp.jpg",
		),
	atendimentoexclusivo: (req, res, to) =>
		enviarTemplateWhatsApp(
			to,
			"falar_com_atendente",
			"https://amandasousaprev.adv.br/wp-content/uploads/2024/04/logo_quad_quad.png",
				...
				fulfillmentMessages: [
					{
						text: {
							text: [
								"Ocorreu um erro inesperado. Nossa equipe já foi notificada. Por favor, tente novamente mais tarde.",
							],
						},
					},
				],
