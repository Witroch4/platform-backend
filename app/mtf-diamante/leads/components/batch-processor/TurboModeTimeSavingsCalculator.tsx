...
import { Clock, TrendingUp, Calculator, Zap, ArrowRight, Timer } from "lucide-react";
...
export function TurboModeTimeSavingsCalculator({
	...
	const formatTimeDetailed = (seconds: number): string => {
		...
		if (seconds < 3600) {
			const minutes = Math.floor(seconds / 60);
			const remainingSeconds = seconds % 60;
			return remainingSeconds > 0 ? `${minutes} minutos e ${remainingSeconds} segundos` : `${minutes} minutos`;
		}
		const hours = Math.floor(seconds / 3600);
		const minutes = Math.floor((seconds % 3600) / 60);
		return minutes > 0 ? `${hours} horas e ${minutes} minutos` : `${hours} horas`;
	};
	...
	const timeSavingsPercentage =
		sequentialTotalTime > 0 ? Math.round((estimatedTimeSavings / sequentialTotalTime) * 100) : 0;

	// Real-time calculations based on current progress
	const estimatedRemainingTime =
		currentProgress > 0 ? Math.round(realTimeElapsed / (currentProgress / 100) - realTimeElapsed) : turboEstimatedTime;
	...
	const realTimeSavingsPercentage =
		sequentialTotalTime > 0 ? Math.round((realTimeSavings / sequentialTotalTime) * 100) : 0;
				...
				<Card className="border-gray-200">
					...
					<CardContent className="space-y-3">
						<div className="text-center">
							<div className="text-2xl font-bold text-gray-700">{formatTime(sequentialTotalTime)}</div>
							<div className="text-xs text-muted-foreground">Tempo total estimado</div>
						</div>
				...
				<Card className="border-blue-200 bg-blue-50">
					...
					<CardContent className="space-y-3">
						<div className="text-center">
							...
							<div className="text-xs text-blue-600">{currentProgress > 0 ? "Tempo projetado" : "Tempo estimado"}</div>
						</div>
				...
				<CardContent className="p-6">
					...
					<div className="mt-4 pt-4 border-t border-green-200">
						<div className="text-sm text-green-700 text-center">
							<strong>Economia detalhada:</strong>{" "}
							{formatTimeDetailed(currentProgress > 0 ? realTimeSavings : estimatedTimeSavings)}
						</div>
					</div>
				</CardContent>
			</Card>
				...
				<Card>
					...
					<CardContent className="space-y-4">
						...
						<div className="flex items-center justify-center gap-2 text-sm text-muted-foreground">
							...
							<span className="text-green-600 font-medium">(economia: {formatTime(realTimeSavings)})</span>
						</div>
					</CardContent>
				</Card>
			)}
				...
				<CardContent>
					<div className="space-y-3">
						...
						<div className="flex items-center justify-between p-3 bg-green-50 rounded-lg">
							...
							<span className="font-bold text-green-600">{Math.round((1 - 1 / config.maxParallelLeads) * 100)}%</span>
						</div>
