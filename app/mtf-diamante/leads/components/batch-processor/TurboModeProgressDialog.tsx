...
import { Loader2, Zap, Clock, TrendingUp, AlertTriangle, CheckCircle2, Activity, Timer, BarChart3 } from "lucide-react";
									...
									<div className="flex items-center justify-between">
										<div className="flex items-center gap-3">
											...
											<div>
												...
												{operation.error && <div className="text-xs text-red-600 mt-1">Erro: {operation.error}</div>}
											</div>
										</div>
						...
						<Card className="p-4 bg-yellow-50 border-yellow-200">
							<div className="flex items-start gap-3">
								...
								<div className="flex-1">
									<div className="font-medium text-yellow-800 mb-1">Erros Detectados no Processamento TURBO</div>
									<div className="text-sm text-yellow-700 mb-3">
										{errorOperations} de {operations.length} operações falharam. O sistema pode continuar com
										processamento sequencial para as operações com erro.
									</div>
									<button
		...
		<div className={`bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg p-3 border border-blue-200 ${className}`}>
			...
			<div className="flex justify-between text-xs text-gray-600">
				...
				<div className="flex gap-2">
					{processingOperations > 0 && <span className="text-blue-600">{processingOperations} processando</span>}
					{errorOperations > 0 && <span className="text-red-600">{errorOperations} erros</span>}
				</div>
			</div>
		</div>
	);
