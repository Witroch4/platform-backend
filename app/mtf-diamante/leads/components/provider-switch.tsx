		...
		<TooltipProvider>
			<Tooltip>
					...
					>
						...
						<div
							className={cn(
								...
								!isGemini ? "bg-emerald-500/20 ring-2 ring-emerald-500/50 scale-110" : "opacity-40 hover:opacity-60",
							)}
						>
							<Image
								...
								width={s.icon}
								height={s.icon}
								className={cn("transition-all duration-200", !isGemini ? "drop-shadow-sm" : "grayscale")}
							/>
						</div>
						...
						<div
							className={cn(
								...
								s.switch,
								isGemini ? "bg-blue-500/30" : "bg-emerald-500/30",
							)}
						>
							{/* Toggle indicator */}
							<span
								className={cn(
									...
									s.toggle,
									isGemini ? `${s.translate} bg-blue-500` : "translate-x-0.5 bg-emerald-500",
								)}
							/>
						</div>
						...
						<div
							className={cn(
								"flex items-center justify-center rounded-md p-0.5 transition-all duration-200",
								isGemini ? "bg-blue-500/20 ring-2 ring-blue-500/50 scale-110" : "opacity-40 hover:opacity-60",
							)}
						>
							<Image
				...
				<TooltipContent side="bottom" className="text-xs">
					...
					<p className="text-muted-foreground">Clique para usar {isGemini ? "GPT" : "Gemini"}</p>
				</TooltipContent>
			</Tooltip>
		</TooltipProvider>
	);
...
export function ProviderSwitchHeader({
	...
	return (
		<div className="flex flex-col items-center gap-0.5">
			<ProviderSwitch column={column} defaultProvider={defaultProvider} onProviderChange={onProviderChange} size="sm" />
 ...
 */
export function useColumnProvider(
	column: LinkedColumnType,
 ...
 */
export function getColumnProvider(
	column: LinkedColumnType,
	defaultProvider: AiProviderType = "GEMINI",
): AiProviderType {
