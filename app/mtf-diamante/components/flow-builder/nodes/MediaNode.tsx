...
export const MediaNode = memo(({ id, data, selected }: MediaNodeProps) => {
	...
	useEffect(() => {
		if (data.mediaType !== mediaType) {
			setNodes((nodes) =>
				nodes.map((node) => (node.id === id ? { ...node, data: { ...node.data, mediaType } } : node)),
			);
		}
	...
	const handleCaptionChange = useCallback(
		(value: string) => {
			setCaption(value);
			setNodes((nodes) =>
				nodes.map((node) => (node.id === id ? { ...node, data: { ...node.data, caption: value } } : node)),
			);
		},
		[id, setNodes],
	);
	...
	return (
		<NodeContextMenu onDuplicate={handleDuplicate} onDelete={handleDelete}>
			...
			>
				...
				<Handle type="target" position={Position.Top} className="!h-3 !w-3 !bg-teal-500 !border-2 !border-white" />
				...
				<div className="flex items-center gap-2 border-b px-3 py-2 bg-teal-100/50 dark:bg-teal-900/30">
					...
					<span className="font-medium text-sm">{data.label || "Enviar Mídia"}</span>
				</div>
							...
							<div className="relative group">
								...
								{mediaType === "video" && data.mediaUrl && (
									<div className="relative aspect-video bg-black rounded-md overflow-hidden">
										<video src={data.mediaUrl} className="w-full h-full object-contain" controls={false} muted />
								...
								{mediaType === "document" && (
									<div className="flex items-center gap-3 p-3 bg-muted/50 rounded-md">
										...
										<div className="flex-1 min-w-0">
											<p className="text-sm font-medium truncate">{data.filename || "documento"}</p>
											<p className="text-xs text-muted-foreground uppercase">{extension || "PDF"}</p>
										</div>
									</div>
								)}
								...
								{mediaType === "audio" && (
									<div className="flex items-center gap-3 p-3 bg-muted/50 rounded-md">
										...
										<div className="flex-1 min-w-0">
											<p className="text-sm font-medium truncate">{data.filename || "áudio"}</p>
											<p className="text-xs text-muted-foreground uppercase">{extension || "AUDIO"}</p>
										</div>
									</div>
								)}
				...
				<Handle type="source" position={Position.Bottom} className="!h-3 !w-3 !bg-teal-500 !border-2 !border-white" />
			</div>
		</NodeContextMenu>
	);
});
