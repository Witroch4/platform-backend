...
export const DelayNode = memo(({ id, data, selected }: DelayNodeProps) => {
	...
	const updateNodeData = useCallback(
		(seconds: number) => {
			...
			setNodes((nodes) =>
				nodes.map((node) =>
					node.id === id ? { ...node, data: { ...node.data, delaySeconds: clampedSeconds, isConfigured: true } } : node,
				),
			);
		},
		[id, setNodes],
	...
	return (
		<NodeContextMenu onDuplicate={handleDuplicate} onDelete={handleDelete}>
			...
			>
				...
				<Handle type="target" position={Position.Top} className="!h-3 !w-3 !bg-cyan-500 !border-2 !border-white" />
				...
				<div className="flex items-center gap-2 border-b px-3 py-2 bg-cyan-100/50 dark:bg-cyan-900/30">
					...
					<span className="font-medium text-sm">{data.label || "Esperar"}</span>
				</div>
				...
				<div className="px-3 py-4">
					<div className="flex items-center justify-center gap-3">
						...
						<div className="flex flex-col items-center min-w-[80px]">
							<span className="text-3xl font-bold text-cyan-700 dark:text-cyan-300 tabular-nums">{localSeconds}</span>
							<span className="text-xs text-muted-foreground mt-1">{localSeconds === 1 ? "segundo" : "segundos"}</span>
						</div>
				...
				<Handle type="source" position={Position.Bottom} className="!h-3 !w-3 !bg-cyan-500 !border-2 !border-white" />
			</div>
		</NodeContextMenu>
	);
});
