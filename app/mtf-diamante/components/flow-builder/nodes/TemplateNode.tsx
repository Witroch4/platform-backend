 ...
 */
export const TemplateNode = memo(({ id, data, selected }: TemplateNodeProps) => {
	...
	const quickReplyButtons = useMemo(() => buttons.filter((b) => b.type === "QUICK_REPLY"), [buttons]);
	...
	return (
		<NodeContextMenu onDuplicate={handleDuplicate} onDelete={handleDelete}>
			...
			>
				...
				<div className="p-3 bg-slate-50 dark:bg-slate-950/20 min-h-[80px]" onDoubleClick={stopPropagation}>
					{isConfigured ? (
						<div className="flex flex-col gap-2">
							...
							{data.header && data.header.type !== "NONE" && (
								<div className="rounded-md border bg-white dark:bg-card px-3 py-2 shadow-sm">
									...
									{(data.header.type === "IMAGE" || data.header.type === "VIDEO") && (
										<div className="flex items-center gap-2 text-xs text-muted-foreground">
											{data.header.type === "IMAGE" ? "🖼️" : "🎬"}
											<span className="truncate">{data.header.mediaUrl || "Mídia não configurada"}</span>
										</div>
									)}
							...
							{data.footer?.text && (
								<div className="rounded-md border bg-white dark:bg-card px-3 py-2 shadow-sm">
									<p className="text-xs text-muted-foreground italic truncate">{data.footer.text}</p>
								</div>
							)}
									...
									{buttons.map((btn) => (
										<div key={btn.id} className="relative group">
											<div className="rounded-md border bg-white dark:bg-card px-3 py-2 shadow-sm flex items-center justify-center gap-2">
												...
												<span className="text-sm font-semibold text-emerald-600 dark:text-emerald-400">{btn.text}</span>
						...
						<div className="p-4 text-center border-2 border-dashed border-border/40 rounded-lg">
							...
							<p className="text-xs text-muted-foreground/70 mb-1">Duplo clique para configurar</p>
							<p className="text-[10px] text-muted-foreground/50">Importe um template aprovado ou crie um novo</p>
						</div>
					)}
				</div>
