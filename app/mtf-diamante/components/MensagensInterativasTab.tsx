...
import { TrashIcon, PencilIcon, Upload, Download, Plus, MessageSquare } from "lucide-react";
...
const normalizeMessage = (m: AnyMsg): Mensagem & { type?: InteractiveMessageType } => {
	...
	const header = m.header ?? m.content?.header ?? m.interactiveContent?.header ?? null;

	const footer = m.footer ?? m.content?.footer ?? m.interactiveContent?.footer ?? null;

	const action = m.action ?? m.content?.action ?? m.interactiveContent?.action ?? null;
	...
	if (Array.isArray(m.content?.action?.buttons)) candidates.push(...m.content.action.buttons);
	if (Array.isArray(m.content?.action?.quick_replies)) candidates.push(...m.content.action.quick_replies);
	if (Array.isArray(m.actionReplyButton?.buttons)) candidates.push(...m.actionReplyButton.buttons);
	...
	const nome = pick(m.name, m.nome, m.title, m.titulo, m.displayName, m.content?.name, m.content?.titulo) || "";
	...
	if (m.type === "generic" && elements.length > 0) {
		...
		const titulos = elements
			.slice(0, 2)
			.map((el) => el.title || el.subtitle)
			.filter(Boolean);
		if (titulos.length > 0) {
			texto += ` - ${titulos.join(", ")}`;
		...
		texto =
			pick(m.body?.text, m.texto, m.content?.body?.text, m.interactiveContent?.body?.text, m.message, m.description) ||
			"";
	}

	const headerTipo = header?.type ?? m.headerTipo ?? null;

	const headerConteudo =
		pick(header?.content, (header as any)?.text, (header as any)?.media_url, m.headerConteudo) ?? null;
	...
	return {
		...
		type:
			(m.type as InteractiveMessageType) ||
			(m.content?.type as InteractiveMessageType) ||
			(() => {
...
const MensagensInterativasTab = ({ caixaId }: MensagensInterativasTabProps) => {
	...
	const mensagens = useMemo<Mensagem[]>(() => (interactiveMessages ?? []).map(normalizeMessage), [interactiveMessages]);
	...
	const channelType = useMemo(() => caixas?.find((c: any) => c.id === caixaId)?.channelType ?? null, [caixas, caixaId]);
		...
		if (detectedType === "cta_url") {
			...
			base.action = {
				...
				buttons: (buttonsSrc || []).map((b: any) => ({
					...
					reply:
						b.reply ||
						(b.title || b.titulo ? { id: b.id || `btn_${Date.now()}`, title: b.title || b.titulo } : undefined),
	...
	const mapMensagemToPayload = (msg: Mensagem) => {
		const header = msg.headerTipo
			? {
					type: msg.headerTipo === "text" ? "text" : msg.headerTipo,
					content: msg.headerTipo === "text" ? msg.headerConteudo || "" : msg.headerConteudo || "",
					media_url: msg.headerTipo !== "text" ? msg.headerConteudo || "" : undefined,
				}
			: undefined;
		...
		const action = hasButtons
			? {
					buttons: msg.botoes?.map((b, i) => ({
						id: b.id || `btn_${Date.now()}_${i}`,
						title: (b as any).title || b.titulo || "",
					})),
				}
			: undefined;
	...
	const handleImportFileChange = async (file: File | null) => {
		...
		try {
			...
			const messages = Array.isArray(json) ? json : Array.isArray(json.messages) ? json.messages : [];
			...
			const valid = messages.filter(
				(m: any) => m && typeof m.name === "string" && m.body && typeof m.body.text === "string",
			);
			if (valid.length === 0) {
	...
	const regenerateButtonIds = (action: any, prefix: string) => {
		...
		const newButtons = action.buttons.map((btn: any, idx: number) => {
			...
			return {
				...
				id: newId,
				reply: btn.reply
					? { ...btn.reply, id: newId, title: btn.reply.title || btn.title || btn.titulo || "" }
					: undefined,
				payload: newId,
			};
		});
	...
	const handleImportNow = async () => {
		...
		try {
			...
			const prefix =
				channelType === "Channel::Instagram" ? "ig_" : channelType === "Channel::FacebookPage" ? "fb_" : "";
	...
	if (currentView === "create" || currentView === "edit") {
		return (
			<div className="space-y-4 sm:space-y-6">
				...
				<InteractiveMessageCreator inboxId={caixaId} onSave={handleSaveMessage} editingMessage={editingMessage} />
			</div>
		);
	}
		...
		<div className="space-y-4 sm:space-y-6">
			...
			<div className="flex flex-col gap-3 sm:gap-4">
				<div className="flex items-start gap-2 sm:gap-3">
					...
					<div className="min-w-0 flex-1">
						<h2 className="text-lg sm:text-xl lg:text-2xl font-bold text-foreground">Mensagens Interativas</h2>
						<p className="text-xs sm:text-sm text-muted-foreground line-clamp-2">
			...
			<Card className="border-border">
				<CardHeader className="px-3 sm:px-6 py-3 sm:py-4">
					...
					<CardDescription className="text-xs sm:text-sm">Gerencie suas mensagens interativas salvas</CardDescription>
				</CardHeader>
										...
										<div className="flex-1 min-w-0">
											<div className="flex items-start sm:items-center gap-2 mb-2 flex-wrap">
												...
												<Badge variant="outline" className="text-[10px] sm:text-xs shrink-0">
													...
													{((msg.botoes ?? []).length || 0) > 0 ? `${(msg.botoes ?? []).length || 0} BOTÕES` : "TEXTO"}
												</Badge>
											</div>
											...
											{((msg.botoes ?? []).length || 0) > 0 && (
												<div className="flex gap-1 flex-wrap">
													{(msg.botoes ?? []).map((botao, idx) => {
														const botaoReaction = buttonReactions.find(
															(reaction) => reaction.buttonId === botao.id && reaction.action === "handoff",
														);
														return (
						...
						{!loading && ((mensagens ?? []).length || 0) === 0 && (
							<div className="text-center text-muted-foreground py-12">
								...
								<p className="text-sm mb-4">
									Crie sua primeira mensagem interativa com botões, listas e mais funcionalidades
								</p>
								<Button onClick={() => setCurrentView("create")} variant="outline">
									<Plus className="h-4 w-4 mr-2" />
			...
			<Dialog open={importDialogOpen} onOpenChange={setImportDialogOpen}>
				<DialogContent className="w-[96vw] sm:max-w-2xl max-h-[85vh]">
					...
					<DialogFooter>
						<Button variant="ghost" onClick={() => setImportDialogOpen(false)} disabled={importing}>
							Cancelar
						</Button>
						<Button
							onClick={handleImportNow}
							disabled={importing || parsedMessages.length === 0 || selectedIndexes.size === 0}
							className="gap-2"
						>
