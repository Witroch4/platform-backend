...
export function ButtonEmojiMapper({
	...
	const updateReaction = (buttonId: string, updates: Partial<ButtonReaction>) => {
		const updatedReactions = reactions.map((reaction) =>
			reaction.buttonId === buttonId ? { ...reaction, ...updates } : reaction,
		);
			...
			<CardContent className="space-y-4">
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4 pl-4">
							...
							<div className="space-y-2">
								...
								<Input
									...
									onChange={(e) =>
										updateReaction(reaction.buttonId, {
								...
								/>
								{reaction.emoji && (
									<p className="text-xs text-muted-foreground">Desative o emoji para usar reação de texto</p>
								)}
							</div>
						</div>
				...
				{showSaveButton && messageId && (
					<div className="flex justify-end pt-4 border-t">
						<Button onClick={saveReactions} disabled={saving} className="min-w-[120px]">
							{saving ? (
								<>
