...
export const UnifiedEditingStep: React.FC<UnifiedEditingStepProps> = ({
	...
	React.useEffect(() => {
		...
		const uniqueFinalButtons = finalButtons.filter((button, index) => {
			if (seenIds.has(button.id)) {
				if (process.env.NODE_ENV === "development") {
					console.error(
						`🚨 [UnifiedEditingStep] DUPLICATE ID DETECTED: ${button.id} at index ${index}. This should NOT happen!`,
					);
				}
	...
	const handleNext = useCallback(async () => {
		...
		if (messageForValidation.type === "generic") {
			const elements = (messageForValidation.action?.elements ||
				messageForValidation.action?.action?.elements ||
				[]) as any[];
