...
import { Plus, X, Image, ExternalLink, Grip, ChevronDown, ChevronUp } from "lucide-react";
	...
	const carouselElements = React.useMemo(() => {
		if (message.type === "generic") {
			...
			return elements.map((el: any, index: number) => ({
				...
				buttons:
					el.buttons?.map((btn: any, btnIndex: number) => ({
						...btn,
						id:
							btn.id ||
							btn.payload ||
							generatePrefixedId(channelType || null, `btn_${index}_${btnIndex}_${Date.now()}`),
					})) || [],
			}));
		}
		return [];
	...
	if (message.type !== "generic") {
		return (
			<Card>
				...
				<CardContent>
					<Button onClick={initializeCarousel} disabled={disabled} className="w-full">
		...
		<Card>
			<CardHeader>
				<div className="flex items-center justify-between">
					...
					{carouselElements.length < 10 && (
						<Button onClick={addElement} disabled={disabled}>
			...
			<CardContent className="space-y-4">
				<DndContext sensors={sensors} onDragEnd={handleDragEnd}>
					<SortableContext
						items={carouselElements.map((el: any, idx: number) => el.id || String(idx))}
									...
									<CardHeader className="pb-2">
										<div className="flex items-center justify-between">
											<div className="flex items-center gap-2">
												...
												<Button variant="ghost" onClick={() => toggleElementExpansion(index)} className="h-6 w-6 p-0">
										...
										<CardContent className="pt-0 space-y-3">
											...
											<div className="space-y-1">
												...
												<Input
													...
													className="text-sm"
												/>
												<div className="text-xs text-muted-foreground text-right">{element.title.length}/80</div>
											</div>
