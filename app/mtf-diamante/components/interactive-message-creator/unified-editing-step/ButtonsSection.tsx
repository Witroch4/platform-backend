	...
	return (
		<Card>
			<CardHeader className="pb-4">
				...
				<p className="text-sm text-muted-foreground">Add up to {getMaxButtons} interactive buttons</p>
					...
					<div className="flex items-start gap-2 p-3 bg-blue-50 dark:bg-blue-950/50 rounded-lg border border-blue-200 dark:border-blue-800">
						...
						<div className="text-sm">
							<p className="font-medium text-blue-900 dark:text-blue-100">
								Tipos de Botão Suportados (Instagram/Facebook):
							</p>
						...
						onEmojiSelect={(value) => {
							if (value === "TEXT_RESPONSE") {
								...
								onReactionChange({
									...
									reaction: { type: "action", value: "handoff" },
								} as any);
							} else if (value.startsWith("send_template:")) {
								const templateId = value.replace("send_template:", "");
								onReactionChange({
									buttonId: reactionConfigButton,
									reaction: { type: "action", value: `send_template:${templateId}` },
								} as any);
								console.log(
									`📋 [ButtonsSection] Template configurado: ${templateId} para botão: ${reactionConfigButton}`,
								);
							} else if (value.startsWith("send_interactive:")) {
								const messageId = value.replace("send_interactive:", "");
								onReactionChange({
									buttonId: reactionConfigButton,
									reaction: { type: "action", value: `send_interactive:${messageId}` },
								} as any);
								console.log(
									`🔗 [ButtonsSection] Mensagem interativa configurada: ${messageId} para botão: ${reactionConfigButton}`,
								);
							} else {
								// É um emoji normal
