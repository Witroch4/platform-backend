...
export const PreviewStep: React.FC<PreviewStepProps> = ({
	...
	const generatePreviewComponents = () => {
		...
		if (message.type === "button" && message.action?.type === "button") {
			...
			components.push({
				type: "buttons",
				buttons: [{ type: "LIST", text: message.action.button || "Ver opções" }],
			});
		}
	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>Revise sua mensagem interativa antes de salvar</CardDescription>
			</CardHeader>
					...
					<div className="flex gap-2">
						<SaveToLibraryButton
							templateData={{
								...
								headerMetaMedia: message.header?.media_url ? [{ url: message.header.media_url }] : [],
							}}
