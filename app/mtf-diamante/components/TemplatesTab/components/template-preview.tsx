		...
		if (useAlternativeFormat) {
			...
			return (
				<div key={index} className="mb-2">
					...
					{comp.type === "header" &&
						comp.format &&
						["image", "video", "document"].includes(comp.format.toLowerCase()) &&
						comp.url && (
							<div className="mb-2">
								...
								{comp.format.toLowerCase() === "video" && (
									<video src={comp.url} controls className="max-w-full h-auto rounded-lg max-h-48" />
								)}
								{comp.format.toLowerCase() === "document" && (
									<div className="flex items-center gap-2 p-2 bg-gray-100 dark:bg-gray-800 rounded">
										<div className="text-blue-600 dark:text-blue-400">📄</div>
										<span className="text-sm text-gray-700 dark:text-gray-300">{comp.filename || "Document"}</span>
									</div>
								)}
							</div>
						)}
					...
					{comp.type === "buttons" && comp.buttons && comp.buttons.length > 0 && (
						<div className="mt-3 space-y-1">
							{comp.buttons.map((button, btnIndex) => (
								...
								>
									{button.text}
									{button.type === "PHONE_NUMBER" && button.phoneNumber && <span className="ml-1">📞</span>}
									{button.type === "URL" && <span className="ml-1">🔗</span>}
									{button.type === "COPY_CODE" && <span className="ml-1">📋</span>}
								</button>
							))}
						</div>
					)}
					...
					{comp.botoes && comp.botoes.length > 0 && (
						<div className="mt-3 space-y-1">
							{comp.botoes.map((button, btnIndex) => (
								...
								>
									{button.texto}
									{button.tipo === "PHONE_NUMBER" && button.telefone && <span className="ml-1">📞</span>}
									{button.tipo === "URL" && <span className="ml-1">🔗</span>}
									{button.tipo === "COPY_CODE" && <span className="ml-1">📋</span>}
								</button>
							))}
						</div>
					)}
			...
			<div className="space-y-4">
				...
				<Tabs defaultValue="whatsapp" className="w-full">
					<TabsList className="grid w-full grid-cols-3 bg-muted">
						...
						<TabsTrigger value="json" className="data-[state=active]:bg-background data-[state=active]:text-foreground">
							JSON
						</TabsTrigger>
					</TabsList>
					...
					<TabsContent value="visual" className="mt-4">
						{(components as CreateTemplateComponent[]).map((c, i) => (
							<div key={i} className="border border-border p-4 rounded-md mb-4 bg-card">
								...
								{c.text && (
									<pre className="bg-muted p-2 rounded mb-2 whitespace-pre-wrap text-sm text-foreground">{c.text}</pre>
								)}
		...
		<div className="space-y-4">
			...
			<Tabs defaultValue="whatsapp" className="w-full">
				<TabsList className="grid w-full grid-cols-3 bg-muted">
					...
					<TabsTrigger value="visual" className="data-[state=active]:bg-background data-[state=active]:text-foreground">
						Visual
					</TabsTrigger>
					<TabsTrigger value="json" className="data-[state=active]:bg-background data-[state=active]:text-foreground">
						JSON
				...
				<TabsContent value="visual" className="mt-4">
					{(components as TemplateComponent[]).map((c, i) => (
						<div key={i} className="border border-border p-4 rounded-md mb-4 bg-card">
							...
							{c.texto && (
								<pre className="bg-muted p-2 rounded mb-2 whitespace-pre-wrap text-sm text-foreground">{c.texto}</pre>
							)}
								...
								<div className="mb-2">
									...
									<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
										{c.variaveis.map((v, idx) => (
											<div key={idx} className="border border-border p-2 rounded text-xs bg-background">
