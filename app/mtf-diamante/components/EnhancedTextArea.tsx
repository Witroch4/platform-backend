...
export const EnhancedTextArea = forwardRef<EnhancedTextAreaRef, EnhancedTextAreaProps>(
	(
		...
		const commonProps = {
			...
			className: cn("transition-colors", "focus:ring-2 focus:ring-primary/20", className),
		};

		const inputElement = multiline ? (
			<Textarea ref={textAreaRef} rows={rows} {...commonProps} />
		) : (
		...
		return (
			<div className="space-y-2">
				...
				<VariableContextMenu onVariableInsert={handleVariableInsert} variables={variables} disabled={disabled}>
					{inputElement}
				</VariableContextMenu>
