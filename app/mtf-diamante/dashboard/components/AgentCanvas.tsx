...
const AgentDetailsNode = memo(function AgentDetailsNodeComponent(props: NodeProps) {
	...
	return (
		<Card
			className="w-[380px] shadow-xl border-2 transition-all hover:shadow-2xl cursor-pointer"
		...
		>
			<CardHeader className="space-y-2 pb-3 bg-gradient-to-br from-primary/5 to-transparent">
				...
				<div className="text-sm font-medium text-foreground/90 mt-2 line-clamp-1">{data.draft.name || "Sem nome"}</div>
				<div className="text-xs text-muted-foreground line-clamp-1">{data.draft.model || "Modelo não definido"}</div>
			</CardHeader>
...
function ModelConfigNode(props: NodeProps) {
	...
	const maxTokensDisplay = data.draft.maxOutputTokens === 0 ? "∞" : (data.draft.maxOutputTokens ?? 1024);

	return (
		<Card
			className="w-[320px] shadow-lg border-2 transition-all hover:shadow-xl cursor-pointer"
			style={{ borderColor: config.color }}
		>
			<CardHeader className="py-3 bg-gradient-to-br from-purple-500/5 to-transparent">
				...
				<div className="text-sm font-medium text-foreground/90 mt-2 font-mono">{data.draft.model}</div>
			</CardHeader>
			<CardContent className="space-y-3 pt-4 pb-3">
				<div className="grid grid-cols-2 gap-3 text-xs">
					<div className="space-y-1">
...
const ToolsConfigNode = memo(function ToolsConfigNodeComponent(props: NodeProps) {
	...
	return (
		<Card
			className="w-[340px] shadow-lg border-2 transition-all hover:shadow-xl cursor-pointer"
			style={{ borderColor: config.color }}
		>
			...
			<CardContent className="space-y-3 pt-4 pb-3">
				<div className="space-y-2 max-h-[120px] overflow-y-auto">
					...
					{(data.draft.toolset || []).length === 0 && (
						<div className="text-xs text-muted-foreground text-center py-4">Nenhuma ferramenta selecionada</div>
					)}
				</div>
				<div className="pt-2 border-t text-xs text-muted-foreground flex items-center justify-center gap-1.5">
					<InfoIcon className="h-3 w-3" />
...
const OutputParserNode = memo(function OutputParserNodeComponent(props: NodeProps) {
	...
	return (
		<Card
			className="w-[360px] shadow-lg border-2 transition-all hover:shadow-xl cursor-pointer"
			style={{ borderColor: config.color }}
		>
			<CardHeader className="py-3 bg-gradient-to-br from-green-500/5 to-transparent">
				...
				<div className="text-xs text-muted-foreground mt-2 font-mono">{schemaType}</div>
			</CardHeader>
			<CardContent className="space-y-3 pt-4 pb-3">
			...
			<div className="h-[600px] rounded-lg border-2 bg-background/50 backdrop-blur shadow-xl overflow-hidden">
				...
				>
					...
					<Panel position="top-left" className="flex gap-2">
						...
						<Button onClick={tidyUp} size="sm" variant="secondary" className="shadow-lg font-medium">
					...
					<Panel
						position="top-right"
						className="bg-background/95 backdrop-blur border-2 rounded-lg shadow-lg px-3 py-2 text-[10px] text-muted-foreground space-y-1"
					>
						<div className="font-mono">⌨️ Atalhos:</div>
						<div className="grid grid-cols-2 gap-x-3 gap-y-0.5">
							<span>Ctrl+1</span>
							<span className="text-foreground/60">Ajustar Zoom</span>
							<span>Shift+Alt+T</span>
							<span className="text-foreground/60">Auto Layout</span>
							<span>+/-</span>
							<span className="text-foreground/60">Zoom In/Out</span>
						</div>
					</Panel>
				</ReactFlow>
			</div>
