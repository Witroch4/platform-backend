...
export async function POST(req: NextRequest) {
	try {
		...
		if (!accountId) {
			...
			if (!mainAccount) {
				return NextResponse.json({ error: "Nenhuma conta principal do Instagram encontrada" }, { status: 404 });
			}
		...
		return NextResponse.json({
			success: true,
			message: isMainAccount ? "Conta principal desconectada." : "Conta secundária desconectada.",
		});
	} catch (error) {
		console.error("Erro ao desconectar conta do Instagram:", error);
		return NextResponse.json({ error: "Ocorreu um erro ao desconectar a conta do Instagram" }, { status: 500 });
	}
}
