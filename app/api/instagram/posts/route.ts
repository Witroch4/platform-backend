...
export async function GET(request: NextRequest) {
	try {
		...
		if (!account.access_token) {
			return NextResponse.json({ error: "Token de acesso não encontrado para esta conta." }, { status: 400 });
		}
		...
		if (!userRes.ok) {
			...
			return NextResponse.json({ error: "Não foi possível obter os dados do Instagram do usuário." }, { status: 500 });
		}
		...
		if (!mediaRes.ok) {
			...
			return NextResponse.json({ error: "Não foi possível obter as mídias do Instagram." }, { status: 500 });
		}
