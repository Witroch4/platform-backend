...
const getInstagramWebhookQueue = () =>
	import("@/lib/queue/instagram-webhook.queue").then((m) => m.instagramWebhookQueue);
...
function verifySignature(body: string, signature: string): boolean {
	...
	const hash = crypto.createHmac("sha256", INSTAGRAM_APP_SECRET).update(body).digest("hex");

	return crypto.timingSafeEqual(Buffer.from(hash), Buffer.from(signature.slice(7)));
}
