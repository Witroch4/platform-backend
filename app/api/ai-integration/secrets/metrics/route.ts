...
async function handleGet(req: NextRequest, context: AccessContext) {
  try {
    ...
    const metrics = {
      ...
      ai_secret_rotation_days_until: statuses.reduce((acc, status) => {
        acc[status.secretName] = status.daysUntilRotation;
        return acc;
      }, {} as Record<string, number>),
      
      // Rotation interval by secret
      ai_secret_rotation_interval_days: Object.entries(SECRET_CONFIGS).reduce((acc, [name, config]) => {
        acc[name] = config.rotationIntervalDays;
        return acc;
      }, {} as Record<string, number>),
      
      // Last rotation timestamp (mock data - in production this would be real)
      ai_secret_rotation_last_rotated_timestamp: statuses.reduce((acc, status) => {
