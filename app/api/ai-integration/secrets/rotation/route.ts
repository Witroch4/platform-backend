...
async function handlePost(req: NextRequest, context: AccessContext) {
	try {
		...
		if (!secretName || typeof secretName !== "string") {
			return NextResponse.json({ error: "Missing or invalid secretName parameter" }, { status: 400 });
		}

		if (!newSecret || typeof newSecret !== "string") {
			return NextResponse.json({ error: "Missing or invalid newSecret parameter" }, { status: 400 });
		}

		// Check if secret exists in configuration
		if (!SECRET_CONFIGS[secretName]) {
			return NextResponse.json({ error: `Unknown secret: ${secretName}` }, { status: 400 });
		}
...
async function handleDelete(req: NextRequest, context: AccessContext) {
	try {
		...
		if (!secretName || !SECRET_CONFIGS[secretName]) {
			return NextResponse.json({ error: "Invalid or unknown secret name" }, { status: 400 });
		}
		...
		if (!secretStatus?.inOverlapWindow) {
			return NextResponse.json({ error: "Secret is not in overlap window" }, { status: 400 });
		}
