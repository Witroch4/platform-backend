...
export async function POST(request: NextRequest) {
	try {
		...
		if (!clientId || !clientSecret || !redirectUri) {
			return NextResponse.json({ error: "Configuração do Instagram incompleta" }, { status: 500 });
		}
		...
		if (!tokenResponse.ok) {
			...
			return NextResponse.json({ error: "Falha ao obter token de acesso do Instagram" }, { status: 400 });
		}
		...
		if (!tokenData.access_token || !tokenData.user_id) {
			return NextResponse.json({ error: "Resposta de token inválida do Instagram" }, { status: 400 });
		}
		...
		if (!userResponse.ok) {
			...
			return NextResponse.json({ error: "Falha ao obter informações do usuário do Instagram" }, { status: 400 });
		}
		...
		return NextResponse.json({ error: "Ocorreu um erro ao conectar a conta do Instagram" }, { status: 500 });
	}
}
