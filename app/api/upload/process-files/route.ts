...
async function convertPdfToImages(pdfBuffer: Buffer, options: PDF2PicOptions): Promise<string[]> {
	try {
		...
		try {
			...
			const uploadResult = await uploadToMinIO(result.buffer, fileName, `image/${pdf2picOptions.format}`, true);
...
export async function POST(request: Request) {
	try {
		...
		log.info(
			`Processando arquivo para purpose: ${purpose}, sessionId: ${sessionId}, convertToImages: ${convertToImages} (padrão automático: ${explicitConvertToImages === null})`,
		);
