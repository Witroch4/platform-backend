...
export async function PATCH(
	...
	try {
		...
		if (!existingAgendamento) {
			return NextResponse.json({ error: "Agendamento não encontrado ou não pertence ao usuário." }, { status: 404 });
		}
		...
		if (updatedData.midias || updatedData.midia) {
			...
			const midiasArray = Array.isArray(midiasField) ? midiasField : [midiasField];
			...
			console.log(
				"[Agendar] Mídias preparadas para atualização:",
