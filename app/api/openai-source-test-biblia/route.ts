...
function normalizeQuickReply(q: TQuickReply): TQuickReply {
	const buttons = q.buttons.slice(0, 3).map((b, i) => {
		...
		const payload = b.payload && b.payload.trim().length > 0 ? b.payload : slugId(title, `BTN${i + 1}`);
...
// ————————————————————————————————————————————————————————————————
const MODEL_CAPS: Record<string, { reasoning: boolean; structured: boolean; label: string; sampling: boolean }> = {
...
function resolveDynamicPrefs(args: {
	...
	const caps = MODEL_CAPS[args.model] ?? { reasoning: false, structured: true, label: args.model, sampling: false };

	const verbosity =
		coerceVerbosity(args.captainVerbosity) ?? coerceVerbosity(args.bodyVerbosity) ?? ("medium" as Verbosity);

	const requestedEffort = coerceEffort(args.captainEffort) ?? coerceEffort(args.bodyEffort) ?? ("medium" as Effort);

	const reasoning = caps.reasoning === true ? ({ effort: requestedEffort } as { effort: Effort }) : undefined;
...
function resolveSamplingPrefs(args: {
	...
	const temperature = Number.isFinite(Number(args.captainTemperature))
		...
		: Number.isFinite(Number(args.bodyTemperature))
			? Number(args.bodyTemperature)
			: undefined;

	const top_p = Number.isFinite(Number(args.captainTopP))
		? Number(args.captainTopP)
		: Number.isFinite(Number(args.bodyTopP))
			? Number(args.bodyTopP)
			: undefined;
...
export async function POST(request: NextRequest) {
	...
	const sessionKeyInput = (body?.sessionId ?? request.headers.get("x-session-id") ?? "dev-session").toString();
	...
	// 9.1) Validações
	if (!userText) {
		return NextResponse.json({ success: false, error: "Campo 'userText' é obrigatório." }, { status: 400 });
	}
	...
	const resolvedMaxOutputTokens = clamp(captainMaxOutputTokens ?? bodyMaxOutputTokens ?? 256, 64, 42000);
	...
	const instructions =
		`${thinkHint}Canal alvo: ${channel}. ` + "Retorne SOMENTE no schema 'quick_reply' (sem texto fora do JSON).";
	...
	try {
		const attempt = async (opts?: { sampling?: Sampling; strict?: boolean }) =>
			structuredOrJson({
				model,
				messages,
		...
		try {
			...
			const unsupportedSampling =
				/Unsupported parameter: 'temperature'|Unsupported parameter: 'top_p'|is not supported with this model/i.test(
					msg,
				);
				...
				debug: {
					...
					request_body: {
						...
						guards: {
							...
							sampling_sent: Boolean(
								(r as any)?.openaiRequestEcho?.temperature ?? (r as any)?.openaiRequestEcho?.top_p ?? sampling,
							),
							notes: [
