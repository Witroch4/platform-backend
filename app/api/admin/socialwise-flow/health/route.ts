...
function determineOverallHealth(components: HealthCheckResult[]): "healthy" | "degraded" | "unavailable" {
	...
	const criticalStatuses = components.filter((c) => criticalComponents.includes(c.component)).map((c) => c.status);
...
export async function POST(request: NextRequest): Promise<NextResponse> {
	try {
		...
		if (!component) {
			return NextResponse.json({ error: "Campo 'component' é obrigatório." }, { status: 400 });
		}
		...
		switch (component) {
			...
			case "embedding_index":
				...
				break;
			default:
				return NextResponse.json({ error: `Componente '${component}' não reconhecido.` }, { status: 400 });
		}
