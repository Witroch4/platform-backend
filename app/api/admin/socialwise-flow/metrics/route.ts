...
export async function POST(request: NextRequest): Promise<NextResponse> {
	try {
		...
		if (!body.band || !body.strategy || typeof body.routeTotalMs !== "number") {
			return NextResponse.json({ error: "Campos obrigatórios: band, strategy, routeTotalMs" }, { status: 400 });
		}

		// Create test metrics
		const testMetrics = createPerformanceMetrics(body.band, body.strategy, body.routeTotalMs, {
