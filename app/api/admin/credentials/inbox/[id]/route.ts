  ...
  try {
    ...
    if (!id) {
      return NextResponse.json(
        { error: 'ID do inbox é obrigatório' },
        { status: 400 }
      );
    }
    ...
    if (!inbox) {
      return NextResponse.json(
        { error: 'Configuração de inbox não encontrada' },
        { status: 404 }
      );
    }
  ...
  try {
    ...
    if (fallbackParaInboxId && fallbackParaInboxId !== existingInbox.fallbackParaInboxId) {
      ...
      if (fallbackParaInboxId === id) {
        return NextResponse.json(
          { error: 'Um inbox não pode fazer fallback para si mesmo' },
          { status: 400 }
        );
      }
      ...
      if (!fallbackInbox) {
        return NextResponse.json(
          { error: 'Inbox de fallback não encontrado' },
          { status: 400 }
        );
      }

      // Verificar se não está criando um loop indireto
      const checkLoop = async (currentId: string, targetId: string, visited: Set<string> = new Set()): Promise<boolean> => {
      ...
      if (await checkLoop(fallbackParaInboxId, id)) {
        return NextResponse.json(
          { error: 'Esta configuração de fallback criaria um loop' },
          { status: 400 }
        );
      }
    }
    ...
    return NextResponse.json({
      ...
      hasCredentials: !!(updatedInbox.whatsappApiKey || updatedInbox.phoneNumberId || updatedInbox.whatsappBusinessAccountId),
