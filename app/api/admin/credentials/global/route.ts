...
export async function POST(request: NextRequest): Promise<Response> {
  try {
    ...
    if (usuarioChatwit.configuracaoGlobalWhatsApp) {
      return NextResponse.json(
        { error: 'Configuração global já existe. Use PUT para atualizar.' },
        { status: 409 }
      );
    }
...
export async function PUT(request: NextRequest): Promise<Response> {
  try {
    ...
    if (!usuarioChatwit.configuracaoGlobalWhatsApp) {
      return NextResponse.json(
        { error: 'Configuração global não encontrada. Use POST para criar.' },
        { status: 404 }
      );
    }
...
export async function DELETE(request: NextRequest): Promise<Response> {
  try {
    ...
    console.log(`[Global Credentials API] Configuração global removida: ${usuarioChatwit.configuracaoGlobalWhatsApp.id}`);
