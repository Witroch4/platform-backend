	...
	try {
		...
		if (lead.textoDOEspelho) {
			try {
				...
				const espelhoString =
					typeof lead.textoDOEspelho === "string" ? lead.textoDOEspelho : JSON.stringify(lead.textoDOEspelho);
				...
				if (espelhoParsed && espelhoParsed.meta && espelhoParsed.aluno && espelhoParsed.itens) {
					...
					console.warn(
						"[Enviar Análise] ⚠️ Estrutura inválida - espelhoParsed não contém meta/aluno/itens obrigatórios",
					);
		...
		if (isInternalAnalysisEnabled()) {
			...
			const textoProva =
				typeof lead.provaManuscrita === "string"
					...
					: lead.provaManuscrita
						? JSON.stringify(lead.provaManuscrita)
						: "";

			const textoEspelhoStr =
				typeof espelhoOtimizado === "string"
					? espelhoOtimizado
					: espelhoOtimizado
						? JSON.stringify(espelhoOtimizado)
						: "";

			if (!textoProva || textoProva.trim().length < 10) {
				return NextResponse.json({ error: "Texto da prova manuscrita ausente ou muito curto." }, { status: 400 });
			}

			if (!textoEspelhoStr || textoEspelhoStr.trim().length < 10) {
				return NextResponse.json({ error: "Texto do espelho ausente ou muito curto." }, { status: 400 });
			}
