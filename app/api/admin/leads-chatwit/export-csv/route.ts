...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    const countFilesByType = (arquivos: any[]): { total: number; pdf: number; image: number } => {
      ...
      const image = arquivos.filter(a => a.fileType?.toLowerCase().includes('image') || a.fileType?.toLowerCase().includes('jpg') || a.fileType?.toLowerCase().includes('png')).length;
    ...
    return NextResponse.json(
      { error: "Erro interno ao exportar leads" },
      { status: 500 }
    );
  }
}
