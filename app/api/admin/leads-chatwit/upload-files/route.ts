...
export async function POST(request: NextRequest): Promise<Response> {
	try {
		...
		if (uploadedFiles.length === 0) {
			return NextResponse.json({ error: "Falha ao fazer upload de todos os arquivos" }, { status: 500 });
		}
		...
		return NextResponse.json({ error: "Erro interno ao fazer upload de arquivos" }, { status: 500 });
	}
}
