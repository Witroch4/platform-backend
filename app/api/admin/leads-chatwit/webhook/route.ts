...
export async function POST(request: Request): Promise<Response> {
	try {
		...
		console.log(
			"[Webhook] Tipo do payload - espelho:",
			isEspelho,
			"espelhoConsultoriaFase2:",
			isEspelhoConsultoriaFase2,
			"espelhoParaBiblioteca:",
			isEspelhoParaBiblioteca,
			"espelhoPadrao:",
			isEspelhoPadrao,
			"manuscrito:",
			isManuscrito,
			...
			"analisePreliminar:",
			isAnalisePreliminar,
			"analiseSimuladoPreliminar:",
			isAnaliseSimuladoPreliminar,
			"analiseValidada:",
			isAnaliseValidada,
			"analiseSimuladoValidada:",
			isAnaliseSimuladoValidada,
			"analiseSimuladoValidadaCamelCase:",
			isAnaliseSimuladoValidadaCamelCase,
			"recursoPreliminar:",
			isRecursoPreliminar,
			"recursoValidado:",
			isRecursoValidado,
		...
		if (isEspelhoPadrao) {
			...
			if (
				webhookData.espelhoPadraotexto &&
				Array.isArray(webhookData.espelhoPadraotexto) &&
				webhookData.espelhoPadraotexto.length > 0
			) {
		...
		if (isEspelhoParaBiblioteca) {
			...
			try {
				...
				if (
					webhookData.arquivos_imagens_espelho &&
		...
		console.error(
			"[Webhook] Payload não identificado como espelho, manuscrito, espelho padrão, pré-análise, pré-análise de simulado, análise, análise de simulado, validação ou validação de simulado",
		);
		console.error(
			"[Webhook] Valores das flags - espelho:",
			webhookData.espelho,
			"espelhoConsultoriaFase2:",
			webhookData.espelhoconsultoriafase2,
			"espelhoParaBiblioteca:",
			webhookData.espelhoparabiblioteca,
			"espelhoPadrao:",
			webhookData.espelhoPadrao,
			"manuscrito:",
			webhookData.manuscrito,
			"análise:",
			webhookData.analise,
			"análiseSimulado:",
			webhookData.analisesimulado,
			"pré-análise:",
			webhookData.analisepreliminar,
			"préAnaliseSimulado:",
			webhookData.analisesimuladopreliminar,
			"analiseValidada:",
			webhookData.analiseValidada,
			"analiseSimuladoValidada:",
			webhookData.analisesimuladovalidado,
		);
		...
		return NextResponse.json({
			success: false,
			message:
				"Payload não identificado como manuscrito, espelho, espelho padrão, espelho para biblioteca, pré-análise, pré-análise de simulado, análise, análise de simulado, validação ou validação de simulado",
...
export async function GET(request: Request): Promise<Response> {
	return NextResponse.json({ status: "Webhook do Chatwit funcionando corretamente" }, { status: 200 });
}
