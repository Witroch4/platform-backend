...
export async function POST(request: Request): Promise<Response> {
	try {
		...
		if (!especialidade || !nome) {
			return NextResponse.json({ error: "Especialidade e nome são obrigatórios" }, { status: 400 });
		}
		...
		if (!usuarioValido) {
			return NextResponse.json({ error: "Nenhum usuário encontrado no sistema Chatwit" }, { status: 404 });
		}
