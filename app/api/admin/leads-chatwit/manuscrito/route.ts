...
export async function PUT(request: Request) {
	try {
		...
		if (!leadId || !texto) {
			return NextResponse.json({ error: "Lead ID e texto são obrigatórios" }, { status: 400 });
		}
		...
		return NextResponse.json({ error: error.message || "Erro interno ao atualizar manuscrito" }, { status: 500 });
	}
}
...
export async function DELETE(request: Request) {
	try {
		...
		return NextResponse.json({ error: error.message || "Erro interno ao excluir manuscrito" }, { status: 500 });
	}
}
