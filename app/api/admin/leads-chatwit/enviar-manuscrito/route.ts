...
export async function POST(request: Request): Promise<Response> {
	try {
		...
		const isEspelho =
			payload.espelho === true || payload.espelhoconsultoriafase2 === true || payload.espelhoparabiblioteca === true;
		...
		const docType = isManuscrito
			? "Manuscrito"
			: isEspelhoBiblioteca
				? "Espelho para Biblioteca"
				: isEspelho
					? "Espelho"
					: isProva
						? "Prova"
		...
		if (isEspelhoBiblioteca) {
			console.log(
				"[Enviar Espelho para Biblioteca] Espelho destinado à biblioteca geral, não atualizando lead específico",
			);
			...
			if (!lead) {
				...
				if (!lead) {
					...
					if (lead) {
						console.log(
							"[Enviar Documento] Lead encontrado usando leadID fornecido como espelhoBibliotecaId:",
							lead.id,
						);
					}
				}
			...
			if (espelhoPadraoId) {
				...
				if (oabRubric) {
					console.log(
						`[Enviar Documento] ✅ OabRubric encontrado (agente local): ${oabRubric.exam} - ${(oabRubric.meta as any)?.area || oabRubric.area}`,
					);
					...
					if (espelhoPadrao?.textoMarkdown) {
						...
						console.log(
							`[Enviar Documento] ✅ Espelho padrão encontrado (legado): ${espelhoPadrao.nome} (${espelhoPadrao.especialidade})`,
						);
					} else {
						console.warn(
							`[Enviar Documento] ⚠️ Espelho padrão ${espelhoPadraoId} não encontrado em OabRubric nem EspelhoPadrao`,
						);
					}
				}
		...
		const shouldUseLocalManuscritoAgent = USE_LOCAL_TRANSCRIBER && isManuscrito && !isEspelho && !isProva;

		const shouldUseLocalMirrorAgent = USE_LOCAL_MIRROR_AGENT && isEspelho && !isManuscrito && !isProva;
		...
		if (shouldUseLocalManuscritoAgent) {
			...
			const imagensManuscrito: IncomingManuscriptImage[] = Array.isArray(payloadFinal.arquivos_imagens_manuscrito)
			...
			const imagensPreparadas: PreparedManuscriptImage[] = imagensManuscrito
				.map(
					(imagem, index): PreparedManuscriptImage => ({
			...
			// ⭐ Extrair provider selecionado pelo usuário
			const selectedProvider = payload.selectedProvider || "GEMINI";
			console.log(`[Enviar Manuscrito][Queue] 🎛️ Provider selecionado: ${selectedProvider}`);
			...
			console.log(`[Enviar Manuscrito][Queue] Job ${job.id} enfileirado com sucesso`);
		...
		if (shouldUseLocalMirrorAgent) {
			...
			const imagensEspelho: IncomingManuscriptImage[] = Array.isArray(payloadFinal.arquivos_imagens_espelho)
			...
			const job = await enqueueMirrorGeneration({
				...
				selectedProvider: payload.selectedProvider || "GEMINI", // ⭐ Provider selecionado pelo usuário
			});

			console.log(`[Enviar Espelho][Queue] Job ${job.id} enfileirado com sucesso`);
