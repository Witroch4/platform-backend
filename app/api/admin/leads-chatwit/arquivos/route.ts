...
export async function GET(request: Request): Promise<Response> {
	try {
		...
		if (!leadId && !usuarioId) {
			return NextResponse.json({ error: "ID do lead ou ID do usuário é obrigatório" }, { status: 400 });
		}
		...
		if (leadId) {
			...
			arquivos = results.map(({ leadOabDataId, leadOabData, ...arquivo }) => ({
				...arquivo,
				leadId: leadOabDataId,
				lead: leadOabData ? { id: leadOabData.id, nomeReal: leadOabData.nomeReal, name: null } : undefined,
			}));
		}
...
export async function POST(request: Request): Promise<Response> {
	try {
		...
		return NextResponse.json({ error: "Erro interno ao adicionar arquivo" }, { status: 500 });
	}
}
...
export async function PATCH(request: Request): Promise<Response> {
	try {
		...
		return NextResponse.json({ error: "Erro interno ao atualizar arquivo" }, { status: 500 });
	}
}
