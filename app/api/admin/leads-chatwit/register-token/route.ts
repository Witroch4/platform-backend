...
export async function POST(request: NextRequest) {
	try {
		...
		if (!chatwitAccountId || typeof chatwitAccountId !== "string") {
			return NextResponse.json({ error: "ID da conta Chatwit é obrigatório" }, { status: 400 });
		}
		...
		if (existingUsuario) {
			return NextResponse.json({ error: "Este token já está sendo usado por outro usuário" }, { status: 400 });
		}
...
export async function GET(request: NextRequest) {
	try {
		...
		if (!user) {
			return NextResponse.json({ error: "Erro ao criar/buscar usuário" }, { status: 500 });
		}
