  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      if ([400, 408, 429, 500, 502, 503, 504].includes(status) && attempt < 3) {
        ...
        console.log(`[TemplateInfo] Tentativa ${attempt} falhou com status ${status}. Aguardando ${waitTime}ms antes de retry...`);
...
export async function GET(req: Request) {
  try {
    ...
    if (error.response?.status === 400) {
      return NextResponse.json(
        {
          error: 'Falha ao buscar template da API do WhatsApp (400 Bad Request). O ID do template pode estar inválido ou as credenciais podem estar expiradas.',
