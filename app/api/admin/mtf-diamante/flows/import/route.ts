...
import { n8nFormatToCanvas, validateFlowImport } from "@/lib/flow-builder/exportImport";
...
export async function POST(request: NextRequest) {
	try {
		...
		if (!inbox) {
			return NextResponse.json({ success: false, error: "Inbox não encontrada ou acesso negado" }, { status: 403 });
		}
		...
		const flowName = newName || flowData.meta.flowName || `Flow Importado ${Date.now()}`;
		...
		const finalName = existingFlow ? `${flowName} (cópia ${Date.now().toString(36)})` : flowName;
