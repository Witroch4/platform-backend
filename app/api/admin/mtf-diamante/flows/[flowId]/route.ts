 ...
 */
async function verifyFlowAccess(flowId: string, userId: string): Promise<{ hasAccess: boolean; flow: any | null }> {
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ flowId: string }> }) {
	try {
		...
		try {
			await syncCanvasToNormalizedFlow(flowId, validation.data.canvas as unknown as FlowCanvas);
