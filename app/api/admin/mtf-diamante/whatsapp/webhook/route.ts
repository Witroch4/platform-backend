...
export async function POST(request: NextRequest) {
	console.warn("[DEPRECATED] Old webhook URL accessed: /api/admin/mtf-diamante/whatsapp/webhook");
	console.warn("[DEPRECATED] Please update to: /api/admin/mtf-diamante/dialogflow/webhook");

	// Get the request body to forward it
	const body = await request.json();

	// Forward the request to the new endpoint
	const newUrl = new URL("/api/admin/mtf-diamante/dialogflow/webhook", request.url);
