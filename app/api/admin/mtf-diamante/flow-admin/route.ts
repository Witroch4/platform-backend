...
export async function GET(request: NextRequest) {
	try {
		...
		if (!inbox) {
			return NextResponse.json({ success: false, error: "Inbox não encontrada ou acesso negado" }, { status: 404 });
		}
		...
		return NextResponse.json({ success: false, error: "dataType inválido" }, { status: 400 });
	} catch (error) {
...
export async function POST(request: NextRequest) {
	try {
		...
		if (action === "abort_sessions") {
			...
			if (ids.length === 0) {
				return NextResponse.json({ success: false, error: "sessionIds é obrigatório" }, { status: 400 });
			}
