...
export async function GET(
	...
	try {
		...
		if (!config.whatsappToken) {
			return NextResponse.json({ error: "Credenciais do WhatsApp não configuradas." }, { status: 400 });
		}
		...
		try {
			...
			if (apiError.response?.status === 404) {
				return NextResponse.json({ error: "Template não encontrado na Meta." }, { status: 404 });
			}
		...
		return NextResponse.json({ error: error.message || "Erro ao verificar status do template." }, { status: 500 });
	}
}
