...
async function getWhatsAppTemplatesFromAPI(userId: string) {
	try {
		...
		if (!config.whatsappBusinessAccountId || !config.whatsappToken) {
			throw new Error(
				"Credenciais da API do WhatsApp não configuradas. Configure as credenciais nas configurações globais.",
			);
		}
		...
		if (refresh) {
			...
			if (category && category !== "all") {
				filteredTemplates = filteredTemplates.filter(
					(template: any) => template.category?.toUpperCase() === category.toUpperCase(),
				);
			}
			if (language && language !== "all") {
				filteredTemplates = filteredTemplates.filter(
					(template: any) => template.language?.toLowerCase() === language.toLowerCase(),
				);
			}
...
export async function POST(request: Request) {
	try {
		...
		console.log(
			"✅ Payload final para API WhatsApp (com variáveis convertidas):",
