...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!usuarioChatwit) {
      return NextResponse.json(
        { success: false, error: 'Usuário Chatwit não encontrado' } as ApiResponse,
        { status: 404 }
      );
    }
    ...
    if (existingCaixa) {
      return NextResponse.json(
        { success: false, error: 'InboxId já existe para este usuário' } as ApiResponse,
        { status: 409 }
      );
    }
    ...
    } as ApiResponse<ChatwitInbox>, { status: 201 });
