...
export async function GET() {
	try {
		...
		console.log(`🔍 [CaixasEntrada] Total de agentes no banco: ${totalAgentes}`);
		...
		caixas.forEach((caixa, index) => {
			...
			if (caixa.agentes && caixa.agentes.length > 0) {
				caixa.agentes.forEach((agente, agenteIndex) => {
					console.log(`     🤖 Agente ${agenteIndex + 1}: ${agente.nome} (${agente.ativo ? "ATIVO" : "INATIVO"})`);
...
export async function POST(request: NextRequest) {
	try {
		...
		if (accessToken && baseURL) {
			try {
				console.log("🔍 [CaixasEntrada] Buscando agentes Dialogflow ativos na origem para inbox:", inboxId);

				const appsResponse = await axios.get(`${baseURL}/api/v1/accounts/${accountId}/integrations/apps`, {
					headers: {
				...
				if (dialogflowApp?.hooks) {
					...
					const hooksParaInbox = dialogflowApp.hooks.filter((h: any) => h.inbox?.id === Number.parseInt(inboxId));

					console.log(`📋 [CaixasEntrada] Encontrados ${hooksParaInbox.length} agentes Dialogflow para esta inbox`);

					// Criar um agente para cada hook encontrado
					for (const hook of hooksParaInbox) {
						try {
							const agenteNome = hook.settings?.agent_name || hook.settings?.project_id || `Dialogflow-${hook.id}`;
							...
							console.error(`❌ [CaixasEntrada] Erro ao criar agente para hook ${hook.id}:`, agenteError.message);
						}
					}

					if (hooksParaInbox.length === 0) {
						console.log("ℹ️ [CaixasEntrada] Nenhum agente Dialogflow encontrado para esta inbox na origem.");
					}
				} else {
					console.log("ℹ️ [CaixasEntrada] Nenhuma integração Dialogflow encontrada na conta.");
				}
			} catch (e: any) {
				console.error("❌ [CaixasEntrada] Erro ao buscar integrações Dialogflow:", e.message);
				console.error("❌ [CaixasEntrada] Detalhes do erro:", e.response?.data || e);
			}
		} else {
			console.log(
				"⚠️ [CaixasEntrada] AccessToken ou baseURL não configurados. Não foi possível sincronizar agentes da origem.",
			);
			console.log("⚠️ [CaixasEntrada] Configure CHATWIT_BASE_URL no .env e verifique o token de acesso.");
		}
