...
import {
  ...
  logDetailedWebhookPayload,
  createSanitizationSummary,
  analyzeOriginalDialogflowRequest,
  logOriginalRequestAnalysis,
  createWebhookErrorResponse,
  createWebhookSuccessResponse,
  parseDialogflowRequest,
  extractTemplateVariables,
  extractFlashIntentData,
  extractUserIdentification,
...
async function isFeatureEnabled(
  ...
  try {
    return await featureFlagManager.isEnabled(
      flagName,
      undefined,
      undefined,
      metadata
...
// Função extractFlashIntentData movida para lib/webhook-utils.ts

// Função parseDialogflowRequest movida para lib/webhook-utils.ts

// Função extractTemplateVariables movida para lib/webhook-utils.ts
    ...
    switch (messageMapping.messageType) {
      ...
      case "enhanced_interactive":
        if (messageMapping.enhancedInteractiveMessage) {
          ...
          console.log(
            `[MTF Diamante Dispatcher] Creating enhanced interactive task with data:`,
            {
              recipientPhone,
              hasWhatsappApiKey: !!(
                messageMapping.whatsappConfig.whatsappToken || whatsappApiKey
          ...
          console.log(
            `[MTF Diamante Dispatcher] Enhanced interactive task created:`,
            {
...
async function handleInstagramTranslation(
  ...
  try {
    logInstagramWithCorrelationId(
      "info",
      "Processing Instagram translation request",
      correlationId
    );
    ...
    logInstagramWithCorrelationId(
      "debug",
      "Extracted webhook data for Instagram translation",
      correlationId,
      {
        ...
        cacheKeyWillInclude:
          "usuarioChatwitId will be extracted from database query",
      }
    );
    ...
    if (
      ...
      logInstagramWithCorrelationId(
        "error",
        "Missing required data for Instagram translation",
        correlationId,
        {
      ...
      return createDialogflowFallbackResponse(
        correlationId,
        "Missing required data for Instagram translation"
      );
    }
    ...
    logInstagramWithCorrelationId(
      "info",
      "Instagram translation job queued",
      correlationId,
      { jobId }
    );

    // Wait for worker completion with 4.5 second timeout
    const result = await waitForInstagramTranslationResult(
      instagramJobData.correlationId,
      4500
    );
    ...
    if (!result) {
      logInstagramWithCorrelationId(
        "error",
        "Instagram translation job returned null result",
        correlationId,
        {
          responseTime,
          jobId,
    ...
    if (result.success && result.fulfillmentMessages) {
      logInstagramWithCorrelationId(
        "info",
        "Instagram translation completed successfully",
        correlationId,
        {
      ...
      if (
        !Array.isArray(result.fulfillmentMessages) ||
        result.fulfillmentMessages.length === 0
      ) {
        logInstagramWithCorrelationId(
          "warn",
          "Empty fulfillment messages received from worker",
          correlationId
        );
        return createDialogflowFallbackResponse(
          correlationId,
          "Empty response from Instagram translation"
        );
      }

      // Validate Socialwise payload structure
      const hasValidSocialwisePayload = result.fulfillmentMessages.some(
        (msg) => msg.payload && msg.payload.socialwiseResponse
      );

      if (!hasValidSocialwisePayload) {
        logInstagramWithCorrelationId(
          "warn",
          "Invalid Socialwise payload structure",
          correlationId,
          {
      ...
      console.log(
        `[MTF Diamante Dispatcher] [${correlationId}] EXACT RESPONSE SENT TO DIALOGFLOW:`,
        JSON.stringify(dialogflowResponse, null, 2)
      );
      console.log(
        `[MTF Diamante Dispatcher] [${correlationId}] FULFILLMENT MESSAGES DETAILS:`,
        result.fulfillmentMessages.map((msg, index) => ({
          index,
          hasPayload: !!msg.payload,
          hasSocialwiseResponse: !!(
          ...
          socialwiseResponseKeys:
            msg.payload && msg.payload.socialwiseResponse
              ? Object.keys(msg.payload.socialwiseResponse)
              : [],
    ...
    logInstagramWithCorrelationId(
      "error",
      "Critical error in Instagram translation",
      correlationId,
      {
        error: errorMessage,
        responseTime,
 ...
 */
function createDialogflowFallbackResponse(
  correlationId: string,
  fallbackMessage: string
): Response {
  console.log(
    `[MTF Diamante Dispatcher] [${correlationId}] CREATING FALLBACK RESPONSE - Message: ${fallbackMessage}`
  );
  ...
  console.log(
    `[MTF Diamante Dispatcher] [${correlationId}] FALLBACK RESPONSE SENT TO DIALOGFLOW:`,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // ========================================
    // LOG ORIGINAL REQUEST FOR PATTERN ANALYSIS
    // ========================================
    console.log(
      `[OriginalRequestDialogflow] [${correlationId}] COMPLETE ORIGINAL PAYLOAD:`,
      {
        ...
        fullPayload: JSON.stringify(rawReq, null, 2),
      }
    );

    // Use the comprehensive analysis function
    logOriginalRequestAnalysis(rawReq, correlationId, {
      "content-type": request.headers.get("content-type"),
      "user-agent": request.headers.get("user-agent"),
      "x-forwarded-for": request.headers.get("x-forwarded-for"),
    });
    ...
    // ========================================
    // DEBUG: SPECIFIC ANALYSIS FOR THIS PAYLOAD
    // ========================================
    const chatwootPayload = rawReq.originalDetectIntentRequest?.payload;
    console.log(
      `[OriginalRequestDialogflow] [${correlationId}] SANITIZATION DEBUG:`,
      {
        correlationId,
        beforeSanitization: {
          intentName: rawReq.queryResult?.intent?.displayName,
          whatsappApiKey: chatwootPayload?.whatsapp_api_key
            ? `${chatwootPayload.whatsapp_api_key.substring(0, 10)}...`
            : "N/A",
          whatsappApiKeyLength: chatwootPayload?.whatsapp_api_key?.length || 0,
          contactPhone: chatwootPayload?.contact_phone,
          interactionType: chatwootPayload?.interaction_type,
          buttonId: chatwootPayload?.button_id,
          hasInteractive: !!chatwootPayload?.interactive,
          inboxId: chatwootPayload?.inbox_id,
          wamid: chatwootPayload?.wamid,
        },
      }
    );

    // Log specific sections for pattern analysis
    console.log(
      `[OriginalRequestDialogflow] [${correlationId}] PAYLOAD STRUCTURE ANALYSIS:`,
      {
        correlationId,
        hasResponseId: !!rawReq.responseId,
        hasQueryResult: !!rawReq.queryResult,
        hasOriginalDetectIntentRequest: !!rawReq.originalDetectIntentRequest,
        hasSession: !!rawReq.session,
        queryResult: {
          hasQueryText: !!rawReq.queryResult?.queryText,
          hasParameters: !!rawReq.queryResult?.parameters,
          hasIntent: !!rawReq.queryResult?.intent,
          intentDisplayName: rawReq.queryResult?.intent?.displayName,
          hasOutputContexts: !!rawReq.queryResult?.outputContexts,
          outputContextsCount: rawReq.queryResult?.outputContexts?.length || 0,
        },
        originalDetectIntentRequest: {
          hasPayload: !!rawReq.originalDetectIntentRequest?.payload,
          payloadKeys: rawReq.originalDetectIntentRequest?.payload
            ? Object.keys(rawReq.originalDetectIntentRequest.payload)
            : [],
          channelType:
            rawReq.originalDetectIntentRequest?.payload?.channel_type,
          interactionType:
            rawReq.originalDetectIntentRequest?.payload?.interaction_type,
          hasWhatsappApiKey:
            !!rawReq.originalDetectIntentRequest?.payload?.whatsapp_api_key,
          hasContactPhone:
            !!rawReq.originalDetectIntentRequest?.payload?.contact_phone,
          hasButtonId: !!rawReq.originalDetectIntentRequest?.payload?.button_id,
          hasInteractive:
            !!rawReq.originalDetectIntentRequest?.payload?.interactive,
          interactiveType:
            rawReq.originalDetectIntentRequest?.payload?.interactive?.type,
        },
      }
    );

    // Log chatwoot payload details if present
    if (rawReq.originalDetectIntentRequest?.payload) {
      const chatwootPayload = rawReq.originalDetectIntentRequest.payload;
      console.log(
        `[OriginalRequestDialogflow] [${correlationId}] CHATWOOT PAYLOAD DETAILS:`,
        {
          ...
          // Contact information
          contactPhone: chatwootPayload.contact_phone
            ? `${String(chatwootPayload.contact_phone).substring(0, 4)}****${String(chatwootPayload.contact_phone).substring(String(chatwootPayload.contact_phone).length - 4)}`
            : "N/A",
          ...
          // WhatsApp specific
          wamid: chatwootPayload.wamid,
          phoneNumberId: chatwootPayload.phone_number_id,
          businessId: chatwootPayload.business_id,
          hasWhatsappApiKey: !!chatwootPayload.whatsapp_api_key,
          whatsappApiKeyLength: chatwootPayload.whatsapp_api_key
            ? chatwootPayload.whatsapp_api_key.length
            : 0,

          // Message details
          messageType: chatwootPayload.message_type,
          messageContentType: chatwootPayload.message_content_type,
          messageContent: chatwootPayload.message_content,
          messageCreatedAt: chatwootPayload.message_created_at,

          // Interaction details
          interactionType: chatwootPayload.interaction_type,
          buttonId: chatwootPayload.button_id,
          buttonTitle: chatwootPayload.button_title,
          listId: chatwootPayload.list_id,
          listTitle: chatwootPayload.list_title,
          listDescription: chatwootPayload.list_description,

          // Interactive object if present
          interactive: chatwootPayload.interactive
            ? {
                type: chatwootPayload.interactive.type,
                buttonReply: chatwootPayload.interactive.button_reply,
                listReply: chatwootPayload.interactive.list_reply,
              }
            : null,
          ...
          timestamp: chatwootPayload.timestamp,
    ...
    /*     logDetailedWebhookPayload(correlationId, rawReq, "raw");

    // ========================================
    // INTERACTION TYPE PATTERN ANALYSIS
    // ========================================
    const chatwootPayload = rawReq.originalDetectIntentRequest?.payload;
      ...
      console.log(
        `[OriginalRequestDialogflow] [${correlationId}] BUTTON INTERACTION PATTERN:`,
        {
          ...
          buttonData: {
            ...
            buttonReplyId: chatwootPayload?.interactive?.button_reply?.id,
            buttonReplyTitle: chatwootPayload?.interactive?.button_reply?.title,
            listReplyId: chatwootPayload?.interactive?.list_reply?.id,
            listReplyTitle: chatwootPayload?.interactive?.list_reply?.title,
          },
          contextData: {
            originalMessageId: chatwootPayload?.context?.id,
            hasContext: !!chatwootPayload?.context,
          },
          queryResult: {
            queryText: rawReq.queryResult?.queryText,
            intentName: rawReq.queryResult?.intent?.displayName,
            parameters: rawReq.queryResult?.parameters,
          },
        }
      );
    } else {
      console.log(
        `[OriginalRequestDialogflow] [${correlationId}] INTENT INTERACTION PATTERN:`,
        {
          ...
          intentData: {
            ...
            queryText: rawReq.queryResult?.queryText,
            parameters: rawReq.queryResult?.parameters,
            confidence: rawReq.queryResult?.intentDetectionConfidence,
            languageCode: rawReq.queryResult?.languageCode,
          },
          messageData: {
            messageType: chatwootPayload?.message_type,
            messageContentType: chatwootPayload?.message_content_type,
            messageContent: chatwootPayload?.message_content,
            hasAttachment: !!chatwootPayload?.attachment,
          },
          contextData: {
            outputContextsCount:
              rawReq.queryResult?.outputContexts?.length || 0,
            outputContexts:
              rawReq.queryResult?.outputContexts?.map((ctx) => ({
                name: ctx.name,
    ...
    // ========================================
    // CHANNEL TYPE PATTERN ANALYSIS
    // ========================================
    const channelType = chatwootPayload?.channel_type;
    console.log(
      `[OriginalRequestDialogflow] [${correlationId}] CHANNEL PATTERN ANALYSIS:`,
      {
        correlationId,
        channelType,
        isWhatsApp: channelType === "Channel::Whatsapp",
        isInstagram: channelType === "Channel::Instagram",
        isTelegram: channelType === "Channel::Telegram",
        isWebWidget: channelType === "Channel::WebWidget",
        channelSpecificData: {
          whatsapp:
            channelType === "Channel::Whatsapp"
              ? {
                  ...
                  businessId: chatwootPayload?.business_id,
                  wamid: chatwootPayload?.wamid,
                  hasApiKey: !!chatwootPayload?.whatsapp_api_key,
                }
              : null,
          instagram:
            channelType === "Channel::Instagram"
              ? {
                  // Instagram specific fields would go here
                  socialwiseActive: chatwootPayload?.socialwise_active,
                }
              : null,
        },
    ...
    // ========================================
    // DEBUG: AFTER SANITIZATION ANALYSIS
    // ========================================
    const sanitizedChatwootPayload = req.originalDetectIntentRequest?.payload;
    console.log(
      `[OriginalRequestDialogflow] [${correlationId}] AFTER SANITIZATION DEBUG:`,
      {
        correlationId,
        afterSanitization: {
          intentName: req.queryResult?.intent?.displayName,
          whatsappApiKey: sanitizedChatwootPayload?.whatsapp_api_key
            ? `${sanitizedChatwootPayload.whatsapp_api_key.substring(0, 10)}...`
            : "N/A",
          whatsappApiKeyLength:
            sanitizedChatwootPayload?.whatsapp_api_key?.length || 0,
          contactPhone: sanitizedChatwootPayload?.contact_phone,
          interactionType: sanitizedChatwootPayload?.interaction_type,
          buttonId: sanitizedChatwootPayload?.button_id,
          hasInteractive: !!sanitizedChatwootPayload?.interactive,
          inboxId: sanitizedChatwootPayload?.inbox_id,
          wamid: sanitizedChatwootPayload?.wamid,
          apiKeyWasModified:
            chatwootPayload?.whatsapp_api_key !==
            sanitizedChatwootPayload?.whatsapp_api_key,
          phoneWasModified:
            chatwootPayload?.contact_phone !==
            sanitizedChatwootPayload?.contact_phone,
        },
      }
    );

    // Log sanitized payload details
    logDetailedWebhookPayload(correlationId, req, "sanitized");

    // Create sanitization summary
    const sanitizationSummary = createSanitizationSummary(rawReq, req);

    // Validate sanitized payload
    const validation = validateSanitizedWebhookPayload(req);

    // Log sanitization results with summary
    logSanitizationResults(
      correlationId,
      originalSize,
      payloadSize,
    ...
    if (sanitizationSummary.fieldsModified.length > 0) {
      console.log(
        `[MTF Diamante Webhook] [${correlationId}] Sanitization summary:`,
        sanitizationSummary
      );
    }
    ...
    if (!validation.isValid) {
      return createWebhookErrorResponse(
        correlationId,
        "Invalid payload after sanitization",
        validation.errors,
        400
      );
    }
    ...
    console.log(
      `[MTF Diamante Dispatcher] [${correlationId}] Received Dialogflow request`
    );

    // Log complete payload for debugging (only for Instagram channels)
    const channelTypeCheck =
      req.originalDetectIntentRequest?.payload?.channel_type;
    if (channelTypeCheck === "Channel::Instagram") {
      console.log(
        `[MTF Diamante Dispatcher] [${correlationId}] COMPLETE DIALOGFLOW PAYLOAD:`,
        JSON.stringify(req, null, 2)
      );

      // Log specific sections for easier analysis
      console.log(
        `[MTF Diamante Dispatcher] [${correlationId}] PAYLOAD SECTIONS:`
      );
      console.log("- queryResult:", JSON.stringify(req.queryResult, null, 2));
      console.log(
        "- originalDetectIntentRequest:",
        JSON.stringify(req.originalDetectIntentRequest, null, 2)
      );
    ...
    console.log(
      `[MTF Diamante Dispatcher] [${correlationId}] Channel detection:`,
      {
    ...
    if (channelDetection.isInstagram) {
      return await handleInstagramTranslation(
        req,
        correlationId,
        startTime,
        payloadSize
    ...
    try {
      ...
      if (
        ...
        await processButtonClickRequest(
          chatwootPayload.button_id,
          chatwootPayload.wamid || chatwootPayload.id,
          webhookData.contactPhone,
          webhookData.whatsappApiKey,
          correlationId,
          req,
    ...
    const useNewWebhookProcessing = await isFeatureEnabled(
      FEATURE_FLAGS.NEW_WEBHOOK_PROCESSING,
      {
    ...
    try {
      const useUnifiedExtraction = await isFeatureEnabled(
        FEATURE_FLAGS.UNIFIED_PAYLOAD_EXTRACTION
      );
      ...
      console.warn(
        `[MTF Diamante Dispatcher] [${correlationId}] Unified extraction failed, falling back to legacy:`,
        extractionError
      );
      ...
      const responseTime = performance.now() - startTime;
      console.log(
        `[MTF Diamante Dispatcher] [${correlationId}] Legacy fallback response in ${responseTime}ms`
      );
    ...
    if (useNewWebhookProcessing) {
      ...
      const useHighPriorityQueue = await isFeatureEnabled(
        FEATURE_FLAGS.HIGH_PRIORITY_QUEUE,
        {
      ...
      const useLowPriorityQueue = await isFeatureEnabled(
        FEATURE_FLAGS.LOW_PRIORITY_QUEUE,
        {
          inboxId: unifiedData.inboxId,
          contactPhone: unifiedData.contactPhone,
        }
      ...
      if (useHighPriorityQueue) {
        setImmediate(async () => {
          try {
            ...
            console.error(
              `[MTF Diamante Dispatcher] [${correlationId}] Error queuing high priority job:`,
              error
            );
          }
        });
      ...
      if (useLowPriorityQueue) {
        setImmediate(async () => {
          try {
            ...
            console.error(
              `[MTF Diamante Dispatcher] [${correlationId}] Error queuing low priority job:`,
              error
            );
          }
        });
    ...
    setImmediate(async () => {
      try {
        ...
        console.error(
          `[MTF Diamante Dispatcher] [${correlationId}] Error queuing legacy tasks:`,
          error
        );
      }
    });
    ...
    const responseTime = performance.now() - startTime;
    console.log(
      `[MTF Diamante Dispatcher] [${correlationId}] Response sent in ${responseTime}ms`
    );
    ...
    // ========================================
    console.error(
      `[OriginalRequestDialogflow] [${correlationId || "error-fallback"}] ERROR PATTERN ANALYSIS:`,
      {
        ...
        requestInfo: {
          method: "POST",
          url: "/api/admin/mtf-diamante/dialogflow/webhook",
          responseTime,
          payloadSize,
          interactionType,
        },
        ...
        debugInfo: {
          ...
          errorOccurredDuring: correlationId ? "processing" : "initialization",
          payloadSizeValid: payloadSize > 0 && payloadSize < 1000000, // 1MB limit
        },
      }
    );

    console.error(
      `[MTF Diamante Dispatcher] [${correlationId}] Critical error in webhook (${responseTime}ms):`,
      error
    );
 ...
 */
async function queueHighPriorityJob(
  data: UnifiedWebhookPayload,
  correlationId: string
): Promise<void> {
  logWithCorrelationId(
    "info",
    "Queuing high priority job for user response",
    correlationId,
    {
  ...
  const { addRespostaRapidaJob, createIntentJob, createButtonJob } =
    await import("@/lib/queue/resposta-rapida.queue");
  ...
  try {
    if (data.interactionType === "intent" && data.intentName) {
      ...
      logWithCorrelationId(
        "info",
        "Intent job queued successfully",
        correlationId,
        {
          intentName: data.intentName,
          inboxId: data.inboxId,
      ...
      logWithCorrelationId(
        "info",
        "Button job queued successfully",
        correlationId,
        {
          buttonId: data.buttonId,
          inboxId: data.inboxId,
    ...
    logWithCorrelationId(
      "error",
      "Failed to queue high priority job",
      correlationId,
      {
 ...
 */
async function queueLowPriorityJob(
  data: UnifiedWebhookPayload,
  correlationId: string
): Promise<void> {
  logWithCorrelationId(
    "info",
    "Queuing low priority job for data persistence",
    correlationId,
    {
      inboxId: data.inboxId,
      contactSource: data.contactSource,
  ...
  // Import the queue functions
  const { addPersistenciaCredenciaisJob, createCredentialsUpdateJob } =
    await import("@/lib/queue/persistencia-credenciais.queue");
  ...
  try {
    ...
    logWithCorrelationId(
      "info",
      "Persistence job queued successfully",
      correlationId,
      {
        inboxId: data.inboxId,
        contactSource: data.contactSource,
    ...
    logWithCorrelationId(
      "error",
      "Failed to queue low priority job",
      correlationId,
      {
        error: error instanceof Error ? error.message : error,
        inboxId: data.inboxId,
...
async function queueLegacyTasks(
  ...
  logWithCorrelationId(
    "info",
    "Queuing legacy tasks for backward compatibility",
    correlationId
  );
  ...
  try {
    ...
    logWithCorrelationId(
      "info",
      "Legacy tasks queued successfully",
      correlationId
    );
  } catch (error) {
    logWithCorrelationId("error", "Error queuing legacy tasks", correlationId, {
      error,
    });
