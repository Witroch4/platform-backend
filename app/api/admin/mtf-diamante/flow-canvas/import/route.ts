 ...
 */
async function fetchMessagesWithButtons(
  inboxId: string
): Promise<MessageWithButtons[]> {
 ...
 */
function buildCanvasFromReactions(
  ...
  for (const msg of messages) {
    ...
    for (const button of msg.buttons) {
      ...
      if (
        reactionType === FlowNodeType.INTERACTIVE_MESSAGE &&
        reactionData.messageId
      ) {
        const existingNodeId = templateToNodeId.get(
          reactionData.messageId as string
        );
        if (existingNodeId) {
...
export async function POST(request: NextRequest) {
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            'Já existe um canvas para esta caixa. Remova-o antes de importar.',
        },
        { status: 409 }
      );
    }
    ...
    const [messages, reactions] = await Promise.all([
      fetchMessagesWithButtons(inboxId),
      fetchButtonReactions(inboxId),
    ]);
