...
async function getMtfDiamanteVariables(userId: string): Promise<MtfDiamanteVariavel[]> {
...
export async function POST(request: Request) {
	try {
		...
		if (!messageId || !recipientPhone) {
			return NextResponse.json({ error: "messageId and recipientPhone are required" }, { status: 400 });
		}
		...
		if (!template || !template.interactiveContent) {
			return NextResponse.json({ error: "Interactive message not found" }, { status: 404 });
		}
		...
		if (!validation.isValid) {
			return NextResponse.json({ error: "Invalid message", details: validation.errors }, { status: 400 });
		}

		// Converter para formato do WhatsApp
		const whatsappMessage = convertInteractiveMessageToWhatsApp(messageData, recipientPhone, variables);

		console.log("[INTERACTIVE-MESSAGE][SEND] WhatsApp message:", JSON.stringify(whatsappMessage, null, 2));
		...
		console.log("[INTERACTIVE-MESSAGE][SEND] WhatsApp response:", response.data);
		...
		try {
			...
			console.error("[INTERACTIVE-MESSAGE][SEND] Error logging to database:", dbError);
		}
