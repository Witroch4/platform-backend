		...
		const transformedMessages: InteractiveMessage[] = messages.map((msg: any) => {
			...
			if (!(interactive as any)?.interactiveType) {
				if (interactive?.actionCtaUrl) {
					...
					actionType =
						hasHeader && hasFooter ? "generic" : bodyText.length <= 640 ? "button_template" : "quick_replies";
				}
			}
...
export async function POST(request: NextRequest) {
	try {
		...
		console.log(
			`[InteractiveMessages API] Message structure:`,
			body.message ? JSON.stringify(body.message, null, 2) : "NO MESSAGE FIELD",
		);
				...
				interactiveContent: {
					create: {
						...
						...((message.type === "button" ||
							...
							(() => {
								console.log(
									`[InteractiveMessages API] Creating actionReplyButton for type ${message.type} with buttons:`,
