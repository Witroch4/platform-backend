...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
	try {
		...
		if (!id) {
			return NextResponse.json({ success: false, error: "ID da mensagem é obrigatório" } as ApiResponse, {
				status: 400,
			});
		}
		...
		if (!message) {
			return NextResponse.json({ success: false, error: "Mensagem não encontrada" } as ApiResponse, { status: 404 });
		}
		...
		const transformedMessage: InteractiveMessage = {
			...
			footer: interactive?.footer
				...
				: undefined,
			action: (transformActionData(actionType, actionData) ||
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
	try {
		...
		if (Object.keys(cleanUpdateData).length === 0) {
			return NextResponse.json({ success: false, error: "Nenhum dado para atualizar" } as ApiResponse, { status: 400 });
		}
		...
		if (
			interactiveUpdateData.header ||
			interactiveUpdateData.body ||
			interactiveUpdateData.footer ||
			interactiveUpdateData.action
		) {
			...
			if (existingInteractive) {
				...
				if (interactiveUpdateData.action) {
					...
					console.log(
						`[InteractiveMessages API UPDATE] Updating action for type ${messageType}:`,
						interactiveUpdateData.action,
					);
					...
					if (
						["button", "quick_replies", "button_template", "generic"].includes(messageType) &&
						(interactiveUpdateData.action.buttons || (interactiveUpdateData.action as any).quick_replies)
					) {
						console.log(
							`[InteractiveMessages API UPDATE] Creating actionReplyButton with buttons:`,
							interactiveUpdateData.action.buttons || (interactiveUpdateData.action as any).quick_replies,
						);
						...
						await prisma.actionList.create({
							data: {
								...
								...(interactiveUpdateData.action.description && {
									description: interactiveUpdateData.action.description,
								}),
							},
						});
						...
						await prisma.actionCtaUrl.create({
							data: {
								interactiveContentId: existingInteractive.id,
								...(interactiveUpdateData.action.displayText && {
									displayText: interactiveUpdateData.action.displayText,
								}),
						...
						await prisma.actionLocationRequest.create({
							data: {
								interactiveContentId: existingInteractive.id,
								...(interactiveUpdateData.action.requestType && {
									requestType: interactiveUpdateData.action.requestType,
								}),
							},
						});
					}
		...
		if (!updatedMessage) {
			return NextResponse.json({ success: false, error: "Mensagem não encontrada após atualização" } as ApiResponse, {
				status: 404,
			});
		}
