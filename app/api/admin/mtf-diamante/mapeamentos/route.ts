  ...
  try {
    ...
    if (!intentName || !templateId || !caixaId) {
      return NextResponse.json(
        { error: "Intenção, template e caixa são obrigatórios." },
        { status: 400 }
      );
    }
    ...
    const data = {
      ...
      inboxId: caixaId,
      templateId,
      customVariables: customVariables || null, // Salvar variáveis customizadas se fornecidas
    };
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const existingMapping =
      await getPrismaInstance().mapeamentoIntencao.findUnique({
        where: { id: mappingId },
    ...
    if (existingMapping) {
      try {
        ...
        if (chatwitInbox) {
          // Use the correct usuarioChatwitId and Chatwit inboxId for cache invalidation
          console.log(
            `[API Cache Invalidation] [DEBUG] Preparing cache invalidation for mapping deletion:`,
            {
          ...
          console.log(
            `[API Cache Invalidation] [SUCCESS] Instagram cache cleared for mapping deletion:`,
            {
              operation: "DELETE /mapeamentos",
              userContext: {
                usuarioChatwitId: chatwitInbox.usuarioChatwitId,
          ...
          console.warn(
            `[API Cache Invalidation] [ERROR] ChatwitInbox not found for cache invalidation:`,
            {
              operation: "DELETE /mapeamentos",
              intentName: existingMapping.intentName,
              mappingId,
        ...
        console.error(
          "[API Cache Invalidation] [ERROR] Error clearing Instagram cache:",
          {
            operation: "DELETE /mapeamentos",
            intentName: existingMapping?.intentName,
            mappingId,
