/**
 * Flow Analytics API - Funnel Endpoint
 * Conversion funnel visualization endpoint with Redis caching
 *
...
import { calculateFunnelData } from "@/lib/flow-analytics/funnel-service";
import type { FunnelStep, ApiResponse } from "@/types/flow-analytics";
...
const CACHE_KEY_PREFIX = "flow-analytics:funnel";
 ...
 */
function getCacheKey(flowId: string, inboxId?: string, dateStart?: string, dateEnd?: string): string {
...
// =============================================================================
// GET - Obter dados de funil para um flow
// =============================================================================
 ...
 * - 3.1: Display visual funnel chart
 * - 3.2: Display absolute count of sessions per step
 * - 3.3: Display percentage relative to START node
 * - 3.4: Calculate and display drop-off percentage between steps
 * - 3.5: Identify and highlight step with highest drop-off
 * - 3.6: Display detailed information when clicking funnel step
 * - 19.3: Funnel API endpoint with caching
 */
export async function GET(request: NextRequest) {
	try {
		...
		try {
			...
			if (cachedData) {
				const parsed = JSON.parse(cachedData) as FunnelStep[];
				return NextResponse.json<ApiResponse<FunnelStep[]>>({
					success: true,
					data: parsed,
				});
			}
			...
			console.warn("[funnel] Cache read error:", cacheError);
		}
		...
		try {
			const redisClient = getRedisInstance();
			await redisClient.setex(cacheKey, CACHE_TTL_SECONDS, JSON.stringify(funnelData));
		} catch (cacheError) {
			// Log cache error but return data anyway
			console.warn("[funnel] Cache write error:", cacheError);
		}
