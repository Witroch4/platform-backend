...
export async function GET(request: NextRequest) {
	try {
		...
		if (reactionId) {
			...
			if (!reaction) {
				return NextResponse.json({ error: "Reação não encontrada ou acesso negado" }, { status: 404 });
			}
...
export async function POST(request: NextRequest) {
	try {
		...
		if (body.messageId && Array.isArray(body.reactions)) {
			...
			if (!message) {
				return NextResponse.json({ error: "Mensagem não encontrada ou acesso negado" }, { status: 404 });
			}
			...
			if (errors.length > 0) {
				return NextResponse.json({ error: "Dados de reação inválidos", details: errors }, { status: 400 });
			}
							...
							data: {
								...
								description:
									reactionData.reaction.type === "text"
										...
										: ((reactionData.reaction.type === "action" ? reactionData.reaction.value : null) as any),
							},
						});
		...
		if (existingReaction) {
			return NextResponse.json({ error: "Já existe uma reação configurada para este botão" }, { status: 409 });
		}
		...
		const createdReaction = await getPrismaInstance().mapeamentoBotao.create({
			data: {
				...
				description:
					reaction?.type === "text"
						? reaction.value
						: (reaction?.type === "action" ? reaction.value : description) || (null as any),
			},
			include: {
				inbox: {
					select: {
		...
		if (error instanceof Error) {
			if (error.message.includes("Unique constraint")) {
				return NextResponse.json({ error: "Já existe uma reação para este botão" }, { status: 409 });
			}
...
export async function PUT(request: NextRequest) {
	try {
		...
		if (messageId && messageId !== existingReaction.inboxId) {
			...
			if (!message) {
				return NextResponse.json({ error: "Nova mensagem não encontrada ou acesso negado" }, { status: 404 });
			}
		}
		...
		if (error instanceof Error) {
			...
			if (error.message.includes("Foreign key constraint")) {
				return NextResponse.json({ error: "Referência inválida (mensagem não encontrada)" }, { status: 400 });
			}
		}
...
export async function DELETE(request: NextRequest) {
	try {
		...
		return NextResponse.json({ error: "id, buttonId ou messageId é obrigatório" }, { status: 400 });
	} catch (error) {
		...
		if (error instanceof Error) {
			if (error.message.includes("Record to delete does not exist")) {
				return NextResponse.json({ error: "Reação não encontrada" }, { status: 404 });
			}
		}
