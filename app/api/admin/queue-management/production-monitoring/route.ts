      ...
      default:
        ...
        return NextResponse.json({
          ...
          data: {
            ...
            connections: {
              status: monitor.getConnectionsHealth().reduce((acc, conn) => {
  ...
  try {
    ...
    switch (action) {
      ...
      case 'execute_recovery':
        if (!data.procedureId) {
          return NextResponse.json(
            { success: false, error: 'procedureId é obrigatório' },
            { status: 400 }
          );
        }
      ...
      case 'rollback_recovery':
        if (!data.executionId) {
          return NextResponse.json(
            { success: false, error: 'executionId é obrigatório' },
            { status: 400 }
          );
        }
      ...
      default:
        return NextResponse.json(
          { success: false, error: `Ação não reconhecida: ${action}` },
          { status: 400 }
        );
    }
  } catch (error) {
