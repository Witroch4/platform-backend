...
function detectSeasonalPatterns(metrics: QueueMetrics[]): AnalyticsData['seasonalPatterns'] {
  ...
  if (metrics.length >= 7 * 24 * 12) { // 7 days of 5-minute intervals
...
function generateInsights(trends: AnalyticsData['trends'], anomalies: AnalyticsData['anomalies'], patterns: AnalyticsData['seasonalPatterns']): AnalyticsData['insights'] {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const allQueueNames = queueName === 'all' 
    ...
    switch (action) {
      ...
      case 'forecast':
        ...
        for (const name of allQueueNames) {
          ...
          const recentAvgThroughput = recentMetrics.reduce((sum, m) => sum + m.throughput.jobsPerMinute, 0) / recentMetrics.length;
          const olderAvgThroughput = olderMetrics.reduce((sum, m) => sum + m.throughput.jobsPerMinute, 0) / olderMetrics.length;
