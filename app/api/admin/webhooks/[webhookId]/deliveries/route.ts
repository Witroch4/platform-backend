		...
		for (const deliveryId of deliveryIds) {
			try {
				...
				switch (action) {
					...
					case "cancel":
						await webhookManager.markDeliveryAsFailed(deliveryId, "Delivery cancelled");
						result = true;
						message = "Delivery cancelled";
						break;

					case "mark_success":
						await webhookManager.markDeliveryAsDelivered(deliveryId, 200, "Manually marked as successful", 0);
						result = true;
...
function calculateAverageResponseTime(deliveries: WebhookDelivery[]): number {
	const successfulDeliveries = deliveries.filter(
		(d: WebhookDelivery) => d.status === "success" && d.responseTime !== undefined && d.responseTime !== null,
	);

	if (successfulDeliveries.length === 0) return 0;

	const sum = successfulDeliveries.reduce((acc: number, d: WebhookDelivery) => acc + (d.responseTime ?? 0), 0);
...
function calculateSuccessRate(deliveries: WebhookDelivery[]): number {
	...
	const completedDeliveries = deliveries.filter(
		(d: WebhookDelivery) => d.status === "success" || d.status === "failed",
	);

	if (completedDeliveries.length === 0) return 0;

	const successfulDeliveries = completedDeliveries.filter((d: WebhookDelivery) => d.status === "success");
