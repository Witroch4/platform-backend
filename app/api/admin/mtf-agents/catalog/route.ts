...
const agentTypeCatalog = [
	...
	},
	{
		...
		capabilities: [
			...
			"Preferível para respostas estruturadas",
			"Melhor com modelos OpenAI recentes",
		],
	},
	{
...
async function loadDynamicModels(): Promise<Array<{ value: string; label: string; category?: string }>> {
	try {
		...
		const geminiCount = isGeminiAvailable()
			? getAvailableVisionModels().filter((m) => m.provider === "gemini").length
			: 0;
		console.log(`[MTF Catalog] ✅ ${formattedModels.length} modelos carregados`);
		console.log(
			`[MTF Catalog] 📊 Gemini: ${geminiCount}, Vision: ${visionModels.length}, Chat: ${chatModels.length}, Reasoning: ${reasoningModels.length}`,
		);
