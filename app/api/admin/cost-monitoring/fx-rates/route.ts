	...
	try {
		...
		if (session.user.role !== "ADMIN" && session.user.role !== "SUPERADMIN") {
			return NextResponse.json(
				{
					error: "Acesso negado. Apenas administradores podem acessar dados de taxa de câmbio.",
				},
				{ status: 403 },
			);
		}
		...
		switch (action) {
			...
			default:
				return NextResponse.json({ error: "Ação não reconhecida. Use: current, history, convert" }, { status: 400 });
		}
	} catch (error) {
...
async function getRateHistory(searchParams: URLSearchParams) {
	try {
		...
		if (isNaN(startDate.getTime()) || isNaN(endDate.getTime())) {
			return NextResponse.json({ error: "Datas inválidas. Use formato YYYY-MM-DD" }, { status: 400 });
		}
		...
		if (endDate.getTime() - startDate.getTime() > oneYearInMs) {
			return NextResponse.json({ error: "Período máximo de consulta é 1 ano" }, { status: 400 });
		}
...
async function convertCurrency(searchParams: URLSearchParams) {
	try {
		...
		if (!amountStr) {
			return NextResponse.json({ error: "Parâmetro amount é obrigatório" }, { status: 400 });
		}
...
export async function POST(request: NextRequest) {
	try {
		...
		if (session.user.role !== "ADMIN" && session.user.role !== "SUPERADMIN") {
			return NextResponse.json(
				{
					error: "Acesso negado. Apenas administradores podem atualizar taxas de câmbio.",
				},
				{ status: 403 },
			);
		}
		...
		switch (action) {
			...
			default:
				return NextResponse.json({ error: "Ação não reconhecida. Use: update, backfill" }, { status: 400 });
		}
	} catch (error) {
