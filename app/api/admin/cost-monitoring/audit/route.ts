  ...
  try {
    ...
    if (session.user.role !== "ADMIN" && session.user.role !== "SUPERADMIN") {
      return NextResponse.json(
        {
          error:
            "Acesso negado. Apenas administradores podem acessar logs de auditoria.",
        },
        { status: 403 }
      );
    }
    ...
    if (limit > 1000) {
      return NextResponse.json(
        { error: "Limite máximo de 1000 registros por consulta." },
        { status: 400 }
      );
    }
  ...
  try {
    ...
    if (session.user.role !== "ADMIN" && session.user.role !== "SUPERADMIN") {
      return NextResponse.json(
        {
          error:
            "Acesso negado. Apenas administradores podem acessar estatísticas de auditoria.",
        },
        { status: 403 }
      );
    }
