...
const budgetSchema = z.object({
	...
	alertAt: z.number().min(0).max(1).default(0.8), // 80% por padrão
		...
		const budgetsWithSpending = await Promise.all(
			budgets.map(async (budget) => {
				...
				return {
					...
					status: percentage >= 1 ? "EXCEEDED" : percentage >= Number(budget.alertAt) ? "WARNING" : "OK",
				};
			}),
		);
...
export async function POST(request: NextRequest) {
	try {
		...
		if (!validatedData.inboxId && !validatedData.userId) {
			return NextResponse.json({ error: "Orçamento deve ser associado a um inbox ou usuário" }, { status: 400 });
		}
