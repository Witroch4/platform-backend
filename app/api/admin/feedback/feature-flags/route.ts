...
export async function GET(request: NextRequest) {
	try {
		...
		return NextResponse.json({ error: "Failed to get feature flag feedback" }, { status: 500 });
	}
}
...
export async function POST(request: NextRequest) {
	try {
		...
		const { userId, flagName, enabled, variant, experience, description, metadata } = body;
		...
		if (!["POSITIVE", "NEGATIVE", "NEUTRAL"].includes(experience)) {
			return NextResponse.json({ error: "Experience must be POSITIVE, NEGATIVE, or NEUTRAL" }, { status: 400 });
		}
		...
		return NextResponse.json({ error: "Failed to submit feature flag feedback" }, { status: 500 });
	}
}
