...
function calculatePercentiles(buckets: Array<{ le: number; count: number }>, totalCount: number): {
  p50: number;
  p95: number;
  p99: number;
} {
...
function checkAlerts(metrics: any[]): DashboardData['alerts'] {
  ...
  latencyMetrics.forEach(metric => {
    if (metric.type === 'histogram') {
      ...
      if (percentiles.p95 > 2500) { // 2.5s threshold
        alerts.push({
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const recentErrors = logAggregator.search({
      level: 'error',
      limit: 10,
    ...
    const recentRateLimits = logAggregator.search({
