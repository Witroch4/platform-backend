  ...
  try {
    ...
    if (query.queueName) {
      ...
      for (const queue of dashboard.queues) {
        metricsData.queues[queue.name] = await getQueueMetrics(
          queue.name, 
          startTime, 
          endTime, 
          query
  ...
  try {
    ...
    if (exportConfig.queueNames && exportConfig.queueNames.length > 0) {
      ...
      for (const queueName of exportConfig.queueNames) {
        exportData.queues[queueName] = await getDetailedQueueMetrics(
          queueName,
          startTime,
          endTime,
          exportConfig
      ...
      for (const queue of dashboard.queues) {
        exportData.queues[queue.name] = await getDetailedQueueMetrics(
          queue.name,
          startTime,
          endTime,
          exportConfig
...
async function getDetailedQueueMetrics(
  queueName: string,
  startTime: Date,
  endTime: Date,
  config: any
  ...
  let filteredJobs = jobMetrics.filter(job => 
    job.createdAt >= startTime && job.createdAt <= endTime
  );
  ...
  if (config.filters) {
    if (config.filters.jobTypes) {
      filteredJobs = filteredJobs.filter(job => 
        config.filters.jobTypes.includes(job.jobName)
      );
    }

    if (config.filters.statuses) {
      filteredJobs = filteredJobs.filter(job => 
        config.filters.statuses.includes(job.status)
      );
    }

    if (config.filters.correlationIds) {
      filteredJobs = filteredJobs.filter(job => 
        job.correlationId && config.filters.correlationIds.includes(job.correlationId)
      );
    }
  }
