...
export async function GET(request: NextRequest) {
	try {
		...
		return NextResponse.json({ error: "Failed to get A/B tests" }, { status: 500 });
	}
}
...
export async function POST(request: NextRequest) {
	try {
		...
		if (!variants.control || !variants.treatment) {
			return NextResponse.json({ error: "Both control and treatment variants are required" }, { status: 400 });
		}
