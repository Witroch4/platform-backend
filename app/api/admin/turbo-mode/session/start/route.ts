...
export async function POST(request: NextRequest) {
	try {
		...
		if (!userId || !leadIds || !Array.isArray(leadIds)) {
			return NextResponse.json({ error: "userId e leadIds são obrigatórios." }, { status: 400 });
		}

		if (leadIds.length === 0) {
			return NextResponse.json({ error: "Pelo menos um lead deve ser fornecido." }, { status: 400 });
		}
