...
export async function POST(request: NextRequest): Promise<Response> {
  try {
    ...
    if (primaryLeadId === secondaryLeadId) {
      return NextResponse.json(
        { error: 'Não é possível mesclar um lead com ele mesmo' },
        { status: 400 }
      );
    }
    ...
    if (!primaryLead || !secondaryLead) {
      return NextResponse.json(
        { error: 'Um ou ambos os leads não foram encontrados' },
        { status: 404 }
      );
    }
    ...
    if (primaryLead.source !== secondaryLead.source) {
      return NextResponse.json(
        { error: 'Não é possível mesclar leads de fontes diferentes' },
        { status: 400 }
      );
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (primaryLead.source === LeadSource.INSTAGRAM) {
        if (secondaryLead.instagramProfile && !primaryLead.instagramProfile) {
          ...
          if (secondaryLead.instagramProfile.lastMessageAt && 
