...
export async function GET(request: NextRequest): Promise<Response> {
  try {
    ...
    console.log(`[Leads API] Buscando leads - Página: ${page}, Limite: ${limit}, Source: ${source}, Busca: "${search}"`);
...
export async function POST(request: NextRequest): Promise<Response> {
  try {
    ...
    if (!source || !Object.values(LeadSource).includes(source)) {
      return NextResponse.json(
        { error: 'Source é obrigatório e deve ser válido' },
        { status: 400 }
      );
    }
    ...
    if (!sourceIdentifier) {
      return NextResponse.json(
        { error: 'sourceIdentifier é obrigatório' },
        { status: 400 }
      );
    }
    ...
    if (existingLead) {
      return NextResponse.json(
        { error: 'Já existe um lead com este identificador para esta fonte' },
        { status: 409 }
      );
    }
