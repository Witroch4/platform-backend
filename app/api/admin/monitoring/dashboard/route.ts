	...
	try {
		...
		if (redis) {
			try {
				...
				console.warn("[Dashboard] Redis ping failed, using fallback mode:", (error as Error).message);
		...
		try {
			...
			const { FeatureFlagManager } = await import("@/lib/feature-flags/feature-flag-manager");
			const { ABTestingManager } = await import("@/lib/feature-flags/ab-testing-manager");
			const { FeedbackCollector, getFeedbackCollector } = await import("@/lib/feedback/feedback-collector");
			...
			const featureFlagMetrics = await getFeatureFlagMetricsSimple(featureFlags);
			...
			console.warn("[Dashboard] Manager initialization failed, using simplified mode:", managerError);
			return NextResponse.json(await getSimplifiedDashboard(timeRangeParam));
		}
	} catch (error: unknown) {
...
async function getFeatureFlagMetricsSimple(flags: any[]) {
	...
	const rolloutFlags = flags.filter((f) => f.enabled && f.rolloutPercentage < 100).length;
...
function getDefaultRecommendations(redisConnected = false) {
	...
	if (!redisConnected) {
		recommendations.push({
			...
			description: "Redis connection failed. Some monitoring features are limited.",
			actions: ["Check Redis container status", "Verify REDIS_URL=redis://redis:6379", "Restart Redis container"],
		});
	} else {
		recommendations.push({
			type: "SYSTEM_HEALTH",
