...
export async function GET(request: NextRequest) {
	try {
		...
		alerts = alerts.sort((a, b) => b.timestamp.getTime() - a.timestamp.getTime()).slice(0, limit);
...
export async function POST(request: NextRequest) {
	try {
		...
		if (!alertId || !action) {
			return NextResponse.json({ error: "Missing required fields: alertId and action" }, { status: 400 });
		}
