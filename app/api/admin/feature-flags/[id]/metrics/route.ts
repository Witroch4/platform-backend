  ...
  try {
    ...
    if (!session?.user?.id || session.user.role !== "SUPERADMIN") {
      return NextResponse.json(
        { error: "Acesso negado. Apenas SUPERADMIN pode acessar métricas." },
        { status: 403 }
      );
    }
    ...
    const averageLatency = metrics.length > 0 
      ? metrics.reduce((sum, m) => sum + m.averageLatencyMs, 0) / metrics.length 
      : 0;
    const successRate = (totalEnabled + totalDisabled) > 0 
      ? (totalEnabled / (totalEnabled + totalDisabled)) * 100 
      : 0;
    const lastEvaluated = metrics.length > 0 && metrics[0].lastEvaluatedAt 
      ? metrics[0].lastEvaluatedAt 
      : null;
