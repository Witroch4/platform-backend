...
export async function GET(request: NextRequest) {
	try {
		...
		if (!session?.user?.id || session.user.role !== "SUPERADMIN") {
			return NextResponse.json({ error: "Acesso negado. Apenas SUPERADMIN pode acessar overrides." }, { status: 403 });
		}
...
export async function POST(request: NextRequest) {
	try {
		...
		if (!session?.user?.id || session.user.role !== "SUPERADMIN") {
			return NextResponse.json({ error: "Acesso negado. Apenas SUPERADMIN pode criar overrides." }, { status: 403 });
		}
		...
		if (!flagId || !userId || enabled === undefined) {
			return NextResponse.json({ error: "flagId, userId e enabled são obrigatórios" }, { status: 400 });
		}
