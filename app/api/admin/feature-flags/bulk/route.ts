...
export async function POST(request: NextRequest) {
	try {
		...
		if (!operation || !flagIds || !Array.isArray(flagIds)) {
			return NextResponse.json({ error: "operation e flagIds são obrigatórios" }, { status: 400 });
		}
		...
		switch (operation) {
			case "toggle":
				...
				if (enabled === undefined) {
					return NextResponse.json({ error: "enabled é obrigatório para operação toggle" }, { status: 400 });
				}
			...
			case "update_rollout":
				...
				if (rolloutPercentage === undefined || rolloutPercentage < 0 || rolloutPercentage > 100) {
					return NextResponse.json({ error: "rolloutPercentage deve estar entre 0 e 100" }, { status: 400 });
				}
