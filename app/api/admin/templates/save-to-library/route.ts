...
export async function POST(request: NextRequest) {
	try {
		...
		if (!templateData.name || !templateData.bodyText) {
			return NextResponse.json({ error: "Nome e texto do corpo são obrigatórios" }, { status: 400 });
		}

		// Extract variables from all text fields
		const allText = [templateData.headerText, templateData.bodyText, templateData.footerText].filter(Boolean).join(" ");
		...
		const content = {
			...
			mediaType:
				templateData.headerType !== "TEXT" && templateData.headerType !== "NONE"
		...
		return NextResponse.json({ error: "Falha ao salvar na biblioteca" }, { status: 500 });
	}
}
