...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }): Promise<Response> {
	try {
		...
		if (!existingTemplate) {
			return NextResponse.json({ error: "Template não encontrado ou sem permissão para editar" }, { status: 404 });
		}
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }): Promise<Response> {
	try {
		...
		if (!existingTemplate) {
			return NextResponse.json({ error: "Template não encontrado ou sem permissão para deletar" }, { status: 404 });
		}
