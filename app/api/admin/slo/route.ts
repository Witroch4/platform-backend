...
export async function GET(request: NextRequest) {
	...
	try {
		...
		switch (action) {
			case "report":
				if (accountId) {
					responseData = sloMeasurementService.generateAccountSLOReport(parseInt(accountId), windowMinutes);
				} else if (channel) {
					responseData = sloMeasurementService.generateChannelSLOReport(channel, windowMinutes);
				} else {
