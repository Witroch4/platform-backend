...
function validateImportData(data: any): { valid: boolean; errors: string[] } {
	...
	if (data.intents && Array.isArray(data.intents)) {
		for (let i = 0; i < data.intents.length; i++) {
			...
			if (
				intent.similarityThreshold !== undefined &&
				(typeof intent.similarityThreshold !== "number" ||
					intent.similarityThreshold < 0 ||
					intent.similarityThreshold > 1)
			) {
			...
			for (const templateData of importData.templates) {
				try {
					...
					result.summary.errors.push(
						`Erro ao importar template "${templateData.name}": ${error instanceof Error ? error.message : String(error)}`,
					);
					result.details.processedTemplates.push({
						originalId: templateData.id,
						name: templateData.name,
		...
		for (const intentData of importData.intents) {
			try {
				...
				const existing = await prisma.intent.findFirst({
					where: { name: intentData.name, createdById: session.user.id },
				...
				if (intentData.embedding && intentData.embedding.centroid && Array.isArray(intentData.embedding.centroid)) {
					...
					// Regenerar embedding
					const newEmbedding = await regenerateEmbeddingForIntent(intentId, intentData.name, intentData.description);
				...
				result.summary.errors.push(
					`Erro ao importar intenção "${intentData.name}": ${error instanceof Error ? error.message : String(error)}`,
				);
				result.details.processedIntents.push({
					originalId: intentData.id,
					name: intentData.name,
