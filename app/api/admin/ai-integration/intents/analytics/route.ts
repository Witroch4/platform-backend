...
export async function GET(request: NextRequest) {
	try {
		...
		// Get analytics data
		const [totalClassifications, successfulMatches, avgProcessingTime, topIntents, confidenceDistribution, dailyStats] =
			await Promise.all([
		...
		const processedTopIntents = topIntents.map((intent) => {
			...
			const averageConfidence =
				totalHits > 0 ? intent.hitLogs.reduce((sum, log) => sum + log.similarity, 0) / totalHits : 0;
