...
export async function POST(request: NextRequest) {
	...
	if (!(prisma as any).aiAssistant) {
		return NextResponse.json(
			{
				error:
					"Modelo AiAssistant indisponível. Rode as migrações/gera o client (npx prisma db push && npx prisma generate).",
			},
			{ status: 503 },
		);
	}
	...
	try {
		const created = await prisma.aiAssistant.create({
			data: {
				...
				tempSchema:
					typeof body?.tempSchema === "number" && body.tempSchema >= 0 && body.tempSchema <= 2 ? body.tempSchema : 0.1,
				tempCopy: typeof body?.tempCopy === "number" && body.tempCopy >= 0 && body.tempCopy <= 2 ? body.tempCopy : 0.4,
				maxOutputTokens:
					typeof body?.maxOutputTokens === "number" && body.maxOutputTokens >= 64 ? body.maxOutputTokens : 1380,
				warmupDeadlineMs:
					typeof body?.warmupDeadlineMs === "number" && body.warmupDeadlineMs > 0 ? body.warmupDeadlineMs : 15000,
				hardDeadlineMs:
					typeof body?.hardDeadlineMs === "number" && body.hardDeadlineMs > 0 ? body.hardDeadlineMs : 15000,
				softDeadlineMs:
					typeof body?.softDeadlineMs === "number" && body.softDeadlineMs > 0 ? body.softDeadlineMs : 15000,
		...
		return NextResponse.json(
			{ error: "Falha ao criar assistente", hint, detail: e?.message || String(e) },
			{ status: 500 },
		);
	}
}
...
export async function PATCH(request: NextRequest) {
	...
	if (typeof body?.disableIntentSuggestion === "boolean")
		updateData.disableIntentSuggestion = body.disableIntentSuggestion;
	if (typeof body?.enableAutoRemarketing === "boolean") updateData.enableAutoRemarketing = body.enableAutoRemarketing;
	if (
		typeof body?.remarketingDelayMinutes === "number" &&
		body.remarketingDelayMinutes >= 5 &&
		body.remarketingDelayMinutes <= 1440
	)
		updateData.remarketingDelayMinutes = body.remarketingDelayMinutes;
	...
	if (["minimal", "low", "medium", "high"].includes(body?.reasoningEffort))
		updateData.reasoningEffort = body.reasoningEffort;
	...
	if (typeof body?.tempSchema === "number" && body.tempSchema >= 0 && body.tempSchema <= 2)
		updateData.tempSchema = body.tempSchema;
	if (typeof body?.tempCopy === "number" && body.tempCopy >= 0 && body.tempCopy <= 2)
		updateData.tempCopy = body.tempCopy;
	...
	if (typeof body?.warmupDeadlineMs === "number" && body.warmupDeadlineMs > 0)
		updateData.warmupDeadlineMs = body.warmupDeadlineMs;
	if (typeof body?.hardDeadlineMs === "number" && body.hardDeadlineMs > 0)
		updateData.hardDeadlineMs = body.hardDeadlineMs;
	if (typeof body?.softDeadlineMs === "number" && body.softDeadlineMs > 0)
		updateData.softDeadlineMs = body.softDeadlineMs;
