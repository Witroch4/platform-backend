...
export async function GET(request: NextRequest) {
	...
	const links =
		(await (prisma as any).aiAssistantInbox?.findMany?.({
			where: { assistantId },
			include: {
				inbox: true,
			},
	...
	const attachedMap = new Map<string, any>(links.map((l: any) => [l?.inbox?.inboxId, l]));
	...
	logger.info("Resultado para UI de conexão", {
		total: result.length,
		attached: result.filter((r) => r.attached).length,
	});
...
export async function POST(request: NextRequest) {
	...
	if (!assistantId || !inboxId)
		return NextResponse.json({ error: "assistantId e inboxId são obrigatórios" }, { status: 400 });
