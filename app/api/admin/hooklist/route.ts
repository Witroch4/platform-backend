  ...
  try {
    ...
    if (!baseURL) {
      return NextResponse.json(
        { error: 'CHATWIT_BASE_URL não configurado' },
        { status: 500 }
      );
    }
    ...
    if (!globalAccessToken) {
      return NextResponse.json(
        { error: 'CHATWIT_ACCESS_TOKEN não configurado' },
        { status: 500 }
      );
    }
      ...
      if (allAccounts.length > 0) {
        ...
        try {
          ...
          const appsResponse = await axios.get(
            `${baseURL}/api/v1/accounts/${account.id}/integrations/apps`,
            {
              headers: {
                'api_access_token': globalAccessToken,
                'Content-Type': 'application/json',
            ...
            if (app.hooks && Array.isArray(app.hooks)) {
              ...
              for (const hook of app.hooks) {
                ...
                allHooks.push({
                  ...
                  usuarioEmail: usuarioLocal ? userMap.get(usuarioLocal.appUserId)?.email || '' : 'external@chatwit.com',
