...
export async function POST(request: NextRequest): Promise<NextResponse> {
	try {
		...
		if (!userText || !agent) {
			return NextResponse.json({ error: "Parâmetros 'userText' e 'agent' são obrigatórios." }, { status: 400 });
		}
		...
		if (!result) {
			return NextResponse.json({ error: "Falha ao gerar botões de chat livre." }, { status: 500 });
		}
