...
import AgendamentoForm from "@/app/gestao-social/[accountid]/dashboard/agendamento/components/AgendamentoForm";
import AgendamentosList from "@/app/gestao-social/[accountid]/dashboard/agendamento/components/AgendamentosList";
...
const AgendamentoDePostagens: React.FC = () => {
	...
	const handleAgendar = async () => {
		...
		try {
			...
			try {
				...
				if (response.status === 200 || response.status === 201) {
					toast("Agendamento Criado com Sucesso!", {
						description: `Data: ${format(dateTime, "dd/MM/yyyy")} às ${format(dateTime, "HH:mm:ss")}`,
						action: (
							<Button
								variant="link"
								onClick={() => {
			...
			const errorMsg =
				error.response?.data?.error || error.response?.data?.details || "Ocorreu um erro ao agendar a postagem.";
				...
				<section>
					...
					{session && (
						<>
							...
							{!loading && agendamentos.length > 0 && (
								<AgendamentosList agendamentos={agendamentos} refetch={refetch} accountid={accountid} />
							)}
						</>
					)}
				</section>
