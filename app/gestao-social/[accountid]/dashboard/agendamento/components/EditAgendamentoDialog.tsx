	...
	const handleEditar = async () => {
		...
		try {
			...
			if (agendamento.isGrupo && agendamento.id) {
				const response = await axios.patch(`/api/${accountid}/agendar/update-grupo/${agendamento.id}`, updatedRow, {
				...
				const response = await axios.patch(`/api/${accountid}/agendar/update/${agendamento.id}`, updatedRow, {
					headers: { "Content-Type": "application/json" },
				});
			...
			<DialogContent className="w-full sm:w-full max-w-lg sm:max-w-[600px] max-h-[calc(100vh-100px)] p-4 md:p-6">
				...
				<ScrollArea className="max-h-[calc(100vh-250px)]">
					<div className="flex flex-col space-y-4 pr-4">
						...
						<DateTimePicker date={date} setDate={handleDateChange} isPostagemDiaria={isPostagemDiaria} />
