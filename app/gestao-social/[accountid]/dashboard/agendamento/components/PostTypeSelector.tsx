...
const PostTypeSelector: React.FC<PostTypeSelectorProps> = ({
	...
	const handlePostagemDiariaChange = (checked: boolean) => {
		if (checked) {
			setTipoPostagem((prev) => (prev.includes("Diario") ? prev : [...prev, "Diario"]));
			toast("Postagem Diária Ativada", {
	...
	const handlePostagemSemanalChange = (checked: boolean) => {
		if (checked) {
			setTipoPostagem((prev) => (prev.includes("Semanal") ? prev : [...prev, "Semanal"]));
			toast("Postagem Semanal Ativada", {
	...
	const handlePostagemAleatoriaChange = (checked: boolean) => {
		if (checked) {
			setTipoPostagem((prev) => (prev.includes("Aleatório") ? prev : [...prev, "Aleatório"]));

			if (isPostagemDiaria) {
				toast("Postagem Diária + Aleatória Ativada", {
					description: "Esta postagem será publicada diariamente em um horário aleatório.",
				});
			}
		} else {
			...
			<div className="space-y-4 pt-2 border-t border-border">
				<div className="flex items-center justify-between">
					<div className="flex items-center space-x-2">
						<Switch id="postagem-diaria" checked={isPostagemDiaria} onCheckedChange={handlePostagemDiariaChange} />
						<Label htmlFor="postagem-diaria" className="text-foreground">
							Postagem Diária
						</Label>
					</div>
				</div>
				...
				<div className="flex items-center justify-between">
					<div className="flex items-center space-x-2">
						<Switch id="postagem-semanal" checked={isPostagemSemanal} onCheckedChange={handlePostagemSemanalChange} />
						<Label htmlFor="postagem-semanal" className="text-foreground">
							Postagem Semanal
						</Label>
					</div>
				</div>
				...
				<div className="flex items-center justify-between">
					<div className="flex items-center space-x-2">
						...
						/>
						<Label htmlFor="postagem-aleatoria" className="text-foreground">
							Postagem Aleatória
						</Label>
						...
						<TooltipProvider>
							<Tooltip>
								...
								<TooltipContent className="max-w-md p-4 text-sm bg-popover border-border text-popover-foreground">
									<div className="space-y-2">
										<p>
											Ativar essa opção permite modificar como o sistema trata múltiplas mídias ao criar uma postagem.
										</p>
										...
										<div>
											...
											<p>
												- Se essa opção for ativada, todas as mídias carregadas serão postadas juntas como um único
												post.
											</p>
										...
										<div>
											...
											<p>
												- Se essa opção for ativada, cada mídia carregada será tratada como uma postagem individual.
											</p>
											<p>- Caso uma descrição seja preenchida, ela será replicada para cada postagem gerada.</p>
											<p>
												- Se a "Postagem Diária" ou "Postagem Semanal" estiver ativada, uma mídia será postada por
												período (dia/semana).
											</p>
										</div>
										<p className="font-semibold mt-2">Importante:</p>
										<p>
											- Se a "Postagem Aleatória" estiver desativada, o comportamento padrão do sistema será postar
											todas as mídias como um único post.
										</p>
										<p>
											- Se houver mais de uma postagem programada para a mesma hora e dia, o sistema escolherá
											aleatoriamente qual delas será publicada.
										</p>
