...
export default function CalendarioPage() {
	...
	const appointmentsForSelectedDay = selectedDay
		? agendamentos.filter((ag) => isSameDay(new Date(ag.Data), selectedDay))
		: [];

	// Cria um conjunto de strings (no formato "yyyy-MM-dd") para os dias que possuem agendamento
	const appointmentDays = new Set(agendamentos.map((ag) => format(new Date(ag.Data), "yyyy-MM-dd")));
	...
	const handleDelete = async (agendamentoId: string) => {
		try {
			...
			toast("Erro ao Excluir", {
				description: err.response?.data?.message || "Ocorreu um erro ao excluir o agendamento.",
			});
		}
	};
	...
	const handleEditarAgendamento = (agendamentoId: string) => {
		...
		router.push(`/gestao-social/${params.accountid}/dashboard/agendamento/editar/${agendamentoId}`);
	};
	...
	return (
		<div className="min-h-screen bg-background">
			<main className="p-4 sm:p-10 space-y-4">
              ...
              */
							modifiers={{
								hasAppointments: (date: Date) => appointmentDays.has(format(date, "yyyy-MM-dd")),
							}}
							modifiersClassNames={{
					...
					<DialogContent className="max-w-md bg-background border-border">
						<DialogHeader>
							<DialogTitle className="text-foreground">
								Agendamentos para {selectedDay ? format(selectedDay, "dd/MM/yyyy") : ""}
							</DialogTitle>
						...
						<div className="space-y-4 my-4">
							{appointmentsForSelectedDay.map((ag) => (
								<div key={ag.id} className="flex items-center justify-between p-3 border border-border rounded bg-card">
									<div>
										<p className="font-semibold text-card-foreground">{format(new Date(ag.Data), "HH:mm:ss")}</p>
										<p className="text-sm text-muted-foreground">{ag.Descricao}</p>
									</div>
									<div className="flex gap-2">
										<Button onClick={() => handleEditarAgendamento(ag.id)}>Editar</Button>
										<Button variant="destructive" onClick={() => handleDelete(ag.id)}>
											Excluir
										</Button>
									</div>
								</div>
