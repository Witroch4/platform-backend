				...
				if (template.interactiveContent) {
					return (
						<div className="text-xs text-muted-foreground">
							<div className="font-medium">{template.interactiveContent.body.text.substring(0, 50)}...</div>
							{template.interactiveContent.header && (
								<div className="text-xs opacity-75">Header: {template.interactiveContent.header.type}</div>
							)}
						</div>
					);
				}
							...
							<TableBody>
								{templates.map((template) => (
									<TableRow key={template.id} className="hover:bg-muted/50">
										...
										<TableCell>
											<div className="text-sm">{template.stats.mapeamentosCount} mapeamentos</div>
											...
											{template.mapeamentos.length > 2 && (
												<div className="text-xs text-muted-foreground">+{template.mapeamentos.length - 2} mais</div>
											)}
										</TableCell>
										<TableCell>
