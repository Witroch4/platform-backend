...
import { Plus, Edit, Trash2, Shield, ExternalLink, Copy, CheckCircle, XCircle } from "lucide-react";
		...
		return (
			<div className="flex items-center justify-center h-screen">
				<div className="text-center">
					...
					<p className="text-muted-foreground">Apenas SUPERADMIN pode acessar esta página.</p>
				</div>
			</div>
		);
	}
		...
		<div className="container mx-auto p-6 space-y-6">
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-muted-foreground">Gerencie domínios autorizados para acesso via iframe</p>
				</div>
				<Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
					...
					<DialogContent>
						<DialogHeader>
							...
							<DialogDescription>Adicione um novo domínio que pode acessar o dashboard via iframe</DialogDescription>
						</DialogHeader>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>Use esta URL para configurar o dashboard como iframe no Chatwit</CardDescription>
				</CardHeader>
				<CardContent>
					<div className="flex items-center gap-2">
						<Input value={iframeUrl || "Carregando..."} readOnly className="font-mono text-sm" />
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>Lista de domínios que podem acessar o dashboard via iframe</CardDescription>
				</CardHeader>
				<CardContent>
					{isLoading ? (
								...
								{domains.length === 0 ? (
									<TableRow>
										<TableCell colSpan={6} className="text-center py-8">
											<p className="text-muted-foreground">Nenhum domínio autorizado cadastrado</p>
										</TableCell>
									</TableRow>
								) : (
									domains.map((domain) => (
										<TableRow key={domain.id}>
											...
											<TableCell>
												...
												>
													{domain.isActive ? <CheckCircle className="h-3 w-3" /> : <XCircle className="h-3 w-3" />}
											...
											<TableCell>
												<div className="flex items-center gap-2">
													<Button variant="ghost" onClick={() => handleEdit(domain)}>
			...
			<Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>Atualize as informações do domínio autorizado</DialogDescription>
					</DialogHeader>
					<div className="space-y-4">
						<div>
							<Label htmlFor="edit-domain">Domínio</Label>
							<Input id="edit-domain" value={editDomain} onChange={(e) => setEditDomain(e.target.value)} />
						</div>
						<div>
