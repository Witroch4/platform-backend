...
export function TrendAnalysis({ 
  ...
  const anomalies = useMemo(() => {
    ...
    const anomalies: { timestamp: Date; metric: string; value: number; expected: number; severity: 'low' | 'medium' | 'high' }[] = [];
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Performance trends, patterns, and anomaly detection
          </p>
        </div>
        <div className="flex items-center space-x-2">
          ...
          <Select value={comparisonPeriod} onValueChange={(value: any) => {
            ...
            <CardContent>
              <div className="space-y-2">
                ...
                <div className="text-xs text-muted-foreground">
                  Previous: {formatValue(trend.previous, trend.unit)} 
                  ({trend.change >= 0 ? '+' : ''}{formatValue(Math.abs(trend.change), trend.unit)})
                </div>
              </div>
            </CardContent>
          </Card>
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <span className="ml-2 font-medium">
                        {formatValue(anomaly.value, anomaly.metric.includes('Rate') ? '%' : 
                                   anomaly.metric.includes('Latency') ? 'ms' : 'jobs/min')}
                      </span>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Expected:</span>
                      <span className="ml-2 font-medium">
                        {formatValue(anomaly.expected, anomaly.metric.includes('Rate') ? '%' : 
                                   anomaly.metric.includes('Latency') ? 'ms' : 'jobs/min')}
                      </span>
                    </div>
                  </div>
              ...
              <div className="space-y-2 text-sm">
                {trendData.filter(t => t.status === 'good').length > 0 && (
                  <div className="flex items-center space-x-2 text-green-600">
                    ...
                    <span>
                      {trendData.filter(t => t.status === 'good').length} metrics showing positive trends
                    </span>
                  </div>
                )}
              ...
              <div className="space-y-2 text-sm">
                {trendData.find(t => t.metric === 'Throughput' && t.trend === 'down') && (
                  <div className="p-2 bg-yellow-50 border border-yellow-200 rounded">
                    Consider investigating throughput decline. Check for resource constraints or increased job complexity.
                  </div>
                )}
