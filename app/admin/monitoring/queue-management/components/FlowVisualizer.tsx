...
export function FlowVisualizer({ 
  ...
  useEffect(() => {
    ...
    const filteredNodes = graphNodes.map(node => ({
      ...node,
      isVisible: (statusFilter === 'all' || node.status === statusFilter) &&
                 (searchTerm === '' || 
    ...
    const calculateLayout = () => {
      ...
      if (layoutType === 'hierarchical') {
        ...
        for (let i = 0; i < iterations; i++) {
          ...
          edges.filter(e => e.isVisible).forEach(edge => {
  ...
  const getEdgeStyle = (edge: GraphEdge) => {
    switch (edge.type) {
      case 'dependency': return '5,5';
      case 'conditional': return '10,5';
      default: return 'none';
    }
  };
            ...
            <CardContent className="p-0">
              ...
              >
                ...
                >
                  ...
                  {/* Enhanced Edges */}
                  {edges.filter(e => e.isVisible).map((edge, index) => {
                    ...
                    const pathData = edge.type === 'dependency' || Math.abs(startX - endX) > 100 ?
                    ...
                    return (
                      <g key={`edge-${index}`}>
                        <path
                          ...
                          fill="none"
                          markerEnd={`url(#${edge.isHighlighted ? 'arrowhead-highlighted' : 
                                           edge.type === 'dependency' ? 'arrowhead-dependency' : 'arrowhead'})`}
                    ...
                    return (
                      <g key={node.id}>
                        ...
                        <rect
                          ...
                          height={node.height}
                          rx="8"
                          fill={isSelected ? '#dbeafe' : 
                                isHovered ? '#f3f4f6' : 
                                node.isHighlighted ? '#fef3c7' : 'white'}
                        ...
                        {showMetrics && (node.processingTime || node.waitTime) && (
                          ...
                          >
                            {node.processingTime ? formatDuration(node.processingTime) : 
                             node.waitTime ? `wait: ${formatDuration(node.waitTime)}` : ''}
                          </text>
                        )}
              ...
              {selectedNodeData ? (
                <div className="space-y-4">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <Badge variant={selectedNodeData.status === 'completed' ? 'default' : 
                                   selectedNodeData.status === 'failed' ? 'destructive' : 'secondary'}>
