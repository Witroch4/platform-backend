...
export default function IntentForm({ intent, onSubmit, onCancel }: IntentFormProps) {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		setLoading(true);
		try {
			const url = intent ? `/api/admin/ai-integration/intents/${intent.id}` : "/api/admin/ai-integration/intents";
			...
			if (response.ok) {
				...
				toast.success(isCreating ? "Intenção criada com sucesso!" : "Intenção atualizada com sucesso!");
				onSubmit();
			} else {
		...
		<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
			<Card className="w-full max-w-2xl max-h-[90vh] overflow-y-auto">
				<CardHeader className="flex flex-row items-center justify-between">
					<CardTitle>{intent ? "Edit Intent" : "Create Intent"}</CardTitle>
					...
					<form onSubmit={handleSubmit} className="space-y-4">
						...
						<div className="grid grid-cols-2 gap-4">
							...
							<div>
								...
								/>
								{errors.similarityThreshold && <p className="text-sm text-red-600">{errors.similarityThreshold}</p>}
							</div>
						</div>
