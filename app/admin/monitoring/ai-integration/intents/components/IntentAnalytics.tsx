							...
							<div key={intent.intentId} className="flex items-center justify-between p-4 border rounded-lg">
								<div className="flex items-center gap-3">
									...
									<div>
										...
										<p className="text-sm text-gray-600">Last used: {new Date(intent.lastUsed).toLocaleDateString()}</p>
									</div>
								</div>
					...
					<div className="space-y-3">
						{analytics.confidenceDistribution.map((item, index) => (
							<div key={index} className="flex items-center gap-4">
								...
								<div className="w-16 text-sm text-gray-600 text-right">{item.count.toLocaleString()}</div>
							</div>
						))}
					</div>
				</CardContent>
