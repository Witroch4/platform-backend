...
import { BarChart3, Download, RefreshCw, TrendingUp, Calendar, Activity } from "lucide-react";
...
export default function AnalyticsPage() {
	...
	const fetchMetrics = async () => {
		try {
			...
			const systemResponse = await fetch(`/api/admin/queue-management/metrics?type=system&timeRange=${timeRange}`);
		...
		<div className="space-y-6">
			...
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-muted-foreground">Advanced analytics, performance insights, and trend analysis</p>
				</div>
						...
						<Card>
							...
							<CardContent>
								<div className="space-y-2 text-sm">
									...
									<div className="flex justify-between">
										<span>Queues:</span>
										<span className="font-medium">{selectedQueue === "all" ? "4 queues" : selectedQueue}</span>
									</div>
									<div className="flex justify-between">
										...
										<>
											<div className="flex justify-between">
												...
												<span className="font-medium">
													{(
														queueMetrics.reduce((sum, m) => sum + m.throughput.jobsPerMinute, 0) / queueMetrics.length
													).toFixed(1)}{" "}
													jobs/min
												</span>
											</div>
											...
											<div className="flex justify-between">
												...
												<span className="font-medium text-green-600">
													{(
														queueMetrics.reduce((sum, m) => sum + m.reliability.successRate, 0) / queueMetrics.length
													).toFixed(1)}
													%
												</span>
											</div>
											<div className="flex justify-between">
												...
												<span className="font-medium text-red-600">
													{(
														queueMetrics.reduce((sum, m) => sum + m.reliability.errorRate, 0) / queueMetrics.length
													).toFixed(1)}
													%
												</span>
											</div>
						...
						<Card>
							...
							<CardContent>
								<div className="space-y-2">
									...
									<Button onClick={fetchMetrics} variant="outline" className="w-full">
										<RefreshCw className="h-4 w-4 mr-2" />
										Refresh Data
									</Button>
								</div>
