									...
									<div className="flex items-start justify-between">
										<div className="flex-1">
											<div className="flex items-center space-x-2 mb-2">
												...
												{alert.queueName && <Badge variant="outline">{alert.queueName}</Badge>}
											</div>
										...
										<div className="flex space-x-2 ml-4">
											{alert.status === "active" && (
												<Button variant="outline" onClick={() => handleAcknowledge(alert)}>
													<Check className="h-4 w-4 mr-1" />
													Acknowledge
												</Button>
											)}
											{alert.status === "acknowledged" && (
												<Button variant="outline" onClick={() => handleResolve(alert)}>
													<CheckCircle className="h-4 w-4 mr-1" />
													Resolve
												</Button>
											)}
											<Button variant="outline" onClick={() => onDismissAlert(alert.id)}>
												<X className="h-4 w-4" />
											</Button>
										</div>
									</div>
				...
				<CardContent className="p-0">
					<div className="space-y-1">
						{filteredAlerts.map((alert) => (
							<div key={alert.id} className={`p-4 border-l-4 hover:bg-gray-50 ${getSeverityColor(alert.severity)}`}>
								<div className="flex items-start justify-between">
									<div className="flex-1">
										<div className="flex items-center space-x-2 mb-2">
											{getSeverityIcon(alert.severity)}
										...
										<div className="flex items-center space-x-4 text-xs text-muted-foreground">
											...
											{alert.acknowledgedAt && <span>Ack: {formatRelativeTime(alert.acknowledgedAt)}</span>}
											{alert.resolvedAt && <span>Resolved: {formatRelativeTime(alert.resolvedAt)}</span>}
										</div>
			...
			<Dialog open={showAcknowledgeDialog} onOpenChange={setShowAcknowledgeDialog}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>Acknowledge this alert to indicate you are aware of the issue.</DialogDescription>
					</DialogHeader>
			...
			<Dialog open={showResolveDialog} onOpenChange={setShowResolveDialog}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>Mark this alert as resolved once the issue has been fixed.</DialogDescription>
					</DialogHeader>
