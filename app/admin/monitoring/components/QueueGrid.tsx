...
import { AlertTriangle, CheckCircle, Clock, Pause, Play, RefreshCw, Settings, TrendingUp, XCircle } from "lucide-react";
...
export function QueueGrid({ queues, onQueueSelect, onBatchAction, onRefreshQueue }: QueueGridProps) {
		...
		<div className="space-y-4">
			...
			<div className="flex items-center justify-between">
				...
				<div className="flex space-x-2">
					<Button variant="outline" onClick={() => queues.forEach((q) => onRefreshQueue(q.name))}>
					...
					return (
						...
						>
							<CardHeader className="pb-3">
								<div className="flex items-center justify-between">
									<CardTitle className="text-lg font-semibold truncate">{queue.name}</CardTitle>
									<div className="flex items-center space-x-2">
										{getStatusIcon(queue.status)}
										<Badge variant={getStatusBadgeVariant(queue.status)}>{queue.status}</Badge>
									</div>
								</div>
							...
							<CardContent className="space-y-4">
								...
								<div className="grid grid-cols-2 gap-3 text-sm">
									<div className="space-y-1">
										<div className="flex justify-between">
											...
											<span className="font-medium text-blue-600">{formatNumber(queue.counts.waiting)}</span>
										</div>
										<div className="flex justify-between">
											<span className="text-muted-foreground">Active</span>
											<span className="font-medium text-green-600">{formatNumber(queue.counts.active)}</span>
										</div>
										<div className="flex justify-between">
											<span className="text-muted-foreground">Delayed</span>
											<span className="font-medium text-yellow-600">{formatNumber(queue.counts.delayed)}</span>
										</div>
									</div>
									<div className="space-y-1">
										<div className="flex justify-between">
											<span className="text-muted-foreground">Completed</span>
											<span className="font-medium text-gray-600">{formatNumber(queue.counts.completed)}</span>
										</div>
										<div className="flex justify-between">
											<span className="text-muted-foreground">Failed</span>
											<span className="font-medium text-red-600">{formatNumber(queue.counts.failed)}</span>
										</div>
										<div className="flex justify-between">
											<span className="text-muted-foreground">Total</span>
											<span className="font-medium">{formatNumber(totalJobs)}</span>
										</div>
									</div>
								</div>
								...
								<div className="space-y-2">
									...
									<div className="flex justify-between text-sm">
										...
										<span className="font-medium">{formatDuration(queue.performance.avgProcessingTime)}</span>
									</div>
									<div className="flex justify-between text-sm">
										<span className="text-muted-foreground">Success Rate</span>
										<span
											className={`font-medium ${
												queue.performance.successRate >= 95
													? "text-green-600"
													: queue.performance.successRate >= 90
														? "text-yellow-600"
														: "text-red-600"
											}`}
										>
								...
								<div className="space-y-1">
									...
									<Progress value={queue.performance.successRate} className="h-2" />
								</div>
