...
export function PerformanceDashboard({ 
  ...
  const [selectedMetric, setSelectedMetric] = useState<'throughput' | 'latency' | 'reliability' | 'resources'>('throughput');
  ...
  const performanceOverview = useMemo(() => {
    ...
    // Calculate trends
    const prevAvgThroughput = previous.length > 0 ? 
      previous.reduce((sum, m) => sum + m.throughput.jobsPerMinute, 0) / previous.length : avgThroughput;
    const prevAvgLatency = previous.length > 0 ? 
      previous.reduce((sum, m) => sum + m.latency.p50, 0) / previous.length : avgLatency;
    const prevSuccessRate = previous.length > 0 ? 
      previous.reduce((sum, m) => sum + m.reliability.successRate, 0) / previous.length : successRate;
    const prevErrorRate = previous.length > 0 ? 
      previous.reduce((sum, m) => sum + m.reliability.errorRate, 0) / previous.length : errorRate;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Comprehensive performance metrics and system analytics
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {performanceOverview.avgThroughput.toFixed(1)}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {formatDuration(performanceOverview.avgLatency)}
            </div>
            <div className="flex items-center text-xs mt-1">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">
              {performanceOverview.successRate.toFixed(1)}%
            </div>
            <div className="flex items-center text-xs mt-1">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">
              {performanceOverview.errorRate.toFixed(1)}%
            </div>
            <div className="flex items-center text-xs mt-1">
            ...
            <div>
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className={`font-medium ${performanceOverview.successRate >= 95 ? 'text-green-600' : 
                    performanceOverview.successRate >= 90 ? 'text-yellow-600' : 'text-red-600'}`}>
                    {performanceOverview.successRate >= 95 ? 'Excellent' :
                     performanceOverview.successRate >= 90 ? 'Good' : 'Needs Attention'}
                  </span>
                </div>
              </div>
            </div>
