...
export function JobList({ 
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      ...
      const mockJobs: Job[] = Array.from({ length: Math.min(pagination.limit, 100) }, (_, i) => ({
        ...
        finishedAt: ['completed', 'failed'].includes(state) ? new Date(Date.now() - Math.random() * 6 * 60 * 60 * 1000) : undefined,
  ...
  const getStatusIcon = (status: JobState) => {
    switch (status) {
      ...
      case 'active': return <TrendingUp className="h-4 w-4 text-blue-600" />;
      case 'waiting': return <Clock className="h-4 w-4 text-yellow-600" />;
      case 'delayed': return <Calendar className="h-4 w-4 text-orange-600" />;
            ...
            <Table>
              ...
              <TableBody>
                {filteredJobs.map((job) => {
                  const duration = job.finishedAt && job.processedAt ? 
                    job.finishedAt.getTime() - job.processedAt.getTime() : 
                    job.processedAt ? Date.now() - job.processedAt.getTime() : null;
