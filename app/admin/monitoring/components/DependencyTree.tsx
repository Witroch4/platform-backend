...
export function DependencyTree({ 
  ...
  const dependencyAnalysis = useMemo(() => {
    ...
    const orphanedJobs = Array.from(allNodes.values()).filter(node => 
      node.jobId !== flowTree.rootJob.jobId && node.dependencies.length === 0
    );
    ...
    return {
      ...
      maxDepth: Math.max(...Array.from(allNodes.values()).map(n => 
        treeData.find(t => t.jobId === n.jobId)?.depth || 0
      ))
    };
  }, [flowTree, treeData]);
  ...
  const getStatusBadgeVariant = (status: JobState) => {
    switch (status) {
      ...
      case 'waiting': return 'secondary' as const;
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-sm text-muted-foreground">
              ...
              {node.dependencyCount > 0 && (
                <span> • {node.dependencyCount} dependencies</span>
              )}
            </div>
          </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {node.children.map(child => (
                      <div key={child.jobId} className="flex items-center space-x-1 text-sm bg-blue-100 px-2 py-1 rounded">
        ...
        <CardHeader>
          <CardTitle>
            {viewMode === 'tree' ? 'Tree View' : 
             viewMode === 'flat' ? 'Flat View' : 'Dependency View'}
