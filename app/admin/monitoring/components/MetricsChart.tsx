...
import { BarChart3, LineChart, PieChart, TrendingUp, Download, Maximize2, RefreshCw } from "lucide-react";
...
export function MetricsChart({
	...
	const renderChart = (chartData: MetricsChartData, width = 800, height = 300) => {
		...
		const getX = (timestamp: Date) => ((timestamp.getTime() - minTime) / timeRange) * chartWidth;
				...
				return (
					<svg width={width} height={height} className="overflow-visible">
						<g transform={`translate(${margin.left}, ${margin.top})`}>
							...
							{[0, 0.25, 0.5, 0.75, 1].map((ratio) => (
								<g key={ratio}>
									<line
										...
										strokeWidth={1}
									/>
									<text x={-10} y={chartHeight * ratio + 4} textAnchor="end" className="text-xs fill-gray-500">
							...
							{/* Line */}
							<path d={pathData} fill="none" stroke={chartData.color} strokeWidth={2} />
							...
							{/* X-axis labels */}
							{chartData.data
								.filter((_, i) => i % Math.ceil(chartData.data.length / 5) === 0)
								.map((d, i) => (
									<text
										key={i}
										x={getX(d.timestamp)}
				...
				return (
					<svg width={width} height={height} className="overflow-visible">
						<g transform={`translate(${margin.left}, ${margin.top})`}>
							...
							{chartData.data.map((d, i) => {
								const x =
									(i / chartData.data.length) * chartWidth + (chartWidth / chartData.data.length - barWidth) / 2;
							...
							<div key={index}>
								{chartData.length > 1 && (
									<h4 className="font-medium mb-2 flex items-center">
										<div className="w-3 h-3 rounded mr-2" style={{ backgroundColor: chart.color }} />
										{chart.title}
									</h4>
								)}
