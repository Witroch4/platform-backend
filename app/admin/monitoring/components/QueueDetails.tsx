		...
		<div className="space-y-6">
			...
			<div className="flex items-center justify-between">
				<div className="flex items-center space-x-4">
					...
					<div>
						...
						<p className="text-muted-foreground">Last updated: {new Date(queueHealth.lastUpdated).toLocaleString()}</p>
					</div>
				</div>
			...
			<Card>
				...
				<CardContent>
					<div className="flex flex-wrap gap-2">
						<Button variant="outline" onClick={() => onBatchAction({ action: "pause_queue", queueName })}>
							<Pause className="h-4 w-4 mr-2" />
							Pause Queue
						</Button>
						<Button variant="outline" onClick={() => onBatchAction({ action: "resume_queue", queueName })}>
			...
			<Tabs value={selectedTab} onValueChange={setSelectedTab}>
				<TabsList className="grid w-full grid-cols-6">
					...
					<TabsTrigger value="waiting">Waiting ({formatNumber(queueHealth.counts.waiting)})</TabsTrigger>
					<TabsTrigger value="active">Active ({formatNumber(queueHealth.counts.active)})</TabsTrigger>
					<TabsTrigger value="completed">Completed ({formatNumber(queueHealth.counts.completed)})</TabsTrigger>
					<TabsTrigger value="failed">Failed ({formatNumber(queueHealth.counts.failed)})</TabsTrigger>
					<TabsTrigger value="delayed">Delayed ({formatNumber(queueHealth.counts.delayed)})</TabsTrigger>
				</TabsList>
				...
				<TabsContent value="waiting">
					<JobList queueName={queueName} state="waiting" onJobAction={onJobAction} onBatchAction={onBatchAction} />
				</TabsContent>

				<TabsContent value="active">
					<JobList queueName={queueName} state="active" onJobAction={onJobAction} onBatchAction={onBatchAction} />
				</TabsContent>

				<TabsContent value="completed">
					<JobList queueName={queueName} state="completed" onJobAction={onJobAction} onBatchAction={onBatchAction} />
				</TabsContent>

				<TabsContent value="failed">
					<JobList queueName={queueName} state="failed" onJobAction={onJobAction} onBatchAction={onBatchAction} />
				</TabsContent>

				<TabsContent value="delayed">
					<JobList queueName={queueName} state="delayed" onJobAction={onJobAction} onBatchAction={onBatchAction} />
				</TabsContent>
			</Tabs>
		</div>
	);
