...
export function FlowTimeline({
	...
	const timelineEvents = useMemo(() => {
		...
		const processNode = (node: FlowNodeExtended) => {
			...
			const isBlocked = blockingDependencies.length > 0 && ["waiting", "delayed"].includes(node.status);
	...
	const filteredEvents = useMemo(() => {
		...
		if (selectedTimeRange !== "all") {
			...
			filtered = filtered.filter((event) => !event.startTime || event.startTime >= cutoff);
		}
	...
	const timelineBounds = useMemo(() => {
		const startTimes = timelineEvents.map((e) => e.startTime).filter(Boolean) as Date[];
		const endTimes = timelineEvents.map((e) => e.endTime).filter(Boolean) as Date[];

		if (startTimes.length === 0) return null;

		const minTime = new Date(Math.min(...startTimes.map((d) => d.getTime())));
		const maxTime = endTimes.length > 0 ? new Date(Math.max(...endTimes.map((d) => d.getTime()))) : new Date();
	...
	const ganttData = useMemo(() => {
		...
		return filteredEvents.map((event) => {
			const startOffset = event.startTime
				? ((event.startTime.getTime() - timelineBounds.minTime.getTime()) / timelineBounds.duration) * 100
				: 0;
			const duration = event.duration || 0;
			const width = duration > 0 ? (duration / timelineBounds.duration) * 100 : 2;
						...
						<div className="space-y-1 max-h-96 overflow-y-auto">
							{ganttData.map((event) => (
								...
								>
									{/* Job bar */}
									<div
										className={`absolute h-full rounded border-l-4 ${getStatusColor(
											event.status,
									...
									>
										<div className="flex items-center h-full px-2">
											...
											<span className="ml-2 text-sm font-medium truncate">{event.jobName}</span>
											{event.progress !== undefined && event.status === "active" && (
												<div className="ml-auto text-xs">{event.progress}%</div>
											)}
										</div>
									</div>
					...
					<CardContent>
						<div className="space-y-3">
							{filteredEvents.map((event) => (
								<div
									key={event.id}
									className={`p-4 rounded-lg border-2 cursor-pointer hover:shadow-md transition-all ${getStatusColor(
										event.status,
									)} ${selectedEvent === event.id ? "ring-2 ring-blue-500" : ""} ${
									...
									<div className="flex items-center justify-between">
										...
										<div className="flex items-center space-x-4 text-sm">
											<div className="text-right">
												<div className="font-medium">{formatRelativeTime(event.startTime)}</div>
												<div className="text-muted-foreground">Duration: {formatDuration(event.duration)}</div>
												{event.waitTime && (
													<div className="text-muted-foreground">Wait: {formatDuration(event.waitTime)}</div>
												)}
											</div>
