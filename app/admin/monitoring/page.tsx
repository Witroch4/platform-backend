...
const navigationSections = [
	...
	},
	{
		...
		description: "Controle dos intents que alimentam experiências com IA.",
		links: [
			// [CLEANUP 2026-02-16] "Filas de IA" removido - eram código morto
			{
		...
		switch (filter) {
			case "healthy":
				return sortedQueues.filter((queue) =>
					queue.paused ? false : queue.status === "healthy" || queue.status === "active",
				);
			case "warning":
				return sortedQueues.filter((queue) => queue.status === "warning");
			case "critical":
				return sortedQueues.filter(
					(queue) => queue.status === "critical" || queue.status === "error" || queue.failed > 0,
				);
			case "paused":
				...
				<div className="@xl/main:grid-cols-2 @5xl/main:grid-cols-4 grid grid-cols-1 gap-4 px-4 *:bg-gradient-to-t *:from-primary/5 *:to-card *:shadow-xs dark:*:from-primary/10 lg:px-6">
					...
					<Card className="@container/card" data-slot="card">
						...
						<CardFooter className="flex-col items-start gap-1 text-sm text-muted-foreground">
							...
							<div>
								Ativos: {numberFormatter.format(queueAggregates.active)} · Concluídos:{" "}
								{numberFormatter.format(queueAggregates.completed)}
							</div>
							{queueAggregates.delayed > 0 && <div>Atrasados: {numberFormatter.format(queueAggregates.delayed)}</div>}
						</CardFooter>
					</Card>
					...
					<Card className="@container/card" data-slot="card">
							...
							<div className="absolute right-4 top-4">
								<Badge
									...
									className={`flex items-center gap-1 rounded-lg text-xs ${
										costChangePositive ? "text-emerald-700 dark:text-emerald-300" : "text-red-600 dark:text-red-400"
									}`}
								>
						...
						<CardFooter className="flex-col items-start gap-1 text-sm text-muted-foreground">
							...
							<div>Processamento OK: {(costData?.systemHealth?.processingRate ?? 0).toFixed(1)}%</div>
						</CardFooter>
					</Card>
				</div>
			</section>
			...
			<section className="grid gap-6 px-4 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)] lg:px-6">
												...
												formatter={(value: any) => (
													<div className="flex w-full items-center justify-between gap-4">
														<span className="text-muted-foreground">
															{chartMetricOptions.find((m) => m.value === chartMetric)?.label}
														</span>
				...
				<Card className="@container/card">
					<CardHeader>
						...
						<CardDescription>Status reportado pelos serviços principais e tempos médios de resposta.</CardDescription>
					</CardHeader>
												...
												<Table>
													...
													<TableBody>
														{queues.slice(0, 12).map((queue) => {
															const statusKey = (
																queue.paused ? "paused" : queue.status
															) as keyof typeof queueStatusStyles;
															const statusClass = queueStatusStyles[statusKey] ?? queueStatusStyles.warning;
															const throughputValue =
																queue.performance?.throughput.jobsPerMinute ??
															...
															return (
																<TableRow key={`${option.value}-${queue.name}`} className="last:border-none">
																	...
																	<TableCell className="text-right">{numberFormatter.format(queue.active)}</TableCell>
