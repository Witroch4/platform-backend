      ...
      <Tabs defaultValue="general" className="space-y-6">
        <TabsList>
          ...
          {lead.source === LeadSource.CHATWIT_OAB && (
            <TabsTrigger value="oab">Dados OAB</TabsTrigger>
          )}
                    ...
                    {lead.automacoes.map((automacao) => (
                      <div key={automacao.id} className="flex items-center justify-between p-3 border rounded">
                        <div>
                          ...
                          <div className="text-sm text-muted-foreground">
                            {automacao.automacao.fraseBoasVindas}
                          </div>
                          <div className="text-xs text-muted-foreground">
                    ...
                    {lead.disparos.map((disparo) => (
                      <div key={disparo.id} className="flex items-center justify-between p-3 border rounded">
                        <div>
                          ...
                          <div className="text-sm text-muted-foreground">
                            Criado em {formatDate(disparo.createdAt)}
                          </div>
                          {disparo.scheduledAt && (
                            <div className="text-xs text-muted-foreground">
                          ...
                          {disparo.sentAt && (
                            <div className="text-xs text-green-600">
                              Enviado em {formatDate(disparo.sentAt)}
                            </div>
                          )}
                          {disparo.errorMessage && (
                            <div className="text-xs text-red-600">
                              Erro: {disparo.errorMessage}
                            </div>
                          )}
                        </div>
                        <Badge 
                          variant={
                            disparo.status === "SENT" ? "default" :
                            disparo.status === "PENDING" ? "outline" :
                            "destructive"
                          }
                        >
                          {disparo.status}
