...
export function UnifiedLeadsList({ onLeadSelect, showActions = true, compact = false }: LeadsListProps) {
				...
				if (lead.oabData) {
					return (
						<div className="text-xs text-muted-foreground">
							...
							{lead.oabData.notaFinal && <span className="text-xs">Nota: {lead.oabData.notaFinal}</span>}
						</div>
					);
				}
				break;
									...
									{leads.map((lead) => (
										<TableRow key={lead.id}>
											<TableCell>
												<div className="flex items-center gap-3">
													...
													<div>
														<div className="font-medium">{lead.name || "Sem nome"}</div>
														<div className="text-sm text-muted-foreground">ID: {lead.sourceIdentifier}</div>
														{lead.tags.length > 0 && (
										...
										<TableCell>												<div className="text-xs text-muted-foreground">
											...
											{showActions && (
												<TableCell>
													<div className="flex items-center gap-1">
														<Button variant="ghost" size="icon" onClick={() => router.push(`/admin/leads/${lead.id}`)}>
