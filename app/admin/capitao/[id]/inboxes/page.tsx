  ...
  useEffect(() => { 
    if (assistantId) {
      ...
      if (showMetrics) {
        ...
        inboxes.filter(i => i.attached).forEach(inbox => {
  ...
  return (
    <div className="p-6 space-y-4 bg-background text-foreground min-h-screen">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="w-[96vw] sm:max-w-2xl max-h-[85vh] bg-background border-border">
            ...
            <div className="space-y-3 overflow-auto max-h-[60vh] pr-2">
              {inboxes.map((i) => (
                <label key={i.inboxId} className="flex items-center gap-3 p-2 rounded-md hover:bg-muted/50 transition-colors">
                  <Checkbox 
      ...
      {inboxes.filter(i => i.attached).length > 0 && (
        <Collapsible open={showMetrics} onOpenChange={setShowMetrics} className="border border-border rounded-md bg-card">
      ...
      <div className="mt-6">
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {inboxes.filter(i => i.attached).map((inbox) => {
                      ...
                      <div className="mt-3 p-3 bg-muted/30 rounded-md border border-border">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-5 gap-3 text-xs">
                          ...
                          <div>
                            ...
                            <div className="font-medium text-foreground">{Math.round(metrics.classificationAccuracy * 100)}%</div>
                          </div>
                          <div>
                            <div className="text-muted-foreground">Cache Hit</div>
                            <div className="font-medium text-foreground">{Math.round(metrics.cacheHitRate * 100)}%</div>
                          </div>
                          <div>
