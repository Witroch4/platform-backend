...
export default function GlobalIntentsPage() {
	...
	const handleExport = async () => {
		...
		try {
			...
			if (response.ok) {
				...
				toast.success(
					`Exportação concluída! ${totalIntents} intenções e ${totalTemplates} templates exportados.${hasEmbeddings ? " Inclui embeddings." : ""}`,
				);
	...
	const handleImport = async () => {
		...
		try {
			...
			if (result.success) {
				toast.success(
					`Importação concluída! ${result.summary.importedIntents} intenções importadas, ${result.summary.skippedIntents} ignoradas.`,
				);
				...
				toast.warning(
					`Importação parcial. ${result.summary.importedIntents} importadas, ${result.summary.errors.length} erros.`,
				);
			} else {
					...
					<div className="flex items-center justify-between">
						<div>
							...
							<p className="text-muted-foreground">
								Crie e gerencie intenções para classificação automática de mensagens
							</p>
						</div>
						<div className="flex gap-2">
							<Button variant="outline" onClick={() => setShowMetrics(!showMetrics)} className="gap-2">
							...
							<div className="p-4 space-y-4">
								...
								<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
									...
									<div className="border rounded-md p-3">
										...
										<Button variant="outline" onClick={prewarmEmbeddings} disabled={selectedIntents.size === 0}>
								...
								<div className="flex items-center gap-2 text-sm">
									<Button variant="outline" onClick={() => setSelectedIntents(new Set(intents.map((i) => i.id)))}>
										Selecionar Todos
									</Button>
									<Button variant="outline" onClick={() => setSelectedIntents(new Set())}>
										Limpar Seleção
									</Button>
									...
									<div key={i.id} className="py-3 space-y-2">
										<div className="flex items-center justify-between">
														...
														<div className="text-xs text-muted-foreground">
															{(() => {
																...
																const mainDescription =
																	aliasesIndex !== -1 ? fullDescription.substring(0, aliasesIndex) : fullDescription;
											...
											<div className="flex items-center gap-2">
												...
												<Button variant="secondary" onClick={() => openEdit(i)}>
													Editar
												</Button>
										...
										{showMetrics && metrics && (
											<div className="ml-6 flex items-center gap-4 text-xs">
												...
												<Badge
													variant={
														metrics.embeddingStatus === "available"
															? "default"
															: metrics.embeddingStatus === "regenerating"
																? "secondary"
																: "destructive"
													}
												>
					...
					<div className="grid grid-cols-1 gap-3">
						...
						<div>
							...
							<p className="text-xs text-muted-foreground mt-1">
								Digite uma frase por linha. Essas frases ajudam o sistema a identificar quando o usuário tem essa
								intenção.
							</p>
						</div>
						<div>
										...
										<div className="space-y-3">
											<div className="flex items-center space-x-2">
												<input
													...
													onChange={(e) =>
														setImportOptions((prev) => ({ ...prev, regenerateEmbeddings: e.target.checked }))
													}
												/>
								...
								<div className="space-y-4">
									<div className="flex items-start gap-2">
										...
										<div>
											...
											<p className="text-sm text-muted-foreground">Resultado do processamento das intenções</p>
										</div>
									</div>
