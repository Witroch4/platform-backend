...
export default function OpenAISourceTestPage() {
			...
			if (resp.ok && data.success) {
				...
				setResult({
					...
					debug: {
						...(data.debug ?? {}),
						server_timing_ms:
							typeof data?.debug?.server_timing_ms === "object" ? data.debug?.server_timing_ms : Math.round(tEnd - t0),
					},
				});
				toast.success("OK!");
			} else {
	...
	const buildMarkdownReport = () => {
		...
		const md = `# 🔬 OpenAI Responses API — Relatório de Teste
		...
		<div className="container mx-auto p-6 space-y-6">
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Inspecione <strong>o que você envia</strong> e <strong>o que recebe</strong> (incluindo o espelho do{" "}
						<strong>pedido à OpenAI</strong>), com Reasoning/Verbosity dinâmicos.
					</CardDescription>
				</CardHeader>
						...
						<CardContent className="space-y-4">
							<div className="grid sm:grid-cols-2 gap-4">
								...
								<div className="space-y-2">
									...
									<Select value={verbosity} onValueChange={(value) => setVerbosity(value as "low" | "medium" | "high")}>
										<SelectTrigger>
								...
								<div className="space-y-2">
									...
									/>
									<p className="text-xs text-muted-foreground">
										Dica: se ver <code>incomplete:max_output_tokens</code>, aumente o valor. Limite baseado na role do
										usuário.
									</p>
								</div>
							</div>
									...
									<div className="grid grid-cols-2 gap-2">
										<div>
											...
											<Select
												...
												onValueChange={(value) => setCaptainVerbosity(value as CaptainVerbosity)}
											>
												<SelectTrigger>
										...
										<div>
											...
											<Select
												value={captainEffort}
												onValueChange={(value) => setCaptainEffort(value as CaptainEffort)}
										...
										<div className="col-span-2">
											...
											/>
											<p className="text-[10px] text-muted-foreground mt-1">Deixe vazio para herdar do top-level.</p>
										</div>
									</div>
									<p className="text-xs text-muted-foreground">
						...
						<Card>
							...
							<CardContent>
								...
								<div className="mt-2 flex gap-2">
									...
									<Button
										variant="outline"
										onClick={() => downloadJSON("response.json", result ?? { info: "sem resposta ainda" })}
									>
										Baixar JSON
									</Button>
								</div>
								...
								{typeof dbg?.server_timing_ms === "object" && (
									<>
										...
										<div className="text-xs">
											<div>
												<strong>Total:</strong> {dbg?.server_timing_ms?.total ?? "n/d"} ms
											</div>
											<div>
												<strong>ensureSession:</strong> {dbg?.server_timing_ms?.ensureSession ?? "n/d"} ms
											</div>
											<div>
												<strong>openai:</strong> {dbg?.server_timing_ms?.openai ?? "n/d"} ms
											</div>
										</div>
									</>
								)}
