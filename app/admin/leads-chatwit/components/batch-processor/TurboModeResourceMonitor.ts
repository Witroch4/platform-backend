...
export class TurboModeResourceMonitor {
	...
	private async checkResources(): Promise<void> {
		try {
			...
			if (this.resourceHistory.length % 6 === 0) {
				// Every 30 seconds
	...
	private getResourceLevel(
		percentage: number,
		type: "memory" | "cpu" | "activeProcesses",
	): ThrottlingStrategy["level"] {
	...
	private getHighestLevel(levels: ThrottlingStrategy["level"][]): ThrottlingStrategy["level"] {
		...
		return levels.reduce(
			(highest, current) => (levelPriority[current] > levelPriority[highest] ? current : highest),
			"none",
		);
	}
	...
	public getHealthStatus(): {
		...
		if (resources.activeProcesses.percentage >= this.thresholds.activeProcesses.critical) {
			issues.push(
				`Too many active processes: ${resources.activeProcesses.count}/${resources.activeProcesses.maxAllowed}`,
			);
