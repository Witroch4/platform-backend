...
export function TurboModePerformanceDashboard({
	...
	const speedImprovement =
		metrics.averageProcessingTime > 0 && metrics.timeSaved > 0
				...
				<Card>
					<CardContent className="p-4">
						<div className="flex items-center gap-3">
							...
							<div>
								<div className="text-2xl font-bold text-purple-600">{formatTime(metrics.timeSaved)}</div>
					...
					<CardContent className="space-y-4">
						<div className="space-y-2">
							<div className="flex justify-between text-sm">
								...
								<span className="font-semibold text-blue-600">{metrics.parallelProcessed} leads</span>
							</div>
							<Progress
						...
						<div className="space-y-2">
							<div className="flex justify-between text-sm">
								...
								<span className="font-semibold text-gray-600">{metrics.sequentialProcessed} leads</span>
							</div>
							<Progress
				...
				<Card>
					...
					<CardContent className="space-y-4">
						<div className="flex items-center justify-between p-3 bg-blue-50 rounded-lg">
							...
							<div className="text-lg font-bold text-blue-700">{formatTime(metrics.averageProcessingTime)}</div>
						</div>
						...
						{speedImprovement > 0 && (
							<div className="flex items-center justify-between p-3 bg-green-50 rounded-lg">
								...
								<div className="text-lg font-bold text-green-700">{speedImprovement}% mais rápido</div>
							</div>
						)}
						...
						<div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
							...
							<div className="text-lg font-bold text-gray-700">{config.maxParallelLeads} leads</div>
						</div>
					</CardContent>
				</Card>
			</div>
			...
			<Card>
				...
				<CardContent>
					<div className="grid grid-cols-2 md:grid-cols-4 gap-4">
						...
						<div className="flex items-center gap-2">
							<div
								className={`w-2 h-2 rounded-full ${metrics.errorRate < 0.1 ? "bg-green-500" : metrics.errorRate < 0.3 ? "bg-yellow-500" : "bg-red-500"}`}
							></div>
		...
		<Card className={`bg-gradient-to-r from-blue-50 to-purple-50 border-blue-200 ${className}`}>
			<CardContent className="p-4">
				...
				<div className="grid grid-cols-3 gap-3 text-center">
					...
					<div>
						<div className="text-lg font-bold text-purple-600">{formatTime(metrics.timeSaved)}</div>
			...
			<CardContent className="p-4">
				<div className="flex items-center gap-3">
					...
					<div className="flex-1">
						<div className="font-semibold text-orange-800 text-sm mb-1">TURBO Mode Indisponível</div>
						<div className="text-xs text-orange-700">{reason}</div>
					</div>
