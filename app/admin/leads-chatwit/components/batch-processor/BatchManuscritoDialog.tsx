// app/admin/leads-chatwit/components/batch-processor/BatchManuscritoDialog.tsx
...
type BatchManuscritoDialogProps = {
	lead: ExtendedLead;
...
export function BatchManuscritoDialog({ lead, progress, onSubmit, onClose }: BatchManuscritoDialogProps) {
	...
	const handleSendManuscrito = async () => {
		...
		setIsSending(true);
		try {
			console.log(`[BatchManuscrito] Enviando ${selectedImages.length} imagens do manuscrito para ${lead.nome}`);
			...
			toast.success(`Manuscrito de ${lead.nome} enviado com sucesso!`);
			onSubmit({ selectedImages });
		} catch (error: any) {
			console.error(`[BatchManuscrito] Erro ao enviar manuscrito:`, error);
			toast.error("Erro", { description: error.message || "Não foi possível enviar o manuscrito" });
		} finally {
			setIsSending(false);
		}
	};
	...
	return (
		<Dialog open onOpenChange={onClose}>
			<DialogContent className="max-w-4xl max-h-[90vh] overflow-auto">
				<DialogHeader>
					<DialogTitle>
						Manuscrito - {lead.nome} ({progress.current + 1}/{progress.total})
					</DialogTitle>
					<DialogDescription>
						Selecione as imagens que serão enviadas para digitação do manuscrito deste lead.
					</DialogDescription>
					<div className="mt-2 text-sm text-muted-foreground">
						💡 Selecione apenas as páginas que contêm texto manuscrito para digitação.
					</div>
				</DialogHeader>
				...
				<div className="py-4">
					...
					{images.length === 0 ? (
						<div className="text-center py-8 text-muted-foreground">Nenhuma imagem disponível para este lead.</div>
					) : (
				...
				<DialogFooter className="flex justify-between">
					...
					<div className="flex gap-2">
						<Button variant="secondary" onClick={() => onSubmit({ selectedImages: [] })} disabled={isSending}>
							Pular este Lead
						</Button>
						<Button
							variant="default"
							onClick={handleSendManuscrito}
