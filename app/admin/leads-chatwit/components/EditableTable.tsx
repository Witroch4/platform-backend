...
export const EditableTable: React.FC<EditableTableProps> = ({ tableNode, onNodeChange }) => {
	const handleCellChange = (rowIndex: number, cellIndex: number, value: string) => {
		...
		newTableNode.children[rowIndex].children[cellIndex].children = [{ type: "text", value }];
	...
	const deleteRow = (rowIndex: number) => {
		const newTableNode = JSON.parse(JSON.stringify(tableNode));
		if (rowIndex > 0 && newTableNode.children.length > 2) {
			// Não permitir deletar o cabeçalho e manter pelo menos 1 linha de dados
	...
	return (
		<div className="my-4 p-3 border rounded-md bg-white dark:bg-gray-800">
			...
			<Button onClick={addRow} variant="outline" className="mt-3">
