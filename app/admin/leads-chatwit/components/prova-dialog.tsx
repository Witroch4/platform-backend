...
export function ProvaDialog({
	...
	return (
		<Dialog open={isOpen} onOpenChange={handleClose}>
			<DialogContent
				className={`${fullScreen ? "max-w-[95vw] w-[95vw] max-h-[95vh]" : "max-w-6xl w-[90vw] max-h-[85vh]"} overflow-hidden flex flex-col`}
			>
				<DialogHeader>
					<DialogTitle>Editar Prova</DialogTitle>
					<DialogDescription>Faça as alterações necessárias no texto da prova.</DialogDescription>
				</DialogHeader>
						...
						<>
							...
							<div className="px-4 pt-2 pb-1 flex items-center gap-4 border-b">
								...
								<div className="flex items-center gap-2">
									...
									<Button size="sm" variant="outline" onClick={() => setFontSize((s) => Math.max(10, s - 1))}>
										A-
									</Button>
									<Button size="sm" variant="outline" onClick={() => setFontSize((s) => Math.min(22, s + 1))}>
										A+
									</Button>
								</div>
								<div className="ml-auto flex items-center gap-2">
									...
									<Input
										...
										className="w-20 h-8"
									/>
									<Button size="sm" variant="secondary" onClick={goToLine}>
										Ir
									</Button>
								</div>
							</div>
