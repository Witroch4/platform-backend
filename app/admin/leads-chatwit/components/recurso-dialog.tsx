  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {recursoValidado ? "Recurso Validado" : "Recurso"}
          </DialogTitle>
          <DialogDescription>
            {aguardandoRecurso
          ...
          <div className="flex flex-col items-center justify-center">
            {aguardandoRecurso ? (
              ...
              <div className="space-y-4">
                {/* Botão do Recurso */}
                <div className="flex flex-col items-center justify-center p-6 border-2 border-dashed rounded-lg cursor-pointer hover:bg-accent/30 transition-colors" onClick={abrirPdfRecurso}>
                ...
                {recursoArgumentacaoUrl && (
                  <div className="flex flex-col items-center justify-center p-6 border-2 border-dashed rounded-lg cursor-pointer hover:bg-accent/30 transition-colors" onClick={abrirPdfArgumentacao}>
                    ...
                    <p className="text-lg font-medium">
                      Argumentação do Recurso Disponível
                    </p>
                    <p className="text-sm text-primary mt-2 flex items-center">
                      Clique para abrir o PDF
                      <ExternalLink className="ml-1 h-3 w-3" />
              ...
              <div className="flex flex-col items-center justify-center py-8">
                ...
                <p className="text-sm text-muted-foreground mt-2">
                  Ainda não recebemos o recurso. Faça um recurso.
                </p>
              </div>
            )}
          </div>
            ...
            <div className="space-y-4 border p-4 rounded-md bg-orange-50">
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  disabled={isValidando || !textoRecurso.trim()}
                >
                  {isValidando ? (
                    <>
        ...
        <DialogFooter className="flex flex-wrap gap-2 justify-between sm:justify-end">
          ...
          {recursoUrl && (
            <Button
              ...
              disabled={isEnviando || !recursoUrl}
            >
              {isEnviando ? (
                <>
                  <Loader2 className="mr-2 h-4 w-4 animate-spin" />
