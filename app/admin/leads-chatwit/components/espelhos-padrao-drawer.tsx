...
import { BookOpen, Upload, Eye, FileUp, Loader2, Send, FileText, HelpCircle, AlertTriangle } from "lucide-react";
...
export function EspelhosPadraoDrawer({ isOpen, onClose, usuarioId }: EspelhosPadraoDrawerProps) {
	...
	useEffect(() => {
		...
		if (espelhosAguardando.length > 0 && isOpen) {
			console.log(
				`[Espelhos Drawer] 🔄 Iniciando polling para ${espelhosAguardando.length} espelho(s) aguardando processamento`,
			);
				...
				try {
					...
					if (response.ok) {
						...
						if (novosEspelhosAguardando.length === 0 || tentativas >= maxTentativas) {
							console.log(
								`[Espelhos Drawer] 🛑 Parando polling - processados: ${novosEspelhosAguardando.length === 0}, timeout: ${tentativas >= maxTentativas}`,
							);
					...
					if (tentativas >= 3) {
						// Parar após 3 erros consecutivos
						clearInterval(interval);
					}
				}
	...
	const handleEnviarParaSistemaExterno = async (espelhoId: string, imageUrls: string[], especialidade: string) => {
		try {
			...
			setEspelhosPadrao((prev) =>
				prev.map((esp) => (esp.id === espelhoId ? { ...esp, aguardandoProcessamento: true } : esp)),
			);
				...
				<DrawerContent className="h-[90vh] max-h-[90vh] flex flex-col">
					<div className="container mx-auto max-w-6xl h-full flex flex-col">
						<DrawerHeader className="px-6 py-4">
							...
							<DrawerDescription>
								Gerencie os espelhos padrão para cada especialidade jurídica. Estes serão usados para gerar espelhos
								mais precisos para os leads.
							</DrawerDescription>
						</DrawerHeader>
											...
											return (
												...
												>
													<div className="flex-1">
														<div className="flex items-center gap-3 mb-2">
															...
															{espelhoExistente && (
																<div className="flex gap-2">
																	...
																	<Badge variant="secondary">{espelhoExistente.totalUsos} uso(s)</Badge>
																</div>
															)}
														</div>
														...
														{espelhoExistente && (
															<p className="text-xs text-muted-foreground">
																Atualizado em {new Date(espelhoExistente.updatedAt).toLocaleDateString()} por{" "}
																{espelhoExistente.atualizadoPor.name}
															</p>
														)}
													</div>
													...
													<div className="flex items-center gap-3">
														...
														{espelhoExistente &&
															espelhoExistente.espelhoCorrecao &&
															!espelhoExistente.aguardandoProcessamento && (
																<Button
																	variant="outline"
																	onClick={() => {
														...
														{espelhoExistente && espelhoExistente.textoMarkdown && (
															<Button variant="outline" onClick={() => handleVisualizarTexto(espelhoExistente)}>
				...
				<DialogContent className="max-w-md">
					<DialogHeader>
						...
						<DialogDescription>
							...
							<br />
							<br />
							<strong>Recomendação:</strong> Use apenas se o envio do PDF falhar. Por padrão, o sistema externo trabalha
							com PDF.
						</DialogDescription>
					</DialogHeader>
			...
			<Dialog open={showMarkdownDialog} onOpenChange={setShowMarkdownDialog}>
				<DialogContent className="max-w-4xl max-h-[80vh] overflow-hidden flex flex-col">
					<DialogHeader>
						...
						<DialogDescription>Espelho padrão para {selectedEspelhoParaVisualizacao?.especialidade}</DialogDescription>
					</DialogHeader>
