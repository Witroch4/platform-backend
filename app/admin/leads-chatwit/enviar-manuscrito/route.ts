  ...
  try {
    ...
    if (isEspelho && USE_INTERNAL_MIRROR_AGENT && leadId) {
      ...
      try {
        ...
        if (!especialidade) {
          throw new Error("Especialidade não definida para o lead. Selecione uma especialidade antes de processar o espelho.");
        }
        ...
        console.log(`[Enviar Espelho] 📋 Espelho Padrão ID: ${lead.espelhoPadraoId || payload.espelhoPadraoId || 'auto'}`);
