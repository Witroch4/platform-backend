		...
		<div className="max-w-screen-2xl mx-auto px-3 sm:px-4 lg:px-6 py-4 sm:py-6 space-y-4 sm:space-y-6 bg-background">
			...
			<div className="flex flex-col gap-4">
				...
				<div className="flex flex-col gap-3">
					{/* Linha de controles - scroll horizontal em mobile se necessário */}
					<div className="flex items-center gap-2 overflow-x-auto pb-2 sm:pb-0 -mx-3 px-3 sm:mx-0 sm:px-0 sm:overflow-visible">
						{/* Select de período - adaptado para mobile */}
						<Select defaultValue="ultimos7" value={filterPeriod} onValueChange={setFilterPeriod}>
							<SelectTrigger className="w-[120px] sm:w-[140px] border-border min-h-[36px] sm:min-h-[40px] text-xs sm:text-sm shrink-0">
						...
						{/* Botão de cadastro de API Key - texto compacto */}
						{!userInfo.isLoading && (
							...
							<Tooltip>
								<TooltipTrigger asChild>
									<Button
										...
										disabled={isLoading}
										className="border-border hover:bg-accent min-h-[36px] sm:min-h-[40px] px-2 sm:px-3 shrink-0"
			...
			<div className={`grid grid-cols-2 gap-3 sm:gap-4 ${isSuperAdmin ? "lg:grid-cols-4" : "sm:grid-cols-3"}`}>
				...
				{isSuperAdmin && (
					<Card className="border-border bg-card">
						<CardHeader className="pb-1 sm:pb-2 px-3 sm:px-6 pt-3 sm:pt-6">
							<CardTitle className="text-xs sm:text-sm font-medium text-card-foreground">Total de Usuários</CardTitle>
						</CardHeader>
						<CardContent className="px-3 sm:px-6 pb-3 sm:pb-6">
							{isLoading ? (
								<div className="h-8 sm:h-12 w-12 sm:w-16 bg-muted animate-pulse rounded-md" />
			...
			{showDashboard && (
				<div className="grid grid-cols-1 gap-3 sm:gap-4">
					<LeadsDashboard period={filterPeriod} refreshCounter={refreshCounter} isOpen={showDashboard} />
				</div>
			)}

			{/* Tabs - Responsivo */}
			<LeadsTabs activeTab={activeTab} onChange={handleTabChange} userRole={userInfo.role} />
			...
			{/* Aba de Mensagens - visão consolidada de todas as mensagens */}
			{activeTab === "mensagens" ? (
				<div className="border border-border rounded-md bg-card overflow-hidden min-h-[500px]">
					<AllMessagesTab searchQuery={searchQuery} />
				</div>
			) : null}
