		...
		<div className="container mx-auto py-6 space-y-6">
			...
			<div className="flex items-center justify-between">
				<div className="flex items-center gap-3">
					...
					<div>
						...
						<p className="text-muted-foreground">Teste webhooks do Dialogflow e SocialWise Flow</p>
					</div>
				</div>
					...
					<div className="space-y-4">
						<div>
							...
							<Input
								...
								className="max-w-md"
							/>
							<p className="text-xs text-muted-foreground mt-1">Este número receberá a mensagem de teste do WhatsApp</p>
						</div>
						...
						<div className="space-y-3">
							<div className="flex items-center space-x-2">
								...
								<p className="text-xs text-muted-foreground">(Recomendado para testes mais precisos)</p>
							</div>
						...
						<div className="space-y-3 border-t pt-4">
							...
							<div className="flex items-center gap-4">
								<Button
									...
									className="flex items-center gap-2"
								>
									{infiniteTestRunning ? <Loader2 className="h-4 w-4 animate-spin" /> : <Zap className="h-4 w-4" />}
								...
								<div className="flex items-center gap-2">
									<Badge variant="secondary">Testes: {testCount}</Badge>
								</div>
							</div>
						...
						<div>
							...
							<Input
								...
								className="max-w-2xl"
							/>
							<p className="text-xs text-muted-foreground mt-1">Se vazio, usaremos o padrão: {DEFAULT_EXTERNAL_DEST}</p>
						</div>
						...
						<div className="space-y-4 border-t pt-4">
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
								<div>
									...
									<p className="text-xs text-muted-foreground mt-1">
										Usado em TODOS os payloads como mensagem ou título do botão •{" "}
										<span className="text-green-600">Salvo automaticamente ao digitar</span>
									</p>
								</div>
								...
								<div>
									...
									<p className="text-xs text-muted-foreground mt-1">
										Usado em TODOS os botões (WhatsApp, Instagram, Facebook) •{" "}
										<span className="text-green-600">Salvo automaticamente ao digitar</span>
									</p>
								</div>
							</div>
			...
			<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
				<Card>
					<CardHeader>
						...
						<CardDescription>Envia o payload real do Dialogflow</CardDescription>
					</CardHeader>
					<CardContent>
						<Button onClick={sendRealPayload} disabled={loading} className="w-full">
				...
				<Card>
					...
					<CardContent>
						<Button onClick={createIntentPayload} disabled={loading} variant="outline" className="w-full">
							{loading ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <Send className="h-4 w-4 mr-2" />}
							Testar Intent
						</Button>
					</CardContent>
				...
				<Card>
					<CardHeader>
						...
						<CardDescription>Copia o payload para área de transferência</CardDescription>
					</CardHeader>
					<CardContent>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>Envie payloads do SocialWise Flow para o destino customizado</CardDescription>
				</CardHeader>
				<CardContent>
					<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>Cole seu próprio payload JSON para teste</CardDescription>
				</CardHeader>
				<CardContent>
					<div className="space-y-4">
						...
						<div className="flex gap-2">
							<Button onClick={sendCustomPayload} disabled={loading || !customPayload.trim()}>
								{loading ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <Send className="h-4 w-4 mr-2" />}
								Enviar Payload Customizado
							</Button>

							<Button
								onClick={() => sendToExternal(JSON.parse(customPayload || "{}"))}
					...
					<CardContent>
						<div className="space-y-2">
							{savedPayloads.map((payload) => (
								<div key={payload.id} className="flex items-center justify-between p-3 border rounded-lg">
									<div>
									...
									<div className="flex gap-2">
										<Button onClick={() => loadPayload(payload)} variant="outline">
											Carregar
										</Button>
										<Button onClick={() => deletePayload(payload.id)} variant="outline">
											<Trash2 className="h-4 w-4" />
										</Button>
									</div>
								</div>
				...
				<Card>
					...
					<CardContent>
						<div className="space-y-4">
							...
							{response.testInfo && (
								<div className="text-sm">
									<strong>Info do teste:</strong> {JSON.stringify(response.testInfo, null, 2)}
								</div>
							)}

							{response.cacheInfo && (
								<div className="text-sm">
									<strong>Cache:</strong> {JSON.stringify(response.cacheInfo, null, 2)}
								</div>
							)}
							...
							<div className="text-xs text-muted-foreground">Timestamp: {response.timestamp}</div>
						</div>
					</CardContent>
				</Card>
			)}
