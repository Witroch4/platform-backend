...
const MapeamentoTab = ({ caixaId }: MapeamentoTabProps) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [mapResponse, msgResponse, templateResponse] = await Promise.all([
        fetch(`/api/admin/atendimento/mapeamentos/${caixaId}`),
        fetch(`/api/admin/atendimento/mensagens-interativas/${caixaId}`),
        fetch(`/api/admin/atendimento/templates/${caixaId}`),
      ]);
  ...
  const handleDelete = async (mappingId: string) => {
    ...
    try {
      const response = await fetch(`/api/admin/atendimento/mapeamentos/${mappingId}`, { method: 'DELETE' });
      if (!response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const response = await fetch(`/api/admin/atendimento/mapeamentos`, {
        method: 'POST',
