							...
							>
								...
								<td className="px-3 py-2">
									...
									<div className="text-xs text-muted-foreground truncate">
										{grupo.descricao_limpa || grupo.descricao}
									</div>
								</td>
...
export default function OabRubricAuditorPage() {
	...
	const handleMergeGroups = useCallback(() => {
		...
		setDraft((prev) => {
			...
			const merged: GabaritoGrupo = {
				...
				peso_maximo: roundTwo([principal, ...restantes].reduce((acc, grupo) => acc + (grupo.peso_maximo ?? 0), 0)),
	...
	const handleAddSubitemToGroup = useCallback((groupId: string, subitemId: string) => {
		setDraft((prev) => {
			if (!prev?.grupos) return prev;
			const gruposAtualizados = prev.grupos.map((grupo) => {
				if (grupo.id !== groupId) return grupo;
							...
							<section className="rounded border border-border bg-card/80 p-4 space-y-4">
								...
								<div className="grid grid-cols-1 gap-3 md:grid-cols-3">
									<div className="rounded border border-border bg-muted/20 p-3">
										...
										{pontuacaoAtual ? (
											<div className="mt-1 text-sm font-semibold">{renderPontuacaoLabel(pontuacaoAtual.peca)}</div>
										) : (
									...
									<div className="rounded border border-border bg-muted/20 p-3">
										...
										{pontuacaoAtual ? (
											<div className="mt-1 text-sm font-semibold">{renderPontuacaoLabel(pontuacaoAtual.questoes)}</div>
										) : (
											<div className="text-sm text-muted-foreground">—</div>
										)}
									</div>
									<div className="rounded border border-border bg-muted/20 p-3">
										...
										{pontuacaoAtual ? (
											<div className="mt-1 text-sm font-semibold">{renderPontuacaoLabel(pontuacaoAtual.geral)}</div>
										) : (
											<div className="text-sm text-muted-foreground">—</div>
										)}
									</div>
								...
								<div className="space-y-4 lg:pr-4">
									...
									{subitemsSemGrupo.length ? (
										<div className="rounded border border-border bg-amber-50/80 p-3 text-xs text-amber-800">
											...
											<p>
												Esses subitens não estão associados a nenhum grupo. Utilize o seletor de subitens para
												adicioná-los onde fizer sentido.
											</p>
											...
											<div className="space-y-2 max-h-[320px] overflow-y-auto pr-1">
												{activeGroup.subitens.map((subId, index) => {
													...
													return (
														...
														>
															<div className="flex items-center justify-between gap-2">
																<button className="font-semibold text-left" onClick={() => setActiveSubitemId(subId)}>
																	{subId}
																</button>
