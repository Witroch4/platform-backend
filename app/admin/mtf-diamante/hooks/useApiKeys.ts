...
export function useApiKeysManager(isPaused: boolean = false): UseApiKeysReturn {
		...
		try {
			...
			await mutate(
				(current) => {
					...
					return current.map((apiKey) => (apiKey.id === optimisticApiKey.id ? result : apiKey));
				},
				{ revalidate: false },
			);
		} catch (error) {
	 ...
	 */
	const updateApiKeyWithRollback = async (
		updatedApiKey: MtfDiamanteApiKey,
		apiPayload: UpdateApiKeyPayload,
	): Promise<void> => {
		const originalApiKeys = apiKeys;
		...
		try {
			...
			await mutate(
				(current) => {
					...
					return current.map((apiKey) => (apiKey.id === updatedApiKey.id ? updatedApiKey : apiKey));
				},
				{ revalidate: false },
			);
			...
			await mutate(
				(current) => {
					...
					return current.map((apiKey) => (apiKey.id === result.id ? result : apiKey));
				},
				{ revalidate: false },
			);
		} catch (error) {
