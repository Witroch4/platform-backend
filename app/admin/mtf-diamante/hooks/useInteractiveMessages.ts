...
export function useInteractiveMessages(
  ...
  const swrConfig = useOptimizedSWRConfig(
  ...
  const addMessage = useCallback(async (
    optimisticMessage: InteractiveMessage, 
    apiPayload: CreateMessagePayload
  ): Promise<void> => {
    ...
    try {
      ...
      await mutate((current?: InteractiveMessage[]) => {
        if (!current) return [result];
        
        return current.map((msg: InteractiveMessage) => 
  ...
  const updateMessage = useCallback(async (
    updatedMessage: InteractiveMessage,
    apiPayload: UpdateMessagePayload
  ): Promise<void> => {
