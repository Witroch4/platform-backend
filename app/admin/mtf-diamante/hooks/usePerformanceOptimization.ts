...
export function useOptimizedValidation(validateFunction: (value: any) => boolean, debounceMs: number = 300) {
	// Debounced validation function
	const debouncedValidate = useDebouncedCallback(validateFunction, debounceMs);
...
export function useReactionsOptimization() {
	...
	const processReactions = useCallback(
		(reactions: any[]) => {
			...
			return reactions.filter((r) => r.emoji || r.textResponse || r.textReaction || r.action);
		},
		[hasReactions],
	);
