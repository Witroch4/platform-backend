...
export function ButtonReactionConfig({
	...
	const handleEmojiSelect = (buttonId: string, emoji: string) => {
		setReactions((prev) =>
			prev.map((reaction) =>
				reaction.buttonId === buttonId ? { ...reaction, emoji, textReaction: undefined } : reaction,
			),
		);
	...
	const handleTextReactionChange = (buttonId: string, textReaction: string) => {
		setReactions((prev) =>
			prev.map((reaction) =>
				reaction.buttonId === buttonId ? { ...reaction, textReaction, emoji: undefined } : reaction,
			),
		);
	};
	...
	const removeReaction = (buttonId: string) => {
		setReactions((prev) =>
			prev.map((reaction) =>
				reaction.buttonId === buttonId ? { ...reaction, emoji: undefined, textReaction: undefined } : reaction,
			),
		);
	};
						...
						<div key={reaction.buttonId} className="border rounded-lg p-4 space-y-3">
							...
							<div className="flex flex-wrap gap-2">
								...
								<div className="flex items-center gap-2">
									...
									{reaction.textReaction && (
										<Button variant="ghost" onClick={() => handleTextReactionChange(reaction.buttonId, "")}>
											<X size={14} />
										</Button>
									)}
								</div>
							...
							{(reaction.emoji || reaction.textReaction) && (
								<div className="bg-gray-50 p-2 rounded text-sm">
									<strong>Preview:</strong> Quando clicarem em "{reaction.buttonText}", o sistema reagirá com:{" "}
									{reaction.emoji || `"${reaction.textReaction}"`}
								</div>
							)}
						</div>
