...
export function InteractivePreview({
	...
	const [actionConfig, setActionConfig] = useState<{
		buttonId: string;
		defaultMode: "send_template" | "send_interactive";
	} | null>(null);
	...
	const buttons = useMemo(() => {
		...
		if (Array.isArray(source.content?.action?.buttons)) candidates.push(...source.content.action.buttons);
		if (Array.isArray(source.content?.action?.quick_replies)) candidates.push(...source.content.action.quick_replies);

		// 5. ActionReplyButton (formato alternativo)
		if (Array.isArray(source.actionReplyButton?.buttons)) candidates.push(...source.actionReplyButton.buttons);
	...
	const handleEmojiSelect = useCallback(
		(buttonId: string, emoji: string) => {
			if (emoji === "TEXT_RESPONSE") {
				...
				toast.success(
					isTemplate ? `📄 Template configurado para o botão` : `📱 Mensagem interativa configurada para o botão`,
					{
				...
				>
					<div className="bg-white dark:bg-gray-800 rounded-lg p-3 shadow-md border border-gray-200 dark:border-gray-700 w-full">
						...
						{(() => {
							...
							if (header) {
								const mediaUrl =
									header.media_url || header.mediaUrl || header.content || (header as any).url || (header as any).link;
						...
						{/* Footer */}
						{(() => {
							const footerText = debouncedMessage.footer?.text || (debouncedMessage as any).content?.footer?.text;
							return (
								footerText && (
									<div
								...
								<div className="flex gap-2 overflow-x-auto pb-2">
									{carouselElements.map((element: any, index: number) => (
										<div key={index} className="flex-shrink-0 w-48 border rounded-lg bg-gray-50 dark:bg-gray-800">
											...
											<div className="p-2">
												{/* Element Title */}
												<div className="text-sm font-medium text-gray-900 dark:text-gray-100 mb-1">{element.title}</div>

												{/* Element Subtitle */}
												{element.subtitle && (
													<div className="text-xs text-gray-600 dark:text-gray-400 mb-2">{element.subtitle}</div>
												)}
															...
															return (
																<button
																	...
																	className={cn(
																		...
																		"text-blue-600 dark:text-blue-400 border-blue-200 dark:border-blue-800",
																		"hover:bg-blue-50 dark:hover:bg-blue-900/20",
																		configMode && showReactionConfig && "ring-1 ring-blue-300 dark:ring-blue-600",
																		hasReaction &&
																...
																>
																	<div className="flex items-center justify-between">
																		...
																		<div className="flex items-center gap-1 flex-shrink-0">
																			...
																			{reaction?.emoji && <span className="text-xs">{reaction.emoji}</span>}
																		</div>
																	</div>
																</button>
															);
								...
								{buttons.length > 3 ? (
									<>
													...
													>
														<div className="flex items-center justify-between">
															...
															<div className="flex items-center gap-1">
																{showReactionIndicators && hasReaction && <Zap className="h-3 w-3 text-yellow-500" />}
																...
																{configMode && showReactionConfig && <Smile className="h-3 w-3 opacity-50" />}
															</div>
														</div>
														{reaction?.textResponse && (
										...
										>
											...
											{showAllButtons ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
										</button>
							...
							<div className="mt-3 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
								<div className="flex items-start gap-2">
									...
									<div className="text-xs text-blue-700 dark:text-blue-300">
										...
										<p>
											Clique nos botões acima para configurar reações automáticas que serão enviadas quando os usuários
											clicarem neles.
										</p>
