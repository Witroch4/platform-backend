...
import type { QuickRepliesNodeData, CarouselNodeData, InteractiveMessageNodeData } from "@/types/flow-builder";
...
function QuickRepliesPreview({ data, isDark }: QuickRepliesPreviewProps) {
	return (
		<div className="space-y-3">
			{/* Message bubble */}
			{data.promptText && (
				<div className={cn("max-w-[220px] rounded-2xl px-3 py-2 shadow-sm", isDark ? "bg-[#3a3d42]" : "bg-white")}>
...
function ButtonTemplatePreview({ data, isDark }: ButtonTemplatePreviewProps) {
	...
	return (
		<div className="space-y-0">
			...
			<div className={cn("max-w-[240px] rounded-2xl overflow-hidden shadow-sm", isDark ? "bg-[#3a3d42]" : "bg-white")}>
				{/* Body text */}
				{bodyText && (
				...
				{data.cards?.map((card, idx) => (
					...
					>
						...
						<div className="p-2 space-y-1">
							...
							{card.subtitle && (
								<p className={cn("text-[10px] truncate", isDark ? "text-gray-400" : "text-gray-500")}>
									{card.subtitle}
								</p>
							)}
							...
							{card.buttons && card.buttons.length > 0 && (
								<div className={cn("border-t pt-1 mt-1", isDark ? "border-gray-600" : "border-gray-100")}>
									...
									{card.buttons.length > 2 && (
										<p className="text-[9px] text-muted-foreground text-center">+{card.buttons.length - 2} mais</p>
									)}
								</div>
							)}
						</div>
			...
			>
				...
				<div
					className={cn(
						...
						channel === "instagram" ? "bg-gradient-to-r from-purple-600 via-pink-500 to-orange-400" : "bg-[#0084ff]",
					)}
				>
				...
				<div className="p-3 min-h-[280px] max-h-[380px] overflow-y-auto">
					{!hasContent ? (
						...
						<div className="flex flex-col items-start">
							...
							{messageType === "carousel" && <CarouselPreview data={data as CarouselNodeData} isDark={isDark} />}
							{messageType === "text" && <TextPreview text={(data as { text: string }).text} isDark={isDark} />}
						</div>
					)}
				</div>
			</div>
