...
export function ButtonReactionPicker({
    ...
    async function loadOptions() {
      ...
      try {
        ...
        if (activeTab === 'interactives' && effectiveInboxId) {
          ...
          console.warn('[ButtonReactionPicker] Aba interativas aberta mas inboxId não disponível:', inboxId || '(url not found)');
        }
      } catch (e) {
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      ...
      const isSelectDropdown = target.closest('[data-radix-popper-content-wrapper]') ||
      ...
      >
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
          ...
          <TabsContent value="templates" className="flex-1 flex flex-col m-0">
            <div className="flex-1 overflow-y-auto overflow-x-hidden p-4 min-h-0" style={{maxHeight: 'calc(100% - 80px)'}}>
              <div className="space-y-4">
                ...
                <Select value={selectedTemplateId} onValueChange={setSelectedTemplateId}>
                  <SelectTrigger>
                    <SelectValue
                      placeholder={loadingOptions ? "Carregando..." : "Selecione um template"}
                    />
                  </SelectTrigger>
                  <SelectContent>
                    {templates.map((t) => (
              ...
              <div className="space-y-4">
                ...
                {(inboxId || getInboxIdFromUrl()) ? (
                  <Select value={selectedInteractiveId} onValueChange={setSelectedInteractiveId}>
                    <SelectTrigger>
                      <SelectValue
                        placeholder={loadingOptions ? "Carregando..." : "Selecione uma mensagem"}
                      />
                    </SelectTrigger>
                    <SelectContent>
                      {interactives.map((m) => (
