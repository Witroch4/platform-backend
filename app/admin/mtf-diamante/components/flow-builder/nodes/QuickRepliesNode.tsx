 ...
 */
export const QuickRepliesNode = memo(
  ({ id, data, selected }: QuickRepliesNodeProps) => {
    ...
    // Update prompt text
    const updatePromptText = useCallback((text: string) => {
      setNodes((nodes) =>
        nodes.map((node) => {
          if (node.id === id) {
    ...
    // Update quick reply
    const updateQuickReply = useCallback((replyId: string, updates: Partial<QuickReplyItem>) => {
      setNodes((nodes) =>
        nodes.map((node) => {
          if (node.id === id) {
            const currentData = node.data as unknown as QuickRepliesNodeData;
    ...
    // Remove quick reply
    const removeQuickReply = useCallback((replyId: string) => {
      setNodes((nodes) =>
        nodes.map((node) => {
          if (node.id === id) {
            const currentData = node.data as unknown as QuickRepliesNodeData;
