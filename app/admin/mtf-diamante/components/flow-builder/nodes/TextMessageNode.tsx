...
export const TextMessageNode = memo(({ id, data, selected }: TextMessageNodeProps) => {
	...
	return (
		<NodeContextMenu onDuplicate={handleDuplicate} onDelete={handleDelete}>
			...
			>
				...
				<Handle type="target" position={Position.Top} className="!h-3 !w-3 !bg-slate-500 !border-2 !border-white" />
				...
				<div className="flex items-center gap-2 border-b px-3 py-2 bg-slate-100/50 dark:bg-slate-900/30">
					...
					<span className="font-medium text-sm">{data.label || "Texto Simples"}</span>
				</div>

				{/* Corpo */}
				<div className="px-3 py-2">
					{data.text ? (
						<p className="text-sm text-muted-foreground line-clamp-3">&ldquo;{data.text}&rdquo;</p>
					) : (
						<p className="text-sm text-muted-foreground italic">Clique duas vezes para configurar</p>
					)}
				</div>

				{/* Handle de saída (bottom) */}
				<Handle type="source" position={Position.Bottom} className="!h-3 !w-3 !bg-slate-500 !border-2 !border-white" />
			</div>
		</NodeContextMenu>
	);
});
