 ...
 */
export const InteractiveMessageNode = memo(
  ({ id, data, selected }: InteractiveMessageNodeProps) => {
          ...
          {showContent || isDragOver ? (
            <div className="flex flex-col gap-1 p-2">
              ...
              {showContent && headerTextElement && (
                <NodeContextMenu
                  onDelete={() => handleRemoveElement(headerTextElement.id)}
                >
                  <div 
              ...
              {elements.some(e => e.type === 'header_image') && (
                ...
                >
                  <div className="relative group rounded-md border bg-white dark:bg-card overflow-hidden shadow-sm">
                   ...
                   {headerImage?.url ? (
                      <>
                        <div className="h-24 w-full bg-cover bg-center relative" style={{ backgroundImage: `url(${headerImage.url})` }}>
              ...
              {bodyElement && (
                <NodeContextMenu
                  onDelete={() => handleRemoveElement(bodyElement.id)}
                >
              ...
              {footerElement && (
                <NodeContextMenu
                  onDelete={() => handleRemoveElement(footerElement.id)}
                >
              ...
              <div className={cn(
                "w-full transition-all duration-300 ease-in-out overflow-hidden pointer-events-none",
                (insertIndex === null || insertIndex >= elements.length) && isDragOver ? "h-12 opacity-100 mt-1" : "h-0 opacity-0"
              )}>
                 <div className="h-full w-full border-2 border-dashed border-blue-400 bg-blue-100/30 rounded-md flex items-center justify-center animate-pulse">
                    <span className="text-xs font-medium text-blue-600">Solte aqui</span>
                 </div>
