...
import MinIOMediaUpload, { type MinIOMediaFile } from '@/app/admin/mtf-diamante/components/shared/MinIOMediaUpload';
 ...
 */
export const CarouselNode = memo(
  ({ id, data, selected }: CarouselNodeProps) => {
    ...
    // Update cards
    const updateCards = useCallback((updater: (cards: CarouselCard[]) => CarouselCard[]) => {
      setNodes((nodes) =>
        nodes.map((node) => {
          if (node.id === id) {
    ...
    // Update card field
    const updateCardField = useCallback((cardId: string, field: keyof CarouselCard, value: string) => {
      updateCards((cards) =>
    ...
    const handleUploadComplete = useCallback((cardId: string, file: MinIOMediaFile) => {
      if (file.url) {
    ...
    const clearCardMedia = useCallback((cardId: string) => {
      setCardMediaFiles(prev => ({ ...prev, [cardId]: [] }));
      updateCardField(cardId, 'imageUrl', '');
    }, [updateCardField]);

    // Add button to card
    const addButtonToCard = useCallback((cardId: string) => {
      ...
      updateCards((cards) =>
        cards.map((c) =>
          c.id === cardId ? { ...c, buttons: [...(c.buttons ?? []), newButton] } : c
        )
      );
    }, [cards, updateCards]);

    // Update button
    const updateButton = useCallback((cardId: string, buttonId: string, updates: Partial<CarouselCardButton>) => {
      updateCards((cards) =>
        cards.map((c) => {
    ...
    // Remove button
    const removeButton = useCallback((cardId: string, buttonId: string) => {
      updateCards((cards) =>
        cards.map((c) => {
          if (c.id !== cardId) return c;
          return {
    ...
    const allPostbackButtons = cards.flatMap((card, cardIndex) =>
      (card.buttons ?? [])
        ...
        .map((btn, btnIndex) => ({
          ...btn,
          cardIndex,
          globalIndex: cards.slice(0, cardIndex).reduce((acc, c) => acc + (c.buttons?.filter(b => b.type === 'postback').length ?? 0), 0) + btnIndex,
        }))
    );
                  ...
                  <div className="space-y-2 p-3 border rounded-lg bg-amber-50 dark:bg-amber-950/30">
                    ...
                    {activeCard.imageUrl ? (
                      ...
                      <MinIOMediaUpload
                        ...
                        setUploadedFiles={(files) => {
                          const updater = typeof files === 'function'
                            ? files(cardMediaFiles[activeCard.id] ?? [])
                            : files;
                    ...
                    <div className="space-y-1.5">
                      <label className="text-xs font-medium text-muted-foreground">
                        Botões ({activeCard.buttons?.length ?? 0}/{INSTAGRAM_VALIDATION.genericTemplate.maxButtonsPerElement})
                      </label>
                      ...
                      {(activeCard.buttons ?? []).map((btn) => (
                        <div key={btn.id} className="flex items-center gap-1.5 group">
                          <button
                            type="button"
                            onClick={() => updateButton(activeCard.id, btn.id, {
