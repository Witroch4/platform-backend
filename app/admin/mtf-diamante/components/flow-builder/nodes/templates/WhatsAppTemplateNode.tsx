 ...
 */
function getButtonTypeFromElementType(
	type: string,
...
export const WhatsAppTemplateNode = memo(({ id, data, selected }: WhatsAppTemplateNodeProps) => {
	...
	const buttons = useMemo(() => {
		const allButtons = elements.filter(
			(e) =>
				e.type === "button" ||
				e.type === "button_url" ||
				e.type === "button_phone" ||
			...
			setNodes((nodes) =>
				nodes.map((node) => {
					if (node.id === id) {
						...
						const buttonCount = currentElements.filter(
							(e) =>
								e.type === "button" ||
								e.type === "button_url" ||
								e.type === "button_phone" ||
			...
			>
				...
				<div className="flex items-center justify-between px-3 py-1.5 bg-slate-100 dark:bg-slate-900/50 border-b text-[10px]">
					...
					<div className="flex items-center gap-2">
						{validation.counts.COPY_CODE > 0 && <span className="text-lime-600">🎫{validation.counts.COPY_CODE}</span>}
						{validation.counts.URL > 0 && <span className="text-rose-600">🔗{validation.counts.URL}/2</span>}
						...
						<div className="flex flex-col gap-1 p-2">
							...
							{headerTextElement && (
								<NodeContextMenu onDelete={isLocked ? undefined : () => handleRemoveElement(headerTextElement.id)}>
									<div
										className={cn(
											"relative group rounded-md border bg-white dark:bg-card px-3 py-2 shadow-sm",
											isLocked && "opacity-80",
										)}
									>
							...
							{bodyElement && (
								<NodeContextMenu onDelete={isLocked ? undefined : () => handleRemoveElement(bodyElement.id)}>
									<div
										className={cn(
											"relative group rounded-md border bg-white dark:bg-card px-3 py-3 shadow-sm min-h-[40px]",
											isLocked && "opacity-80",
										)}
									>
										<div className="flex items-start gap-2">
							...
							{footerElement && (
								<NodeContextMenu onDelete={isLocked ? undefined : () => handleRemoveElement(footerElement.id)}>
									<div
										className={cn(
											"relative group rounded-md border border-dashed bg-slate-50 dark:bg-slate-900/50 px-3 py-2 shadow-sm",
											isLocked && "opacity-80",
										)}
									>
										<div className="flex items-start gap-2">
											...
											>
												<div className="relative group">
													...
													>
														<div className="flex items-center gap-2">
															{/* Ícone do tipo */}
															{btnIcon && <span className="shrink-0">{btnIcon}</span>}
