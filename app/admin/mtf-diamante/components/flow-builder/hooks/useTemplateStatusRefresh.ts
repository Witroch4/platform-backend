...
export function useTemplateStatusRefresh({
	...
	useEffect(() => {
		...
		const recover = async () => {
			try {
				...
				const match = templates.find((t: { name?: string; id?: string }) => t.name === templateName && t.id);
				if (match?.id && mountedRef.current) {
					...
					setNodes((nodes) =>
						nodes.map((node) =>
							node.id === nodeId ? { ...node, data: { ...node.data, metaTemplateId: match.id } } : node,
						),
					);
				}
			} catch (err) {
	...
	const refreshStatus = useCallback(
		async (silent = false) => {
			...
			try {
				const res = await fetch(`/api/admin/mtf-diamante/templates/${caixaId}/${metaTemplateId}/status`);
				const data = await res.json();
				...
				if (data.statusChanged) {
					...
					toast.success(
						`Template ${data.status === "APPROVED" ? "aprovado" : data.status === "REJECTED" ? "rejeitado" : `atualizado: ${label}`}!`,
					);
				} else if (!silent) {
