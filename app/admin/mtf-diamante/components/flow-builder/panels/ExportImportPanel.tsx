...
export function ExportImportPanel({
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : 'Erro ao exportar flow'
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      const message =
        error instanceof Error ? error.message : 'Erro ao importar flow';
        ...
        <Dialog open={isPreviewDialogOpen} onOpenChange={setIsPreviewDialogOpen}>
          <DialogContent className="w-[96vw] sm:max-w-2xl max-h-[85vh]">
            <DialogHeader>
              ...
              <DialogDescription>
                Visualização das conexões em formato legível. Útil para debug e
                verificação da estrutura do flow.
              </DialogDescription>
            </DialogHeader>
