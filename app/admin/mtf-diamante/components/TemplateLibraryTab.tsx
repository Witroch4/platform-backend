...
export function TemplateLibraryTab() {
  ...
  const getApprovalStatus = (template: TemplateLibraryWithCreator) => {
    ...
    switch (latestRequest.status) {
      case 'pending':
        return <Badge variant="default"><Clock className="h-3 w-3 mr-1" />Pendente</Badge>;
      case 'approved':
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              Gerencie templates compartilhados e mensagens interativas
            </p>
          </div>
        </div>
        ...
        {isSuperAdmin && (
          <TabsContent value="management">
            <LibraryManagementInterface
              onCreateGlobal={() => setCreateDialogOpen(true)}
            />
          </TabsContent>
        )}
      </Tabs>
