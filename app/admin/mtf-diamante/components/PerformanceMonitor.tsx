...
export function PerformanceMonitor() {
  ...
  const refreshStats = () => {
    ...
    const newStats: PerformanceStats[] = operations.map(op => {
  ...
  const getPerformanceBadge = (avgTime: number) => {
    if (avgTime < 100) return <Badge variant="default" className="bg-green-500">Excelente</Badge>;
    if (avgTime < 300) return <Badge variant="default" className="bg-yellow-500">Bom</Badge>;
    if (avgTime < 500) return <Badge variant="default" className="bg-orange-500">Regular</Badge>;
  ...
  const getSuccessRateBadge = (rate: number) => {
    ...
    if (rate >= 90) return <Badge variant="default" className="bg-yellow-500">Bom</Badge>;
    if (rate >= 80) return <Badge variant="default" className="bg-orange-500">Regular</Badge>;
                ...
                <div key={stat.operation} className="space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium">
                      {stat.operation.replace('-', ' ').toUpperCase()}
                    </span>
                    <span className="text-xs text-muted-foreground">
                      {stat.totalOperations} ops
                    </span>
                  </div>
