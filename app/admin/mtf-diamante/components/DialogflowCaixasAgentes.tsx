...
import { Loader2, Plus, Trash2, Settings, Bot, Inbox as InboxIcon, CheckCircle2, RefreshCw } from "lucide-react";
...
function DebugAgentesButton() {
	...
	const handleDebug = async () => {
		...
		try {
			...
			toast.error(error.response?.data?.error || "Erro no debug");
		} finally {
			setLoading(false);
		}
	};
	...
	return (
		<Button variant="outline" onClick={handleDebug} disabled={loading}>
...
function SincronizarAgentesButton({ onSincronizacao }: { onSincronizacao: () => void }) {
	...
	const handleSincronizar = async () => {
		...
		try {
			...
			toast.error(error.response?.data?.error || "Erro ao sincronizar agentes");
		} finally {
			setLoading(false);
		}
	};
...
export function DialogflowCaixasAgentes({ onCaixaSelected, filterCaixaId, hideToolbar }: DialogflowCaixasAgentesProps) {
		...
		<>
			<Card
				className={cn(
					...
					isSelected ? "border-primary ring-2 ring-primary" : "hover:border-muted-foreground/50",
				)}
				...
				<CardHeader>
					<div className="flex justify-between items-start">
						<div>
							...
							<CardDescription>{caixa.channelType}</CardDescription>
						</div>
				...
				<CardContent>
					...
					<div className="space-y-2 mb-4">
						{!caixa.agentes || caixa.agentes.length === 0 ? (
							...
							(caixa.agentes || []).map((agente) => (
								<AgenteItem key={agente.id} agente={agente} onUpdate={onUpdate} refreshCaixas={refreshCaixas} />
							))
						)}
					</div>
					...
					<div className="space-y-2 mb-4">
						{!caixa.assistentes || caixa.assistentes.length === 0 ? (
							<p className="text-muted-foreground text-xs">Nenhum assistente conectado.</p>
						) : (
...
function AgenteItem({
	...
	const handlePessimisticToggle = (toggledAgentId: string) => {
		...
		const caixaAtual = caixas.find((c) => (c.agentes || []).some((a) => a.id === toggledAgentId));
		const agenteAtual = caixaAtual?.agentes?.find((a) => a.id === toggledAgentId);
	...
	const handleToggleAgenteWithState = (toggledAgentId: string, estadoOriginal: boolean) => {
		const togglePromise = async () => {
			...
			const response = estadoOriginal
				? await axios.delete(`/api/admin/mtf-diamante/dialogflow/agentes/${toggledAgentId}/toggle`)
				: await axios.post(`/api/admin/mtf-diamante/dialogflow/agentes/${toggledAgentId}/toggle`);
	...
	const handleDeleteAgente = async () => {
		const deletePromise = axios.delete(`/api/admin/mtf-diamante/dialogflow/agentes/${agente.id}`);
...
function AssistenteItem({
	...
	const handleToggleAssistente = async () => {
		...
		try {
			...
			if (!assistente.conectado) {
				...
				console.log(
					`📍 [DEBUG] Atualizando link existente "${assistente.linkId}" para assistente "${assistente.nome}"`,
				);
	...
	const handleDesconectarAssistente = async () => {
		const deletePromise = axios.delete(`/api/admin/ai-integration/assistant-links/${assistente.linkId}`);
		...
		<>
			<div className="flex items-center justify-between p-2 border rounded-lg text-sm bg-blue-50 dark:bg-blue-950">
				<div className="flex items-center gap-2">
					...
					{!optimisticConectado && <Badge variant="secondary">Disponível</Badge>}
			...
			<Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							Tem certeza que deseja desconectar o assistente "{assistente.nome}" desta caixa? O assistente não será
							excluído, apenas desconectado.
						</DialogDescription>
					</DialogHeader>
					<DialogFooter>
						<Button variant="outline" onClick={() => setShowDeleteDialog(false)}>
		...
		<Dialog open={open} onOpenChange={setOpen}>
			...
			<DialogContent className="sm:max-w-2xl">
				<DialogHeader>
					...
					<DialogDescription>
						Listando caixas de entrada da sua conta Chatwit que ainda não foram configuradas.
					</DialogDescription>
				</DialogHeader>
