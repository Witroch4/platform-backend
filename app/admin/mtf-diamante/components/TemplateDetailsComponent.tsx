...
import { Loader2, AlertCircle, Send, Trash2, CheckCircle, Copy, Phone, ExternalLink } from "lucide-react";
...
export default function TemplateDetailsComponent({ templateId, onBack }: TemplateDetailsComponentProps) {
		...
		async function fetchTemplate() {
			...
			try {
				...
				if (bodyComp && Array.isArray(bodyComp.parameters)) {
					setTestVariables(bodyComp.parameters.map((v: any) => v.example || ""));
				}
				...
				if (btnComp?.buttons) {
					const copyBtn = btnComp.buttons.find((b: any) => b.type === "COPY_CODE");
		...
		<div className="max-w-6xl mx-auto space-y-6">
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>Teste o template enviando para um número específico</CardDescription>
				</CardHeader>
				<CardContent className="space-y-4">
								...
								{testVariables.map((val, idx) => (
									<div key={idx} className="grid grid-cols-1 sm:grid-cols-4 gap-2 items-center">
										<Label className="text-xs sm:text-sm truncate">
											{"{{" +
												template.componentes.flatMap((c) => (Array.isArray(c.parameters) ? c.parameters : []))[idx]
													?.type +
												"}}"}
										</Label>
					...
					{template.componentes.some((c) => c.type === "BUTTONS" && c.buttons?.some((b) => b.type === "COPY_CODE")) && (
						<div>
							<Label>Cupom (copy_code)</Label>
							<Input placeholder="CODE123" value={couponCode} onChange={(e) => setCouponCode(e.target.value)} />
						</div>
					)}

					<div className="flex gap-2">
						<Button onClick={handleTestSend} disabled={isSending}>
							{isSending ? <Loader2 className="animate-spin h-4 w-4 mr-2" /> : <Send className="h-4 w-4 mr-2" />}
							Enviar teste
						</Button>
