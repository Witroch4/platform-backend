...
export function ExecutiveKPICards({ filters }: ExecutiveKPICardsProps) {
  ...
  // Fetch KPI data
  const { data, error, isLoading } = useSWR<{ success: boolean; data: ExecutiveKPIs }>(
    apiUrl,
    fetcher,
    {
  ...
  const kpiCards = useMemo<KPICardData[]>(() => {
    ...
    return [
      ...
      },
      {
        ...
        colorClass: kpis.completionRate >= 70 
      ...
      },
      {
        ...
        colorClass: kpis.abandonmentRate <= 30 
          ? 'text-green-600 dark:text-green-400' 
