...
function transformToChartData(steps: FunnelStep[]): ChartDataPoint[] {
  ...
  return steps.map(step => ({
    name: step.nodeName.length > 20 
      ? `${step.nodeName.substring(0, 20)}...` 
      : step.nodeName,
    ...
    <div className="bg-background border rounded-lg shadow-lg p-3 space-y-2">
      ...
      <div className="space-y-1 text-xs">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <span className={`font-medium ${data.dropOff > 30 ? 'text-red-500' : data.dropOff > 15 ? 'text-orange-500' : 'text-green-500'}`}>
...
export function FunnelChart({ flowId, filters }: FunnelChartProps) {
  ...
  // Fetch funnel data
  const { data: funnelSteps, error, isLoading } = useSWR<FunnelStep[]>(
    apiUrl,
    fetcher,
    {
  ...
  const summary = useMemo(() => {
    ...
    const conversionRate = firstStep.sessionCount > 0 
      ? (lastStep.sessionCount / firstStep.sessionCount) * 100 
      : 0;
      ...
      <CardHeader>
        <div className="flex items-start justify-between">
          <div>
            ...
            <CardDescription>
              Progressão de usuários através das etapas do flow
            </CardDescription>
          </div>
      ...
      <CardContent>
        ...
        {!isLoading && !error && chartData.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Execute o flow para gerar dados de conversão
            </p>
          </div>
        )}
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-red-700 dark:text-red-300 mt-1">
                      <span className="font-semibold">{summary.highestDropOff.nodeName}</span>
                      {' '}tem taxa de abandono de{' '}
              ...
              >
                ...
                <Bar dataKey="sessions" name="sessions" radius={[8, 8, 0, 0]}>
                  {chartData.map((entry, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={getBarColor(entry.dropOff, entry.isHighestDropOff)}
                    />
                  ))}
                  <LabelList
                    dataKey="percentage"
