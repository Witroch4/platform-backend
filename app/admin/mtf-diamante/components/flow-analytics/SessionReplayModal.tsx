...
export function SessionReplayModal({ sessionId, open, onOpenChange }: SessionReplayModalProps) {
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="w-[96vw] sm:max-w-4xl max-h-[85vh]">
				<DialogHeader>
					...
					<DialogDescription>Timeline cronológica de execução com logs detalhados</DialogDescription>
				</DialogHeader>
							...
							<div className="bg-muted/50 rounded-lg p-4 space-y-2">
								<div className="grid grid-cols-2 gap-4 text-sm">
									...
									<div>
										...
										<code className="text-xs bg-background px-1 py-0.5 rounded">{session.conversationId}</code>
									</div>
									<div>
										<span className="text-muted-foreground">Contato:</span>
										<code className="text-xs bg-background px-1 py-0.5 rounded">{session.contactId}</code>
									</div>
								</div>
							</div>
							...
							<div>
								...
								<div className="space-y-3">
									{session.executionLog.map((entry, index) => (
										...
										>
											{/* Icon */}
											<div className="flex-shrink-0 mt-0.5">{getNodeTypeIcon(entry.nodeType)}</div>
											...
											<div className="flex-1 min-w-0 space-y-1">
												...
												{entry.action && <p className="text-xs text-muted-foreground">Ação: {entry.action}</p>}

												{entry.errorDetail && (
													<div className="bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900 rounded p-2 mt-2">
														<p className="text-xs text-red-700 dark:text-red-300">{entry.errorDetail}</p>
													</div>
												)}
											</div>
										</div>
								...
								<>
									...
									<div>
										...
										<div className="bg-muted/50 rounded-lg p-3">
											<pre className="text-xs overflow-x-auto">{JSON.stringify(session.variables, null, 2)}</pre>
										</div>
									</div>
								</>
							)}
