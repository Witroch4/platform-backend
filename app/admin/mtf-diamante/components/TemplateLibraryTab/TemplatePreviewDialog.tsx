							...
							<div className="space-y-2 text-sm">
								{content?.header && (
									<div>
										...
										<div className="bg-muted p-2 rounded text-xs font-mono">{content?.header}</div>
									</div>
								)}
								<div>
									<span className="text-muted-foreground">Body:</span>
									<div className="bg-muted p-2 rounded text-xs font-mono">{content?.body}</div>
								</div>
								{content?.footer && (
									<div>
										<span className="text-muted-foreground">Footer:</span>
										<div className="bg-muted p-2 rounded text-xs font-mono">{content.footer}</div>
									</div>
								)}
							</div>
						</div>
					...
					<div className="space-y-4">
						...
						<div
							className={`whatsapp-preview rounded-lg p-4 max-w-sm mx-auto bg-cover bg-center ${
								theme === "dark" ? "bg-[url('/fundo_whatsapp_black.jpg')]" : "bg-[url('/fundo_whatsapp.jpg')]"
							}`}
						>
										...
										template.approvalRequests.map((request) => (
											<div key={request.id} className="p-3 border rounded-lg">
												...
												{request.requestMessage && (
													<p className="text-sm text-muted-foreground mb-2">Request: {request.requestMessage}</p>
												)}
												{request.responseMessage && <p className="text-sm">Response: {request.responseMessage}</p>}
											</div>
										))
									) : (
