...
export const TemplatePreview = ({ formState }: TemplatePreviewProps) => {
  ...
  const isEmptyTemplate = useMemo(() => {
    const noHeader = !formState.headerType || formState.headerType === 'NONE'
      || (formState.headerType === 'TEXT' && !formState.headerText)
      || (['IMAGE','VIDEO','DOCUMENT'].includes(formState.headerType) && (!formState.headerMetaMedia || formState.headerMetaMedia.length === 0));
