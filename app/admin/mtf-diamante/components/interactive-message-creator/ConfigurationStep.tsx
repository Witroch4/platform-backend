...
export const ConfigurationStep: React.FC<ConfigurationStepProps> = ({ 
  ...
  const generatePreviewComponents = () => {
    ...
    if (message.type === "button" && message.action && isButtonAction(message.action)) {
      components.push({ type: "buttons", buttons: message.action.buttons.map((btn: any) => ({ type: "QUICK_REPLY", text: btn.title })) });
      ...
      <CardContent>
        <Tabs defaultValue="basic" className="w-full">
          ...
          <TabsContent value="advanced" className="space-y-6">
            ...
            {message.type === "location_request" && <LocationRequestConfig message={message} updateAction={updateAction} />}
            {message.type === "reaction" && <ReactionConfig message={message} updateMessage={updateMessage} />}
            {message.type === "sticker" && <StickerConfig message={message} updateMessage={updateMessage} uploadedFiles={uploadedFiles} setUploadedFiles={setUploadedFiles} />}
          </TabsContent>
