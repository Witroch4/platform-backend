...
export const convertCentralToLocal = (reaction: CentralButtonReaction): LocalButtonReaction => {
	...
	console.log("🔄 [convertCentralToLocal] Converting reaction:", {
		...
		output: {
			...
			reaction:
				reaction.type === "emoji" || reaction.type === "text" || reaction.type === "action"
...
export const generatePrefixedId = (channelType: string | null, fallbackSuffix: string): string => {
	...
	const uniqueSuffix =
		fallbackSuffix.includes("_") && fallbackSuffix.length > 15
			? fallbackSuffix
			: (() => {
					const timestamp = Date.now();
...
export const validateInstagramMessage = (messageType: string, content: any): { isValid: boolean; errors: string[] } => {
	const errors: string[] = [];
		...
		case "quick_replies":
			...
			content.quick_replies?.forEach((reply: any, index: number) => {
				if (reply.title?.length > MESSAGE_LIMITS.INSTAGRAM_QUICK_REPLY_TITLE_MAX_LENGTH) {
					errors.push(
						`Quick reply ${index + 1}: título excede ${MESSAGE_LIMITS.INSTAGRAM_QUICK_REPLY_TITLE_MAX_LENGTH} caracteres`,
					);
				}
			});
			break;
		...
		case "generic":
			...
			content.elements?.forEach((element: any, index: number) => {
				if (element.title?.length > MESSAGE_LIMITS.INSTAGRAM_GENERIC_TITLE_MAX_LENGTH) {
					errors.push(
						`Elemento ${index + 1}: título excede ${MESSAGE_LIMITS.INSTAGRAM_GENERIC_TITLE_MAX_LENGTH} caracteres`,
					);
				}
				if (element.subtitle?.length > MESSAGE_LIMITS.INSTAGRAM_GENERIC_SUBTITLE_MAX_LENGTH) {
					errors.push(
						`Elemento ${index + 1}: subtítulo excede ${MESSAGE_LIMITS.INSTAGRAM_GENERIC_SUBTITLE_MAX_LENGTH} caracteres`,
					);
				}
				if (element.buttons?.length > MESSAGE_LIMITS.INSTAGRAM_BUTTON_TEMPLATE_MAX_BUTTONS) {
					errors.push(
						`Elemento ${index + 1}: máximo ${MESSAGE_LIMITS.INSTAGRAM_BUTTON_TEMPLATE_MAX_BUTTONS} botões permitido`,
					);
				}
			});
			break;
