...
export const ListConfig: React.FC<ListConfigProps> = ({ message, updateAction }) => {
	...
	const addSection = () => {
		...
		updateAction({
			sections: [...currentSections, { title: "", rows: [{ id: newRowId, title: "", description: "" }] }],
			type: "list",
		});
	};
	...
	const removeRow = (sectionIndex: number, rowIndex: number) => {
		const sections = [...getSections()];
		sections[sectionIndex].rows = sections[sectionIndex].rows.filter((_: any, i: number) => i !== rowIndex);
		updateAction({ sections, type: "list" });
	};
							...
							<Card key={sectionIndex} className="border-dashed">
								<CardHeader className="pb-3">
									<div className="flex items-center justify-between">
										...
										<Button variant="ghost" onClick={() => removeSection(sectionIndex)}>
								...
								<CardContent className="space-y-4">
									...
									<div className="space-y-2">
										<div className="flex items-center justify-between">
											...
											<Button variant="outline" onClick={() => addRow(sectionIndex)}>
												<Plus className="h-4 w-4 mr-1" />
												Adicionar Item
											</Button>
										</div>
										...
										{section.rows.map((row, rowIndex) => (
											<div key={rowIndex} className="border rounded p-3 space-y-2">
												<div className="flex items-center justify-between">
													<Badge variant="secondary" className="text-xs">
														Item {rowIndex + 1}
													</Badge>
													<Button variant="ghost" onClick={() => removeRow(sectionIndex, rowIndex)}>
