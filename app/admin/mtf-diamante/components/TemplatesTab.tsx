...
const TemplatesTab = ({ caixaId }: TemplatesTabProps) => {
  ...
  const fetchTemplates = async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch(`/api/admin/mtf-diamante/templates/${caixaId}`);
  ...
  const handleDelete = async (templateId: string) => {
    ...
    try {
      const response = await fetch(`/api/admin/mtf-diamante/templates/${templateId}`, { method: 'DELETE' });
      if (!response.ok) {
