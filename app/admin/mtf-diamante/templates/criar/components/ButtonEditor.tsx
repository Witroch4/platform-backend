...
export const ButtonEditor = ({ buttons, setButtons }: ButtonEditorProps) => {
  ...
  const addButton = (type: ButtonType, opts?: Partial<TemplateButton>) => {
    ...
    const defaults: Partial<TemplateButton> = {
      ...
      PHONE_NUMBER: { text: opts?.callVariant === "whatsapp" ? "Ligar no WhatsApp" : "Ligar", phone_number: "+5500000000000", callVariant: opts?.callVariant || "phone" },
                ...
                return (
                  <SortableItem key={button.id} id={button.id}>
                    <div className="flex items-start gap-2 p-3 border rounded-md bg-background">
                      <div className="mt-1"><Icon className="h-4 w-4" /></div>
                      <div className="flex-1 space-y-2">
                        <div className="flex items-center gap-2">
                          <Badge variant="outline">{button.type}</Badge>
                          <span className="text-xs text-muted-foreground">{countByType(button.type)}/{limits[button.type]}</span>
                          <Button variant="ghost" size="icon" className="ml-auto h-6 w-6" onClick={() => removeButton(index)}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                          <div>
                            ...
                            {button.type === "COPY_CODE" && (
                              <p className="text-[10px] text-muted-foreground mt-1">O texto deste botão é fixo para aprovação.</p>
                            )}
                          </div>
                          {button.type === "URL" && (
                            <div>
                            ...
                            <div>
                              ...
                              <Alert className="mt-2">
                                ...
                                <AlertDescription className="text-[11px]">
                                  Ative as ligações no gerenciador do WhatsApp. Consulte a documentação oficial sobre chamadas WhatsApp em <a className="underline" href="https://developers.facebook.com/docs/whatsapp/cloud-api/calling/" target="_blank" rel="noreferrer">Calling API</a> e o gerenciador em <a className="underline" href="https://business.facebook.com/latest/whatsapp_manager/phone_numbers" target="_blank" rel="noreferrer">WhatsApp Manager</a>.
                                </AlertDescription>
                              </Alert>
                            </div>
                          )}
