...
def generate_presigned_upload_url(
    ...
    logger.debug("presigned_upload_url key=%s expiry=%d", s3_key, expiry_seconds)
    return presigned
...
def generate_presigned_download_url(
    ...
    logger.debug("presigned_download_url key=%s expiry=%d", s3_key, expiry)
    return presigned
...
def upload_bytes_to_s3(
    ...
    logger.info("upload_to_s3 key=%s size=%d", s3_key, len(data))
    return url
...
def upload_bytes_to_bucket(
    ...
    logger.info("upload_to_bucket bucket=%s key=%s size=%d", bucket, s3_key, len(data))
    return url
...
def download_bytes_from_s3(s3_key: str) -> bytes:
    ...
    data = resp["Body"].read()
    logger.debug("download_from_s3 key=%s size=%d", s3_key, len(data))
    return data
...
def delete_s3_object(s3_key: str) -> None:
    ...
    logger.info("delete_s3_object key=%s", s3_key)
