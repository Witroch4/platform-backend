# ANTIGRAVITY.md — Socialwise Chatwit

> Instruções para o agente **Antigravity** (Google DeepMind). Baseado no `CLAUDE.md`.
...
- **Flow Engine vs Intents (Regressão)**: NUNCA interceptar todas as mensagens no webhook. Flow só executa se: 1) Botão `flow_` clicado (resume), ou 2) Intent classificada mapeada p/ Flow. O pipeline de classificação (alias/embedding) tem prioridade sobre `FlowOrchestrator` default.
