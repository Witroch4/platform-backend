...
const { parseGabaritoDeterministico, verificarPontuacao, debugResumo } = require('./lib/oab/gabarito-parser-deterministico');
const { extractTextFromPdf } = require('./lib/oab-eval/rubric-from-pdf');
...
async function testSinglePdf(pdfPath) {
  ...
  try {
    ...
    console.log(`   🎯 PEÇA: ${verificacao.peca.total} pts (esperado: ${verificacao.peca.esperado}) - ${pecaOk ? '✅ OK' : '❌ FALHOU'}`);
    console.log(`   🎯 QUESTÕES: ${verificacao.questoes.total} pts (esperado: ${verificacao.questoes.esperado}) - ${questoesOk ? '✅ OK' : '❌ FALHOU'}`);
    console.log(`   🎯 GERAL: ${verificacao.geral.total} pts (esperado: ${verificacao.geral.esperado}) - ${geralOk ? '✅ OK' : '❌ FALHOU'}`);
...
async function runBatchTest() {
  ...
  console.log(`${'='.repeat(80)}`);
  console.log(`✅ Passou: ${passed}/${pdfFiles.length}`);
  console.log(`❌ Falhou: ${failed}/${pdfFiles.length}`);
  console.log(`🚨 Erro: ${errors}/${pdfFiles.length}`);
  ...
  if (failedResults.length > 0) {
    ...
    failedResults.forEach(result => {
      ...
      console.log(`   QUESTÕES: ${result.verificacao.questoes.total} (esperado: ${result.verificacao.questoes.esperado})`);
      ...
      result.parsed.itens.forEach((item, idx) => {
        const peso = item.peso || 0;
        const grupo = item.ou_group_id ? ` [OU:${item.ou_group_id}]` : '';
        console.log(`   ${idx + 1}. [${item.questao}] ${peso.toFixed(2)} pts${grupo} - ${item.descricao.substring(0, 100)}...`);
      });
    });
  }
