...
export async function selectMidiaForSending(agendamentoId: string): Promise<Midia | null> {
	try {
		...
		console.log(
			`[AgendamentoService] Agendamento ${agendamentoId} tem ${agendamento.midias.length} mídias. TratarComoPostagensIndividuais: ${agendamento.tratarComoPostagensIndividuais}, Randomizar: ${agendamento.randomizar}`,
		);
		...
		if (agendamento.tratarComoPostagensIndividuais) {
			...
			if (selectedMidia) {
				...
				console.log(
					`[AgendamentoService] Incrementado contador da mídia ${selectedMidia.id} para ${selectedMidia.contador + 1}`,
				);
...
export async function prepareWebhookData(agendamentoId: string): Promise<any> {
	try {
		...
		const tokenExpired = instagramAccount.expires_at ? instagramAccount.expires_at * 1000 < Date.now() : false;
		...
		if (agendamento.tratarComoPostagensIndividuais) {
			...
			console.log(
				`[AgendamentoService] Preparando webhook para agendamento ${agendamentoId} com mídia única ${midia.id}`,
			);
		} else {
			// Se não for para tratar como postagens individuais, envia todas as mídias
			allMidias = agendamento.midias;
			console.log(
				`[AgendamentoService] Preparando webhook para agendamento ${agendamentoId} com ${allMidias.length} mídias para carrossel`,
			);
		}
