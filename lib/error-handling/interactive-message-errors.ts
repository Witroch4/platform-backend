...
export function handleApiCall<T>(apiCall: () => Promise<Response>, context?: StructuredError["context"]): Promise<T> {
	return errorHandler.withRetry(
		...
		`api_${context?.action || "call"}_${Date.now()}`,
		context,
	);
}

export function handleValidation(validationFn: () => any, context?: StructuredError["context"]): any {
	try {
		return validationFn();
	} catch (error) {
...
export function withErrorBoundary<T>(operation: () => T, context?: StructuredError["context"]): T {
	try {
		return operation();
	} catch (error) {
