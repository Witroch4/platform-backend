...
export function checkRateLimit(inboxId: string): {
	...
	try {
		...
		const validationError = createValidationError("rate_limit_check", sanitizeErrorMessage(error), undefined);
...
export async function handleWebhookError(
	...
	if (error instanceof InstagramTranslationError) {
		...
		instagramError = createValidationError("webhook_error", sanitizeErrorMessage(error), correlationId);
	}
