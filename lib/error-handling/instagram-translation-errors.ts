...
export function createInvalidChannelError(channelType: string, correlationId?: string): InvalidChannelError {
	return new InvalidChannelError(channelType, correlationId);
}

export function createDatabaseError(operation: string, originalError: Error, correlationId?: string): DatabaseError {
...
export function createValidationError(field: string, reason: string, correlationId?: string): ValidationError {
	return new ValidationError(field, reason, correlationId);
}

export function createTimeoutError(timeoutMs: number, correlationId?: string): TimeoutError {
	...
	getSummary(timeRangeMs?: number): ErrorSummary {
		...
		if (timeRangeMs) {
			...
			relevantErrors = this.errors.filter((error) => {
				const errorTime = error.metadata?.timestamp ? new Date(error.metadata.timestamp).getTime() : Date.now();
		...
		summary.recentErrors = relevantErrors.slice(-10).map((error) => ({
			timestamp: new Date(),
			code: error.code,
