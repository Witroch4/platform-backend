 ...
 */
export function calculateHeatmapData(sessions: FlowSessionInput[], flow: RuntimeFlowData): NodeHeatmapData[] {
	...
	for (const [nodeId, metrics] of nodeMetrics.entries()) {
		...
		const avgTimeBeforeLeaving = metrics.dropOffs > 0 ? metrics.totalTimeBeforeLeaving / metrics.dropOffs : 0;

		// Calculate visit percentage relative to START
		const visitPercentage = startNodeVisits > 0 ? (metrics.visitCount / startNodeVisits) * 100 : 0;
