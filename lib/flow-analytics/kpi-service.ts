  ...
  try {
    ...
    for (const session of sessionsWithTiming) {
      ...
      if (Array.isArray(executionLog) && executionLog.length > 0) {
        ...
        if (hasStartNode) {
          const hasInteraction = executionLog.some(entry => 
    ...
    const avgTimeToAbandonment = abandonmentCount > 0 
      ? totalAbandonmentTime / abandonmentCount 
      : 0;

    const startToEndRate = totalSessions > 0 
      ? (startToEndCount / totalSessions) * 100 
      : 0;

    const startToFirstInteractionRate = totalSessions > 0 
      ? (startToFirstInteractionCount / totalSessions) * 100 
      : 0;

    const avgClickThroughRate = totalInteractiveMessages > 0 
      ? (totalButtonClicks / totalInteractiveMessages) * 100 
      : 0;

    const avgResponseRateAfterDelay = totalDelayNodes > 0 
      ? (responsesAfterDelay / totalDelayNodes) * 100 
      : 0;
