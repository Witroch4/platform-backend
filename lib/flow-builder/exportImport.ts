...
export function getNodeOutputCount(node: FlowNode): number {
	if (node.type === "interactive_message") {
		...
		const elementsButtonCount = data.elements?.filter((e) => e.type === "button").length ?? 0;
 ...
 */
function getOutputIndexFromHandle(sourceHandle: string | undefined | null, node: FlowNode, _edge: FlowEdge): number {
	if (!sourceHandle) {
		return 0;
	}
	...
	if (node.type === "interactive_message") {
		...
		const elementsIndex = elementsButtons.findIndex((b) => b.id === sourceHandle);
		if (elementsIndex >= 0) {
			return elementsIndex;
		}

		// Busca no array buttons (legado)
		const legacyIndex = data.buttons?.findIndex((b) => b.id === sourceHandle) ?? -1;
		if (legacyIndex >= 0) {
			return legacyIndex;
		}
	}
...
export function canvasToN8nFormat(
	...
	for (const node of canvas.nodes) {
		...
		const outputs: N8nConnectionTarget[][] = Array.from({ length: Math.max(outputCount, 1) }, () => []);

		// Processa cada edge
		for (const edge of nodeEdges) {
			const outputIndex = getOutputIndexFromHandle(edge.sourceHandle, node, edge);
...
export function n8nFormatToCanvas(exportData: FlowExportFormat): FlowCanvas {
	...
	for (const [sourceId, sourceConnections] of Object.entries(exportData.connections)) {
		...
		for (let outputIdx = 0; outputIdx < outputs.length; outputIdx++) {
			...
			for (const target of targets) {
				const sourceHandle = getHandleFromOutputIndex(outputIdx, sourceNode.type as string, sourceNode);
		...
		for (const outputs of conn.main) {
			...
			for (const target of outputs) {
				...
				if (!nodeIds.has(target.node)) {
					warnings.push(`Conexão para nó inexistente: ${sourceId} → ${target.node}`);
				}
			}
		}
	}
 ...
 */
export function generateConnectionsDebugView(exportData: FlowExportFormat): string {
	const lines: string[] = [
		"=".repeat(60),
	...
	for (const node of exportData.nodes) {
		...
		for (let i = 0; i < outputs.length; i++) {
			...
			if (node.type === "interactive_message") {
				...
				outputLabel = button ? `Botão ${i}: "${(button as { title?: string }).title || button.id}"` : `Output ${i}`;
			...
			for (let j = 0; j < targets.length; j++) {
				...
				const targetType = targetNode ? (targetNode.type as string).toUpperCase().replace(/_/g, " ") : "?";
 ...
 */
export function generateConnectionsGraph(exportData: FlowExportFormat): string {
	const lines: string[] = [];
	const nodeMap = new Map(exportData.nodes.map((n) => [n.id, n]));
	...
	lines.push(
		`│ Nodes: ${String(exportData.nodes.length).padEnd(3)} | Connections: ${countConnections(exportData.connections).toString().padEnd(25)} │`,
	);
	lines.push("├─────────────────────────────────────────────────────────────┤");
