...
function parseLinhasItens(bloco: string, questao: "PEÇA" | `Q${1 | 2 | 3 | 4}`): LinhaItem[] {
	...
	const texto = mHdr ? bloco.slice(mHdr.index + mHdr[0].length) : bloco;
	let rawLines = texto
		.split(/\n/)
		.map((s) => s.trim())
		.filter(Boolean)
		.filter((line) => {
			if (
				/^(ITEM\s+PONTUA[ÇC][AÃ]O|ITEM|PONTUA[ÇC][AÃ]O|Endereçamento|Qualificação das partes|Alegações iniciais|Fundamentação|Pedidos|Pedidos e requerimentos|Fechamento)$/i.test(
					line,
				)
			)
				return false;
	...
	const flush = () => {
		...
		if (process.env.DEBUG_GABARITO === "1" && currentLabel) {
			...
			console.log(
				`[LOG]   matriz disponível: [${currentPesosFromMatrix ? currentPesosFromMatrix.join(", ") : "nenhuma"}]`,
			);
		}
		...
		let descricao = raw.replace(/0[,\.]00(?:\s*\/\s*(?:0?\d{1,2}|\d{2})[,\.]\d{2})+(?:\s*)?$/g, "").trim();

		descricao = descricao.replace(/\b(Endereçamento|Mérito|Pedidos e requerimentos|Fechamento)\b$/i, "").trim();
		...
		if (process.env.DEBUG_GABARITO === "1") {
			console.log(
				`[LOG] Item finalizado: ${currentLabel} | pesos: [${pesos.join(", ")}] | temOU: ${temOU} | segmento: ${currentItemSegment || currentSegment || "nenhum"}`,
			);
		}
...
function extrairFundamentos(s: string): string[] {
	...
	const leis = (s.match(RX_LEI) || []).map((x) =>
		x
			.replace(/\s+/g, " ")
			.trim()
			.replace(/\bn[ºo]\b/gi, ""),
	);
	const decr = (s.match(RX_DECR) || []).map((x) =>
		x
			.replace(/\s+/g, " ")
			.trim()
			.replace(/\bn[ºo]\b/gi, ""),
...
function fundamentosCanonicos(descricao: string, brutos: string[]): string[] {
	...
	for (const f of brutos) {
		...
		t = t.replace(
			/\bArt\.\s*([\dº§\.,\sIVXLC\-]+)\s+do\s+(CPC|CC)\b/i,
			(_m, art, dip) => `${dip.toUpperCase()} art. ${String(art).replace(/\s+/g, " ")}`,
		);
		// Art. X da Lei N -> Lei N art. X
		if (/^Art\./i.test(t) && lei) {
			t = t.replace(
				/^Art\.\s*([\dº§\.,\sIVXLC\-]+)/i,
				(_m, art) => `${lei.replace(/\bn[ºo]\b/gi, "")} art. ${String(art).replace(/\s+/g, " ")}`,
			);
		}
		...
		if (/^Art\./i.test(t) && decr) {
			t = t.replace(
				/^Art\.\s*([\dº§\.,\sIVXLC\-]+)/i,
				(_m, art) => `${decr.replace(/\bn[ºo]\b/gi, "")} art. ${String(art).replace(/\s+/g, " ")}`,
			);
		}
		// Art. X da CF/88|CRFB -> CF/88 art. X
		t = t.replace(
			/\bArt\.\s*([\dº§\.,\sIVXLC\-]+)\s+da\s+(?:CF\/?88|CRFB\/?88|Constituiç(?:a|ã)o(?: Federal)?(?: de 1988)?)\b/i,
...
export function parseGabaritoDeterministico(textoBruto: string, metaIn: ParseMetaInput): GabaritoAtomico {
		...
		linhas.forEach((li) => {
			...
			const ensureUniqueGroupId = (baseId: string, suffixSource: string | undefined, fallbackIndex: number) => {
				const normalizedSuffix =
					(suffixSource || `ALT${fallbackIndex}`)
						.normalize("NFD")
						.replace(/[\u0300-\u036f]/g, "")
						.replace(/[^A-Za-z0-9]/g, "_")
						.replace(/^_+|_+$/g, "")
			...
			if (process.env.DEBUG_GABARITO === "1") {
				console.log(
					`[LOG] ${questao}-${li.rotulo}: pesos=${li.pesos.length}, temOU=${hasAlternative}, deveDividir=${deveDividirPorOU}, ramos=${ramos.length}`,
				);
			...
			const ouGroupId = hasAlternative
				? `${questao}-${li.rotulo}-OU`.replace(/\s+/g, "").replace(/[^A-Za-z0-9\-]/g, "")
				: undefined;
			...
			ramos.forEach((ramoOriginal, ramoIdx) => {
				...
				const branchPesos = pesosPorRamo[ramoIdx] && pesosPorRamo[ramoIdx].length ? pesosPorRamo[ramoIdx] : li.pesos;
						...
						if (!descricaoFrag.trim()) {
							if (process.env.DEBUG_GABARITO === "1") {
								console.log("[GABARITO::SKIP_EMPTY]", {
									questao,
									fragIdx,
									original: partes[fragIdx] || "undefined",
								});
							}
							continue;
						}
				...
				if (process.env.DEBUG_GABARITO === "1" && questao === "PEÇA") {
					...
					console.log(
						`[LOG]   pesoTotal antes cap: ${branchPesos.length ? branchPesos.reduce((acc, p) => acc + (p ?? 0), 0) : "null"}`,
					);
	...
	if (process.env.DEBUG_GABARITO === "1") {
		...
		console.log(
			`[GABARITO::GRUPOS] PEÇA=${gruposPeca.length} | QUESTOES=${gruposQuestoes.length} | TOTAL=${gruposBuilders.length}`,
		);
...
export function verificarPontuacao(itens: Subitem[]): {
		...
		for (const item of lista) {
			...
			if (item.ou_group_id) {
				...
				const fundamentos = grupo.items.filter((g) => {
					...
					if (
						/^(conforme|com\s+base|com\s+fundamento|nos\s+termos|na\s+forma|pelo\s+art|pela\s+aplica|fundamentos\s+da|lastreado|previsto\s+no)/.test(
							texto,
						)
					)
						return true;
	...
	return {
		peca: {
			...
			desvio: desvioPeca,
			ok: Math.abs(desvioPeca) <= 0.2, // tolerância atual do parser
		},
		questoes: {
			total: somaQuestoes,
			esperado: esperadoQuestoes,
			desvio: desvioQuestoes,
			ok: Math.abs(desvioQuestoes) <= 0.2,
			porQuestao,
		},
		geral: {
			total: somaGeral,
