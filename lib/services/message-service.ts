...
export class MessageService {
  ...
  /**
   * Salva mensagem com verificação de idempotência
  ...
  async saveMessage(options: SaveMessageOptions): Promise<Message | null> {
    ...
    if (options.externalId) {
      ...
      if (existing) {
        console.log(
          `[MessageService] Mensagem duplicada detectada: ${options.externalId}`
        );
        return null; // Idempotente: não criar duplicata
      }
    }
