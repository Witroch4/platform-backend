...
export class FlashIntentChecker {
	...
	async isFlashIntentEnabledForUser(userId: string): Promise<boolean> {
		try {
			...
			const [webhookEnabled, highPriorityEnabled, lowPriorityEnabled, unifiedModelEnabled, cachingEnabled] =
				await Promise.all([
			...
			return webhookEnabled && highPriorityEnabled && lowPriorityEnabled && unifiedModelEnabled && cachingEnabled;
		} catch (error) {
