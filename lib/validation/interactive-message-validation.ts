...
export const BodySchema = z.object({
	text: z.string().max(MESSAGE_LIMITS.BODY_TEXT_MAX_LENGTH, VALIDATION_MESSAGES.BODY_TOO_LONG),
});

export const FooterSchema = z.object({
	text: z.string().max(MESSAGE_LIMITS.FOOTER_TEXT_MAX_LENGTH, VALIDATION_MESSAGES.FOOTER_TOO_LONG),
});
...
const BaseInteractiveMessageSchema = z.object({
	...
	name: z.string().min(1, VALIDATION_MESSAGES.NAME_REQUIRED).max(255, VALIDATION_MESSAGES.NAME_TOO_LONG),
	type: z.enum([
		"button",
		"list",
		"cta_url",
...
export const InteractiveMessageSchema = BaseInteractiveMessageSchema.refine(
	(data) => {
...
export const ButtonReactionSchema = z
	.object({
		id: z.string().optional(),
...
export class InteractiveMessageValidator {
	...
	static validateButtonReactions(reactions: ButtonReaction[], buttons: QuickReplyButton[]): ValidationResult {
		...
		const validReactions = reactions.filter((reaction) => {
			...
			if (
				reaction.type === "emoji" &&
				(!reaction.emoji || (typeof reaction.emoji === "string" && reaction.emoji.trim().length === 0))
			) {
				return false;
			}
			if (
				...
				(!reaction.textResponse ||
					(typeof reaction.textResponse === "string" && reaction.textResponse.trim().length === 0))
			) {
				return false;
			}
			if (
				reaction.type === "action" &&
				(!reaction.action || (typeof reaction.action === "string" && reaction.action.trim().length === 0))
			) {
				return false;
			}
	...
	private static validateBodyText(
		text: string,
		errors: ValidationError[],
		warnings: ValidationError[],
		messageType?: InteractiveMessageType,
	...
	private static validateHeaderContent(
		content: string,
		errors: ValidationError[],
		warnings: ValidationError[],
		type?: string,
	...
	private static validateBusinessRules(
		...
		if (message.type === "button") {
			...
			const hasButtons =
				buttonAction?.buttons && Array.isArray(buttonAction.buttons) && buttonAction.buttons.length > 0;
