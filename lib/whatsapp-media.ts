...
export async function downloadMetaMediaAndUploadToMinio(
	...
	try {
		...
		console.log(
			`[WhatsAppMedia] ${existingPublicUrl === null ? "URL null" : "URL da Meta"} - prosseguindo com download...`,
		);
		...
		if (template) {
			const existingComponents = (template.whatsappOfficialInfo?.components as Record<string, unknown>) || {};
 ...
 */
export async function getPublicMediaUrl(templateId: string, userId: string, metaUrl?: string): Promise<string | null> {
	try {
