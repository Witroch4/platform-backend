...
export class UserCache {
   ...
   */
  async getLoginAttempts(userId: string, limit: number = 10): Promise<Array<{ success: boolean; timestamp: number; ip?: string }>> {
    const key = `user:login:${userId}`
   ...
   */
  async setUserNotificationSettings(userId: string, settings: Record<string, any>, ttl: number = 86400): Promise<boolean> {
   ...
   */
  async addUserActivity(userId: string, activity: { action: string; resource: string; timestamp: number }): Promise<number> {
   ...
   */
  async getUserActivity(userId: string, limit: number = 20): Promise<Array<{ action: string; resource: string; timestamp: number }>> {
    const key = `user:activity:${userId}`
