...
export class QueueCache {
   ...
   */
  async setQueueError(queueName: string, error: { message: string; timestamp: number; count: number }, ttl: number = 300): Promise<boolean> {
   ...
   */
  async setQueueWorkers(queueName: string, workers: Array<{ id: string; status: string; lastSeen: number }>, ttl: number = 60): Promise<boolean> {
