...
export class CacheInvalidationManager {
   ...
   */
  async handleEvent(eventType: EventType, context: { queueName?: string; userId?: string; [key: string]: any }): Promise<void> {
   ...
   */
  async smartInvalidate(changeType: 'queue' | 'job' | 'user' | 'alert' | 'metric', identifier: string, operation: 'create' | 'update' | 'delete'): Promise<void> {
    switch (changeType) {
      case 'queue':
   ...
   */
  scheduleCleanup(intervalMs: number = 3600000): NodeJS.Timeout { // Default 1 hour
