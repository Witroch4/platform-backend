...
import { JOB_STATES, QUEUE_STATES, EVENT_TYPES, CACHE_KEYS, DEFAULTS, ERROR_CODES } from "../constants";
...
import {
	getFlowControlService,
	FlowControlService,
	FlowControlConfig,
	FlowControlMetrics,
} from "./flow-control.service";
...
export class QueueManagerService extends EventEmitter implements QueueManagerServiceInterface {
	...
	public async retryAllFailed(queueName: string, user?: User): Promise<BatchResult> {
		try {
			...
			this.logger.info(
				`Batch retry completed for queue ${queueName}: ${result.processed}/${failedJobs.length} successful`,
				{
					queueName,
					result,
					userId: user?.id,
	...
	public async cleanCompleted(
		...
		try {
			...
			this.logger.info(
				`Batch cleanup completed for queue ${queueName}: ${result.processed}/${jobsToClean.length} jobs cleaned`,
				{
					queueName,
					olderThan,
	...
	private validateQueueConfig(config: QueueConfig): void {
		if (!config.name || config.name.trim() === "") {
			throw new QueueManagementError("Queue name is required", ERROR_CODES.VALIDATION_ERROR);
		}

		if (this.queues.has(config.name)) {
			throw new QueueManagementError(`Queue ${config.name} already exists`, ERROR_CODES.QUEUE_ALREADY_EXISTS);
		}
	}
	...
	private determineQueueStatus(counts: any, performance: any, resources: any, queueName: string): QueueState {
	...
	private applyJobFilters(jobs: Job[], filters: JobFilters): Job[] {
		...
		if (filters.search) {
			...
			filteredJobs = filteredJobs.filter(
				(job) => job.name.toLowerCase().includes(searchTerm) || job.id?.toString().includes(searchTerm),
			);
		}
		...
		if (filters.dateRange) {
			filteredJobs = filteredJobs.filter((job) => {
				...
				return jobTime >= filters.dateRange!.start.getTime() && jobTime <= filters.dateRange!.end.getTime();
			});
		}
	...
	private async clearQueueCache(queueName: string): Promise<void> {
		const keys = [CACHE_KEYS.QUEUE_CONFIG(queueName), CACHE_KEYS.QUEUE_HEALTH(queueName)];
	...
	private validatePagination(pagination: Pagination): void {
		...
		if (pagination.limit < 1 || pagination.limit > DEFAULTS.MAX_PAGE_SIZE) {
			throw new ValidationError(`Limit must be between 1 and ${DEFAULTS.MAX_PAGE_SIZE}`, "limit", pagination.limit);
		}
	}
