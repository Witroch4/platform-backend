...
import { JobAction, BatchAction } from "../../../types/queue-management";
import { User } from "../types/user.types";
import { DEFAULTS, ERROR_CODES, EVENT_TYPES } from "../constants";
import { QueueManagementError, ValidationError } from "../errors";
...
export class BatchOperationService extends EventEmitter {
	...
	public async executeBatchJobOperation(
		...
		try {
			...
			throw new QueueManagementError(`Batch operation failed: ${errorMessage}`, ERROR_CODES.INTERNAL_ERROR, 500, {
				operationId,
	...
	public async executeBatchQueueOperation(
		...
		try {
			...
			const errorMessage =
				error instanceof Error ? (error instanceof Error ? error.message : "Unknown error") : "Unknown error";
			throw new QueueManagementError(`Batch queue operation failed: ${errorMessage}`, ERROR_CODES.INTERNAL_ERROR, 500, {
				operationId,
				originalError: errorMessage,
			});
		} finally {
	...
	public async cancelBatchOperation(operationId: string, user?: User): Promise<boolean> {
		...
		if (!progress) {
			throw new QueueManagementError(`Batch operation not found: ${operationId}`, ERROR_CODES.NOT_FOUND);
		}
	...
	public async rollbackBatchOperation(operationId: string, user?: User): Promise<boolean> {
		...
		try {
			...
			const errorMessage =
				error instanceof Error ? (error instanceof Error ? error.message : "Unknown error") : "Unknown error";
			throw new QueueManagementError(`Failed to rollback batch operation: ${errorMessage}`, ERROR_CODES.INTERNAL_ERROR);
		}
	}
		...
		for (const action of progress.rollbackInfo.rollbackActions.reverse()) {
			try {
				...
				const errorMessage =
					error instanceof Error ? (error instanceof Error ? error.message : "Unknown error") : "Unknown error";
				rollbackErrors.push(
					`Failed to rollback ${action.type} for ${action.jobId || action.queueName}: ${errorMessage}`,
				);
			}
		}
