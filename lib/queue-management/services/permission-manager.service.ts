...
import { User, QueueConfig } from "../../../types/queue-management";
import { UserRole } from "../constants";
import { PERMISSIONS, ROLE_PERMISSIONS, USER_ROLES, ERROR_CODES, Permission } from "../constants";
...
export class PermissionManagerService implements PermissionManagerInterface {
	...
	validateJobOperation(user: User, action: string, queueName: string, jobId?: string): void {
		...
		this.logger.audit(`job:${action}`, user.id, `job:${jobId || "unknown"}`, { queueName, action });
	}
	...
	validateQueueOperation(user: User, action: string, queueName: string): void {
		...
		// Log the permission check
		this.logger.audit(`queue:${action}`, user.id, `queue:${queueName}`, { queueName, action });
	}
	...
	validateBatchOperation(user: User, action: string, queueName: string, jobCount: number): void {
		...
		if (jobCount > 1000 && user.role !== USER_ROLES.ADMIN && user.role !== USER_ROLES.SUPERADMIN) {
			throw new InsufficientPermissionsError(`batch operation with ${jobCount} jobs`, `queue:${queueName}`, user.id);
		}

		// Log the batch operation
		this.logger.audit(`batch:${action}`, user.id, `queue:${queueName}`, { queueName, action, jobCount });
	}
	...
	validateSystemOperation(user: User, action: string): void {
		...
		this.logger.audit(`system:${action}`, user.id, "system", { action });
	}
	...
	validateAlertOperation(user: User, action: string, alertId?: string): void {
		...
		this.logger.audit(`alert:${action}`, user.id, `alert:${alertId || "unknown"}`, { action, alertId });
	}
	...
	validateFlowOperation(user: User, action: string, flowId: string): void {
		...
		this.logger.audit(`flow:${action}`, user.id, `flow:${flowId}`, { action, flowId });
	}
	...
	validateRateLimitOperation(user: User, requestCount: number, windowMs: number): void {
		...
		if (requestCount > maxRequests) {
			throw new InsufficientPermissionsError(`rate limit exceeded (${requestCount}/${maxRequests})`, "api", user.id);
		}
	}
}
