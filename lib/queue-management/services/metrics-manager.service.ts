...
export class MetricsManagerService extends EventEmitter {
	...
	async getDashboardMetrics(): Promise<DashboardMetrics> {
		try {
			...
			const errorMessage =
				error instanceof Error ? (error instanceof Error ? error.message : "Unknown error") : "Unknown error occurred";
			throw new QueueManagementError(`Failed to get dashboard metrics: ${errorMessage}`, "DASHBOARD_METRICS_ERROR");
		}
	}
	...
	async getAggregatedMetrics(
		...
		try {
			...
			const errorMessage =
				error instanceof Error ? (error instanceof Error ? error.message : "Unknown error") : "Unknown error occurred";
			throw new QueueManagementError(`Failed to get aggregated metrics: ${errorMessage}`, "AGGREGATED_METRICS_ERROR");
		}
	}
	 ...
	 */
	async getTrendAnalysis(queueName: string, metric: string, timeRange: TimeRange): Promise<TrendPrediction> {
		try {
			...
			const errorMessage =
				error instanceof Error ? (error instanceof Error ? error.message : "Unknown error") : "Unknown error occurred";
			throw new QueueManagementError(`Failed to get trend analysis: ${errorMessage}`, "TREND_ANALYSIS_ERROR");
		}
	}
	...
	async detectAnomalies(timeRange?: TimeRange): Promise<Anomaly[]> {
		try {
			...
			const errorMessage =
				error instanceof Error ? (error instanceof Error ? error.message : "Unknown error") : "Unknown error occurred";
			throw new QueueManagementError(`Failed to detect anomalies: ${errorMessage}`, "ANOMALY_DETECTION_ERROR");
		}
	}
	 ...
	 */
	async createBaseline(queueName: string, metric: string, timeRange: TimeRange): Promise<PerformanceBaseline> {
		try {
			return await this.anomalyDetector.createBaseline(queueName, metric, timeRange);
		} catch (error) {
			const errorMessage =
				error instanceof Error ? (error instanceof Error ? error.message : "Unknown error") : "Unknown error occurred";
			throw new QueueManagementError(`Failed to create baseline: ${errorMessage}`, "BASELINE_CREATION_ERROR");
		}
	}
	 ...
	 * Export metrics data
	 */
	async exportMetrics(format: "csv" | "json", filters: MetricFilters): Promise<ExportResult> {
		try {
			if (!this.config.exportEnabled) {
				throw new QueueManagementError("Metrics export is disabled", "EXPORT_DISABLED");
			}
			...
			const errorMessage =
				error instanceof Error ? (error instanceof Error ? error.message : "Unknown error") : "Unknown error occurred";
			throw new QueueManagementError(`Failed to export metrics: ${errorMessage}`, "EXPORT_ERROR");
		}
	}
	...
	async getStorageStats(): Promise<{
		...
		try {
			...
			const errorMessage =
				error instanceof Error ? (error instanceof Error ? error.message : "Unknown error") : "Unknown error occurred";
			throw new QueueManagementError(`Failed to get storage stats: ${errorMessage}`, "STORAGE_STATS_ERROR");
		}
	}
	...
	async cleanupOldData(): Promise<{ deletedRecords: number; freedSpace: number }> {
		try {
			...
			const errorMessage =
				error instanceof Error ? (error instanceof Error ? error.message : "Unknown error") : "Unknown error occurred";
			throw new QueueManagementError(`Failed to cleanup old data: ${errorMessage}`, "CLEANUP_ERROR");
		}
	}
	...
	async optimizeStorage(): Promise<void> {
		try {
			...
			const errorMessage =
				error instanceof Error ? (error instanceof Error ? error.message : "Unknown error") : "Unknown error occurred";
			throw new QueueManagementError(`Failed to optimize storage: ${errorMessage}`, "OPTIMIZATION_ERROR");
		}
	}
	...
	private async getRecentAlerts(limit: number): Promise<
		Array<{
			id: string;
			severity: string;
			title: string;
	...
	private calculateSystemHealth(queueMetrics: any[], systemMetrics: SystemMetrics): "healthy" | "warning" | "critical" {
	...
	private async getHistoricalMetrics(queueName: string, metric: string, timeRange: TimeRange): Promise<any[]> {
