...
export class MetricsCollectorService extends EventEmitter {
	...
	constructor(prisma: PrismaClient, redis?: ReturnType<typeof getRedisInstance>) {
		...
		this.config = {
			...
			aggregationIntervals: queueConfig.metrics.aggregationIntervals.map((interval) => interval as any),
	...
	async collectQueueMetrics(queueName: string): Promise<QueueMetrics> {
		...
		try {
			...
			const successRate = totalJobsLastHour > 0 ? (totalJobsLastHour - failedJobsLastHour) / totalJobsLastHour : 1;
			const errorRate = totalJobsLastHour > 0 ? failedJobsLastHour / totalJobsLastHour : 0;
			const retryRate = totalJobsLastHour > 0 ? retriedJobsLastHour / totalJobsLastHour : 0;
	...
	async collectJobMetrics(jobId: string): Promise<any> {
		try {
			...
			if (!jobMetrics) {
				throw new QueueManagementError(`Job metrics not found for job ${jobId}`, "JOB_METRICS_NOT_FOUND");
			}
	...
	private async getQueueResourceUsage(
		queueName: string,
	): Promise<{ memoryUsage: number; cpuTime: number; ioOperations: number }> {
