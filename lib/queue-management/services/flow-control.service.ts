...
import { QueueConfig, User, RateLimiterConfig } from "../../../types/queue-management";
import { CACHE_KEYS, DEFAULTS, ERROR_CODES, EVENT_TYPES } from "../constants";
import { QueueManagementError, QueueNotFoundError, ValidationError, RateLimitExceededError } from "../errors";
		...
		try {
			...
			const metrics: FlowControlMetrics = {
				...
				rateLimitStatus: rateLimiter
					? await rateLimiter.getStatus(queueName)
					: {
							current: 0,
							limit: 0,
							resetTime: new Date(),
		...
		if (config.rateLimiter) {
			...
			if (config.rateLimiter.duration < 1000) {
				throw new ValidationError(
					"Rate limiter duration must be at least 1000ms",
					"rateLimiter.duration",
					config.rateLimiter.duration,
				);
			}
		}
...
class RateLimiter {
	...
	async checkLimit(
		key: string,
	): Promise<{ allowed: boolean; current: number; limit: number; resetTime: Date; windowMs: number }> {
		const now = Date.now();
...
class CircuitBreaker {
	...
	private shouldAttemptReset(): boolean {
		return !!(this.lastFailureTime && Date.now() - this.lastFailureTime.getTime() >= this.config.recoveryTimeout);
	}
}
