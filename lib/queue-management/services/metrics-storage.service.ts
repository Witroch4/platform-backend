...
export class MetricsStorageService implements MetricStorage {
	...
	async store(metrics: Metric[]): Promise<void> {
		...
		try {
			...
			throw new QueueManagementError(`Failed to store metrics: ${errorMessage}`, "STORAGE_ERROR");
		}
	}
	...
	async query(query: MetricQuery): Promise<Metric[]> {
		try {
			...
			const errorMessage =
				error instanceof Error ? (error instanceof Error ? error.message : "Unknown error") : "Unknown error occurred";
			throw new QueueManagementError(`Failed to query metrics: ${errorMessage}`, "QUERY_ERROR");
		}
	}
	...
	async aggregate(query: AggregationQuery): Promise<AggregatedMetrics> {
		try {
			...
			const errorMessage =
				error instanceof Error ? (error instanceof Error ? error.message : "Unknown error") : "Unknown error occurred";
			throw new QueueManagementError(`Failed to aggregate metrics: ${errorMessage}`, "AGGREGATION_ERROR");
		}
	}
	...
	async createPartition(tableName: string, startDate: Date, endDate: Date): Promise<void> {
      ...
      `);
			...
			console.log(
				`Created partition ${partitionName} for period ${startDate.toISOString()} to ${endDate.toISOString()}`,
			);
		} catch (error) {
			const errorMessage =
				error instanceof Error ? (error instanceof Error ? error.message : "Unknown error") : "Unknown error occurred";
			throw new QueueManagementError(`Failed to create partition: ${errorMessage}`, "PARTITION_ERROR");
		}
	}
	...
	private async cleanupOldCacheData(cutoffDate: Date): Promise<void> {
		try {
			...
			const patterns = ["metrics:*", "queue:metrics:*", "pre_agg:*"];
	...
	async optimizeIndexes(): Promise<void> {
		try {
			...
			const errorMessage =
				error instanceof Error ? (error instanceof Error ? error.message : "Unknown error") : "Unknown error occurred";
			throw new QueueManagementError(`Failed to optimize indexes: ${errorMessage}`, "INDEX_OPTIMIZATION_ERROR");
		}
	}
	...
	async getStorageStats(): Promise<{
      ...
      `;
			...
			// Get partition count
			const partitionCount = Array.from(this.partitions.values()).reduce(
			...
			const errorMessage =
				error instanceof Error ? (error instanceof Error ? error.message : "Unknown error") : "Unknown error occurred";
			throw new QueueManagementError(`Failed to get storage stats: ${errorMessage}`, "STATS_ERROR");
		}
	}
	...
	async preAggregateMetrics(granularity: string): Promise<void> {
		try {
			...
			const errorMessage =
				error instanceof Error ? (error instanceof Error ? error.message : "Unknown error") : "Unknown error occurred";
			throw new QueueManagementError(`Failed to pre-aggregate metrics: ${errorMessage}`, "PRE_AGGREGATION_ERROR");
		}
	}
	...
	async compressOldData(): Promise<{ compressedPartitions: number; spaceSaved: number }> {
		try {
			...
			const errorMessage =
				error instanceof Error ? (error instanceof Error ? error.message : "Unknown error") : "Unknown error occurred";
			throw new QueueManagementError(`Failed to compress old data: ${errorMessage}`, "COMPRESSION_ERROR");
		}
	}
