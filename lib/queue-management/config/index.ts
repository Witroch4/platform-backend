...
export const ConfigTemplates = {
	...
	background: (name: string, createdBy: string) =>
		createQueueConfig(name, createdBy)
			...
			.alertThresholds(500, 120000, 0.1) // 500 waiting jobs, 2min processing, 10% error rate
	...
	batch: (name: string, createdBy: string) =>
		createQueueConfig(name, createdBy)
			...
			.alertThresholds(50, 600000, 0.2) // 50 waiting jobs, 10min processing, 20% error rate
			.cleanupPolicy(10, 10)
...
export const SystemConfigUtils = {
	...
	async setMultiple(
		...
		await Promise.all(
			configs.map(({ key, value, description }) => manager.setSystemConfig(key, value, updatedBy, description)),
		);
	},
		...
		const defaults = [
			{
				...
				value: 90,
				description: "Default retention period for queue metrics in days",
			},
			{
				key: SYSTEM_CONFIG_KEYS.QUEUE_DEFAULT_CLEANUP_INTERVAL,
				value: 3600,
				description: "Default cleanup interval in seconds",
			},
			{
				key: SYSTEM_CONFIG_KEYS.QUEUE_DEFAULT_MAX_CONCURRENT_JOBS,
				value: 100,
				description: "Default maximum concurrent jobs per queue",
			},
			{
				key: SYSTEM_CONFIG_KEYS.ALERTS_DEFAULT_COOLDOWN_MINUTES,
				value: 5,
				description: "Default cooldown period for alerts in minutes",
			},
			{ key: SYSTEM_CONFIG_KEYS.ALERTS_DEFAULT_CHANNELS, value: ["email"], description: "Default alert channels" },
			{
				key: SYSTEM_CONFIG_KEYS.METRICS_COLLECTION_INTERVAL_SECONDS,
				value: 60,
			...
			{
				key: SYSTEM_CONFIG_KEYS.RATE_LIMIT_MAX_REQUESTS,
				value: 100,
				description: "Maximum requests per rate limit window",
			},
			{ key: SYSTEM_CONFIG_KEYS.AUTO_RETRY_ENABLED, value: true, description: "Enable automatic retry of failed jobs" },
			{
				key: SYSTEM_CONFIG_KEYS.AUTO_CLEANUP_ENABLED,
				value: true,
				description: "Enable automatic cleanup of old jobs",
			},
			{
				key: SYSTEM_CONFIG_KEYS.AUTO_SCALING_ENABLED,
				value: false,
				description: "Enable automatic scaling of workers",
			},
		];
