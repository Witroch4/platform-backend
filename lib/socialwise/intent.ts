...
async function summarizeShortTitle(
	name: string,
...
Saída: retorne apenas o rótulo final.`;
		...
		try {
			classifierLogger.info("Short title prompt (input)", { model: usedModel, maxLen, instructions: sys, user });
		} catch {}
		...
		try {
			classifierLogger.info("Short title raw response", data);
		} catch {}
		const out = extractResponsesApiText(data);
		// Log do resultado do resumo
		try {
			classifierLogger.info("Short title result (output)", { result: out });
		} catch {}
		if (!out) return null;
...
Saída: retorne apenas o rótulo final.`;
			...
			classifierLogger.info("Short title prompt (fallback try)", {
				model: "gpt-4o-mini",
				maxLen,
				instructions: sys,
				user,
			...
			try {
				classifierLogger.info("Short title raw response (fallback)", j2);
			} catch {}
	...
	try {
		...
		if (options?.userId && userText.trim()) {
			...
			if (candidates.length > 0) {
				...
				try {
					classifierLogger.info("Embedding input (userText)", { text: userText });
				} catch {}
				...
				if (q) {
					...
					const top = scored
						.slice(0, 5)
						.map((s: { name: string; shortTitle: string; score: number; threshold: number }) => ({
							name: s.name,
					...
					if (best && best.score >= best.threshold) {
						...
						classifierLogger.info("Intent chosen by embeddings", {
							intent: intentName,
					...
					classifierLogger.info("No intent passed threshold", {
						best: best ? { name: best.name, score: Number(best.score.toFixed(4)), threshold: best.threshold } : null,
					});
		...
		try {
			classifierLogger.info("LLM classification prompt (input)", input);
		} catch {}
