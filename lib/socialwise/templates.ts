 ...
 */
export async function buildWhatsAppByIntentRaw(
	intentRaw: string,
	inboxId: string,
	wamid?: string,
	contactContext?: { contactName?: string; contactPhone?: string },
) {
	...
	let mapping = await prisma.mapeamentoIntencao.findFirst({
		where: {
			inbox: { inboxId },
			OR: [{ intentName: norm.original }, { intentName: norm.plain }],
		},
		include: {
			template: {
				include: {
	...
	if (mapping.template.type === "WHATSAPP_OFFICIAL" && mapping.template.whatsappOfficialInfo) {
		...
		if (customVariables && Object.keys(customVariables).length > 0) {
			components = applyCustomVariablesToComponents(components, customVariables, contactContext?.contactPhone || "", {
 ...
 */
export async function buildWhatsAppByGlobalIntent(
	intentRaw: string,
	inboxId: string,
	wamid?: string,
	contactContext?: { contactName?: string; contactPhone?: string },
	...
	let intent = await prisma.intent.findFirst({
		where: {
			createdById: userId,
			isActive: true,
			OR: [{ name: nameRaw }, { name: norm.plain }, { slug }, { slug: slugify(nameRaw) }],
		},
		select: {
			id: true,
			name: true,
	...
	if (!intent) {
		...
		const caseInsensitiveMatch = allIntents.find(
			(i) =>
	...
	if (t.type === "WHATSAPP_OFFICIAL" && t.whatsappOfficialInfo) {
		...
		const metaTemplateId: string | undefined = typeof wi?.metaTemplateId === "string" ? wi.metaTemplateId : undefined;
		const content = await builder.buildTemplatePayload(t.name || "default", language, components, metaTemplateId);
		return { whatsapp: content };
	}
...
class InstagramTemplateConverter {
	...
	static convertWhatsAppPayloadToInstagram(whatsappPayload: any): any {
		console.log(
			"[Instagram Converter] Converting WhatsApp payload to Instagram:",
		...
		if (hasImage) {
			...
			imageUrl =
				whatsappPayload.header?.image?.link || whatsappPayload.header?.content || whatsappPayload.header?.image?.url;
		}
		...
		switch (templateType) {
			case "GENERIC_TEMPLATE":
				...
				result = InstagramTemplateConverter.buildGenericTemplate(
					baseText,
					imageUrl,
					instagramButtons,
					undefined,
			...
			case "BUTTON_TEMPLATE":
				...
				result = InstagramTemplateConverter.buildButtonTemplate(
					baseText,
					imageUrl,
					instagramButtons,
					undefined,
	...
	static determineInstagramTemplateType(
		...
		const hasExplicitQuickReplies = buttonList.some(
			(b: any) =>
	 ...
	 */
	static buildQuickReplies(bodyText: string, buttons: any[] = [], footerText?: string): any {
		const fullText = footerText ? `${bodyText}\n\n${footerText}` : bodyText;
 ...
 */
export async function buildInstagramByIntentRaw(
	intentRaw: string,
	inboxId: string,
	contactContext?: { contactName?: string; contactPhone?: string },
): Promise<any> {
 ...
 * EXATAMENTE O MESMO PADRÃO DO WHATSAPP mas para Instagram
 */
export async function buildInstagramByGlobalIntent(
	intentRaw: string,
	inboxId: string,
	contactContext?: { contactName?: string; contactPhone?: string },
): Promise<any> {
 ...
 */
export async function buildFacebookPageByIntentRaw(
	intentRaw: string,
	inboxId: string,
	contactContext?: { contactName?: string; contactPhone?: string },
): Promise<any> {
 ...
 * EXATAMENTE O MESMO PADRÃO DO INSTAGRAM - Facebook Page segue as mesmas regras
 */
export async function buildFacebookPageByGlobalIntent(
	intentRaw: string,
	inboxId: string,
	contactContext?: { contactName?: string; contactPhone?: string },
): Promise<any> {
	...
	let intent = await prisma.intent.findFirst({
		where: {
			createdById: userId,
			isActive: true,
			OR: [{ name: norm.original }, { name: norm.plain }],
		},
		select: {
			id: true,
			name: true,
	...
	if (!intent) {
		...
		const caseInsensitiveMatch = allIntents.find(
			(i) => i.name.toLowerCase() === norm.original.toLowerCase() || i.name.toLowerCase() === norm.plain.toLowerCase(),
		);
	...
	if (t.type === "INTERACTIVE_MESSAGE" && t.interactiveContent) {
		...
		if (converted && (converted as any).text)
			return { facebook: { message_format: "TEXT", text: (converted as any).text } };
