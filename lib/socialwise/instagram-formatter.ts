...
import { clampTitle, clampBody, clampPayload, validatePayloadFormat } from "./clamps";
...
export function buildInstagramButtons(
	...
	const { enableFallback = true, allowedPayloads, dropInvalidInsteadOfFallback = true } = options;
...
export function buildInstagramTextFallback(text: string, buttons: InstagramButtonOptions[]): InstagramTextMessage {
...
export function validateInstagramMessage(message: InstagramMessage): {
	...
	if ("message_format" in message) {
		...
		if (!("quick_replies" in message) || !Array.isArray((message as any).quick_replies))
			v.push("Quick replies required");
		else {
