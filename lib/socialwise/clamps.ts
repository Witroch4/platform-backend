...
export function clampButtonData(
  ...
  const isValid =
    clampedTitle.length > 0 && clampedPayload.length > 0 && isPayloadValid;
...
export function validateChannelLimits(
  ...
  if (content.body && content.body.length > limits.bodyText) {
    violations.push(
      `Body text exceeds ${limits.bodyText} characters (${content.body.length})`
    );
  }
  ...
  if (content.buttons && content.buttons.length > limits.maxButtons) {
    violations.push(
      `Too many buttons: ${content.buttons.length} (max: ${limits.maxButtons})`
    );
  }
  ...
  if (content.buttons) {
    content.buttons.forEach((button, index) => {
      if (button.title && button.title.length > limits.buttonTitle) {
        violations.push(
          `Button ${index + 1} title exceeds ${limits.buttonTitle} characters`
        );
      }
      ...
      if (
        ...
        violations.push(
          `Button ${index + 1} ID exceeds ${CHANNEL_LIMITS.whatsapp.buttonId} characters`
        );
      }
      ...
      if (
        ...
        violations.push(
          `Button ${index + 1} payload exceeds ${CHANNEL_LIMITS.instagram.payload} characters`
        );
      }

      if (!validatePayloadFormat(button.payload)) {
        violations.push(
          `Button ${index + 1} payload format invalid (must match ^@[a-z0-9_]+$)`
        );
      }
    });
  }
