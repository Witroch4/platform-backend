...
export class RateLimitError extends ApiError {
	constructor(limit: number, window: number) {
		super(`Rate limit exceeded: ${limit} requests per ${window}s`, API_ERROR_CODES.RATE_LIMIT_EXCEEDED, 429, {
			limit,
			window,
		});
	}
...
export function handleApiError(
	...
	if (error instanceof z.ZodError) {
		return createErrorResponse(API_ERROR_CODES.VALIDATION_ERROR, "Invalid input data", 400, {
	...
	if (error instanceof Error) {
		return createErrorResponse(API_ERROR_CODES.INTERNAL_SERVER_ERROR, error.message || defaultMessage, 500);
	}

	// Handle unknown errors
	return createErrorResponse(API_ERROR_CODES.INTERNAL_SERVER_ERROR, defaultMessage, 500);
}
...
export function applySorting<T>(items: T[], sortBy: string, sortOrder: "asc" | "desc" = "asc"): T[] {
 ...
 * Search helper
 */
export function applySearch<T>(items: T[], searchTerm: string, searchFields: string[]): T[] {
