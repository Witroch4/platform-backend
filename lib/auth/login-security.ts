...
export async function recordFailedAttempt(
  ...
  try {
    ...
    await redis.multi()
      .incr(ipKey)
      .expire(ipKey, CONFIG.ipRateLimit.windowSeconds)
      .exec();
    ...
    if (newCount >= CONFIG.accountLockout.maxFailedAttempts) {
      ...
      await redis.setex(
        lockoutKey,
        CONFIG.accountLockout.lockoutMinutes * 60,
        "locked"
      );
      ...
      console.warn(
        `[LoginSecurity] Conta bloqueada por tentativas excessivas: ${normalizedEmail}`
      );
    }

    // 4. Calcular delay progressivo
    const delay = Math.min(
      CONFIG.progressiveDelay.baseDelay +
        newCount * CONFIG.progressiveDelay.delayPerAttempt,
