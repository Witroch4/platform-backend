...
export class WebhookManager {
	...
	async getWebhookStats(): Promise<{
		...
		const successfulDeliveries = deliveries.filter(
			(d: WebhookDelivery) => d.responseStatus != null && d.responseStatus >= 200 && d.responseStatus < 300,
		).length;
		...
		const delivered = deliveries.filter((d: WebhookDelivery) => d.deliveredAt != null && d.createdAt != null);
		const averageResponseTime =
			delivered.reduce(
				(sum: number, d: WebhookDelivery) => sum + ((d.deliveredAt?.getTime() ?? 0) - d.createdAt.getTime()),
				0,
			) / (delivered.length || 1);
		...
		return {
			...
			averageResponseTime,
			lastSuccessAt:
				deliveries.find((d) => d.responseStatus != null && d.responseStatus >= 200 && d.responseStatus < 300)
					?.deliveredAt ?? null,
			lastFailureAt:
				deliveries.find((d) => d.responseStatus != null && (d.responseStatus < 200 || d.responseStatus >= 300))
					?.deliveredAt ?? null,
	...
	private formatWebhookDelivery(delivery: any): WebhookDelivery {
		...
		return {
			...
			error:
				delivery.responseStatus != null && (delivery.responseStatus < 200 || delivery.responseStatus >= 300)
