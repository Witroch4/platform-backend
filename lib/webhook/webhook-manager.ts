...
const defaultEvent = Object.values(Prisma.WebhookEvent)[0] as Prisma.WebhookEvent;
...
export class WebhookManager {
	...
	async getWebhookStatsById(webhookId: string): Promise<WebhookStats> {
		...
		for (const delivery of deliveries) {
			if (delivery.responseStatus && (delivery.responseStatus < 200 || delivery.responseStatus >= 300)) {
				consecutiveFailures++;
			} else if (delivery.responseStatus && delivery.responseStatus >= 200 && delivery.responseStatus < 300) {
				break;
			}
		}
	...
	async cancelDelivery(deliveryId: string): Promise<boolean> {
		const result = await prisma.webhookDelivery.updateMany({
			...
			data: {
				responseStatus: 499, // Client Closed Request
				responseBody: "Delivery cancelled by user",
			},
		});
	...
	async markDeliverySuccess(deliveryId: string): Promise<boolean> {
		const result = await prisma.webhookDelivery.updateMany({
			...
			data: {
				responseStatus: 200,
				responseBody: "Manually marked as successful",
				deliveredAt: new Date(),
			},
		});
	...
	async deliverWebhook(deliveryId: string): Promise<{
		...
		try {
			...
			await prisma.webhookDelivery.update({
				where: { id: deliveryId },
				data: {
					responseStatus: 0, // Indicates network/timeout error
