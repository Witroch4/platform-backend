...
export class UnifiedLeadManager {
	 ...
	 */
	private static async resolveOrCreateChatwitAccountIdFromInbox(
		inboxId: string,
		fallbackNumericAccountId?: number,
	): Promise<string | null> {
		try {
			...
			const chatwitAccountId =
				inbox.usuarioChatwit.chatwitAccountId ||
				(fallbackNumericAccountId !== undefined ? String(fallbackNumericAccountId) : null);
	...
	static async findLeadByContactSource(
		...
		try {
			...
			const chatwitAccountPk = inboxId ? await this.resolveOrCreateChatwitAccountIdFromInbox(inboxId, accountId) : null;
	...
	static async updateLeadWithMessageMetadata(
		...
		try {
			...
			if (lead.source === LeadSource.INSTAGRAM && lead.instagramProfile) {
				...
				const updatedAnnotations = currentAnnotations ? `${currentAnnotations}\n${newAnnotation}` : newAnnotation;
...
export async function findLeadByContactSource(contactSource: string, accountId: number): Promise<ExtendedLead | null> {
