...
export async function verifyBackwardCompatibility(): Promise<{
  ...
  try {
    ...
    try {
      ...
      const { BatchProcessorOrchestrator } = await import('@/app/admin/leads-chatwit/components/batch-processor/BatchProcessorOrchestrator')
 ...
 */
export async function runCompleteIntegrationTest(testUserId: string, testAccountId: string): Promise<{
  success: boolean
  results: {
  ...
  const success = results.systemIntegration.success && 
                 results.backwardCompatibility.success && 
                 results.userInitialization.success
