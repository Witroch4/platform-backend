...
export class ABTestingManager {
	...
	async assignUserToVariant(testId: string, userId: string, metadata?: Record<string, any>): Promise<ABTestAssignment> {
		const test = await this.getABTest(testId);
		if (!test) {
			throw new Error(`A/B test ${testId} not found`);
		}
	...
	private async calculateVariantResults(
		...
		for (const metric of metrics) {
			...
			if (metric.type === "CONVERSION") {
				result.metrics[metric.name].conversionRate = result.sampleSize > 0 ? (count / result.sampleSize) * 100 : 0;
			}
		}
