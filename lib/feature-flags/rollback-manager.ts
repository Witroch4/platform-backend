...
export class RollbackManager {
	...
	async canRollback(flagName: string): Promise<{
		...
		try {
			...
			const recentRollback = recentExecutions.find(
				(exec) => exec.success && Date.now() - new Date(exec.executedAt).getTime() < 5 * 60 * 1000, // 5 minutes
			);
	...
	private async sendRollbackAlert(plan: RollbackPlan, execution: RollbackExecution, reason: string): Promise<void> {
		try {
			const alert = {
