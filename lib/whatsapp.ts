...
export async function sendTemplateMessage(toRaw: string, templateName: string, opts: SendOpts = {}): Promise<boolean> {
	try {
			...
			switch (c.type) {
				case "HEADER": {
					if (["IMAGE", "VIDEO", "DOCUMENT"].includes(c.format)) {
						...
						if (!mediaUrl) {
							console.warn(
								`[sendTemplateMessage] Tentando enviar template '${templateName}' com mídia, mas publicMediaUrl está vazia.`,
							);
							continue;
						}
...
export async function testWhatsAppApiConnection(cfg: any) {
	try {
		...
		await axios.get(`${cfg.fbGraphApiBase}/${id}/whatsapp_business_profile`, {
			headers: {
