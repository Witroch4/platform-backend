...
describe('WhatsApp Messages Service', () => {
  ...
  describe('sanitizeCouponCode', () => {
    it('should sanitize coupon codes correctly', () => {
      ...
      testCases.forEach(({ input, expected }) => {
        if (expected === Error) {
          expect(() => sanitizeCouponCode(input)).toThrow();
        } else {
          const result = sanitizeCouponCode(input);
          expect(result).toBe(expected);
        }
      });
    });
  ...
  describe('sendTemplateMessage', () => {
    it('should send template message successfully', async () => {
      ...
      const result = await sendTemplateMessage(templateData, templateComponents);
      ...
      expect(mockedAxios.post).toHaveBeenCalledWith(
        'https://graph.facebook.com/v22.0/test-phone-number-id/messages',
        {
          messaging_product: 'whatsapp',
    ...
    it('should handle WhatsApp API errors', async () => {
      ...
      const result = await sendTemplateMessage(templateData, []);
  ...
  describe('sendInteractiveMessage', () => {
    it('should send interactive button message successfully', async () => {
      const interactiveData: InteractiveMessageData = {
        recipientPhone: '5511999999999',
        whatsappApiKey: 'test-api-key',
        header: {
          type: 'text',
      ...
      const result = await sendInteractiveMessage(interactiveData);
    ...
    it('should send interactive list message successfully', async () => {
      const listData: InteractiveMessageData = {
        recipientPhone: '5511999999999',
        whatsappApiKey: 'test-api-key',
      ...
      const result = await sendInteractiveMessage(listData);
    ...
    it('should send CTA URL message successfully', async () => {
      const ctaData: InteractiveMessageData = {
        recipientPhone: '5511999999999',
        whatsappApiKey: 'test-api-key',
      ...
      const result = await sendInteractiveMessage(ctaData);
    ...
    it('should send location request message successfully', async () => {
      const locationData: InteractiveMessageData = {
        recipientPhone: '5511999999999',
        whatsappApiKey: 'test-api-key',
      ...
      const result = await sendInteractiveMessage(locationData);
    ...
    it('should handle interactive message with image header', async () => {
      const imageHeaderData: InteractiveMessageData = {
        recipientPhone: '5511999999999',
        whatsappApiKey: 'test-api-key',
        header: {
          type: 'image',
      ...
      const result = await sendInteractiveMessage(imageHeaderData);
