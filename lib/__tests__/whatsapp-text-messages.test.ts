...
describe('WhatsApp Text Messages Service', () => {
  ...
  describe('sendTextMessage', () => {
    it('should send text message successfully', async () => {
      const textData: TextMessageData = {
      ...
      const result = await sendTextMessage(textData);
    ...
    it('should handle phone number formatting', async () => {
      ...
      for (const { input, expected } of testCases) {
        ...
        await sendTextMessage(textData);
  ...
  describe('Edge Cases', () => {
    it('should handle missing environment variables', async () => {
      ...
      expect(result.success).toBe(true);
      expect(mockedAxios.post).toHaveBeenCalledWith(
        'https://graph.facebook.com/v22.0//messages', // Empty phone number ID
        expect.any(Object),
        expect.any(Object)
      );
    });
