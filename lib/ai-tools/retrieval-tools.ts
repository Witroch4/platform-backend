...
export const RetrievalToolsSchema = {
  search_business_info: {
    ...
    function: {
      ...
      description: "Busca informações sobre o negócio/empresa nos documentos carregados pelo usuário (horários, endereço, serviços, políticas, etc.)",
      parameters: {
        type: "object",
        properties: {
          query: {
            type: "string",
            description: "Termo de busca sobre o negócio (ex: 'horário funcionamento', 'endereço', 'serviços', 'políticas')"
          },
          userId: {
            type: "string",
...
export async function searchBusinessInfo(query: string, userId: string, assistantId?: string): Promise<string> {
  ...
  try {
    ...
    matchingDocuments.slice(0, 3).forEach(doc => { // Limita a 3 resultados
      result += `📄 **${doc.title}**\n`;
      if (doc.sourceUrl) {
...
export async function searchIntents(query: string, accountId: string): Promise<string> {
  ...
  try {
    ...
    const matchingIntents = intents.filter(intent => {
      ...
      return searchableText.includes(searchTerms) || 
             searchTerms.split(' ').some(term => searchableText.includes(term));
    });
    ...
    matchingIntents.slice(0, 3).forEach(intent => { // Limita a 3 resultados
    ...
    matchingDocuments.slice(0, 3).forEach(doc => {
      ...
      if (doc.contentText) {
        ...
        if (bestParagraph) {
          ...
          const excerpt = bestParagraph.length > 400 
            ? bestParagraph.substring(0, 400) + '...'
            : bestParagraph;
          result += `💡 ${excerpt}\n\n`;
        }
      }
...
export async function executeRetrievalTool(
  ...
  switch (toolName) {
    ...
    case "search_intents":
      return await searchIntents(
        parameters.query, 
        parameters.accountId || context.accountId || ''
      );
...
export function getAvailableTools(context: { hasBusinessInfo?: boolean; hasIntents?: boolean; hasDocuments?: boolean }) {
  const tools = [];
