...
export class InstagramTemplateCache {
	...
	async getTemplateMapping(
		...
		const logContext = createCacheLogContext(usuarioChatwitId, inboxId, intentName, "getTemplateMapping");
		logCacheKeyGeneration(
			logContext,
			cacheKeyComponents,
		...
		try {
			...
			if (cached) {
				...
				this.redis.setex(key, TTL.TEMPLATE_MAPPING, JSON.stringify(cachedData)).catch(() => {});
				...
				this.stats.misses++;
				logCacheMiss({ ...logContext, cacheKey: key }, { latency }, "Key not found in cache");
				return null;
			}
		} catch (error) {
			this.stats.errors++;
	...
	async setTemplateMapping(
		...
		// Log cache key generation for debugging
		const logContext = createCacheLogContext(usuarioChatwitId, inboxId, intentName, "setTemplateMapping");
		logCacheKeyGeneration(
			logContext,
			cacheKeyComponents,
			"chatwit:instagram_template_mapping:intentName:usuarioChatwitId:inboxId",
	...
	async getConversionResult(
		...
		// Log cache key generation for debugging
		const logContext = createCacheLogContext(usuarioChatwitId, inboxId, intentName, "getConversionResult");
		logCacheKeyGeneration(
			logContext,
			cacheKeyComponents,
		...
		try {
			...
			if (cached) {
				...
				this.stats.misses++;
				logCacheMiss({ ...logContext, cacheKey: key }, { latency }, "Key not found in cache", { bodyLength, hasImage });
				return null;
			}
		} catch (error) {
			this.stats.errors++;
	...
	async setConversionResult(
		...
		// Log cache key generation for debugging
		const logContext = createCacheLogContext(usuarioChatwitId, inboxId, intentName, "setConversionResult");
		logCacheKeyGeneration(
			logContext,
			cacheKeyComponents,
			"chatwit:instagram_conversion_result:intentName:usuarioChatwitId:inboxId:bodyLength:hasImage[:u:uniqueUserKey]",
	...
	async recordQueryPerformance(queryType: string, executionTime: number, isSlowQuery: boolean = false): Promise<void> {
	...
	async getQueryPerformanceMetrics(queryType?: string): Promise<QueryPerformanceMetrics[]> {
		try {
			if (queryType) {
				const key = this.getCacheKey(CACHE_PREFIXES.QUERY_PERFORMANCE, queryType);
				const cached = await this.redis.get(key);
				...
				return values.filter((value: any) => value !== null).map((value: any) => JSON.parse(value!));
			}
		} catch (error) {
	...
	async invalidateTemplateMapping(intentName: string, usuarioChatwitId: string, inboxId: string): Promise<void> {
		const cacheKeyComponents = { intentName, usuarioChatwitId, inboxId };
		...
		// Log cache key generation for debugging
		const logContext = createCacheLogContext(usuarioChatwitId, inboxId, intentName, "invalidateTemplateMapping");
		logCacheKeyGeneration(
			logContext,
			cacheKeyComponents,
			"chatwit:instagram_template_mapping:intentName:usuarioChatwitId:inboxId",
		...
		try {
			...
			if (keysToDelete.length > 0) {
				...
				logCacheInvalidation(logContext, keysToDelete, "Template mapping invalidation", {
	...
	async batchGetTemplateMappings(
		...
		const keys = requests.map((req) =>
			this.getCacheKey(CACHE_PREFIXES.TEMPLATE_MAPPING, `${req.intentName}:${req.inboxId}`),
		);
...
export async function getCachedTemplateMapping(
	...
	return instagramTemplateCache.getTemplateMapping(intentName, usuarioChatwitId, inboxId);
}
...
export async function setCachedTemplateMapping(
	...
	return instagramTemplateCache.setTemplateMapping(intentName, usuarioChatwitId, inboxId, mapping, ttl);
}
...
export async function invalidateTemplateMappingCache(
	...
	return instagramTemplateCache.invalidateTemplateMapping(intentName, usuarioChatwitId, inboxId);
}
...
export async function recordQueryPerformance(
	...
	return instagramTemplateCache.recordQueryPerformance(queryType, executionTime, isSlowQuery);
}
