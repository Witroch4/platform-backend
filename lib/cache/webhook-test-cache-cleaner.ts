...
export class WebhookTestCacheCleaner {
	...
	async clearAllCaches(): Promise<CacheClearResult> {
		...
		try {
			...
			console.log(
				`[WebhookTestCacheCleaner] Successfully cleared ${result.clearedCaches.length} cache types, ${result.totalKeysCleared} total keys`,
			);
		} catch (error) {
	...
	private async clearTestCache(): Promise<number> {
		try {
			const patterns = ["test:*", "chatwit:test:*", "*:test:*", "webhook-test:*"];
