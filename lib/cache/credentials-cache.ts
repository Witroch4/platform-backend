...
export class CredentialsCache {
	...
	async invalidateCredentials(inboxId: string, options?: { suppressQueue?: boolean }): Promise<void> {
		const keys = [
				...
				try {
					...
					if (result.credentials) {
						const creds: WhatsAppCredentials = {
							whatsappApiKey: result.credentials.whatsappApiKey,
							phoneNumberId: result.credentials.phoneNumberId,
							businessId: result.credentials.whatsappBusinessAccountId,
							inboxId: result.credentials.resolvedFromInboxId ?? inboxId,
							source: result.credentials.source === "inbox" ? "inbox" 
								: result.credentials.source === "fallback_inbox" ? "fallback" 
								: "global",
							updatedAt: new Date(),
						};
						await credentialsCache.setCredentials(inboxId, creds);
					}
				} catch (error) {
...
export class CacheHealthMonitor {
	...
	getPerformanceStats(): {
		...
		const hitRate =
			this.healthStats.totalRequests > 0 ? (this.healthStats.cacheHits / this.healthStats.totalRequests) * 100 : 0;

		const errorRate =
			this.healthStats.totalRequests > 0 ? (this.healthStats.errors / this.healthStats.totalRequests) * 100 : 0;
