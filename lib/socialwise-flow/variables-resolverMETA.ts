...
export class METAVariablesResolver {
	...
	async initialize(): Promise<void> {
		if (this.userId) {
			console.log(`[META Variables] Already initialized with userId: ${this.userId}`);
			return;
		}

		if (!this.context.inboxId) {
			console.warn("[META Variables] No inboxId provided, cannot resolve user-specific variables");
			return;
		}
		...
		try {
			...
			if (this.userId) {
				console.log(`[META Variables] Initialized with userId: ${this.userId} from inboxId: ${this.context.inboxId}`);
			} else {
				console.warn(`[META Variables] No userId found for inboxId: ${this.context.inboxId}`);
			}
		} catch (error) {
			console.error(`[META Variables] Error getting userId from inboxId ${this.context.inboxId}:`, error);
		}
	}
	...
	async resolveText(text: string): Promise<string> {
		...
		console.log(`[META Variables] Resolving text: "${text.substring(0, 100)}${text.length > 100 ? "..." : ""}"`);
		...
		try {
			resolvedText = resolveTextWithVariables(
				resolvedText,
				{},
				{
		...
		if (this.userId) {
			try {
				...
				if (originalText !== resolvedText) {
					console.log(`[META Variables] MTF variables (including lote_ativo) resolved for user ${this.userId}`);
					console.log(
					...
					console.log(`[META Variables] No MTF variables found in text for user ${this.userId}`);
				}
			} catch (error) {
				console.error(`[META Variables] Error resolving MTF variables for user ${this.userId}:`, error);
			}
		} else {
			console.log("[META Variables] No userId available, skipping MTF variable resolution");
		}
	...
	private resolveSystemVariables(text: string): string {
		...
		for (const [placeholder, value] of Object.entries(systemVariables)) {
			if (resolvedText.includes(placeholder)) {
				...
				resolvedText = resolvedText.replace(new RegExp(placeholder.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "g"), value);
			}
		}
	...
	async resolveInteractiveMessage(data: any): Promise<any> {
		...
		if (processedData.header?.type === "text" && processedData.header.content) {
			...
			processedData.header.content = await this.resolveText(processedData.header.content);
		}
		...
		if (processedData.footer?.text) {
			...
			processedData.footer.text = await this.resolveText(processedData.footer.text);
		}

		// Resolve variables in action.buttons (builder-style shape)
		if (processedData.action?.buttons) {
			console.log(`[META Variables] Resolving ${processedData.action.buttons.length} button titles (action.buttons)`);
			for (let i = 0; i < processedData.action.buttons.length; i++) {
				const button = processedData.action.buttons[i];
				if (button.title) {
					console.log(`[META Variables] Resolving button ${i + 1} title: "${button.title}"`);
		...
		if (processedData.action?.sections) {
			console.log(`[META Variables] Resolving ${processedData.action.sections.length} list sections`);
			for (let i = 0; i < processedData.action.sections.length; i++) {
				const section = processedData.action.sections[i];
				if (section.title) {
					console.log(`[META Variables] Resolving section ${i + 1} title: "${section.title}"`);
				...
				if (section.rows) {
					for (let j = 0; j < section.rows.length; j++) {
						...
						if (row.title) {
							console.log(`[META Variables] Resolving row ${j + 1} title: "${row.title}"`);
							row.title = await this.resolveText(row.title);
						}
						if (row.description) {
							console.log(`[META Variables] Resolving row ${j + 1} description: "${row.description}"`);
		...
		if (processedData.actionLocationRequest?.requestText) {
			processedData.actionLocationRequest.requestText = await this.resolveText(
				processedData.actionLocationRequest.requestText,
			);
		}

		console.log("[META Variables] Interactive message variables resolved successfully");
	...
	async resolveTemplateComponents(components: any[]): Promise<any[]> {
		...
		console.log(`[META Variables] Resolving ${components.length} template components`);
		...
		const resolvedComponents = await Promise.all(
			components.map(async (component, index) => {
				...
				if (component.type === "BODY" && component.text) {
					console.log(`[META Variables] Resolving BODY component ${index + 1}: "${component.text}"`);
					processedComponent.text = await this.resolveText(component.text);
				}

				if (component.type === "HEADER" && component.text) {
					console.log(`[META Variables] Resolving HEADER component ${index + 1}: "${component.text}"`);
					processedComponent.text = await this.resolveText(component.text);
				}

				if (component.type === "FOOTER" && component.text) {
					console.log(`[META Variables] Resolving FOOTER component ${index + 1}: "${component.text}"`);
					processedComponent.text = await this.resolveText(component.text);
				}
		...
		console.log("[META Variables] Template components resolved successfully");
	 ...
	 */
	static fromUserId(userId: string, context: Partial<METAVariableContext> = {}): METAVariablesResolver {
