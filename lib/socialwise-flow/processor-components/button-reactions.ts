...
export async function processButtonReaction(context: ProcessorContext): Promise<ButtonReactionMeta | undefined> {
	...
	if (isWhatsAppChannel(context.channelType)) {
		...
		isButton = !!(payload.context?.message?.content_attributes?.button_reply?.id || payload.button_id);
		...
		isButton =
			!!(payload.context?.message?.content_attributes?.postback_payload || payload.postback_payload) &&
	...
	const replyToMessageId = payload.context?.message?.source_id || payload.context?.wamid || payload.wamid;
