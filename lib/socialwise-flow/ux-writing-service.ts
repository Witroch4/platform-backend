...
import { IntentCandidate, WarmupButtonsResponse, AgentConfig, IOpenAIService } from "@/services/openai";
import {
...
import { buildChannelResponse, buildFallbackResponse, ButtonOption } from "./channel-formatting";
...
export class UXWritingService {
	...
	async generateWarmupButtons(
		...
		try {
			...
			const result = await this.openaiService.generateWarmupButtons(userText, candidates, agent);
	 ...
	 */
	async generateShortTitlesBatch(intents: IntentCandidate[], agent: AgentConfig): Promise<string[] | null> {
		if (!intents.length) return [];

		try {
			// Use the OpenAI service method
			const result = await this.openaiService.generateShortTitlesBatch(intents, agent);
	 ...
	 */
	async generateDomainTopics(userText: string, agent: AgentConfig): Promise<WarmupButtonsResponse> {
		try {
			...
			return this.generateFallbackDomainTopics(legalContext.primaryArea || undefined);
		} catch (error) {
	 ...
	 */
	private generateFallbackWarmupButtons(userText: string, candidates?: IntentCandidate[]): WarmupButtonsResponse {
		const legalContext = analyzeLegalContext(userText);
	 ...
	 */
	private generateFallbackDomainTopics(primaryArea?: string): WarmupButtonsResponse {
		...
		const buttons = domainButtons[primaryArea || "default"] || domainButtons.default;
 ...
 */
export function createUXWritingService(openaiService: IOpenAIService): UXWritingService {
