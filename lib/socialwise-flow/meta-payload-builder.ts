...
export class METAPayloadBuilder {
	...
	public async buildInteractiveMessagePayload(dbInteractiveContent: any): Promise<any> {
		...
		if (!dbInteractiveContent || !dbInteractiveContent.body?.text) {
			throw new Error("Conteúdo interativo inválido: o corpo da mensagem é obrigatório.");
		}
		...
		if (this.variablesResolver) {
			...
			console.log(
				"[META PayloadBuilder] Variables resolved, processed data:",
		...
		const detectInteractiveType = (): string | undefined => {
			try {
				...
				const hasQR =
					Array.isArray(buttons) && buttons.some((b: any) => String(b?.content_type || "").toLowerCase() === "text");
				...
				const hasCarousel =
					...
					(processedContent?.actionReplyButton?.elements &&
						Array.isArray(processedContent.actionReplyButton.elements)) ||
		...
		if (processedContent.actionCarousel) {
			...
			interactivePayload.action = this._buildCarouselAction(processedContent.actionCarousel);
		} else if (
			...
			interactivePayload.action = this._buildButtonAction(processedContent.actionReplyButton);
		} else if (processedContent.actionList) {
			interactivePayload.type = "list";
			interactivePayload.action = this._buildListAction(processedContent.actionList);
		} else if (processedContent.actionCtaUrl) {
			interactivePayload.type = "cta_url";
			interactivePayload.action = this._buildCtaUrlAction(processedContent.actionCtaUrl);
		} else if (processedContent.actionFlow) {
			interactivePayload.type = "flow";
			interactivePayload.action = this._buildFlowAction(processedContent.actionFlow);
				...
				buttons: [
					{
						...
						reply: {
							id: "send_location",
							title: dbInteractiveContent.actionLocationRequest.requestText || "Enviar Localização",
						},
					},
				],
			};
		} else {
			throw new Error("Ação interativa não reconhecida ou ausente no 'interactiveContent'.");
		}
	...
	public async buildTemplatePayload(
		...
		const metaComponents = await this._transformOfficialTemplateComponents(metaTemplateId, components);
	...
	private _buildHeader(dbHeader: any): any {
		...
		switch (dbHeader.type) {
			...
			case "video":
			case "document":
				if (!dbHeader.content) throw new Error(`Header de ${dbHeader.type} requer o campo 'content' com a URL.`);
	...
	private _buildButtonAction(dbAction: any): any {
		if (!dbAction.buttons || !Array.isArray(dbAction.buttons) || dbAction.buttons.length === 0) {
			throw new Error("Ação de botão requer um array de 'buttons' no payload JSON.");
		}
	...
	private _buildListAction(dbAction: any): any {
		if (
			...
			throw new Error("Ação de lista requer 'buttonText' e um array de 'sections'.");
		}
		return {
	...
	private _buildCarouselAction(dbAction: any): any {
		if (!dbAction.elements || !Array.isArray(dbAction.elements) || dbAction.elements.length === 0) {
			throw new Error("Ação de carrossel requer um array de 'elements' no payload JSON.");
		}
			...
			elements: dbAction.elements.slice(0, 10).map((element: any) => {
				...
				if (element.default_action?.url) {
					mappedElement.default_action = {
						...
						...(element.default_action.messenger_extensions && {
							messenger_extensions: element.default_action.messenger_extensions,
						}),
						...(element.default_action.webview_height_ratio && {
	...
	private async _transformOfficialTemplateComponents(
		...
		try {
			...
			// HEADER
			const headerIndex = resolvedList.findIndex((c) => String(c?.type || "").toUpperCase() === "HEADER");
			if (headerIndex >= 0) {
			...
			// BUTTONS
			const buttonsComp = resolvedList.find((c) => String(c?.type || "").toUpperCase() === "BUTTONS");
			if (buttonsComp && Array.isArray(buttonsComp.buttons)) {
				...
				for (const btn of buttons) {
					...
					if (btnType === "COPY_CODE") {
						...
						const couponCode =
							(btn.coupon_code && String(btn.coupon_code)) || (Array.isArray(btn.example) && btn.example[0]) || "";
						resultComponents.push({
							type: "button",
							sub_type: "copy_code",
