...
const CriticalConversationSchema = z
	.object({
...
export const SocialWiseFlowPayloadSchema = z
	.object({
		session_id: z
			.union([z.string(), z.number()])
			.transform((val) => String(val))
			.refine((val) => val.length > 0, "Session ID is required"),
...
export const BearerTokenSchema = z.string().min(32, "Bearer token too short").max(256, "Bearer token too long");
...
export function sanitizeUserText(text: string): {
	...
	const sanitized = result.data.replace(/\s+/g, " ").trim().slice(0, 4096); // Hard limit
