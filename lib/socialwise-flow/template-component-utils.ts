...
export function normalizeComponentsToArray(raw: any): any[] {
	...
	if (raw && typeof raw === "object") {
		...
		if (numericKeys.length > 0) {
			return numericKeys.sort((a, b) => Number(a) - Number(b)).map((key) => raw[key]);
		}
	}
	return [];
}
...
export function applyCustomVariablesToComponents(
	...
	const processed = list.map((component) => {
		...
		if (component?.type === "BODY" && typeof component.text === "string") {
			const params = buildTemplateParameters(component.text, customVariables, component.example);
			if (params.length > 0) {
				processedComponent.__resolvedBodyParams = params.map((param) => ({ ...param }));
			}
			processedComponent.text = replaceVariablesInTemplateText(
				component.text,
				Array.isArray(component.example?.body_text?.[0]) ? component.example.body_text[0] : [],
				customVariables,
				contactPhone,
			);
		}
		...
		if (component?.type === "HEADER" && component.format === "TEXT" && typeof component.text === "string") {
			const params = buildTemplateParameters(component.text, customVariables, component.example, true);
			if (params.length > 0) {
				processedComponent.__resolvedHeaderParams = params.map((param) => ({ ...param }));
			}
			processedComponent.text = replaceVariablesInTemplateText(
				component.text,
				Array.isArray(component.example?.header_text?.[0]) ? component.example.header_text[0] : [],
				customVariables,
				contactPhone,
			);
		}
		...
		if (component?.type === "BUTTONS" && Array.isArray(component.buttons)) {
			processedComponent.buttons = component.buttons.map((button: any) => {
				...
				if (String(clonedButton?.type || "").toUpperCase() === "COPY_CODE" && customVariables?.coupon_code) {
...
function buildTemplateParameters(
	...
	if (example) {
		const namedArray = (example?.body_text_named_params as any[]) || (example?.header_text_named_params as any[]) || [];
	...
	const positionalExamples: string[] =
		(example?.body_text?.[0] as string[]) || (example?.header_text?.[0] as string[]) || [];
