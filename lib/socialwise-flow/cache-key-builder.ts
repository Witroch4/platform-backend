...
export class SocialWiseCacheKeyBuilder {
	...
	constructor() {
		...
		this.hmacSecret =
			process.env.SOCIALWISE_CACHE_HMAC_SECRET || process.env.NEXTAUTH_SECRET || "fallback-secret-for-development-only";

		// Skip validation during build process or when NEXTAUTH_SECRET is available
		const isBuildProcess =
			process.env.NEXT_PHASE === "phase-production-build" || process.env.CI === "true" || !process.env.DATABASE_URL;

		if (
			this.hmacSecret === "fallback-secret-for-development-only" &&
	 ...
	 */
	buildWarmupKey(config: CacheKeyConfig, userText: string, candidates: Array<{ slug: string; desc?: string }>): string {
		const namespace = this.buildNamespace(config);
	 ...
	 */
	buildConfirmationKey(config: CacheKeyConfig, userText: string, intentSlug: string): string {
		const namespace = this.buildNamespace(config);
	...
	getTTL(keyType: CacheKeyType): number {
		switch (keyType) {
			case "classify":
				return CACHE_TTL.CLASSIFY;
			case "warmup":
				return CACHE_TTL.WARMUP;
			case "stitle":
				return CACHE_TTL.STITLE;
			case "confirm":
				return CACHE_TTL.CONFIRM;
			case "emb":
				return CACHE_TTL.EMBEDDING;
			case "idem":
				return CACHE_TTL.IDEMPOTENCY;
			case "nonce":
				return CACHE_TTL.NONCE;
			case "rate":
				return CACHE_TTL.RATE_LIMIT;
			case "health":
	 ...
	 */
	buildKeyWithTTL(keyType: CacheKeyType, config: CacheKeyConfig, ...args: any[]): { key: string; ttl: number } {
		let key: string;
