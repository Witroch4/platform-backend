...
import { clampBody as clampBodyCentralized, CHANNEL_LIMITS } from "@/lib/socialwise/clamps";
 ...
 */
export function buildWhatsAppButtons(bodyText: string, buttons: ButtonOption[]): WhatsAppMessage {
 ...
 */
export function buildInstagramButtons(text: string, buttons: ButtonOption[]): InstagramMessage {
 ...
 */
export function buildChannelResponse(channelType: string, text: string, buttons?: ButtonOption[]): ChannelResponse {
	const lowerChannelType = (channelType || "").toLowerCase();

	// Palavras-chave para handoff - mas NÃO se há botões válidos
	const lowerText = (text || "").toLowerCase();
	const hasHandoffKeywords =
		lowerText.includes("atendente") || lowerText.includes("humano") || lowerText.includes("falar");
	...
	if (hasHandoffKeywords && (!buttons || buttons.length === 0)) {
		formattingLogger.info("🚨 HANDOFF AUTOMÁTICO DETECTADO", {
			originalText: text,
			triggerWord: lowerText.includes("atendente") ? "atendente" : lowerText.includes("humano") ? "humano" : "falar",
...
export function buildDefaultLegalTopics(channelType: string): ChannelResponse {
	...
	return buildChannelResponse(channelType, "Posso ajudar com qual área do direito?", defaultButtons);
}

/**
 * Degradação graciosa quando a LLM falha ou userId não está disponível
 * Indica claramente que o sistema está indisponível
 */
export function buildFallbackResponse(channelType: string, userText: string): ChannelResponse {
	const fallbackButtons: ButtonOption[] = [
		{ title: "Falar com atendente", payload: "@falar_atendente" },
		{ title: "Recomeçar", payload: "@recomecar" },
		{ title: "Sair", payload: "@sair" },
	];

	return buildChannelResponse(
		channelType,
		"⚠️ Sistema indisponível no momento. Tente novamente ou escolha uma opção:",
		fallbackButtons,
	);
}
