...
export class SocialWiseFlowCacheManager {
	 ...
	 */
	async getClassificationResult(config: CacheKeyConfig, userText: string): Promise<ClassificationResult | null> {
		try {
	 ...
	 * Short Titles Cache
	 */
	async getShortTitle(config: CacheKeyConfig, intentSlug: string): Promise<string | null> {
		try {
	...
	async setShortTitle(config: CacheKeyConfig, intentSlug: string, title: string, customTTL?: number): Promise<void> {
		try {
			const key = this.keyBuilder.buildShortTitleKey(config, intentSlug);
	 ...
	 */
	async batchGetShortTitles(config: CacheKeyConfig, intentSlugs: string[]): Promise<Map<string, string | null>> {
		const results = new Map<string, string | null>();

		try {
			const keys = intentSlugs.map((slug) => this.keyBuilder.buildShortTitleKey(config, slug));
	...
	async batchSetShortTitles(config: CacheKeyConfig, titleMap: Map<string, string>, customTTL?: number): Promise<void> {
	 ...
	 */
	async getMicrocopy(config: CacheKeyConfig, userText: string, intentSlug: string): Promise<MicrocopyResult | null> {
		try {
	 ...
	 * Embeddings Cache
	 */
	async getEmbedding(config: CacheKeyConfig, text: string): Promise<EmbeddingResult | null> {
		try {
	...
	async setEmbedding(config: CacheKeyConfig, text: string, result: EmbeddingResult, customTTL?: number): Promise<void> {
		try {
			const key = this.keyBuilder.buildEmbeddingKey(config, text);
	 ...
	 */
	async checkIdempotency(config: CacheKeyConfig, wamid: string): Promise<boolean> {
		try {
	...
	async setIdempotency(config: CacheKeyConfig, wamid: string, customTTL?: number): Promise<void> {
		try {
			const key = this.keyBuilder.buildIdempotencyKey(config, wamid);
	 ...
	 */
	async checkNonce(config: CacheKeyConfig, nonce: string): Promise<boolean> {
		try {
	...
	async setNonce(config: CacheKeyConfig, nonce: string, customTTL?: number): Promise<void> {
		try {
			const key = this.keyBuilder.buildNonceKey(config, nonce);
	 ...
	 * Cache Invalidation
	 */
	async invalidateUserCache(config: CacheKeyConfig, userText?: string): Promise<void> {
		try {
	...
	async invalidateIntentCache(config: CacheKeyConfig, intentSlug: string): Promise<void> {
		try {
...
export async function checkMessageIdempotency(config: CacheKeyConfig, wamid: string): Promise<boolean> {
	return socialWiseFlowCache.checkIdempotency(config, wamid);
}

export async function setMessageIdempotency(config: CacheKeyConfig, wamid: string): Promise<void> {
	return socialWiseFlowCache.setIdempotency(config, wamid);
}

export async function checkAntiReplayNonce(config: CacheKeyConfig, nonce: string): Promise<boolean> {
	return socialWiseFlowCache.checkNonce(config, nonce);
}

export async function setAntiReplayNonce(config: CacheKeyConfig, nonce: string): Promise<void> {
