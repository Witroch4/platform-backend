...
export class SocialWiseMonitoringDashboard {
	 ...
	 */
	recordQualitySample(
		sample: Omit<QualitySample, "user_input_hash" | "timestamp"> & {
			user_input: string;
		},
	): void {
		...
		const userInputHash = crypto.createHash("sha256").update(sample.user_input).digest("hex").substring(0, 16);
		...
		try {
			...
			const hardAccuracy =
				hardSamples.length > 0
					? (hardSamples.filter((s) => s.user_satisfaction_score && s.user_satisfaction_score >= 4).length /
							hardSamples.length) *
						100
					: 0;
			...
			const softCtr =
				softSamples.length > 0
					? (softSamples.filter((s) => s.generated_buttons && s.generated_buttons.length > 0).length /
							softSamples.length) *
						100
					: 0;
	...
	async getDashboardMetrics(): Promise<DashboardMetrics> {
		try {
			...
			const [classificationRates, errorRates, performancePercentiles, embeddingHealth, llmHealth, qualityMetrics] =
			...
			const embeddingStatus = embeddingHealth.status === "fulfilled" ? embeddingHealth.value.status : "unavailable";
			const llmStatus = llmHealth.status === "fulfilled" ? llmHealth.value.status : "unavailable";
			...
			return {
				currentLatency: {
					...
					low: 0,
					router: 0,
					overall_p95: performancePercentiles.status === "fulfilled" ? performancePercentiles.value.p95 : 0,
				},
				classificationRates:
					classificationRates.status === "fulfilled"
				...
				qualityMetrics:
					qualityMetrics.status === "fulfilled"
...
// Utility functions
export function recordSocialWiseQualitySample(
	sample: Omit<QualitySample, "user_input_hash" | "timestamp"> & {
		user_input: string;
	},
): void {
