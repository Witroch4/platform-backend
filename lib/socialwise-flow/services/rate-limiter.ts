...
export class SocialWiseRateLimiterService {
   ...
   */
  async checkSocialWiseRateLimit(
    context: SocialWiseRateLimitContext
  ...
  async checkEnhancedRateLimit(
    ...
    const inboxResult = await this.checkCustomScopeLimit(
      'inbox',
    ...
    const sessionResult = await this.checkCustomScopeLimit(
      'session',
   ...
   */
  async getSocialWiseRateLimitStatus(
    context: SocialWiseRateLimitContext
  ): Promise<{
   ...
   */
  async resetSocialWiseRateLimits(
    context: SocialWiseRateLimitContext
  ): Promise<void> {
    try {
      const keys = [
        `sw-rl:inbox:inbox:${context.inboxId}`,
        `sw-rl:session:session:${context.sessionId}`,
      ];
