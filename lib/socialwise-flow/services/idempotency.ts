...
export class SocialWiseIdempotencyService {
	...
	extractIdempotencyKey(payload: SocialWiseFlowPayloadType): SocialWiseIdempotencyKey {
		...
		const accountId = String(socialwiseContext?.account_data?.id || payload.context.inbox?.account_id || 0);
		const inboxId = String(socialwiseContext?.inbox_data?.id || payload.context.inbox?.id || 0);
