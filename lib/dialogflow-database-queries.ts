...
export interface CompleteMessageMapping {
	...
	messageType: "template" | "interactive" | "unified_template" | "enhanced_interactive";
...
export async function findCompleteMessageMappingByIntent(
	...
	try {
		...
		if (!mapping.template) {
			console.log(`[DB Query] MapeamentoIntencao found but no template associated (may have flowId): ${mapping.id}`);
			return null;
		}
 ...
 */
export async function validateInboxFallbackChain(externalInboxId: string): Promise<{
	isValid: boolean;
	issues: string[];
	chain: string[];
 ...
 */
export async function findActionByButtonId(buttonId: string): Promise<ButtonActionMapping | null> {
	try {
 ...
 */
export async function findReactionByButtonId(buttonId: string): Promise<ButtonReactionMapping | null> {
	try {
