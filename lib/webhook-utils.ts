 ...
 */
export function logWebhookError(error: Error, correlationId: string, context?: any): void {
	const errorData = {
		correlationId,
		timestamp: new Date().toISOString(),
		error: {
 ...
 */
export function createWebhookSuccessResponse(correlationId: string, data: any, processingTime?: number): Response {
...
export function sanitizeRespostaRapidaJobData(data: {
	...
	return {
		...
		personName:
			data.personName && typeof data.personName === "string" ? sanitizeTextContent(data.personName) : undefined,
	};
}
...
// [CLEANUP 2026-02-16] processWebhookWithFlashIntentFromUtils REMOVIDA
// Era usada apenas pelo webhook Dialogflow que foi removido
// O SocialWise Flow processa mensagens inline (síncrono)
...
// [CLEANUP 2026-02-16] processLegacyWebhookFallback REMOVIDA
// Era usada apenas pelo webhook Dialogflow que foi removido
