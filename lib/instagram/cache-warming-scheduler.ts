...
class CacheWarmingScheduler {
	...
	private async warmTemplate(template: TemplateAccessInfo): Promise<boolean> {
		try {
			...
			const warmingPromise = findOptimizedCompleteMessageMapping(template.intentName, template.inboxId);
			...
			console.error(
				`[CacheWarmingScheduler] Error warming template ${template.intentName}:${template.inboxId}:`,
				error,
			);
			return false;
		}
