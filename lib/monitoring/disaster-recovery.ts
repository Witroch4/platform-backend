						...
						try {
							...
							for (const key of queueKeys) {
								...
								if (waitingCount > 2000) {
									// Pausar filas com mais de 2000 jobs
		...
		for (const alert of activeAlerts) {
			...
			for (const procedure of this.procedures.values()) {
				if (
					...
					this.executeProcedure(procedure.id, `auto_trigger_${alert.id}`).catch((error) => {
			...
			for (let i = 0; i < procedure.steps.length; i++) {
				...
				try {
					...
					const result = await Promise.race([
						step.action(),
						new Promise<RecoveryResult>((_, reject) => setTimeout(() => reject(new Error("Timeout")), step.timeout)),
					]);
