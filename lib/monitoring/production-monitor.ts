...
class ProductionMonitor {
	...
	async performHealthChecks(): Promise<void> {
		const checks = [this.checkPrismaHealth(), this.checkRedisHealth(), this.checkSystemMetrics()];
	...
	private async backupRecentMetrics(): Promise<BackupStatus> {
		...
		try {
			...
			const metrics =
				(await this.prisma.queueMetrics?.findMany({
					where: {
						timestamp: {
							gte: yesterday,
						},
	...
	getMonitoringStatus() {
		...
		return {
			...
			connectionsHealth: connectionsHealth.reduce(
				(acc, conn) => {
