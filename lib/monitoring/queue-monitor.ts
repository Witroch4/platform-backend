...
export class QueueMonitor {
	...
	private recordJobCompletion(job: Job, queueName: string, status: "completed" | "failed", error?: Error): void {
		const now = new Date();
		const processingTime = job.finishedOn && job.processedOn ? job.finishedOn - job.processedOn : undefined;
		const waitTime = job.processedOn && job.timestamp ? job.processedOn - job.timestamp : undefined;
	...
	private extractCorrelationId(job: Job): string | undefined {
		try {
			...
			if (data && typeof data === "object") {
				...
				return data.correlationId || data.data?.correlationId || data.payload?.correlationId || undefined;
			}
		} catch (error) {
	...
	getQueuePerformanceStats(queueName: string, timeWindowMinutes: number = 60): QueuePerformanceStats | null {
		...
		const averageProcessingTime =
			jobsWithProcessingTime.length > 0
				? jobsWithProcessingTime.reduce((sum, job) => sum + (job.processingTime || 0), 0) /
	...
	getFailedJobs(queueName: string, limit: number = 50): JobMetrics[] {
		const history = this.jobMetricsHistory.get(queueName) || [];
		return history.filter((job) => job.status === "failed").slice(-limit);
	}
...
export async function initializeQueueMonitoring(): Promise<void> {
	// [CLEANUP 2026-02-16] Filas resposta-rapida e persistencia-credenciais REMOVIDAS
	// SocialWise Flow processa mensagens inline (síncrono), não usa mais BullMQ
	try {
		console.log("[QueueMonitor] Queue monitoring initialized (no legacy queues registered)");
	} catch (error) {
