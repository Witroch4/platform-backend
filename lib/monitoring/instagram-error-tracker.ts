...
export class InstagramErrorTracker {
	...
	private determineSeverity(errorCode: string, category: ErrorCategory, retryCount: number): ErrorSeverity {
		...
		const criticalErrors = [InstagramTranslationErrorCodes.SYSTEM_ERROR, InstagramTranslationErrorCodes.DATABASE_ERROR];
	...
	private logError(
		...
		const logLevel = severity === ErrorSeverity.CRITICAL ? "error" : severity === ErrorSeverity.HIGH ? "error" : "warn";

		console[logLevel](
			`[Instagram Error Tracker] [${severity.toUpperCase()}] [${correlationId}] ${errorCode}: ${error.message}`,
			{
				category,
				severity,
				errorCode,
	...
	private updateErrorPattern(errorCode: string, category: ErrorCategory, correlationId: string, timestamp: Date): void {
	...
	async getErrorStatistics(hoursBack: number = 1): Promise<ErrorStatistics> {
		try {
			...
			// Get active patterns
			const activePatterns = Array.from(this.errorPatterns.values()).filter(
				(p) => p.isActive && p.lastSeen.getTime() > windowStart,
			);
	...
	private async getRecentErrors(hoursBack: number): Promise<ErrorTrackingEntry[]> {
		...
		if (hoursBack > 1) {
			try {
				...
				const errorBatches = await Promise.all(recentKeys.map((key: any) => this.redis.get(key)));
