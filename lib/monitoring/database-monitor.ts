...
export class DatabaseMonitor {
					...
					return new Proxy(originalMethod as Record<string, any>, {
						get: (modelTarget: Record<string, any>, modelProp: string) => {
							...
							if (typeof modelMethod === "function" && this.isDatabaseOperation(modelProp as string)) {
								return this.wrapDatabaseOperation(modelMethod.bind(modelTarget), prop as string, modelProp as string);
							}
	...
	getSlowQueryAlerts(): SlowQueryAlert[] {
		return Array.from(this.slowQueries.values()).sort((a, b) => b.averageExecutionTime - a.averageExecutionTime);
	}
	...
	getFailedQueries(limit: number = 50): DatabaseQueryMetrics[] {
		return this.queryMetrics.filter((query) => !query.success).slice(-limit);
	}
	...
	getDatabaseDashboard(): {
		...
		const connectionStatus =
			this.connectionMetrics.length > 0 ? this.connectionMetrics[this.connectionMetrics.length - 1] : null;
