...
export class ApplicationPerformanceMonitor {
	...
	private async checkSocialWiseLatencyP95(): Promise<void> {
		...
		if (recentSocialWiseMetrics.length >= MIN_SAMPLES) {
			// Calculate P95
			const responseTimes = recentSocialWiseMetrics.map((m) => m.responseTime).sort((a, b) => a - b);
	...
	private evictOldMetrics(now: number): void {
		...
		this.metricsBuffer.worker = this.metricsBuffer.worker.filter(
			(m) => now - m.timestamp.getTime() <= EVICTION_WINDOW_MS,
		);
		this.metricsBuffer.database = this.metricsBuffer.database.filter(
			(m) => now - m.timestamp.getTime() <= EVICTION_WINDOW_MS,
		);
		this.metricsBuffer.cache = this.metricsBuffer.cache.filter(
			(m) => now - m.timestamp.getTime() <= EVICTION_WINDOW_MS,
		);
	}
	...
	private async checkQueueDepths(): Promise<void> {
		...
		// TODO: Adicionar monitoramento de novas filas aqui quando necessário
		try {
			// No queues to monitor currently
		} catch (error) {
	...
	async getPerformanceSummary(): Promise<{
		...
		const databaseStats = {
			avgQueryTime:
				recentDatabaseMetrics.length > 0
