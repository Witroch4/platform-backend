...
import { databaseMonitor, createMonitoredPrismaClient } from "./database-monitor";
...
function setupGracefulShutdown(): void {
	const shutdownHandler = async (signal: string) => {
		...
		try {
			...
			await Promise.all([apm.shutdown(), queueMonitor.shutdown(), databaseMonitor.shutdown()]);
...
export async function performHealthCheck(): Promise<{
	...
	components: Record<string, { status: string; message?: string; latency?: number }>;
	timestamp: string;
}> {
	const healthCheck = {
		status: "healthy" as "healthy" | "degraded" | "unhealthy",
		components: {} as Record<string, { status: string; message?: string; latency?: number }>,
	...
	try {
		...
		const costStatus =
			costHealth.status === "HEALTHY" ? "healthy" : costHealth.status === "DEGRADED" ? "degraded" : "unhealthy";
