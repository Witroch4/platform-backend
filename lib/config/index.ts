...
class ConfigManager {
  ...
  private applyEnvOverrides(yamlConfig: AppConfig): AppConfig {
    ...
    if (process.env.SOCIALWISE_CONCURRENCY_LIMIT) {
      config.socialwise_flow.concurrency.max_concurrent_llm_calls_per_inbox =
        parseInt(process.env.SOCIALWISE_CONCURRENCY_LIMIT, 10);
    }

    if (process.env.SOCIALWISE_GLOBAL_CONCURRENCY_LIMIT) {
      config.socialwise_flow.concurrency.max_concurrent_llm_calls_global =
        parseInt(process.env.SOCIALWISE_GLOBAL_CONCURRENCY_LIMIT, 10);
    }

    if (process.env.SOCIALWISE_QUEUE_TIMEOUT_MS) {
      config.socialwise_flow.concurrency.queue_timeout_ms =
        parseInt(process.env.SOCIALWISE_QUEUE_TIMEOUT_MS, 10);
    }

    if (process.env.SOCIALWISE_DEGRADATION_ENABLED !== undefined) {
      config.socialwise_flow.concurrency.degradation_enabled =
