...
export function sanitizeCouponCode(raw?: string): string {
	...
	if (!sanitized) throw new Error("Invalid coupon code - use 1-32 letters/numbers without spaces");
	return sanitized;
}
 ...
 */
function buildTemplateComponents(templateComponents: any[], data: TemplateMessageData): any[] {
	...
	for (const component of templateComponents) {
		switch (component.type) {
			...
			case "BODY":
				const placeholders = (component.text?.match(/\{\{(\d+)\}\}/g) || []).length;
				if (placeholders && data.bodyVars && data.bodyVars.length >= placeholders) {
					const parameters = data.bodyVars.slice(0, placeholders).map((v) => ({ type: "text", text: String(v) }));
...
export async function sendTemplateMessage(
	...
	try {
		...
		console.log(`[WhatsApp Messages] Template message sent successfully: ${result.messageId}`);
...
function buildInteractivePayload(data: InteractiveMessageData): any {
	...
	if (data.header) {
		basePayload.interactive.header = {
			...
			[data.header.type]: data.header.type === "text" ? data.header.content : { link: data.header.content },
		};
	}
 ...
 */
export async function sendInteractiveMessage(data: InteractiveMessageData): Promise<MessageResult> {
	try {
		...
		console.log(`[WhatsApp Messages] Interactive message sent successfully: ${result.messageId}`);
 ...
 */
export async function sendTextMessage(data: TextMessageData): Promise<MessageResult> {
	try {
		console.log(
		...
		console.log(`[WhatsApp Messages] Text message sent successfully: ${result.messageId}`);
