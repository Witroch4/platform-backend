...
class SseManager {
		...
		if (!this.connectionsByLead.has(leadId)) {
			...
			if (this.subscriber) {
				this.subscriber.subscribe(`sse:${leadId}`, (err: any, count: any) => {
					...
					console.log(
						`[SSE Redis] 📡 Inscrição no canal sse:${leadId} confirmada. Total de inscrições nesta instância: ${count}`,
					);
				});
			}
		}
	...
	public getConnectionsCount(): number {
		return Array.from(this.connectionsByLead.values()).reduce(
			(total, leadConnections) => total + leadConnections.size,
			0,
		);
	}
