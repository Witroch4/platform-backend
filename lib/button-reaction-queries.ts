 ...
 */
export async function getReactionByButtonId(buttonId: string, userId: string): Promise<ButtonReactionData | null> {
 ...
 */
export async function upsertButtonReaction(data: {
	buttonId: string;
	emoji?: string;
	textReaction?: string;
	action?: string;
 ...
 */
export async function deleteButtonReaction(buttonId: string, userId: string): Promise<boolean> {
	const prisma = getPrismaInstance();
 ...
 */
export function formatReactionData(reaction: ButtonReactionData, channelType: string, wamid: string): any {
...
export async function getIntentMappingByButtonId(
	...
	try {
		...
		const intentWhereClause: any = {
			intentName: searchCandidates.length ? { in: searchCandidates } : intentName,
			inbox: {
				usuarioChatwit: {
					appUserId: userId,
				},
