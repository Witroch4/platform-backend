...
export class CostMonitor {
  ...
  async collectMetrics(): Promise<CostMetrics> {
    try {
      ...
      const eventsSuccessful = completed.filter(
        (job) => job.timestamp && job.timestamp > windowStart.getTime()
      ).length;
      const eventsFailed = failed.filter(
        (job) => job.timestamp && job.timestamp > windowStart.getTime()
      ).length;

      // Calcular tempo médio de processamento
      const jobsWithProcessingTime = recentJobs.filter(
      ...
      const costByProvider = await this.getCostsByProvider(
        windowStart,
        timestamp
      );
      const costByModel = await this.getCostsByModel(windowStart, timestamp);
      const totalCostUSD = Object.values(costByProvider).reduce(
        (sum, cost) => sum + cost,
        0
      );

      const errorRate =
        eventsProcessed > 0 ? (eventsFailed / eventsProcessed) * 100 : 0;
   ...
   */
  private async getCostsByProvider(
    start: Date,
    end: Date
  ...
  private async checkMetricsAlerts(metrics: CostMetrics): Promise<void> {
    ...
    if (
      metrics.averageProcessingTime > COST_ALERT_THRESHOLDS.MAX_PROCESSING_TIME
    ) {
      await this.createAlert({
        type: "PROCESSING_DELAY",
        severity: "MEDIUM",
  ...
  private async createAlert(
    alertData: Omit<CostAlert, "id" | "timestamp" | "resolved">
  ): Promise<void> {
  ...
  getSystemHealth(): CostSystemHealth {
    ...
    const avgLatency =
      recentMetrics.reduce((sum, m) => sum + m.averageProcessingTime, 0) /
      recentMetrics.length;
    const avgEventsPerMinute =
      recentMetrics.reduce((sum, m) => sum + m.eventsProcessed, 0) /
      recentMetrics.length;
    ...
    if (avgErrorRate > 15 || latestMetrics.pendingEvents > 2000) {
      ...
      avgErrorRate > 5 ||
      latestMetrics.pendingEvents > 500 ||
      avgLatency > 30000
    ) {
      status = "DEGRADED";
    }
  ...
  getDashboard(): {
    ...
    const totalEventsToday = todayMetrics.reduce(
      (sum, m) => sum + m.eventsProcessed,
      0
    );
    const totalCostToday = todayMetrics.reduce(
      (sum, m) => sum + m.totalCostUSD,
      0
    );
    const errorRateToday =
      todayMetrics.length > 0
        ? todayMetrics.reduce((sum, m) => sum + m.errorRate, 0) /
          todayMetrics.length
        : 0;
    const averageProcessingTime =
      todayMetrics.length > 0
        ? todayMetrics.reduce((sum, m) => sum + m.averageProcessingTime, 0) /
          todayMetrics.length
        : 0;
