...
export async function whatsappWithCost(
  ...
  try {
    ...
    const budgetGuard = await guardWhatsAppOperation(
      args.meta?.inboxId,
      args.meta?.userId
    );
...
export async function whatsappMarketingWithCost(
  ...
  return whatsappWithCost(
    (name, phone) => sendFunction(name, phone, variables),
    { templateName, to, meta }
  );
}
...
export async function whatsappUtilityWithCost(
  ...
  return whatsappWithCost(
    (name, phone) => sendFunction(name, phone, data),
    { templateName, to, meta }
  );
}
...
export async function whatsappAuthWithCost(
  ...
  return whatsappWithCost(
    (name, phone) => sendFunction(name, phone, otp),
    { templateName, to, meta }
  );
}
