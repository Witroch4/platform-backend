...
export class EvaluationPipeline {
			...
			if (classification.band === "SOFT" || classification.band === "ROUTER") {
				try {
					...
					const compatibleClassification = {
						...
						strategy:
							classification.strategy.includes("_degraded") || classification.strategy.includes("_error")
	...
	private generateReport(results: EvaluationResult[], errors: string[]): EvaluationReport {
		...
		const qualityMetrics: QualityMetrics = {
			...
			overallClassificationAccuracy:
				successfulResults.length > 0 ? successfulResults.filter((r) => r.correct).length / successfulResults.length : 0,
			averageResponseTime,
			errorRate,
		};
	...
	private calculateSoftBandCTR(results: EvaluationResult[]): number {
		const softResults = results.filter(
			(r) => r.expectedBand === "SOFT" && r.generatedContent?.buttons && r.generatedContent.buttons.length > 0,
		);
	...
	private calculateLowBandValidTopics(results: EvaluationResult[]): number {
		const lowResults = results.filter((r) => r.expectedBand === "LOW" && r.generatedContent?.buttons);
	 ...
	 */
	private generateRecommendations(metrics: QualityMetrics, bandResults: any): string[] {
