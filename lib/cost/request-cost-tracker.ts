...
export class RequestCostTracker {
  ...
  async finalizeRequest(
    requestId: string,
    responseTimeMs: number
  ): Promise<RequestCostBreakdown | null> {
    const breakdown = this.requestCosts.get(requestId);
    if (!breakdown) {
    ...
    try {
      ...
      if (breakdown.userId && this.config.enableBudgetAlerts) {
        breakdown.budgetImpact = await this.calculateBudgetImpact(
          breakdown.userId,
  ...
  private async checkBudgetAlerts(
    userId: string,
    budgetImpact: RequestCostBreakdown['budgetImpact']
  ): Promise<void> {
  ...
  private async generateOptimizationRecommendations(
    ...
    if (breakdown.embeddingCost > breakdown.totalCost * 0.3) {
      recommendations.push({
        type: 'embedding',
        priority: 'medium',
        description: 'Embedding costs are high relative to total request cost. Consider caching embeddings or using smaller models.',
 ...
 */
export function createRequestCostTracker(
  overrides: Partial<RequestCostConfig> = {}
): RequestCostTracker {
