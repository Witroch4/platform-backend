...
import { budgetNotificationService, BudgetNotificationData } from "./notification-service";
...
export async function sendBudgetAlert(
	...
	try {
		...
		const result = await budgetNotificationService.sendBudgetNotification(budget, notificationData, notificationConfig);

		if (result.success) {
			console.log(`📧 Alerta de orçamento enviado: ${budget.name} (${alertType})`);
			return true;
		} else {
			console.warn(`⚠️ Falha ao enviar alerta de orçamento ${budget.name}:`, result.errors);
			return false;
		}
	} catch (error) {
...
export async function applyBudgetControls(budget: CostBudget): Promise<void> {
	try {
		...
		if (percentage >= BUDGET_CONTROLS_CONFIG.THRESHOLDS.CRITICAL_LIMIT) {
			...
			console.log(`⚠️ Controles parciais aplicados para orçamento ${budget.name}`);
		}
...
export async function removeBudgetControls(budget: CostBudget): Promise<void> {
	try {
		...
		if (budget.inboxId) {
			keys.push(`${BUDGET_CONTROLS_CONFIG.REDIS_PREFIXES.INBOX_BLOCKED}${budget.inboxId}`);
		}

		if (budget.userId) {
			keys.push(`${BUDGET_CONTROLS_CONFIG.REDIS_PREFIXES.USER_BLOCKED}${budget.userId}`);
		}
		...
		if (keys.length > 0) {
			...
			if (deleted > 0) {
				console.log(`✅ Controles removidos para orçamento ${budget.name} (${deleted} flags)`);
 ...
 */
export async function getDowngradedModel(originalModel: string, inboxId?: string, userId?: string): Promise<string> {
	try {
		// Verificar se há downgrade ativo para o contexto
		const keys: string[] = [];
		if (inboxId) keys.push(`${BUDGET_CONTROLS_CONFIG.REDIS_PREFIXES.MODEL_DOWNGRADE}inbox:${inboxId}`);
		if (userId) keys.push(`${BUDGET_CONTROLS_CONFIG.REDIS_PREFIXES.MODEL_DOWNGRADE}user:${userId}`);
		...
		for (const key of keys) {
			...
			if (downgrade) {
				...
				if (downgradedModel) {
					console.log(`📉 Modelo downgraded: ${originalModel} → ${downgradedModel}`);
 ...
 */
async function logBudgetAction(budget: CostBudget, action: string, metadata: any): Promise<void> {
	try {
