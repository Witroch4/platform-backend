 ...
 */
export async function openaiWithCost(client: OpenAI, args: OpenAIHookArgs): Promise<OpenAIResponse> {
	const started = Date.now();
	const costQueue = getCostQueue();

	try {
		// Verificar orçamento antes da operação
		const budgetGuard = await guardOpenAIOperation(args.model, args.meta?.inboxId, args.meta?.userId);

		if (!budgetGuard.allowed) {
			logBlockedOperation("OpenAI Chat Completion", budgetGuard.reason || "Orçamento excedido", {
		...
		const isChatCompletionsShape =
			...
			args.input.length > 0 &&
			args.input.every((m: any) => typeof m?.role === "string" && typeof m?.content === "string");
