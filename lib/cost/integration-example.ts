...
export async function processRequestWithCostTracking(
	...
	try {
		...
		await costTracker.trackEmbeddingCost(requestId, userText.length, "text-embedding-3-small");

		// Step 2: Simulate classification (in real implementation, this would call the actual classification)
		const classificationResult = {
			band: "SOFT" as const,
			score: 0.72,
			candidates: [{ slug: "consulta_juridica", name: "Consulta Jurídica", desc: "Consulta geral", score: 0.72 }],
...
function analyzeCostsFromEvaluation(
	...
	if (costByBand.SOFT && costByBand.SOFT > totalCost * 0.6) {
		recommendations.push(
			"SOFT band processing is expensive. Consider degrading to deterministic responses under load.",
		);
	}
...
export async function monitorBudgetAndQuality(
	...
	const dailyRequests = analytics.topCostlyRequests.filter((r) => r.timestamp >= startOfDay);
	const monthlyRequests = analytics.topCostlyRequests.filter((r) => r.timestamp >= startOfMonth);
	...
	if (analytics.averageCostPerRequest > 0.1) {
		qualityStatus.recommendedActions.push(
			"High average cost per request detected. Run quality evaluation to check for inefficiencies.",
		);
	}

	if (analytics.requestCount > 1000 && analytics.averageCostPerRequest > 0.05) {
		qualityStatus.recommendedActions.push(
			"High volume with elevated costs. Consider implementing more aggressive caching.",
		);
	}
