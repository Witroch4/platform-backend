...
export class CostErrorHandler {
	 ...
	 */
	private getErrorSeverity(errorType: CostErrorType, context: ErrorContext): "low" | "medium" | "high" | "critical" {
		...
		if (errorType === CostErrorType.NETWORK_ERROR || errorType === CostErrorType.TIMEOUT_ERROR) {
			return "low";
		}
	 ...
	 */
	private calculateRetryDelay(attemptsMade: number, errorType: CostErrorType): number {
		...
		const exponentialDelay = Math.min(baseDelay * Math.pow(multiplier, attemptsMade), maxDelay);
	 ...
	 */
	private async moveToDeadLetterQueue(errorLog: CostErrorLog, context: ErrorContext): Promise<void> {
		try {
	 ...
	 */
	private async incrementErrorMetrics(errorType: CostErrorType, severity: string): Promise<void> {
		try {
	...
	async getErrorStats(days: number = 7): Promise<{
		...
		try {
			...
			for (let i = 0; i < days; i++) {
				...
				const dayErrors = await this.redis.hgetall(`cost:errors:daily:${dateStr}`);
				...
				for (const [key, value] of Object.entries(dayErrors)) {
					...
					if (key.startsWith("severity:")) {
						...
						stats.errorsBySeverity[severity] = (stats.errorsBySeverity[severity] || 0) + count;
					} else if (Object.values(CostErrorType).includes(key as CostErrorType)) {
			...
			stats.topErrors = Object.entries(stats.errorsByType)
				.map(([type, count]) => ({
					...
					count,
					percentage: stats.totalErrors > 0 ? (count / stats.totalErrors) * 100 : 0,
				}))
