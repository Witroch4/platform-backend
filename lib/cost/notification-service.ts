...
import { sendBudgetAlertEmail } from "@/lib/mail/resend";
...
export class BudgetNotificationService {
  ...
  async sendBudgetNotification(
    ...
    if (lastNotification) {
      results.errors.push(
        `Notificação em cooldown (${cooldownMinutes} minutos)`
      );
      return results;
    }
    ...
    for (const channel of config.channels) {
      try {
        ...
        switch (channel) {
          case "email":
            channelSuccess = await this.sendEmailNotification(
              notificationData,
              config
            );
            break;
          case "webhook":
            channelSuccess = await this.sendWebhookNotification(
              notificationData,
              config
            );
            break;
          case "dashboard":
            channelSuccess =
              await this.sendDashboardNotification(notificationData);
            break;
        }
  ...
  private async sendEmailNotification(
    ...
    try {
      const recipients =
        config.emailRecipients || (await this.getDefaultEmailRecipients());

      if (recipients.length === 0) {
        console.warn(
          "⚠️ Nenhum destinatário de email configurado para alertas de orçamento"
        );
        return false;
      }
      ...
      const emailPromises = recipients.map((recipient) =>
        sendBudgetAlertEmail(
          ...
          data.limitUSD,
          data.percentage,
          data.type === "EXCEEDED" || data.type === "CRITICAL"
            ? "EXCEEDED"
            : "WARNING"
        )
      );
      ...
      console.log(
        `📧 Notificação de orçamento enviada por email para ${recipients.length} destinatários`
      );
      return true;
    } catch (error) {
  ...
  private async sendWebhookNotification(
    ...
    try {
      ...
      console.log(
        `🔗 Notificação de orçamento enviada via webhook: ${config.webhookUrl}`
      );
      return true;
    } catch (error) {
  ...
  async cleanupOldNotifications(olderThanDays: number = 7): Promise<number> {
    try {
      ...
      console.log(
        `🧹 Limpeza de notificações: ${deletedCount} notificações antigas removidas`
      );
      return deletedCount;
    } catch (error) {
...
export const budgetNotificationService =
  BudgetNotificationService.getInstance();
