...
export class PricingService {
  ...
  async resolveUnitPrice(
    ...
    try {
      ...
      const priceCard = await this.findBestPriceCard(
        provider,
        product,
        unit,
        when,
   ...
   */
  private async findBestPriceCard(
    provider: Provider,
    product: string,
    unit: Unit,
    when: Date,
...
export async function processPendingPricingEvents(limit: number = 100): Promise<{
  ...
  try {
    ...
    console.log(`Processamento de eventos pendentes: ${processed} processados, ${failed} falharam de ${pendingEvents.length} total`);
