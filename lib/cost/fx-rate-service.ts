...
export class FxRateService {
  private static readonly FALLBACK_RATE = 5.50; // último recurso
  ...
  private static async fromBacenPTAX(target?: Date): Promise<number> {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const data = await this.fetchJson(url);
      const values: Array<{ cotacaoCompra: number; cotacaoVenda: number; dataHoraCotacao: string; }> =
        data?.value ?? [];
   ...
   * Converte USD → BRL
   */
  static async convertUsdToBrl(usdAmount: number, date?: Date): Promise<{ brlAmount: number; rate: number; date: Date }> {
