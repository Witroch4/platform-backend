...
export async function checkAllBudgets(): Promise<{
  ...
  try {
    ...
    console.log(`✅ Verificação concluída: ${results.checked} orçamentos verificados, ${results.alerts} alertas, ${results.blocked} bloqueios`);
    return results;
...
async function checkBudgetStatus(budget: CostBudget): Promise<{
  ...
  console.log(`📊 Orçamento ${budget.name} (${budget.id}): ${(percentage * 100).toFixed(1)}% usado ($${currentSpending.toFixed(2)}/$${budget.limitUSD})`);
