...
export class ParallelProcessingManager {
	...
	private async processBatch<T>(
		...
		try {
			...
			const processingPromises = tasks.map((task) => this.processTaskWithTimeout(task, processor, config.timeoutMs));
	...
	private async processTaskWithTimeout<T>(
		...
		for (let attempt = 1; attempt <= this.MAX_RETRIES; attempt++) {
			try {
				...
				const result = await Promise.race([processor(task), timeoutPromise]);
