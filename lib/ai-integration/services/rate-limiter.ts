...
export class RateLimiterService {
  ...
  async checkRateLimit(
    ...
    const conversationResult = await this.checkScopeLimit(
      'conversation',
      conversationId,
    ...
    const contactResult = await this.checkScopeLimit(
      'contact',
      contactId,
      this.config.contact
    );
    ...
    const accountResult = await this.checkScopeLimit(
      'account',
      accountId,
      this.config.account
    );
    ...
    if (clientIp) {
      const ipResult = await this.checkScopeLimit(
        'ip',
        clientIp,
        { limit: 60, window: 10 }
      );
...
export function parseRateLimitConfig(): RateLimitConfig {
  const parseRateLimit = (envVar: string, defaultValue: string): { limit: number; window: number } => {
    ...
    if (!match) {
      throw new Error(`Invalid rate limit format for ${envVar}: ${value}. Expected format: "limit/window" (e.g., "8/10s")`);
    }
