...
${context.channel === "instagram" ? "- Prefira quick replies para múltiplas opções" : ""}`;
	...
	private async adaptToChannel(
		...
		WhatsAppInteractiveMessage | InstagramQuickReplyMessage | InstagramButtonTemplateMessage | { text: string }
	> {
		...
		switch (channel) {
			...
			case "instagram":
				...
				return hasWebUrls ? this.adaptToInstagramButtonTemplate(response) : this.adaptToInstagramQuickReply(response);
	...
	private triggerDegradation(reason: "high_latency" | "consecutive_failures" | "429_errors" | "5xx_errors"): void {
		...
		console.warn(
			`LLM degradation triggered: ${reason}. Degraded until: ${new Date(this.performanceMetrics.degradedUntil).toISOString()}`,
		);
	}
	 ...
	 */
	private createDegradedResponse(context: LlmPromptContext, startTime: number, reason: string): ChannelAdaptedResponse {
	...
	getPerformanceMetrics() {
		const avgLatency =
			this.performanceMetrics.averageLatency.length > 0
				? this.performanceMetrics.averageLatency.reduce((a, b) => a + b, 0) /
					this.performanceMetrics.averageLatency.length
				: 0;
