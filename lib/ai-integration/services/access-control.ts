...
export function requireAllPermissions(context: AccessContext, permissions: AIPermission[]): void {
	const missingPermissions = permissions.filter((permission) => !hasPermission(context.userRole, permission));
...
export async function getUserAuditTrail(
	...
	try {
		const { startDate, endDate, action, resourceType, limit = 50, offset = 0 } = options;
...
export async function getAIAuditTrail(
	...
	try {
		const { userId, action, resourceType, resourceId, startDate, endDate, limit = 50, offset = 0 } = options;
 ...
 */
export async function getAuditStatistics(timeframe: "day" | "week" | "month" = "day"): Promise<{
	totalEntries: number;
	...
	try {
		...
		const [totalEntries, successfulOperations, failedOperations, uniqueUsers, topActions, topResources] =
			await Promise.all([
				prisma.auditLog.count({ where }),
 ...
 */
export async function cleanupOldAuditLogs(retentionDays: number = 365): Promise<{ deletedCount: number }> {
	try {
