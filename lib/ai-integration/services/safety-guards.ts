...
export class SafetyGuards {
  ...
  constructor(config?: Partial<ContentPolicyConfig>) {
    this.config = {
      ...
      prohibitedTerms: config?.prohibitedTerms || [
        'senha', 'password', 'cartão', 'card', 'token',
        'api_key', 'secret', 'private', 'confidencial'
      ],
   ...
   */
  private checkExternalUrls(content: string, buttons?: Array<{ title: string; id: string; url?: string }>): SafetyCheckResult {
    const violations: string[] = [];
    let sanitizedContent = content;
