...
const ConfigSchema = z.object({
  ...
  // Rate Limiting
  RL_CONV: z.string().regex(/^\d+\/\d+s$/, 'RL_CONV must be in format "8/10s"').default('8/10s'),
  RL_ACC: z.string().regex(/^\d+\/\d+s$/, 'RL_ACC must be in format "80/10s"').default('80/10s'),
  RL_CONTACT: z.string().regex(/^\d+\/\d+s$/, 'RL_CONTACT must be in format "15/10s"').default('15/10s'),
...
export async function performConfigHealthCheck(): Promise<ConfigHealthResult> {
  ...
  try {
    ...
    const enabledFlags = featureFlags.filter(flag => 
 ...
 */
export function validateConfigValue(key: string, value: string): {
  isValid: boolean;
  error?: string;
  warning?: string;
} {
