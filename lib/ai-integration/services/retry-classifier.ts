...
export class RetryClassifier {
  ...
  private handleServerErrorRetry(context: RetryContext, statusCode: number): RetryDecision {
    ...
    const delay = Math.min(
      this.baseDelay * Math.pow(2, context.currentAttempt),
      this.maxDelay
    );
   ...
   */
  private classifyError(error: Error, statusCode: number): 'no_retry' | 'rate_limit' | 'server_error' | 'network_error' {
    ...
    const isNetworkError = this.retryMatrix.networkError.some(code => 
