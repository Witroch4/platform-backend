 ...
 * Requirements: 9.1, 9.4
 */
export async function validateUrlForAccount(
  url: string,
  accountId: number
...
export async function getDomainAllowlistStats(accountId: number): Promise<{
  ...
  const customDomains = allowedDomains.filter(domain => 
    !DEFAULT_ALLOWED_DOMAINS.includes(domain)
  );
...
export function isDefaultAllowedDomain(domain: string): boolean {
  ...
  return DEFAULT_ALLOWED_DOMAINS.some(allowedDomain => 
