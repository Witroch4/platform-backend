...
export class HumanHandoffService {
   ...
   */
  private prepareHandoffMessage(context: HandoffContext, handoffId: string): {
    content: string;
    ...
    const tags = [
      ...this.defaultConfig.addTags,
      `handoff_reason_${context.reason.type}`,
      `handoff_id_${handoffId}`
    ];
   ...
   */
  async getHandoffStats(accountId?: number, timeRange?: { start: Date; end: Date }): Promise<{
    total: number;
  ...
  shouldHandoff(context: {
    ...
    if (context.processingTime > 30000) { // 30 seconds
      return true;
    }
