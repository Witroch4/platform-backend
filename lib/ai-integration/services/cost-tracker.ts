...
import { CostTracker, BudgetConfig, BudgetStatus, CostMetrics, TokenPricing } from "../types/cost-control";
...
export class CostTrackingService {
	...
	async trackCost(params: {
		...
		const { accountId, model, inputTokens, outputTokens, operation, traceId } = params;
		...
		const cost =
			operation === "embedding"
				...
				: (inputTokens / 1000) * pricing.inputTokenCostPer1k + (outputTokens / 1000) * pricing.outputTokenCostPer1k;
	...
	async getBudgetStatus(accountId: number): Promise<BudgetStatus> {
		...
		const economicModeActive = percentageUsed >= budgetConfig.economicModeThreshold;
	 ...
	 */
	async getCostSummary(accountId: number, days: number = 7): Promise<CostTracker[]> {
