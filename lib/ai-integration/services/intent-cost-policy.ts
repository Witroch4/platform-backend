...
export class IntentCostPolicyService implements IIntentCostPolicyService {
	...
	async evaluateIntentCost(intentId: string, accountId: number): Promise<IntentCostDecision> {
		try {
			...
			if (economicModeActive) {
				const economicDecision = await this.applyEconomicModeRestrictions(policy, accountId, budgetStatus);
	...
	private getPredefinedIntentPolicy(intentId: string): IntentCostPolicy | null {
		...
		return {
			...
			fallbackStrategy:
				predefined.category === "expensive" || predefined.category === "premium" ? "mini_model" : "template",
			budgetThresholdPercent: predefined.category === "expensive" ? 85 : predefined.category === "premium" ? 80 : 95,
			enabled: true,
			createdAt: new Date(),
			updatedAt: new Date(),
		};
