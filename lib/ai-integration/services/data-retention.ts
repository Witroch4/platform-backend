	...
	try {
		...
		return {
			llmAudit: {
				...
				recordsByMode: llmAuditByMode.reduce(
					(acc, item) => {
...
${metrics.intentHitLog.topIntents
	.map(
		(intent) =>
			`- ${intent.name}: ${intent.count} hits (${(intent.averageSimilarity * 100).toFixed(1)}% avg similarity)`,
	)
	.join("\n")}
	...
	try {
		...
		if (stats.llmAudit.expiringSoon > 1000) {
			recommendations.push(
				`Large number of LLM audit records expiring soon (${stats.llmAudit.expiringSoon}). Consider running cleanup.`,
			);
		}

		if (stats.intentHitLog.expiringSoon > 1000) {
			recommendations.push(
				`Large number of intent hit log records expiring soon (${stats.intentHitLog.expiringSoon}). Consider running cleanup.`,
			);
		}
