...
export class ThresholdTunerService {
	 ...
	 */
	async addNegativeExample(intentId: string, text: string): Promise<NegativeExample> {
		try {
			...
			const negativeExample = await this.prisma.$queryRaw<
				Array<{
					id: string;
					intentId: string;
					text: string;
	 ...
	 */
	async generateROCCurve(intentId: string, positiveExamples: string[], testThresholds?: number[]): Promise<ROCPoint[]> {
	...
	async recommendThreshold(
		...
		try {
			...
			const confidence =
				sortedPoints.length > 1 ? Math.min(1, (sortedPoints[0].score - sortedPoints[1].score) * 2) : 0.5;
	 ...
	 */
	async validateThreshold(intentId: string, threshold: number, positiveExamples: string[]): Promise<ValidationResult> {
		try {
			const negativeExamples = await this.getNegativeExamples(intentId);
			const positiveEmbeddings = await embeddingGenerator.generateEmbeddings(positiveExamples);
	 ...
	 */
	async getIntentRejectionMetrics(timeRange?: { from: Date; to: Date }): Promise<{
		...
		try {
			...
			const rejectionsByIntent = rejectionsByIntentRaw
				.map((item: any) => {
			...
			throw new Error(
				`Failed to get intent rejection metrics: ${error instanceof Error ? error.message : "Unknown error"}`,
			);
		}
	}
