...
import { LlmConfig, LlmResponse, CircuitBreakerState, LlmPromptContext } from "../types/llm";
...
export class OpenAIStructuredClient {
	 ...
	 */
	async generateStructuredOutput<T>(options: StructuredOutputOptions<T>): Promise<LlmResponse<T>> {
	 ...
	 */
	private async executeWithRetry<T>(options: StructuredOutputOptions<T>): Promise<{ data: T; tokensUsed: number }> {
	...
	async healthCheck(): Promise<boolean> {
		try {
			...
			const response = await openaiChatWithCost(this.client, this.config.model, [{ role: "user", content: "test" }], {
