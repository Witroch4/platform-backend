...
export class TemplateRegistryService {
	 ...
	 */
	getTemplateByName(name: string, channel: Channel, language: Language, version?: string): Template | undefined {
		...
		return templates.filter((t) => t.metadata.isActive).sort((a, b) => this.compareVersions(b.version, a.version))[0];
	}
	 ...
	 */
	renderTemplate(templateId: string, context: TemplateRenderContext): RenderedTemplate | null {
	 ...
	 */
	recordABTestMetric(templateId: string, metric: "impression" | "click" | "conversion"): void {
		const template = this.templates.get(templateId);
	...
	private initializeDefaultTemplates(): void {
		...
		this.registerTemplate({
			...
			variables: [{ name: "name", type: "string", required: true, defaultValue: "Cliente" }],
			metadata: {
