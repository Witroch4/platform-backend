...
export class EmbeddingGenerator {
	 ...
	 */
	async generateEmbedding(text: string, options?: TextPreprocessingOptions): Promise<EmbeddingVector> {
		// Preprocess text
	 ...
	 */
	async generateEmbeddings(texts: string[], options?: TextPreprocessingOptions): Promise<EmbeddingVector[]> {
	...
	private preprocessText(
		...
		const { normalize = true, trim = true, lowercase = false, removeExtraSpaces = true } = options;
	...
	private getCacheKey(text: string): string {
		...
		const hash = crypto.createHash("sha256").update(`${this.config.model}:${text}`).digest("hex");
