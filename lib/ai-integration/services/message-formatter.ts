...
export class MessageFormatterService {
   ...
   */
  formatInstagramQuickReply(
    input: InstagramQuickReplyInput,
    context: FormatContext
  ): FormattedChatwitMessage {
    return {
      content: input.text,
   ...
   */
  formatInstagramButtonTemplate(
    input: InstagramButtonTemplateInput,
    context: FormatContext
  ): FormattedChatwitMessage {
    return {
      content: input.text,
...
export class MessageFormatter {
  ...
  formatMessage(params: FormatMessageParams): FormattedMessage {
    ...
    if (params.interactiveData) {
      baseResult.contentAttributes = this.formatInteractiveContent(
  ...
  private formatInteractiveContent(
    ...
    switch (channel) {
      ...
      case 'instagram':
      case 'messenger':
        return this.formatInstagramContent(
          data as InstagramQuickReplyMessage | InstagramButtonTemplateMessage
        );
   ...
   */
  private formatInstagramContent(
    data: InstagramQuickReplyMessage | InstagramButtonTemplateMessage
  ): { ig: InstagramContent } {
  ...
  detectChannelFromData(
    ...
    if ('quick_replies' in data || 
        ('buttons' in data && data.buttons.length > 0 && 
         ('payload' in data.buttons[0] || 'url' in data.buttons[0]))) {
      return 'instagram';
    }
