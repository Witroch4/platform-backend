...
export class OutboundIdempotencyService {
	 ...
	 */
	async checkIdempotency(key: IdempotencyKey, traceId: string): Promise<IdempotencyCheckResult> {
		try {
	 ...
	 */
	async markAsSending(key: IdempotencyKey, traceId: string, attemptCount: number = 1): Promise<void> {
		try {
			const { getRedisInstance } = await import("@/lib/connections");
			const redis = getRedisInstance();
	 ...
	 */
	async markAsSent(key: IdempotencyKey, traceId: string, attemptCount: number = 1): Promise<void> {
		try {
			const { getRedisInstance } = await import("@/lib/connections");
			const redis = getRedisInstance();
	 ...
	 */
	async markAsFailed(key: IdempotencyKey, traceId: string, error: string, attemptCount: number = 1): Promise<void> {
		try {
			const { getRedisInstance } = await import("@/lib/connections");
			const redis = getRedisInstance();
	...
	async cleanupExpiredRecords(conversationId?: number): Promise<number> {
		try {
			...
			const pattern = conversationId ? `${this.keyPrefix}:${conversationId}:*` : `${this.keyPrefix}:*`;
	...
	private hashPayload(payload: ChatwitMessagePayload): string {
		...
		return crypto.createHash("sha256").update(payloadString).digest("hex").substring(0, 16);
	}
