...
export class FeatureFlagManager {
   ...
   */
  async enableForInbox(flagId: string, accountId: number, inboxId: number, reason: string, userId: string): Promise<void> {
    await this.flagService.override({
      flagId,
      accountId,
      inboxId,
   ...
   */
  async disableForInbox(flagId: string, accountId: number, inboxId: number, reason: string, userId: string): Promise<void> {
    await this.flagService.override({
      flagId,
      accountId,
      inboxId,
   ...
   */
  async performGradualRollout(flagId: string, targetPercentage: number, incrementPercentage: number = 5): Promise<void> {
