...
export class ButtonRouterService {
	 ...
	 */
	async routeButtonClick(payload: string, context: ButtonClickContext): Promise<ButtonRouteResult> {
		const startTime = Date.now();
		this.metrics.totalRoutes++;

		// Update channel metrics
		this.metrics.routesByChannel[context.channel] = (this.metrics.routesByChannel[context.channel] || 0) + 1;
		...
		try {
			...
			if (routeResult.success) {
				...
				this.metrics.routesByType[routeType] = (this.metrics.routesByType[routeType] || 0) + 1;
	...
	validateButtonPayload(payload: string): {
		...
		const routeExists =
			parsed.isValid && payloadRouterService.getRoutingRule(parsed.namespace, parsed.slug) !== undefined;
	...
	getRoutingAnalytics(timeRange?: { from: Date; to: Date }): {
		...
		const topRoutes = Object.entries(this.metrics.routesByType)
			.map(([route, count]) => ({
				route,
				count,
				successRate: this.metrics.successfulRoutes > 0 ? count / this.metrics.totalRoutes : 0,
			}))
		...
		const channelPerformance = Object.entries(this.metrics.routesByChannel)
			.map(([channel, routes]) => ({
				channel,
				routes,
				successRate: this.metrics.successfulRoutes > 0 ? routes / this.metrics.totalRoutes : 0,
			}))
