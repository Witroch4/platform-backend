...
export class IntentClassifier {
	 ...
	 */
	async classifyIntent(text: string, options: ClassificationOptions = {}): Promise<IntentClassificationResult> {
		const startTime = Date.now();
		const { traceId, accountId, conversationId, messageId, customThreshold, skipCache = false } = options;
	 ...
	 */
	async classifyBatch(texts: string[], options: ClassificationOptions = {}): Promise<IntentClassificationResult[]> {
		const startTime = Date.now();
	 ...
	 */
	async getMetrics(options: { traceId?: string } = {}): Promise<ClassificationMetrics> {
		...
		try {
			...
			return {
				...
				successfulClassifications: parseInt(metrics.successfulClassifications || "0"),
	...
	private async updateMetrics(data: {
		...
		try {
			...
			if (results) {
				const currentAvg = parseFloat((results[results.length - 2]?.[1] as string) || "0");
				const totalCount = parseInt((results[results.length - 1]?.[1] as string) || "1");

				// Calculate new average
				const newAvg = (currentAvg * (totalCount - 1) + data.latency) / totalCount;
...
export function createIntentClassifier(
	...
	return new IntentClassifier(embeddingGenerator, prisma, redis, config);
}
