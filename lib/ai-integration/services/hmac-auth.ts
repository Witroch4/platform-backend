...
export class HmacAuthService {
	...
	constructor(secret: string, timestampWindow: number = 300) {
		// 5 minutes
		if (!secret) {
	...
	validateSignature(
		...
		try {
			...
			const expectedSignature = crypto.createHmac("sha256", this.secret).update(canonicalString, "utf8").digest("hex");

			// Remove 'sha256=' prefix if present
			const cleanSignature = signature.startsWith("sha256=") ? signature.slice(7) : signature;

			// Timing-safe comparison
			const isValid = crypto.timingSafeEqual(Buffer.from(expectedSignature, "hex"), Buffer.from(cleanSignature, "hex"));
	...
	generateSignature(rawBody: string, timestamp?: string): { signature: string; timestamp: string } {
		...
		const signature = crypto.createHmac("sha256", this.secret).update(canonicalString, "utf8").digest("hex");
...
export function getHmacAuthService(): HmacAuthService {
	if (!hmacAuthInstance) {
		...
		const timestampWindow = process.env.HMAC_TIMESTAMP_WINDOW ? parseInt(process.env.HMAC_TIMESTAMP_WINDOW, 10) : 300; // 5 minutes default
