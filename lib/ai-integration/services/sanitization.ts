...
export class SanitizationService {
	 ...
	 */
	sanitizeWhatsAppMessage(input: WhatsAppSanitizationInput): SanitizationResult<WhatsAppSanitizationInput> {
		...
		if (buttons.length > CHANNEL_LIMITS.whatsapp.buttons.max) {
			...
			warnings.push(
				`Buttons limited to ${CHANNEL_LIMITS.whatsapp.buttons.max} (removed ${originalLength - buttons.length})`,
			);
		}
		...
		buttons = buttons.map((button) => ({
			...button,
			title:
				button.title.length > CHANNEL_LIMITS.whatsapp.buttonTitle
	...
	sanitizeInstagramQuickReply(
		...
		quick_replies = quick_replies.map((reply) => ({
			...reply,
			title:
				reply.title.length > CHANNEL_LIMITS.instagram.quickReply.title
	...
	sanitizeInstagramButtonTemplate(
		...
		if (buttons.length > CHANNEL_LIMITS.instagram.buttonTemplate.buttons.max) {
			...
			buttonsRemoved += originalLength - buttons.length;
			warnings.push(
				`Buttons limited to ${CHANNEL_LIMITS.instagram.buttonTemplate.buttons.max} (removed ${originalLength - buttons.length})`,
			);
		}
		...
		buttons = buttons.map((button) => ({
			...button,
			title:
				button.title.length > CHANNEL_LIMITS.instagram.buttonTemplate.title
	 ...
	 */
	private removeDuplicateButtons(buttons: Array<{ title: string; id: string }>): Array<{ title: string; id: string }> {
