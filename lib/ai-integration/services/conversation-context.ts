...
export class ConversationContextStore {
  ...
  async getContext(
    ...
    try {
      ...
      // Parse context data
      const context = this.config.enableCompression
        ? this.decompressContext(contextData)
    ...
    try {
      ...
      if (context.messages.length === 0) {
        return includeSystemPrompt
          ? "Esta é uma nova conversa. Seja útil e cordial."
          : "";
      }
      ...
      return includeSystemPrompt
        ? "Erro ao carregar histórico. Continue a conversa normalmente."
        : "";
    }
  }
  ...
  async getContextSummary(
    ...
    try {
      const contextKey = this.getContextKey(conversationId);
      const [contextData, ttl] = await Promise.all([
        this.redis.get(contextKey),
        this.redis.ttl(contextKey),
      ]);
      ...
      const lastMessageTime =
        context.messages.length > 0
          ? Math.max(...context.messages.map((m: any) => m.timestamp))
          : 0;
   ...
   */
  async getActiveConversationsCount(
    options: { traceId?: string } = {}
  ): Promise<number> {
    const { traceId } = options;
