...
function createEmojiRegex(): RegExp {
  const ranges = EMOJI_RANGES.map(range => 
 ...
 * Requirements: 9.1
 */
export function validateEmojiUsage(text: string, options: {
  maxTotal?: number;
  ...
  if (allowedCategories.length > 0) {
    const allowedEmojis = allowedCategories.flatMap(cat => EMOJI_CATEGORIES[cat as keyof typeof EMOJI_CATEGORIES] || []);
 ...
 * Requirements: 9.1
 */
export function normalizeForChannelUX(text: string, channel: 'whatsapp' | 'instagram' = 'whatsapp'): {
  normalized: string;
  changes: string[];
} {
 ...
 * Requirements: 9.1
 */
export function suggestEmojiReplacements(text: string, channel: 'whatsapp' | 'instagram' = 'whatsapp'): {
  suggestions: Array<{
    original: string;
    replacement: string;
    reason: string;
