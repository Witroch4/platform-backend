...
export class QueueLagMonitor {
	...
	constructor(config: Partial<QueueLagConfig> = {}) {
		this.config = {
			...
			queues:
				config.queues ??
				(process.env.QUEUE_LAG_MONITOR_QUEUES?.split(",") || ["ai:incoming-message", "ai:embedding-upsert"]),
		};
	}
	...
	private async measureQueueLag(queueName: string): Promise<QueueLagMeasurement> {
		...
		try {
			...
			const queueMetrics = aiMetrics
				.getMetrics()
				.filter((m) => m.name === "ai_jobs_in_queue" && m.labels.queue_name === queueName);

			const jobsWaiting =
				queueMetrics.length > 0
					? queueMetrics[0].type === "histogram"
						? queueMetrics[0].count
						: queueMetrics[0].value
					: 0;
	...
	private async measureAllQueues(): Promise<QueueLagReport> {
		...
		try {
			...
			const measurements = await Promise.all(this.config.queues.map((queueName) => this.measureQueueLag(queueName)));
	...
	// Clear history
	clearHistory(): void {
		const totalMeasurements = Array.from(this.lagHistory.values()).reduce((sum, history) => sum + history.length, 0);
