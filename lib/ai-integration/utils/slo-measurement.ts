...
export class SLOMeasurementService {
	...
	private measureAvailability(windowMinutes: number, labels: Record<string, string> = {}): number {
		...
		const jobMetrics = metrics.filter(
			(m) => m.name === "ai_jobs_total" && Object.entries(labels).every(([key, value]) => m.labels[key] === value),
		);
	...
	private measureSLO(target: SLOTarget, windowMinutes: number, labels: Record<string, string> = {}): SLOMeasurement {
		let value: number;
		...
		const violated =
			target.measurement === "error_rate"
				? value > target.target
				: target.measurement === "availability"
					? value < target.target
					: value > target.target;
	...
	generateSLOReport(windowMinutes: number = 60, labels: Record<string, string> = {}): SLOReport {
		...
		try {
			const measurements = this.sloTargets.map((target) => this.measureSLO(target, windowMinutes, labels));
