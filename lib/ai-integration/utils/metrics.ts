...
class MetricsCollector {
	...
	private readonly defaultLatencyBuckets = [10, 25, 50, 100, 250, 500, 1000, 2500, 5000, 10000, 30000, Infinity];

	// Default histogram buckets for token counts
	private readonly defaultTokenBuckets = [10, 50, 100, 250, 500, 1000, 2500, 5000, 10000, 25000, Infinity];
...
export class AIMetrics {
	...
	incrementJobsTotal(stage: string, status: string, labels: MetricLabels = {}): void {
		this.collector.incrementCounter("ai_jobs_total", { stage, status, ...labels }, "Total number of AI jobs processed");
	}
	...
	setJobsInQueue(queueName: string, count: number): void {
		this.collector.setGauge("ai_jobs_in_queue", count, { queue_name: queueName }, "Number of jobs currently in queue");
	}
	...
	incrementRateLimitHits(scope: string, labels: MetricLabels = {}): void {
		this.collector.incrementCounter("ai_ratelimit_hits_total", { scope, ...labels }, "Total number of rate limit hits");
	}
	...
	setActiveWorkers(workerType: string, count: number): void {
		this.collector.setGauge("ai_active_workers", count, { worker_type: workerType }, "Number of active workers");
	}

	// SLO violation metrics
	incrementSLOViolations(labels: MetricLabels = {}): void {
		this.collector.incrementCounter("ai_slo_violation_total", labels, "Total number of SLO violations");
	}
	...
	incrementSecretRotations(): void {
		this.collector.incrementCounter("ai_secret_rotation_events_total", {}, "Total number of secret rotation events");
	}
