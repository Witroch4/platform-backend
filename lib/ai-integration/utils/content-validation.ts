 ...
 * Requirements: 9.1, 9.2, 9.3, 9.4
 */
export function validateAndSanitizeText(text: string, options: ContentValidationOptions): ValidationResult {
	const errors: string[] = [];
	...
	if (sanitized.length > options.maxLength) {
		...
		sanitized = smartTruncate(sanitized, options.maxLength, options.preserveWordBoundaries !== false);
		truncated = true;
 ...
 * Requirements: 9.1, 9.2, 9.3, 9.4
 */
export function createFallbackText(originalContent: any, reason: string): string {
	const fallbackMessages = {
		invalid_buttons:
			'Desculpe, não foi possível exibir as opções. Digite sua mensagem ou "ajuda" para falar com um atendente.',
		url_validation_failed:
			'Alguns links não puderam ser exibidos por questões de segurança. Digite "ajuda" para falar com um atendente.',
 ...
 */
export function validateButtonTitle(title: string, maxLength: number = 20): ValidationResult {
	const errors: string[] = [];
	const warnings: string[] = [];
	let sanitized = title || "";
	let truncated = false;
 ...
 */
export function validateWebUrl(url: string, allowedDomains?: string[]): ValidationResult {
	const errors: string[] = [];
	const warnings: string[] = [];
