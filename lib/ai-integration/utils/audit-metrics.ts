...
export async function collectAuditMetrics(): Promise<AuditMetrics> {
	...
	try {
		...
		const [llmAuditTotal, llmAuditByMode, llmAuditAvgScore, llmAuditLast24h, llmAuditLast7d, llmAuditTopConversations] =
			await Promise.all([
				// Total de registros
		...
		const successRate = intentHitLogTotal > 0 ? (intentHitLogSuccessful / intentHitLogTotal) * 100 : 0;
