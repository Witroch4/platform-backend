...
export const REDIS_KEYS = {
	...
	llmCache: (textHash: string, channel: string, accountId: number) => `llm:cache:${textHash}:${channel}:${accountId}`,
	circuitBreaker: (service: string) => `cb:${service}`,
	outboundJournal: (conversationId: number, payloadHash: string) => `out:${conversationId}:${payloadHash}`,
} as const;
