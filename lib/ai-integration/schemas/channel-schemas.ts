...
export const InstagramQuickReplySchema = z.object({
	...
	quick_replies: z
		...
		.min(1)
		.max(13), // Instagram allows up to 13, but we'll cap at 3 for UX
});
			...
			z.discriminatedUnion("type", [
				...
				z.object({
					...
					url: z
						.string()
						.url()
						.refine((url) => url.startsWith("https://"), { message: "Instagram web_url buttons must use HTTPS" }),
				}),
			]),
		)
		.min(1)
...
export class ChannelSchemaValidator {
	 ...
	 */
	static validateInstagramQuickReply(data: any): {
		valid: boolean;
		data?: InstagramQuickReplyMessage;
		errors?: string[];
	} {
		try {
	 ...
	 */
	static validateInstagramButtonTemplate(data: any): {
		valid: boolean;
		data?: InstagramButtonTemplateMessage;
		errors?: string[];
	} {
		try {
