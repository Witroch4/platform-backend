 ...
 */
async function emitRotationMetrics(
  overdueCount: number,
  dueSoonCount: number,
  inOverlapCount: number
): Promise<void> {
  try {
...
export async function getRotationMonitorJobStatus(queue: any): Promise<{
  ...
  try {
    ...
    const lastDaily = completedJobs.find((job: any) => 
      job.data?.checkType === 'daily' && job.returnvalue
    );
    
    const lastWeekly = completedJobs.find((job: any) => 
      job.data?.checkType === 'weekly' && job.returnvalue
    );
