		...
		try {
			...
			if (this.config.accounts && this.config.accounts.length > 0) {
				for (const accountId of this.config.accounts) {
					try {
						const accountReport = sloMeasurementService.generateAccountSLOReport(accountId, this.config.windowMinutes);
			...
			if (this.config.channels && this.config.channels.length > 0) {
				for (const channel of this.config.channels) {
					try {
						const channelReport = sloMeasurementService.generateChannelSLOReport(channel, this.config.windowMinutes);
			...
			for (const sloName of criticalSLOs) {
				try {
					...
					if (burnRate.burnRate > 1) {
						// Burning error budget faster than sustainable
