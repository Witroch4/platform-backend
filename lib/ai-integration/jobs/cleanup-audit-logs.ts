 ...
 */
export async function processCleanupAuditLogsJob(job: Job<CleanupJobData>): Promise<CleanupJobResult> {
	...
	try {
		...
		const totalExpired = retentionStatsBefore.llmAudit.expiringSoon + retentionStatsBefore.intentHitLog.expiringSoon;
		...
		if (totalExpired > maxRecordsToDelete && !force) {
			warnings.push(
				`Too many records to delete (${totalExpired} > ${maxRecordsToDelete}). Use force=true to override.`,
			);
 ...
 */
export async function createManualCleanupJob(queue: any, options: CleanupJobData = {}): Promise<Job<CleanupJobData>> {
	try {
