...
export const instagramTranslationQueue = new Queue<InstagramTranslationJobData>(INSTAGRAM_TRANSLATION_QUEUE_NAME, {
 ...
 */
export async function addInstagramTranslationJob(data: InstagramTranslationJobData): Promise<string> {
	...
	try {
		...
		const job = await instagramTranslationQueue.add(
			jobName,
			{
				...data,
				metadata: {
 ...
 */
export async function getInstagramTranslationResult(correlationId: string): Promise<InstagramTranslationResult | null> {
	try {
