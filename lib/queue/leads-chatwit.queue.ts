...
export async function addLeadJob(data: ILeadJobData) {
	...
	try {
		...
		const job = await leadsQueue.add(`lead-${sourceId}`, data, {
			// Não define novas opções aqui para usar as padrões,
			// evitando sobrescrever os valores definidos acima
		});
		console.log(
			`[BullMQ] Job ${job.id} enfileirado para lead ${sourceId} com ${data.payload.origemLead.arquivos?.length || 0} arquivos`,
		);
		return job;
	} catch (error) {
...
export async function addFinalAnalysisJob(data: IFinalAnalysisJobData) {
	await leadsQueue.add("process-final-analysis", data, {
