	...
	try {
		...
		if (!fs.existsSync(tempDir)) {
			...
			try {
				...
				throw new Error(
					`Não foi possível criar o diretório temporário na raiz do projeto: ${error instanceof Error ? error.message : String(error)}`,
				);
			}
		}
		...
		for (let i = 0; i < fileUrls.length; i++) {
			...
			try {
				...
				if (isPdf(fileUrl)) {
					...
					console.log(
						`[iLovePDF] Arquivo ${i + 1} não é PDF, imagem ou Office conhecido. Tentando baixar e processar como PDF`,
					);
					fileBuffer = await downloadUrlAsBuffer(fileUrl);
				}
...
export async function unifyAndSavePdf(fileUrls: string[], fileName: string): Promise<string> {
	try {
		console.log(
			`[iLovePDF] Iniciando processo de unificação e salvamento de ${fileUrls.length} arquivos com nome "${fileName}"`,
		);
...
export async function convertOfficeToPdf(officeUrl: string): Promise<Buffer> {
	try {
		...
		throw new Error(
			`Falha ao converter arquivo Office em PDF: ${error instanceof Error ? error.message : String(error)}`,
		);
	}
}
