...
async function buildRubricFromPdfLLM(buffer: Buffer, options: BuildRubricOptions = {}, preExtractedText?: string) {
  ...
  const prompt = `Você receberá a transcrição integral de um padrão oficial de respostas da prova prático-profissional da OAB.\n` +
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: "Você é um analista jurídico da FGV especializado em normalizar padrões de resposta da OAB. Sempre responda com JSON válido.",
      },
      {
        role: "user",
        content: prompt,
  ...
  for (const fn of tryParsers) {
    try { rawObj = fn(); break; } catch (e) { lastErr = e; }
  }
  if (!rawObj) {
...
function extractMetaFromText(rawText: string, fallback: { exam?: string; area?: string; data_aplicacao?: string }): ParseMetaInput {
  const exam = rawText.match(/\d+º Exame de Ordem Unificado/i)?.[0]?.trim()
    ?? fallback.exam
    ?? "Exame OAB";
...
export async function buildRubricFromPdf(buffer: Buffer, options: BuildRubricOptions = {}) {
  ...
  console.warn("[OAB::RUBRIC_UPLOAD::FALLBACK] Acionando LLM devido a inconsistências", {
    ...
    diagnostico: {
      ...
      itensProblematicos: deterministic.itens.filter((it) => it.peso == null).map(it => ({
        id: it.id,
        questao: it.questao,
        escopo: it.escopo,
