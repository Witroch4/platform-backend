...
export function startTranscriptionWorker() {
  ...
  console.log(`[TranscriptionQueue] 🚀 Iniciando worker (max concurrent: ${maxConcurrentJobs}, page concurrency: ${concurrency})`);
...
async function processTranscriptionJob(
  ...
  const { leadID, images, userId } = job.data;
...
export async function getTranscriptionStatus(leadID: string): Promise<{
  ...
  if (state === 'waiting') {
    ...
    return { status: 'queued', progress: { leadID, currentPage: 0, totalPages: job.data.images.length, percentage: 0, startedAt: new Date() } };
  }
...
// Graceful shutdown
process.on('SIGTERM', stopTranscriptionWorker);
process.on('SIGINT', stopTranscriptionWorker);
