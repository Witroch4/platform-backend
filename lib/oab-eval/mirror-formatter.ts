 ...
 * - schema_docs
 */
export function optimizeMirrorPayload(payload: StudentMirrorPayload): OptimizedMirrorPayload {
	return {
		meta: {
 ...
 */
export function formatMirrorToMarkdown(structuredMirror: any, rubric: any): string {
...
export function formatMirrorToJson(
	...
	if (mirror?.avaliacoes?.peca?.itens) {
		for (const item of mirror.avaliacoes.peca.itens) {
			const rubricItem =
				rubric?.grupos?.find((g: any) => g.id === item.id) || rubric?.itens?.find((i: any) => i.id === item.id);
