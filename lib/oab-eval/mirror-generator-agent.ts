...
function selectConsistentTotal({
	...
	const referenceDiff = referenceClamped != null ? Math.abs(extractedClamped - referenceClamped) : null;
	const referenceComputedDiff = referenceClamped != null ? Math.abs(computedClamped - referenceClamped) : null;
...
function rebalanceOverridesForScope({
	...
	let sum = roundToTwo(entries.reduce((acc, [groupId]) => acc + (groupOverrides.get(groupId) ?? 0), 0));
	...
	const prioritized = (
		entries.filter(([, info]) => !info.reliable).length ? entries.filter(([, info]) => !info.reliable) : entries
	)
		.map(([groupId, info]) => ({
			groupId,
			info,
	...
	const adjustedSum = roundToTwo(entries.reduce((acc, [groupId]) => acc + (groupOverrides.get(groupId) ?? 0), 0));
	const residual = roundToTwo(adjustedSum - extractedTotal);
	if (residual > SCORE_TOLERANCE) {
		console.warn(`[MirrorGenerator::Rebalance] ⚠️ Residual após ajuste em ${scopeLabel}: ${residual.toFixed(2)}.`);
	}
}
...
function isAttendanceColumn(label: string | null): boolean {
	...
	const patterns = [/ATENDIMENTO AO QUESITO/, /\bATENDIMENTO\b/, /PONTUACAO OBTIDA/, /NOTA OBTIDA/, /PONTOS OBTIDOS?/];
 ...
 */
async function getMirrorExtractorConfig(selectedProvider?: "OPENAI" | "GEMINI"): Promise<{
	model: string;
	...
	console.log(
		`[MirrorGenerator] 🎛️ Provider efetivo: ${effectiveProvider} (original: ${selectedProvider || "undefined"})`,
	);
	...
	try {
		...
		if (blueprint) {
			...
			if (!blueprintMatchesProvider) {
				...
				console.log(
					`[MirrorGenerator] 🔄 Provider switch ativado: ${previousModel} → ${model} (provider: ${effectiveProvider})`,
				);
			} else {
...
async function loadRubric(especialidade: string, espelhoPadraoId?: string): Promise<RubricPayload> {
	...
	if (espelhoPadraoId) {
		...
		if (rubric) {
			console.log(
				`[MirrorGenerator] ✅ Rubrica encontrada por ID: ${rubric.exam || "Exame"} - ${(rubric.meta as any)?.area || rubric.area}`,
			);
		}
	}
...
function flattenExtractedResponse(parsed: Record<string, unknown>): ExtractedMirrorData {
	...
	const notaKeys = Object.keys(result).filter(
		(k) => k.startsWith("nota_obtida_") || k.startsWith("nota_total_"),
	).length;
...
async function extractMirrorDataFromImages(
	...
	const notasIntro = grupos.length
		? ["3. NOTAS DOS GRUPOS:", "Para cada grupo abaixo, informe a nota total obtida (formato: X.XX):"]
		: ["3. NOTAS DOS ITENS:", "Para cada item abaixo, extraia a nota obtida (formato: X.XX):"];
	...
	try {
		...
		console.log(
			`[MirrorGenerator] 📊 Aluno: ${extracted.nome_do_examinando || "N/A"}, Nota: ${extracted.nota_final || "N/A"}`,
		);
		...
		if (expectedGroupIds.length) {
			...
			if (notasExtraidas > 0) {
				console.log(
					`[MirrorGenerator::Response] (info) Subitens extraídos: ${notasExtraidas}/${expectedSubitemIds.length}`,
				);
			}
			...
			console.log(
				`[MirrorGenerator::Response] (info) Colunas extraídas: ${colunasExtraidas}/${expectedGroupIds.length}`,
			);
			console.log(
				`[MirrorGenerator::Response] (info) Fontes totais por questão: ${fontesTotaisQuestoes}/4, Colunas totais por questão: ${colunasTotaisQuestoes}/4`,
			);
		} else {
 ...
 */
function buildStructuredMirror(rubric: RubricPayload, extractedData: ExtractedMirrorData): StructuredMirror {
	...
	if (possuiGrupos) {
		for (const grupo of gruposRubrica) {
			...
			if (
				...
				fonteDetected != null &&
				Math.abs(fonteDetected - normalizedOverride) > SCORE_TOLERANCE
			) {
				console.warn(
	...
	const computedTotalQuestoes = roundToTwo(
		Math.min(sumQuestoesSelecionadas, expected.questoes.esperado ?? sumQuestoesSelecionadas),
	);
...
export async function generateMirrorLocally(input: MirrorGeneratorInput): Promise<MirrorGeneratorOutput> {
	console.log(`[MirrorGenerator] 🚀 Iniciando geração de espelho para lead ${input.leadId} (${input.especialidade})`);
	...
	const jsonMirror = formatter.formatMirrorToJson(extractedData, rubric, structuredMirror, {
		leadId: input.leadId,
		nome: input.nome,
