...
export function useChatwitIA(chatId?: string | null, initialModel = "gpt-5-chat-latest") {
	...
	const loadChatFromDB = async (id: string) => {
		...
		try {
			...
			const convertedMessages: Message[] = sessionData.messages.map((msg) => {
				...
				if (msg.contentType === "audio" && msg.audioData) {
					...
					const hasImageMarkdown =
						typeof msg.content === "string" && (msg.content.includes("![Imagem gerada]") || msg.content.includes("!["));
			...
			console.log(
				`🖼️ Mensagens com imagens do MinIO:`,
				convertedMessages.filter(
					(m) => typeof m.content === "string" && m.content.includes("![Imagem gerada](https://"),
				).length,
			);
			console.log(
				`🎨 Mensagens com imagens base64:`,
				convertedMessages.filter(
					(m) => typeof m.content === "string" && m.content.includes("![Imagem gerada](data:image/"),
				).length,
			);

			// 🔧 CORREÇÃO: Evitar sobrescrever mensagens locais quando já há conteúdo
			const shouldUpdateMessages = messages.length === 0 || currentSessionId !== id;
	...
	const sendMessage = async (content: string, systemPrompt?: string, modelParam?: string) => {
		...
		try {
			...
			const updateMessageWithStream = (
				content: string,
				images: any[] = [],
				partialImageData?: { index: number; base64: string },
			) => {
					...
					setMessages((prev) => {
						...
						if (lastIndex >= 0 && updatedMessages[lastIndex].role === "assistant") {
							...
							const hasBase64Images =
								currentContent && currentContent.includes("![Imagem gerada](data:image/png;base64,");
				...
				for (const line of lines) {
					...
					try {
						...
						if (data.type === "chunk") {
							...
							const lastContent = streamContentRef.current.replace(
								"🎨 Gerando imagem...",
								"🎨 Gerando imagem... (progresso)",
							);
							...
							if (data.response_id) {
								...
								console.log(
									`✅ LastResponseId atualizado IMEDIATAMENTE (ref: ${lastResponseIdRef.current}, state: ${data.response_id})`,
								);
							...
							setMessages((prevMessages) => {
								...
								if (lastMessage && lastMessage.role === "assistant") {
									...
									const hasPartialImages =
										currentContent && currentContent.includes("![Gerando imagem...](data:image/png;base64");
	...
	const listFiles = async (purpose?: string) => {
		try {
			...
			const url = purpose ? `/api/chatwitia/files?purpose=${purpose}` : "/api/chatwitia/files";
	...
	const testUpload = async (file: File, purpose = "test") => {
		try {
			...
			console.log(
				`Teste de upload: Iniciando com arquivo: ${file.name}, tamanho: ${(file.size / 1024 / 1024).toFixed(2)}MB, tipo: ${file.type}, propósito: ${purpose}`,
			);
