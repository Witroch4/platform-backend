...
export const useTemplateForm = (initialState: TemplateFormState, onSuccessCallback?: () => void) => {
	...
	const isValidHeaderMedia = useMemo(() => {
		if (state.headerType === "VIDEO" || state.headerType === "IMAGE") {
			return (
				state.headerMetaMedia.length > 0 &&
				state.headerMetaMedia[0]?.status === "success" &&
				!!state.headerMetaMedia[0]?.mediaHandle
			);
		}
		return true;
					...
					.map((b: any) => {
						switch (b.type) {
							...
							case "COPY_CODE":
								return {
									type: "COPY_CODE",
									text: b.text || "Copiar código da oferta",
									example: [b.example?.[0] || ""],
								};
							case "FLOW": {
