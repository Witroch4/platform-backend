...
interface UseVariableManagerReturn {
  ...
  // Template helpers
  createTemplateWithAutoFooter: (header: string, body: string, footer: string) => {
    header: string;
    body: string;
    footer: string;
  };
...
export function useVariableManager(): UseVariableManagerReturn {
  ...
  const saveVariables = useCallback(async (variablesToSave: MtfDiamanteVariavel[]): Promise<boolean> => {
    try {
      setSaving(true);
  ...
  const getAutoFooterForTemplate = useCallback((currentFooter?: string): string => {
