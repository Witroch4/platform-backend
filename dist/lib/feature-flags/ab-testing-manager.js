...
exports.createWebhookPerformanceTest = createWebhookPerformanceTest;
...
class ABTestingManager {
    ...
    async createABTest(name, description, hypothesis, variants, metrics, targetSampleSize = 1000, confidenceLevel = 0.95, createdBy = 'system') {
        ...
        await this.featureFlagManager.setFeatureFlag(controlFlagName, false, // Start disabled
        variants.control.percentage, { variant: 'control', testId, ...variants.control.config }, createdBy);
        await this.featureFlagManager.setFeatureFlag(treatmentFlagName, false, // Start disabled
        variants.treatment.percentage, { variant: 'treatment', testId, ...variants.treatment.config }, createdBy);
        const test = {
            id: testId,
            name,
            description,
        ...
        await this.redis.setex(`ab_test:${testId}`, 60 * 60 * 24 * 90, // 90 days
        JSON.stringify(test));
        console.log(`[ABTest] Created A/B test: ${testId}`);
        return test;
    }
    async startABTest(testId, startedBy = 'system') {
        ...
        await this.featureFlagManager.setFeatureFlag(test.variants.control.flagName, true, test.variants.control.percentage, { variant: 'control', testId, ...test.variants.control.config }, startedBy);
        await this.featureFlagManager.setFeatureFlag(test.variants.treatment.flagName, true, test.variants.treatment.percentage, { variant: 'treatment', testId, ...test.variants.treatment.config }, startedBy);
        // Update test status
        test.status = 'RUNNING';
        test.startDate = new Date();
        await this.redis.setex(`ab_test:${testId}`, 60 * 60 * 24 * 90, JSON.stringify(test));
        console.log(`[ABTest] Started A/B test: ${testId}`);
    }
    async stopABTest(testId, stoppedBy = 'system') {
        const test = await this.getABTest(testId);
        if (!test) {
            throw new Error(`A/B test ${testId} not found`);
        }
        // Disable feature flags
        await this.featureFlagManager.setFeatureFlag(test.variants.control.flagName, false, 0, {}, stoppedBy);
        await this.featureFlagManager.setFeatureFlag(test.variants.treatment.flagName, false, 0, {}, stoppedBy);
        // Update test status
    ...
    async assignUserToVariant(testId, userId, metadata) {
        const test = await this.getABTest(testId);
        if (!test) {
            throw new Error(`A/B test ${testId} not found`);
        }
        ...
        // Store assignment
        await this.redis.setex(`ab_test_assignment:${testId}:${userId}`, 60 * 60 * 24 * 90, // 90 days
    ...
    async recordMetric(testId, userId, metricName, value, metadata) {
        ...
        await this.redis.lpush(`ab_test_metrics:${testId}:${assignment.variant}:${metricName}`, JSON.stringify(metricRecord));
    ...
    async calculateVariantResults(testId, variant, metrics, stats) {
        const result = {
            testId,
            variant,
            metrics: {},
...
async function createWebhookPerformanceTest() {
    try {
        ...
        const test = await abTestManager.createABTest('Webhook Performance Test', 'Test new webhook processing vs legacy processing', 'New webhook processing will improve response times by 50%', {
            control: {
                name: 'Legacy Webhook',
