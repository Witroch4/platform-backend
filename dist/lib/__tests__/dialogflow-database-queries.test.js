...
const dialogflow_database_queries_1 = require("../dialogflow-database-queries");
// Mock Prisma
const mockPrisma = {
    mapeamentoIntencao: {
        findUnique: jest.fn()
...
describe('Dialogflow Database Queries', () => {
    ...
    describe('findCompleteMessageMappingByIntent', () => {
        it('should find complete template message mapping', async () => {
            ...
            const result = await (0, dialogflow_database_queries_1.findCompleteMessageMappingByIntent)('welcome', 'caixa-456');
        ...
        it('should find complete interactive message mapping', async () => {
            ...
            mockPrisma.mapeamentoIntencao.findUnique.mockResolvedValue(mockMapping);
            const result = await (0, dialogflow_database_queries_1.findCompleteMessageMappingByIntent)('menu', 'caixa-789');
            expect(result).toEqual({
                id: 'mapping-456',
                intentName: 'menu',
                caixaEntradaId: 'caixa-789',
        ...
        it('should use environment fallback when WhatsApp config is missing', async () => {
            ...
            mockPrisma.mapeamentoIntencao.findUnique.mockResolvedValue(mockMapping);
            const result = await (0, dialogflow_database_queries_1.findCompleteMessageMappingByIntent)('test', 'caixa-123');
        ...
        it('should return null when no mapping is found', async () => {
            ...
            const result = await (0, dialogflow_database_queries_1.findCompleteMessageMappingByIntent)('nonexistent', 'caixa-123');
        ...
        it('should return null when mapping has no message data', async () => {
            ...
            mockPrisma.mapeamentoIntencao.findUnique.mockResolvedValue(mockMapping);
            const result = await (0, dialogflow_database_queries_1.findCompleteMessageMappingByIntent)('empty', 'caixa-123');
            expect(result).toBeNull();
        });
        it('should handle database errors gracefully', async () => {
            mockPrisma.mapeamentoIntencao.findUnique.mockRejectedValue(new Error('Database connection failed'));
            await expect((0, dialogflow_database_queries_1.findCompleteMessageMappingByIntent)('test', 'caixa-123')).rejects.toThrow('Database query failed: Database connection failed');
        });
    });
    describe('findReactionByButtonId', () => {
        it('should find button reaction from database', async () => {
            ...
            const result = await (0, dialogflow_database_queries_1.findReactionByButtonId)('like_button');
            expect(result).toEqual({
                id: 'reaction-123',
                buttonId: 'like_button',
                emoji: '👍',
        ...
        it('should fallback to config when database model does not exist', async () => {
            ...
            button_reaction_mapping_1.getEmojiForButton.mockReturnValue('❤️');
            const result = await (0, dialogflow_database_queries_1.findReactionByButtonId)('love_button');
            expect(result).toEqual({
            ...
            expect(button_reaction_mapping_1.getEmojiForButton).toHaveBeenCalledWith('love_button');
        });
        it('should return null when no reaction is found', async () => {
            mockPrisma.buttonReactionMapping = {
                findUnique: jest.fn().mockResolvedValue(null)
            };
            button_reaction_mapping_1.getEmojiForButton.mockReturnValue(null);
            const result = await (0, dialogflow_database_queries_1.findReactionByButtonId)('nonexistent_button');
            expect(result).toBeNull();
        });
        it('should ignore inactive database reactions', async () => {
            ...
            button_reaction_mapping_1.getEmojiForButton.mockReturnValue('⚡');
            const result = await (0, dialogflow_database_queries_1.findReactionByButtonId)('inactive_button');
            expect(result).toEqual({
        ...
        it('should handle database errors gracefully', async () => {
            ...
            await expect((0, dialogflow_database_queries_1.findReactionByButtonId)('error_button')).rejects.toThrow('Database query failed: Database error');
        });
    });
    describe('getAllActiveButtonReactions', () => {
        it('should get all active reactions from database', async () => {
            ...
            const result = await (0, dialogflow_database_queries_1.getAllActiveButtonReactions)();
            expect(result).toEqual([
                {
                    id: 'reaction-1',
                    buttonId: 'like',
        ...
        it('should fallback to config when database is empty', async () => {
            ...
            button_reaction_mapping_1.getAllButtonReactions.mockReturnValue(mockConfigReactions);
            const result = await (0, dialogflow_database_queries_1.getAllActiveButtonReactions)();
            expect(result).toEqual([
                {
                    id: 'config-0',
        ...
        it('should handle database errors gracefully', async () => {
            ...
            await expect((0, dialogflow_database_queries_1.getAllActiveButtonReactions)()).rejects.toThrow('Database query failed: Database error');
        });
    });
    describe('testDatabaseConnection', () => {
        it('should return true for successful connection', async () => {
            mockPrisma.$queryRaw.mockResolvedValue([{ '?column?': 1 }]);
            const result = await (0, dialogflow_database_queries_1.testDatabaseConnection)();
    ...
    describe('closeDatabaseConnection', () => {
        it('should close database connection', async () => {
            ...
            await (0, dialogflow_database_queries_1.closeDatabaseConnection)();
    ...
    describe('Message Type Priority', () => {
        it('should prioritize unified template over other types', async () => {
            ...
            mockPrisma.mapeamentoIntencao.findUnique.mockResolvedValue(mockMapping);
            const result = await (0, dialogflow_database_queries_1.findCompleteMessageMappingByIntent)('priority_test', 'caixa-priority');
