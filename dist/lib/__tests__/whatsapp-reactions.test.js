...
const whatsapp_reactions_1 = require("../whatsapp-reactions");
// Mock dependencies
jest.mock('axios');
jest.mock('@/app/lib', () => ({
...
describe('WhatsApp Reactions Service', () => {
    ...
    describe('sendReactionMessage', () => {
        it('should send reaction with provided API key successfully', async () => {
            ...
            auth_1.auth.mockResolvedValue(mockSession);
            lib_1.getWhatsAppConfig.mockResolvedValue(mockConfig);
            lib_1.getWhatsAppApiUrl.mockReturnValue(mockApiUrl);
            const mockResponse = {
                data: {
            ...
            const result = await (0, whatsapp_reactions_1.sendReactionMessage)(reactionData);
            expect(result).toEqual({
                success: true,
                messageId: 'wamid.reaction123'
            });
            expect(mockedAxios.post).toHaveBeenCalledWith(mockApiUrl, {
        ...
        it('should send reaction without provided API key using session config', async () => {
            ...
            expect(mockedAxios.post).toHaveBeenCalledWith(mockApiUrl, expect.objectContaining({
                reaction: {
        ...
        it('should handle phone number formatting correctly', async () => {
            ...
            auth_1.auth.mockResolvedValue(mockSession);
            lib_1.getWhatsAppConfig.mockResolvedValue(mockConfig);
            lib_1.getWhatsAppApiUrl.mockReturnValue('https://api.test.com');
            mockedAxios.post.mockResolvedValue({
            ...
            for (const { input, expected } of testCases) {
                ...
                await (0, whatsapp_reactions_1.sendReactionMessage)(reactionData);
        ...
        it('should handle authentication errors', async () => {
            ...
            auth_1.auth.mockResolvedValue(null);
            const result = await (0, whatsapp_reactions_1.sendReactionMessage)(reactionData);
            expect(result).toEqual({
                success: false,
        ...
        it('should handle configuration errors', async () => {
            ...
            const mockSession = { user: { id: 'user123' } };
            auth_1.auth.mockResolvedValue(mockSession);
            lib_1.getWhatsAppConfig.mockRejectedValue(new Error('WhatsApp configuration not found'));
            const result = await (0, whatsapp_reactions_1.sendReactionMessage)(reactionData);
            expect(result).toEqual({
                success: false,
    ...
    describe('logReactionAttempt', () => {
        ...
        it('should log successful reaction attempt', async () => {
            ...
            await (0, whatsapp_reactions_1.logReactionAttempt)(logData);
            expect(consoleSpy).toHaveBeenCalledWith('[WhatsApp Reactions] Tentativa de reação registrada:', expect.objectContaining({
        ...
        it('should handle logging errors gracefully', async () => {
            ...
            await expect((0, whatsapp_reactions_1.logReactionAttempt)(logData)).resolves.toBeUndefined();
            expect(consoleErrorSpy).toHaveBeenCalledWith('[WhatsApp Reactions] Erro ao registrar tentativa de reação:', expect.any(Error));
