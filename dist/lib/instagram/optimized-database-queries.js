...
exports.findOptimizedCompleteMessageMapping = findOptimizedCompleteMessageMapping;
exports.findBatchOptimizedTemplateMappings = findBatchOptimizedTemplateMappings;
exports.checkDatabaseConnectionHealth = checkDatabaseConnectionHealth;
exports.getQueryPerformanceStats = getQueryPerformanceStats;
exports.warmInstagramTemplateCache = warmInstagramTemplateCache;
...
class QueryPerformanceMonitor {
    ...
    recordQuery(metrics) {
        ...
        (0, instagram_template_cache_1.recordQueryPerformance)(metrics.queryName, metrics.executionTime, metrics.executionTime > PERFORMANCE_THRESHOLDS.SLOW_QUERY_MS).catch(error => {
...
class DatabaseError extends Error {
    ...
    executionTime;
    originalError;
    constructor(message, queryName, executionTime, originalError) {
        super(message);
...
async function findOptimizedCompleteMessageMapping(intentName, inboxId) {
    ...
    try {
        ...
        if (quickInboxLookup) {
            ...
            cached = await (0, instagram_template_cache_1.getCachedTemplateMapping)(intentName, usuarioChatwitId, inboxId);
        }
        else {
        ...
        await (0, instagram_template_cache_1.setCachedTemplateMapping)(intentName, result.usuarioChatwitId, inboxId, result);
        queryMonitor.recordQuery({
            queryName,
            executionTime,
            cacheHit: false,
            timestamp: new Date(),
            parameters: { intentName, inboxId, usuarioChatwitId: result.usuarioChatwitId, result: 'success', messageType: result.messageType },
        });
        ...
        queryMonitor.recordQuery({
            ...
            cacheHit,
            timestamp: new Date(),
            parameters: { intentName, inboxId, usuarioChatwitId, error: error instanceof Error ? error.message : 'Unknown error' },
        });
        ...
        throw new DatabaseError(`Failed to find template mapping for ${intentName}:${usuarioChatwitId}:${inboxId}`, queryName, executionTime, error instanceof Error ? error : undefined);
    }
}
/**
 * Batch query for multiple template mappings with optimized caching
 */
async function findBatchOptimizedTemplateMappings(requests) {
    const queryName = 'findBatchOptimizedTemplateMappings';
    const startTime = Date.now();
    const results = new Map();
    try {
        // Try batch cache lookup first
        const cacheResults = await instagram_template_cache_1.instagramTemplateCache.batchGetTemplateMappings(requests);
        const cacheMisses = [];
        ...
        if (cacheMisses.length > 0) {
            const dbResults = await Promise.allSettled(cacheMisses.map(request => findOptimizedCompleteMessageMapping(request.intentName, request.inboxId)));
        ...
        throw new DatabaseError(`Batch template mapping query failed`, queryName, executionTime, error instanceof Error ? error : undefined);
    }
}
/**
 * Optimized WhatsApp config builder with minimal data transfer
 */
function buildOptimizedWhatsAppConfig(chatwitInbox) {
 ...
 */
function getOptimizedTemplateMessageType(template) {
    if (!template)
        return "template";
 ...
 */
async function checkDatabaseConnectionHealth() {
    const startTime = Date.now();
    try {
...
function getQueryPerformanceStats() {
    ...
    const cacheStats = instagram_template_cache_1.instagramTemplateCache.getStats();
    const recommendations = [];
    // Generate performance recommendations
    if (cacheStats.hitRate < PERFORMANCE_THRESHOLDS.CACHE_MISS_THRESHOLD) {
        recommendations.push(`Cache hit rate is low (${cacheStats.hitRate.toFixed(1)}%). Consider warming the cache or increasing TTL.`);
    }
    if (monitorStats.slowQueries > 0) {
        recommendations.push(`${monitorStats.slowQueries} slow queries detected. Consider optimizing database indexes or query structure.`);
    }
    if (monitorStats.averageExecutionTime > PERFORMANCE_THRESHOLDS.SLOW_QUERY_MS / 2) {
        recommendations.push(`Average query time is high (${monitorStats.averageExecutionTime.toFixed(0)}ms). Consider query optimization.`);
    }
 ...
 */
async function warmInstagramTemplateCache(limit = 100) {
 ...
 */
function recordDatabaseQuery(queryName, executionTime, success, error) {
    queryMonitor.recordQuery({
        queryName,
        executionTime,
        cacheHit: false,
