...
const whatsapp_reactions_worker_1 = require("../../lib/whatsapp-reactions-worker");
...
/**
 * Extrai phoneNumberId e businessAccountId da URL de configuração do WhatsApp
 * Tenta extrair valores reais do payload ou gera IDs únicos baseados no token
 */
function extractWhatsAppConfigFromUrl(whatsappApiKey, payload) {
    try {
        // Primeiro, tentar extrair do payload se disponível
        const chatwootPayload = payload?.originalDetectIntentRequest?.payload;
        // Verificar se há informações de configuração no payload
        if (chatwootPayload?.whatsapp_phone_number_id) {
            return {
                phoneNumberId: chatwootPayload.whatsapp_phone_number_id,
                businessAccountId: chatwootPayload.whatsapp_business_account_id ||
                    `business_${chatwootPayload.whatsapp_phone_number_id}`,
            };
        }
        // Tentar extrair da URL se estiver presente
        if (chatwootPayload?.webhook_url) {
            const urlMatch = chatwootPayload.webhook_url.match(/\/(\d+)\/messages/);
            if (urlMatch) {
                const phoneNumberId = urlMatch[1];
                return {
                    phoneNumberId,
                    businessAccountId: `business_${phoneNumberId}`,
                };
            }
        }
        // Se o token contém informações estruturadas, extrair delas
        if (whatsappApiKey.includes("|")) {
            // Alguns tokens têm formato: app_id|app_secret
            const parts = whatsappApiKey.split("|");
            return {
                phoneNumberId: `phone_${parts[0]}`,
                businessAccountId: `business_${parts[0]}`,
            };
        }
        // Para tokens padrão, gerar IDs únicos baseados no hash do token
        const tokenHash = whatsappApiKey.substring(0, 16);
        return {
            phoneNumberId: `phone_${tokenHash}`,
            businessAccountId: `business_${tokenHash}`,
        };
    }
    catch (error) {
        console.error("Erro ao extrair configuração do WhatsApp:", error);
        // Fallback para valores padrão se não conseguir extrair
        return {
            phoneNumberId: "default_phone_number_id",
            businessAccountId: "default_business_account_id",
        };
    }
}
async function updateWhatsAppApiKey(data) {
    try {
        ...
        // Extrair configurações reais do WhatsApp
        const whatsappConfig = extractWhatsAppConfigFromUrl(data.whatsappApiKey, data.payload);
        console.log(`[MTF Diamante Webhook Worker] Configuração extraída:`, {
            phoneNumberId: whatsappConfig.phoneNumberId,
            businessAccountId: whatsappConfig.businessAccountId,
            inboxId: data.chatwootInboxId,
        });
        ...
        console.log(`[MTF Diamante Webhook Worker] API key atualizada para inbox: ${data.chatwootInboxId} com phoneNumberId: ${whatsappConfig.phoneNumberId}`);
    }
    catch (error) {
    ...
    try {
        ...
        if (messageData.type === "template") {
            ...
            result = await (0, whatsapp_messages_1.sendTemplateMessage)({
                ...
                whatsappApiKey,
                phoneNumberId: metadata?.phoneNumberId, // Add phoneNumberId from metadata
...
async function processSendReaction(taskData) {
    ...
    try {
        ...
        const result = await (0, whatsapp_reactions_worker_1.sendReactionMessage)({
            recipientPhone,
            messageId,
            emoji,
            whatsappApiKey,
        ...
        try {
            await (0, whatsapp_reactions_worker_1.logReactionAttempt)({
                recipientPhone,
                messageId,
                emoji,
...
async function processButtonClick(jobData) {
    ...
    try {
        ...
        const targetMessageId = buttonClickData.originalMessageId ||
        ...
        console.error("[MTF Diamante Webhook Worker] Error processing button click:", {
            ...
            processingTimeMs: processingTime,
            buttonId: jobData.payload?.originalDetectIntentRequest?.payload?.interactive
                ?.button_reply?.id,
        });
        throw error;
    }
...
async function processButtonReactions(params) {
    ...
    if (buttonReaction.emoji) {
        ...
        try {
            const reactionResult = await (0, whatsapp_reactions_worker_1.sendReactionMessage)({
                recipientPhone,
