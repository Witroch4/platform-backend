...
export async function generateImage(this: { client: OpenAI }, prompt: string, options: ImageGenerationOptions = {}) {
	...
	try {
		...
		console.log(`Servidor: Imagem gerada com sucesso - ${response.data?.length || 0} imagem(ns)`);
		return response;
	} catch (error) {
 ...
 */
export async function generateImageWithResponses(this: { client: OpenAI }, prompt: string, options: any = {}) {
	try {
