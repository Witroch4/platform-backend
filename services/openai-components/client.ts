...
export class ClientOpenAIService implements IOpenAIService {
	...
	async uploadFileFromPath(filePath: string, opts: { filename: string; mimeType: string; purpose: FilePurpose }) {
		try {
	...
	async withDeadlineAbort<T>(fn: (signal: AbortSignal) => Promise<T>, ms?: number): Promise<T | null> {
