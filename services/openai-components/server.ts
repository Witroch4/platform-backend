...
export class ServerOpenAIService implements IOpenAIService {
	...
	generateWarmupButtons: (
		...
		agent: AgentConfig,
		opts?: { channelType?: import("./types").ChannelType },
	...
	routerLLM: (
		...
		opts?: {
			...
			sessionId?: string;
			intentHints?: import("./types").IntentCandidate[];
	...
	withDeadlineAbort<T>(fn: (signal: AbortSignal) => Promise<T>, ms = 250): Promise<T | null> {
