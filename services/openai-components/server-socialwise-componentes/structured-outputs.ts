			...
			if (obj && typeof obj === "object") {
				for (const key in obj) {
					...
					if (
						...
						(obj[key].length > 0 || key.includes("_decision") || key.includes("_response"))
					) {
						return true;
					}
				}
...
export async function structuredOrJson<T>(args: StructuredOrJsonArgs<T>): Promise<StructuredOrJsonResult<T>> {
	...
	if (sessionId && channel && !previous_response_id && !args.disableEnsureSession) {
		...
		try {
			...
			if (sessionResult.isNewSession) {
				console.log(
					`📚 [HistoryStrategy: ${historyStrategy}] Nova sessão iniciada. ${useNativeHistory ? "OpenAI gerenciará contexto via previous_response_id" : "Histórico manual via Redis"}`,
				);
			}
	...
	try {
		...
		const hasSizeIssue = issues.some((it: any) => it?.path?.includes("response_text") || it?.path?.includes("buttons"));
		if (hasSizeIssue) {
			try {
				candidate = coerceLengths(candidate, channel);
				parsed = schema.parse(candidate);
