...
- Use apenas slugs de INTENT_HINTS no payload.${
		proposeHumanHandoff
			? ` Se houver ambiguidade real OU o usuário pedir humano, pode incluir 1 handoff:
...
- mode='chat': use slugs de INTENT_HINTS para hipóteses.${
		proposeHumanHandoff
			? ` Se ambiguidade real OU pedido explícito por humano, inclua 1 botão de handoff:
  - payload: @falar_atendente
  - título: "Atendimento Humano" (ou variações permitidas)
  - por último.`
			: ""
